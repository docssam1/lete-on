...
`lab/`은 모든 인쇄 학습지를 찾는 통합 문제은행 입구입니다. 쌓기나무, 평면도형,
이동·대칭, 관찰·경로, 입체·전개도, 색종이 접기의 여섯 영역으로 나누고, 각 카드에서
기존 검증 문제은행과 인쇄 화면으로 이동합니다. 점판·각도·사각형·원·둘레·단위넓이·
도형의 변화·거울대칭은 이 입구에서 최대 20문항을 정해 넘길 수 있습니다. 문제 크기에
따라 한 장 구성이 고정된 숨은 도형·길 잇기·주사위 굴리기·전개도 활동지는 문항 수를
임의로 늘리지 않습니다.
...
- games/angle-studio/ - 각 탐구실: 각도 어림·점판 직각·다각형 내각·평행선과 각 4영역, 80문제. worksheet/angle-studio/에서 표지 포함 1~20문항을 구성합니다.
- games/unit-area/ - 단위넓이 탐구: 온칸 세기·반칸 묶기·넓이 비교·직접 만들기 4영역, 80문제. worksheet/unit-area/에서 표지 포함 1~20문항을 구성합니다. 둘레는 포함하지 않습니다.
- games/perimeter/ - 둘레 탐구: 바깥 경계·붙인 도형·둘레 비교·같은 둘레의 다른 모양 4영역, 80문제. worksheet/perimeter/에서 표지 포함 1~20문항을 구성합니다.
...
각 탐구실의 문제와 검증 기준은 `games/angle-studio/README.md`를 따릅니다.
필즈 1·2과정의 추가 후보와 기존 활동의 중복 검토는 `docs/ANGLE_STUDIO_SOURCE_REVIEW.md`에 정리했습니다.
추가 후보는 구현 완료 목록이 아닙니다.

## 둘레 탐구

`games/perimeter/`는 바깥 경계 세기, 붙인 도형의 공통 변 구별, 둘레 비교,
같은 둘레의 다른 도형 만들기를 독립 영역으로 제공합니다. 영역마다 신규 20문항이며,
`worksheet/perimeter/`가 같은 문제은행과 그림을 사용합니다. 단위넓이 활동과 섞지 않습니다.
자세한 학습 조건과 검증 방법은 `games/perimeter/README.md`를 따릅니다.
로컬 구현·검증 상태와 실제 배포 상태는 별개입니다.
