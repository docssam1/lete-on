...
try {
  ...
  const initialCubePreviews = await page.evaluate(() => ({
    expected: window.GW_GEN.TYPES.length,
    cards: document.querySelectorAll('.type-card.has-visual').length,
    drawings: document.querySelectorAll('.type-card.has-visual .type-visual svg').length,
    audit: window.__LABTYPEPREVIEWS,
    largeType: window.__LABPREVIEW && window.__LABPREVIEW.type,
    selected: document.querySelectorAll('.type-card.has-visual input:checked').length
  }));
  assert.equal(initialCubePreviews.cards, initialCubePreviews.expected, "every generated cube type needs a card preview");
  assert.ok(initialCubePreviews.drawings >= initialCubePreviews.expected, "cube type previews must contain real renderer SVGs");
  assert.equal(Object.keys(initialCubePreviews.audit || {}).length, initialCubePreviews.expected, "cube preview audit missing types");
  Object.entries(initialCubePreviews.audit).forEach(([code, item]) => {
    assert.equal(item.type, code, `${code}: preview generated by a different type`);
    assert.ok(item.kind, `${code}: preview figure kind missing`);
    assert.ok(item.questionId, `${code}: preview question-bank identity missing`);
  });
  assert.equal(initialCubePreviews.largeType, "TC", "cube preview should be visible before selection");
  assert.equal(initialCubePreviews.selected, 0, "opening preview must not select a worksheet type");

  await page.locator('.type-card[data-type="IC"]').hover();
  const hoveredPreview = await page.evaluate(() => ({
    type: window.__LABPREVIEW && window.__LABPREVIEW.type,
    cardType: document.querySelector('#previewBody .ws-card')?.dataset.type,
    questionId: document.querySelector('#previewBody .ws-card')?.dataset.questionId,
    problemQuestionId: window.__LABPREVIEW && window.__LABPREVIEW.problem.identity.questionId,
    selected: document.querySelectorAll('.type-card.has-visual input:checked').length,
    highlighted: document.querySelector('.type-card.is-previewing')?.dataset.type
  }));
  assert.equal(hoveredPreview.type, "IC", "hover should show the matching cube problem");
  assert.equal(hoveredPreview.cardType, "IC", "large preview card type mismatch");
  assert.equal(hoveredPreview.questionId, hoveredPreview.problemQuestionId, "large preview did not use the generated question identity");
  assert.equal(hoveredPreview.selected, 0, "preview hover must not select a worksheet type");
  assert.equal(hoveredPreview.highlighted, "IC", "previewed type is not identified in the list");
  ...
  for (const width of [1280, 768, 390, 320]) {
    ...
    await page.goto(labUrl, { waitUntil: "networkidle" });
    const cubeLayout = await page.evaluate(() => ({
      overflow: document.documentElement.scrollWidth > window.innerWidth,
      previews: document.querySelectorAll('.type-card.has-visual .type-visual svg').length,
      previewCardsFit: [...document.querySelectorAll('.type-card.has-visual')].every((node) => {
        const box = node.getBoundingClientRect();
        const visual = node.querySelector('.type-visual').getBoundingClientRect();
        return box.left >= 0 && box.right <= window.innerWidth && visual.left >= box.left && visual.right <= box.right;
      }),
      largePreview: Boolean(window.__LABPREVIEW && document.querySelector('#previewBody .ws-card')),
      selected: document.querySelectorAll('.type-card.has-visual input:checked').length
    }));
    assert.equal(cubeLayout.overflow, false, `${width}: cube preview horizontal overflow`);
    assert.ok(cubeLayout.previews >= 19, `${width}: cube preview drawings missing`);
    assert.equal(cubeLayout.previewCardsFit, true, `${width}: cube thumbnail clipped outside its card`);
    assert.equal(cubeLayout.largePreview, true, `${width}: initial cube preview missing`);
    assert.equal(cubeLayout.selected, 0, `${width}: preview changed cube selections`);
    await page.screenshot({ path: `${out}/lab-cube-${width}.png`, fullPage: true });
    await page.locator('.domain-btn[data-domain="plane"]').click();
  ...
  const report = {
    ...
    learnerFit: {
      ...
      representations: "영역 탭, 실제 문제은행 도형 미리보기, 생성 유형 체크박스, 독립 학습지 라디오, 문항 수와 실제 인쇄 화면 연결",
