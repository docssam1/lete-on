const copy = {
  ko: {
    ...
    hint:"힌트", clear:"비우기", skip:"다음 문제", levelPick:"단계를 골라요", pieces:"{done}/{total}조각 · {cells}/{totalCells}칸", viewModes:["입체","위","앞","옆","자동"], rotateActions:["옆으로 돌리기","위로 돌리기","뒤집기"],
    ...
    hintText:"이제 반짝이는 방향과 자리를 살펴보세요.", hintPiece:"먼저 다음에 쓸 조각을 찾아보세요.", hintEliminate:"이 보기는 모양이 달라요. 남은 보기를 돌려 생각해 보세요.", clearConfirm:"고정된 조각을 빼고 모두 비울까요?", skipConfirm:"이 문제는 포인트 없이 넘어갈까요?",
    rotate:"가로로 돌려 주세요", exit:"마을로 나가기", nextLevel:"다음 단계", practice:"같은 단계 더 풀기", district:"입체 지구로",
    completeTitle:"한 회차 끝!", completeText:"이번 회차의 다섯 문제를 모두 살펴봤어요. 다음 단계로 갈까요?",
  ...
  en: {
    ...
    hint:"Hint", clear:"Clear", skip:"Next", levelPick:"Choose a stage", pieces:"{done}/{total} pieces · {cells}/{totalCells} cells", viewModes:["3D","Top","Front","Side","Auto"], rotateActions:["Side turn","Up turn","Flip"],
    ...
    hintText:"Now study the glowing direction and place.", hintPiece:"First find the piece to use next.", hintEliminate:"This choice has a different shape. Rotate the remaining choices in your mind.", clearConfirm:"Clear every movable piece?", skipConfirm:"Skip this problem without points?",
    ...
    completeTitle:"Round complete!", completeText:"You looked through all five problems in this round. Move to the next stage?",
  ...
  zh: {
    brand:"索玛立方工坊", level:"阶段", target:"目标形状", build:"我的作品", tray:"拼块托盘", hint:"提示", clear:"清空", skip:"下一题", levelPick:"选择阶段", pieces:"{done}/{total}块 · {cells}/{totalCells}格", viewModes:["立体","俯视","正面","侧面","自动"], rotateActions:["侧转","上转","翻转"], targetViewerLabel:"由{count}个单位立方体组成的三维目标。", buildViewerLabel:"三维作品，已放置{done}/{total}块。", choiceLabel:"选项{number}", pieceLabel:"选择索玛拼块{id}", soundOn:"关闭声音", soundOff:"打开声音",
    selectPiece:"请先选择一个拼块。", selectPlace:"点击淡淡发光的可放位置。", invalid:"这里不能放。", removed:"拼块已放回托盘。", solved:"所有拼块严丝合缝！", twoWaysSolved:"你用两种不同方法完成了立方体！", firstWaySolved:"第一种拼法完成！接着换一种方法再拼。", secondWayPrompt:"请用不同的位置和方向再次完成立方体。", wrongChoice:"转动后会发现是不同的拼块。", correctChoice:"对！转动后还是同一个拼块。", hintText:"现在观察发光的方向和位置。", hintPiece:"先找出下一步要用的拼块。", hintEliminate:"这个选项的形状不同。请转动想象剩下的选项。", clearConfirm:"清空所有可移动拼块吗？", skipConfirm:"不计分跳过这题吗？", rotate:"请横放设备", exit:"返回立体区", nextLevel:"下一阶段", practice:"同阶段再练", district:"返回立体区", completeTitle:"本轮结束！", completeText:"你已经看完本轮五题。要进入下一阶段吗？", tutorialSkip:"跳过", tutorialNext:"下一步", tutorialDone:"开始", tutorial:["索玛立方用七个不同拼块组成各种立体。","先选择下方拼块，再用向侧面旋转、向上旋转或翻转来改变方向。","点击淡淡发光的位置放下拼块。点击已放拼块可收回托盘。"], prompts:["找出旋转后相同的拼块。","旋转两个拼块，搭成左边的形状。","用三个不同拼块搭成同样的目标形状。","观察固定拼块，填满立方体。","先用一种方法完成七块立方体。"], stageNames:["儿童","Pre","入门","初级","中级"]
  },
  ja: {
    brand:"ソーマキューブ工房", level:"段階", target:"作る形", build:"自分の形", tray:"ピース置き場", hint:"ヒント", clear:"空にする", skip:"次の問題", levelPick:"段階を選ぼう", pieces:"{done}/{total}ピース · {cells}/{totalCells}マス", viewModes:["立体","上","正面","横","自動"], rotateActions:["横回転","上回転","裏返す"], targetViewerLabel:"単位立方体{count}個でできた3Dの目標。", buildViewerLabel:"{total}ピース中{done}ピースを置いた3D作品。", choiceLabel:"選択肢{number}", pieceLabel:"ソーマピース{id}を選ぶ", soundOn:"音を消す", soundOff:"音を出す",
    selectPiece:"先にピースを選んでください。", selectPlace:"うすく光る場所をタップしてください。", invalid:"そこには置けません。", removed:"ピースをトレイに戻しました。", solved:"すき間なく全部入りました！", twoWaysSolved:"2つの違う組み方で完成しました！", firstWaySolved:"1つ目の組み方が完成！次は別の方法で作ります。", secondWayPrompt:"ピースの位置と向きを変えて、もう一度完成させましょう。", wrongChoice:"回してみると別のピースです。", correctChoice:"正解！回しても同じピースです。", hintText:"次に光る向きと場所を見てください。", hintPiece:"まず次に使うピースを探しましょう。", hintEliminate:"この選択肢は形が違います。残りを頭の中で回してみましょう。", clearConfirm:"動かせるピースを全部戻しますか？", skipConfirm:"ポイントなしで次へ進みますか？", rotate:"横向きにしてください", exit:"エリアへ戻る", nextLevel:"次の段階", practice:"同じ段階を続ける", district:"立体エリアへ", completeTitle:"1回分終了！", completeText:"この回の5問をすべて見ました。次の段階へ進みますか？", tutorialSkip:"スキップ", tutorialNext:"次へ", tutorialDone:"始める", tutorial:["ソーマキューブは7つの違うピースで立体を作るパズルです。","下のピースを選び、横に回す・上に回す・裏返すボタンで向きを変えます。","うすく光る場所をタップして置きます。置いたピースをタップするとトレイへ戻ります。"], prompts:["回転しても同じピースを探しましょう。","2つのピースで左と同じ形を作りましょう。","3つの違うピースで同じ目標の形を作りましょう。","固定されたピースを見て立方体を埋めましょう。","まず1つの方法で7ピースの立方体を完成させましょう。"], stageNames:["キッズ","Pre","入門","初級","中級"]
  }
};
