...
let alternateCount=0,enumeratedCount=0;
...
for(const problem of alternateSamples){const solutions=findSolutions(problem,100);if(solutions.length<2)throw new Error(`Expected alternate answers for ${problem.id}.`);for(const regions of solutions){if(!validateCuts(problem,cutsForRegions(regions)).valid)throw new Error(`Valid partition rejected for ${problem.id}.`);}enumeratedCount+=solutions.length;alternateCount+=solutions.length-1;}
...
console.log(`equal-partition selftest: ${solved} problems, ${enumeratedCount} valid partitions checked (${alternateCount} alternatives beyond authored samples)`);
