...
function toggleEdge(key){if(state.solved)return;state.history.push(new Set(state.cuts));state.cuts.has(key)?state.cuts.delete(key):state.cuts.add(key);tone("tap");renderBoard();}
