...
try {
  ...
  for (const domain of domains) {
    await page.goto(`${base}/geometry/games/angle-studio/?domain=${domain.id}`);
    await page.waitForSelector("#board svg");
  }
  await page.goto(base + "/geometry/worksheet/angle-studio/");
  await page.waitForSelector(".problem");
  await context.setOffline(true);
  for (const domain of domains) {
    await page.goto(`${base}/geometry/games/angle-studio/?domain=${domain.id}&level=${domain.level}&practice=1&lang=ko`);
    await page.waitForSelector("#board svg");
    assert.equal(await page.locator("#title").textContent(), domain.names.ko);
  }
  await page.goto(base + "/geometry/worksheet/angle-studio/?domain=all&count=20");
  await page.waitForSelector(".problem");
