...
function renderBoard(p) {
  ...
  ui.board.append(grid,routeSvg(p));
...
function updateMarker() {
  ...
  marker.classList.toggle("solid",state.method==="solid");
  marker.classList.toggle("flat",state.method==="flat");
  if(state.method==="solid") marker.append(dieSvg(state.orientation ?? problem().startOrientation,{compact:true}));
  else marker.append(flatDieSvg(state.orientation ?? problem().startOrientation,{revealed:state.flatRevealed}));
...
function updateDie() {
  ...
  ui.directionButtons.forEach((button) => {
    button.disabled = state.solved || problem().interaction === "route-answer" || !total || state.step >= total;
...
function answer(value,button) {
  ...
  const p=problem(); state.orientation=p.finalOrientation; state.step=p.directions.length;state.flatRevealed=new Set(Object.keys(flatFaceQuads));updateMarker(); updateDie();
...
function rollOne(selectedDirection) {
  const p=problem(); if(state.solved||state.step>=p.directions.length)return;
  ...
  state.orientation=roll(state.orientation,direction); state.step+=1;state.flatRevealed=new Set(["top"]);
  ui.die.classList.remove("rolling");void ui.die.offsetWidth;ui.die.classList.add("rolling"); updateMarker(); updateDie(); playTone("roll");
}

function resetRoll() { state.orientation=problem().startOrientation; state.step=0;state.flatRevealed=new Set(["top"]); updateMarker(); updateDie(); }
...
function selectLevel(id){state.level=id;state.problemIndex=0;ui.levels.hidden=true;state.queue=sessionProblems("dice-roll",id,level().problems,SESSION_SIZE);history.replaceState({},"",`?level=${id}`);renderProblem();}
