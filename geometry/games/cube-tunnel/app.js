...
const tutorialStorageKey = "gfield-cube-tunnel-tutorial-v1";
const tutorialKeys = ["tutorialTunnel1", "tutorialTunnel2", "tutorialTunnel3"];
...
function updatePrompt() {
  elements.numberPrompt.textContent = text(state.lang, "chooseTunnel");
}
...
function checkAnswer() {
  ...
  if (Number(state.answer) !== currentProblem().answer.remaining) {
    state.wrongAttempts += 1;
    elements.hiddenDisplay.classList.add("wrong");
    showToast(text(state.lang, "tunnelWrong"));
    setGuide("tunnelWrong", false);
    return;
  }
  completeProblem();
}
...
function completeProblem() {
  state.solved = true;
  awardPoints(`cube-tunnel:${currentProblem().id}`, 15);
  ...
  // the red tunnels revealed through the block before the next problem loads.
...
function renderLevelList() {
  ...
  levels.forEach((level, index) => {
    ...
    button.addEventListener("click", () => {
      ...
      state.problemIndex = 0;
      localStorage.setItem("cube-tunnel-level", String(index));
...
// Faint shell showing the original solid block, so the drilled holes read as
// holes rather than floating cubes.
const containerMaterial = new THREE.MeshStandardMaterial({
  color: 0xf3e6cf, transparent: true, opacity: 0.06, roughness: 0.6, metalness: 0.0,
  depthWrite: false, side: THREE.DoubleSide
});
const containerEdgeMaterial = new THREE.LineBasicMaterial({ color: 0x9a7a4e, transparent: true, opacity: 0.5 });
// Red ghost cube marking a drilled-out cell while "구멍 보기" (Show Tunnels) is on.
const tunnelMaterial = new THREE.MeshStandardMaterial({
  color: 0xff6a5a, emissive: 0xd12617, emissiveIntensity: 0.45,
  transparent: true, opacity: 0.32, roughness: 0.45, metalness: 0.02, depthWrite: false
});
...
const sharedMaterials = new Set([cubeMaterial, containerMaterial, containerEdgeMaterial, tunnelMaterial, edgeMaterial, edgeMaterialDim]);
let hiddenMeshes = [];
...
function renderModel() {
  ...
  const removed = removedCells(currentProblem().box, currentProblem().tunnels);
  ...
  // Faint outline of the original solid block, so the drilled channels read as
  // holes rather than the remaining cubes looking like they float.
  ...
  for (let z = 0; z < depth; z += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      for (let y = 0; y < height; y += 1) {
        const isRemoved = removed.has(`${x},${y},${z}`);
        if (isRemoved) {
          // Drilled-out cell: an empty hole normally, a red ghost on "구멍 보기".
          if (!state.xray) continue;
          const ghost = new THREE.Mesh(cubeGeometry, tunnelMaterial);
          ghost.position.set(wx, y + 0.5, wz);
          ghost.userData.glow = true;
          hiddenMeshes.push(ghost);
          modelGroup.add(ghost);
        } else {
          // A cube that remains in the block.
