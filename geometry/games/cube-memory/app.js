...
const viewOverride = Number(params.get("view")) || 0;   // test hook: shorten the look
const gameProgress = readGameProgress("cubeMemory");
const savedLevel = Number.isInteger(Number(gameProgress.levelIndex))
  ? Number(gameProgress.levelIndex)
  : Number(localStorage.getItem("cube-memory-level")) || 0;
const state = {
  ...
  phase: "memorize",             // "memorize" | "rebuild" | "solved"
  placed: {},                    // "x,z" -> color index
  selectedColor: 0,
  hintsUsed: 0,
  wrongAttempts: 0,
...
const tutorialStorageKey = "gfield-cube-memory-tutorial-v1";
const tutorialKeys = ["tutorialMem1", "tutorialMem2", "tutorialMem3"];
...
function updateXrayButtons() {
  const label = text(state.lang, "revealShow");
...
function setGuide(key, values = {}, shouldSpeak = true) {
  const message = format(key, values);
  elements.guide.textContent = message;
  if (shouldSpeak) speak(message);
}

function updatePrompt() {
  elements.numberPrompt.textContent = text(state.lang, state.phase === "memorize" ? "memorizeHint" : "rebuildPrompt");
}
...
function openConceptTutorial() {
  // Pause the countdown while the intro is open so the child reads it first.
  clearTimers();
...
function advanceConceptTutorial() {
  ...
  speechSynthesis?.cancel();
  startMemorize();   // begin the look now that the intro is closed
}
...
function startMemorize() {
  state.phase = "memorize";
  clearTimers();
  elements.coverBadge.hidden = true;
  renderModel(true);
  renderStatus();
  renderPalette();
  renderRebuildGrid();
  updatePrompt();
  const viewMs = viewOverride || currentProblem().viewMs;
  const seconds = Math.max(1, Math.round(viewMs / 1000));
  setGuide("guideMemorize", { n: seconds }, false);
  // Countdown ring.
  elements.countdown.hidden = false;
  let remaining = seconds;
  elements.countdownNum.textContent = String(remaining);
  state.countdownTimer = setInterval(() => {
    remaining -= 1;
    if (remaining >= 1) elements.countdownNum.textContent = String(remaining);
  }, viewMs / seconds);
  state.memorizeTimer = setTimeout(enterRebuild, viewMs);
}
...
function enterRebuild() {
  ...
  state.phase = "rebuild";
  elements.countdown.hidden = true;
  elements.coverBadge.hidden = false;
  renderModel(false);
  renderStatus();
  renderPalette();
  renderRebuildGrid();
  updatePrompt();
  setGuide("guideRebuild", {}, true);
}

function renderStatus() {
  elements.hiddenDisplay.classList.toggle("rebuild", state.phase === "rebuild");
  elements.hiddenDisplay.classList.toggle("solved", state.phase === "solved");
  if (state.phase === "memorize") {
    elements.phaseLabel.textContent = text(state.lang, "memorize");
    elements.phaseLabel.className = "phase-memorize";
    elements.hiddenValue.textContent = "👀";
  } else if (state.phase === "solved") {
    elements.phaseLabel.textContent = text(state.lang, "rebuild");
    elements.phaseLabel.className = "";
    elements.hiddenValue.textContent = "✓";
  } else {
    elements.phaseLabel.textContent = text(state.lang, "rebuild");
    elements.phaseLabel.className = "";
    elements.hiddenValue.textContent = `${Object.keys(state.placed).length}/${currentProblem().cells.length}`;
  }
}

// ---------------------------------------------------------------------------
// Color palette + rebuild grid (the child's input surface)
// ---------------------------------------------------------------------------
function renderPalette() {
  elements.colorPalette.replaceChildren();
  const active = state.phase === "rebuild";
  palette.forEach((color) => {
    const swatch = document.createElement("button");
    swatch.type = "button";
    swatch.className = "swatch";
    swatch.style.background = color.css;
    swatch.setAttribute("aria-label", text(state.lang, color.key));
    swatch.title = text(state.lang, color.key);
    swatch.classList.toggle("selected", color.id === state.selectedColor);
    swatch.disabled = !active;
    swatch.addEventListener("click", () => selectColor(color.id));
    elements.colorPalette.append(swatch);
  });
}
...
function renderRebuildGrid() {
  const [width, depth] = currentProblem().grid;
  elements.rebuildGrid.style.setProperty("--cols", String(width));
  elements.rebuildGrid.replaceChildren();
  const active = state.phase === "rebuild";
  // Back row (z=0) at the top, front row (z=depth-1) at the bottom, so the grid
  // is laid out the same way the child saw the board in the 3/4 view.
  for (let z = 0; z < depth; z += 1) {
    for (let x = 0; x < width; x += 1) {
      const key = cellKey(x, z);
      const cell = document.createElement("button");
      cell.type = "button";
      cell.className = "rebuild-cell";
      cell.dataset.x = String(x);
      cell.dataset.z = String(z);
      if (key in state.placed) {
        cell.classList.add("filled");
        cell.style.background = palette[state.placed[key]].css;
      }
      cell.disabled = !active;
      cell.addEventListener("click", () => placeCell(x, z));
      elements.rebuildGrid.append(cell);
    }
  }
}

function placeCell(x, z) {
  if (state.phase !== "rebuild") return;
  const key = cellKey(x, z);
  if (state.placed[key] === state.selectedColor) {
    delete state.placed[key];             // tap the same color again to clear
  } else {
    state.placed[key] = state.selectedColor;
  }
  renderRebuildGrid();
  renderStatus();
}
...
function peek() {
  ...
  elements.coverBadge.hidden = true;
  renderModel(true);
  elements.xrayButtons.forEach((b) => b.setAttribute("aria-pressed", "true"));
  setGuide("guideHint", {}, false);
  state.peekTimer = setTimeout(() => {
    if (state.phase !== "rebuild") return;
    elements.coverBadge.hidden = false;
    renderModel(false);
    elements.xrayButtons.forEach((b) => b.setAttribute("aria-pressed", "false"));
  }, 1600);
}
...
function checkAnswer() {
  ...
  if (placedKeys.length === 0) {
    showToast(text(state.lang, "memEmpty"));
    setGuide("memEmpty", {}, false);
    return;
  }
  const target = new Map(currentProblem().cells.map((c) => [cellKey(c.x, c.z), c.color]));
  let correct = target.size === placedKeys.length;
  if (correct) {
    for (const [key, color] of target) {
      if (state.placed[key] !== color) { correct = false; break; }
    }
  }
  if (correct) { completeProblem(); return; }
  // Mark mismatches (wrong color or wrong/extra cell).
  state.wrongAttempts += 1;
  $$(".rebuild-cell").forEach((cell) => {
    const key = cellKey(Number(cell.dataset.x), Number(cell.dataset.z));
    const want = target.get(key);
    const have = state.placed[key];
    cell.classList.remove("right", "wrong");
    if (have !== undefined && have !== want) cell.classList.add("wrong");
  });
  showToast(text(state.lang, "memWrong"));
  setGuide("memWrong", {}, false);
}

function completeProblem() {
  state.phase = "solved";
  clearTimers();
  awardPoints(`cube-memory:${currentProblem().id}`, 15);
  celebrateEvolution(syncEvolution(), state.lang);
  updateLevelBadge(state.lang, { left: "25%" });
  elements.coverBadge.hidden = true;
  renderModel(true);                 // reveal the original arrangement
  renderPalette();
  renderRebuildGrid();
  $$(".rebuild-cell.filled").forEach((cell) => cell.classList.add("right"));
  renderStatus();
...
function renderLevelList() {
  ...
  levels.forEach((level, index) => {
    ...
    button.addEventListener("click", () => {
      ...
      state.problemIndex = 0;
      localStorage.setItem("cube-memory-level", String(index));
...
// ---------------------------------------------------------------------------
// Three.js stage — same wood/board/lighting as the rest of the suite; cubes are
// tinted with the palette color so they read as painted wood blocks.
// ---------------------------------------------------------------------------
...
// One tinted-wood material per palette color.
const colorMaterials = palette.map((color) => new THREE.MeshStandardMaterial({
  color: color.hex, map: cubeWoodTexture, roughness: 0.5, metalness: 0.02, bumpMap: cubeWoodTexture, bumpScale: 0.012
}));
const edgeGeometry = new THREE.EdgesGeometry(cubeGeometry, 28);
const edgeMaterial = new THREE.LineBasicMaterial({ color: 0x5a3d20, transparent: true, opacity: 0.28 });

const sharedMaterials = new Set([...colorMaterials, edgeMaterial]);
let glowMeshes = [];
...
function renderModel(showCubes) {
  clearModel();
  ...
  if (showCubes) {
    problem.cells.forEach(({ x, z, color }) => {
      const cube = new THREE.Mesh(cubeGeometry, colorMaterials[color]);
      cube.position.set(x - centerX, 0.5, z - centerZ);
      cube.castShadow = true;
      cube.receiveShadow = true;
      cube.add(new THREE.LineSegments(edgeGeometry, edgeMaterial));
      if (state.phase === "solved") { cube.userData.glow = true; glowMeshes.push(cube); }
      modelGroup.add(cube);
    });
  }
...
function setCameraView(view) {
  ...
  if (view === "free") camera.position.set(7.2, 6.3, 8.2);
...
function animate() {
  ...
  if (glowMeshes.length) {
    const pulse = 1 + Math.sin(time * 3.4) * 0.06;
    glowMeshes.forEach((mesh) => { mesh.scale.setScalar(pulse); });
  }
  controls.update();
...
elements.xrayButtons.forEach((button) => button.addEventListener("click", peek));
...
$$('[data-lang]').forEach((button) => button.addEventListener("click", () => {
  ...
  applyLanguage();
  renderModel(state.phase !== "rebuild");
}));
