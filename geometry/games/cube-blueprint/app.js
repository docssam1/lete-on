...
import { levels, validateLevels, viewsOfHeightGrid, viewsMatch } from "./levels.js?v=blueprint-1";
import { text } from "./i18n.js?v=blueprint-1";
...
const gameProgress = readGameProgress("cubeBlueprint");
const savedLevel = Number.isInteger(Number(gameProgress.levelIndex))
  ? Number(gameProgress.levelIndex)
  : Number(localStorage.getItem("cube-blueprint-level")) || 0;
const state = {
...
const tutorialStorageKey = "gfield-cube-blueprint-tutorial-v1";
const tutorialKeys = ["tutorialBlueprint1", "tutorialBlueprint2", "tutorialBlueprint3"];
...
function loadProblem() {
  ...
  saveGameProgress("cubeBlueprint", {
...
function renderBuildGrid() {
  ...
  for (let z = 0; z < depth; z += 1) {
    for (let x = 0; x < width; x += 1) {
      ...
      const allowed = problem.target.top[z][x] === 1;
      cell.classList.toggle("unavailable", !allowed);
      cell.disabled = state.solved || !allowed;
      cell.setAttribute("aria-label", allowed ? `${text(state.lang, "height")} ${h}` : "");
...
function checkAnswer() {
  ...
  if (!anyCube) {
    showToast(text(state.lang, "blueprintEmpty"));
    setGuide("blueprintEmpty", false);
    return;
  }
  ...
  showToast(text(state.lang, "blueprintWrong"));
  setGuide("blueprintWrong", false);
}

function completeProblem() {
  state.solved = true;
  awardPoints(`cube-blueprint:${currentProblem().id}`, 15);
...
function renderLevelList() {
  ...
  levels.forEach((level, index) => {
    ...
    button.addEventListener("click", () => {
      ...
      state.problemIndex = 0;
      localStorage.setItem("cube-blueprint-level", String(index));
...
// ---------------------------------------------------------------------------
// Three.js stage — opaque premium wood build on a gridded drafting tray.
// ---------------------------------------------------------------------------
...
const cubeMaterial = new THREE.MeshStandardMaterial({
  color: 0xffe2aa, map: cubeWoodTexture, roughness: 0.5, metalness: 0.012,
  bumpMap: cubeWoodTexture, bumpScale: 0.016
});
