"""Inspect and rasterize browser-generated QA PDFs, never final deliverables."""
import json
import os
...
for item in manifest["pdfs"]:
    ...
    has_cover = "no-cover" not in name
    ...
    with pdfplumber.open(path) as document:
        for index, (page, measured) in enumerate(zip(reader.pages, document.pages)):
            ...
            assert text.count("GFIELD") >= 2, (name, index, "watermark")
            ...
            if has_cover and index == 0:
                assert numbers == [], (name, "cover contains a numbered problem")
            else:
            ...
            for char in measured.chars:
                assert char["x0"] >= -1 and char["x1"] <= measured.width + 1, (name, index, "horizontal text clipping")
                assert char["top"] >= -1 and char["bottom"] <= measured.height + 1, (name, index, "vertical text clipping")
                total_characters += 1
            total_pages += 1
