...
/* ===== 유형 9: 목표 합이 되게 색칠하기 (역방향) =====
   ...
   색칠할 칸을 학생이 고른다. 접은 칸 하나의 굤적(거울 네 칸) 합을 S라 하면
...
function genNumInverse(){
  ...
  return {
    ...
    info:`사분면 (${pr},${pc}) · 굤적 합 ${S.flat().join('·')} · 목표 ${target} · 답 ${answerSet.map(posName).join(', ')}`,
    src:''
  };
}
// 직사각형 숫자 격자 (중간 단계가 2x4라 정사각 전용 drawNumberGrid를 못 쓰다)
...
function genUnfoldShape(){
  ...
  const cands=[wrongOnlyOne, wrongShift, wrongCorner]
    .filter(w=>maskDiff(rasterMask(w,M), mCorrect) > M*M*0.01);              // 눈에 띄게 다르겜야 오답이다
...
/* ===== 유형 5: 겹친 색종이 순서 (교재 35~36·44~46쪽) =====
   같은 크기의 정사각형 8장을 3x3 자리에 격칩 놓는다(가장자리 중앙 한 자리는 비움 — 교재 문법).
   답이 유일하려면 "맨 위를 거둘어내면 남은 것 중 온전히 보이는 종이가 늘 하나뿐이어야 한다".
   즉 맨 위를 빼 모든 종이가 자기보다 위 종이 중 하나와 겹쳤야 한다. */
...
function genStack(askOrder){
  ...
  for(let attempt=0;attempt<800;attempt++){
    ...
    // 거둘어내기 유일성: 바로 위 종이와 격쳤야 한다. "위 종이 중 아무나"로는 부족하다 —
    // 그 종이가 먼저 거쳠 나가면 중간 종이가 조기에 온전히 드러나 답이 두 갈래로 된다.
    let ok=true;
    for(let k=1;k<8;k++) if(!over(z[k], z[k-1])){ok=false;break;}
    if(!ok) continue;
    // 모든 종이에 라벨 놓을 보이는 지점이 있어야 한다 (내부 표본점 중 위 종이에 안 덮이고 가장 여유로운 겳)
    const spots=[];
    ...
    return {
      ...
      text:['다음은 크기가 모두 같은 정사각형 모양의 색종이 8장을 격쳐 놓은 것입니다.',
