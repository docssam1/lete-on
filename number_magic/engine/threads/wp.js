...
(function () {
  ...
  function measureSituation(rng, kind, opt) {
    ...
    /* 합병은 두 그릇을 '큰 그릇 / 작은 그릇'으로 부른다(아래 labA·labB). 그런데
       분수 갈래는 두 분자를 따로 뽑기 때문에 **큰 그릇이 작은 그릇보다 적게 든**
       이야기가 나온다(큰 병 3/10 L · 작은 병 5/10 L). 그림으로 헤아린 아이가
       바르게 생각하고도 틀리는 자리라 이름과 양을 맞춘다 — 큰 쪽에 큰 수를 준다.
       ⚠️ rng를 쓰지 않는다(자리만 바꾼다) — 같은 학습지의 다른 문항이 밀리지 않는다.
       합은 그대로라 **정답은 바뀌지 않는다**(레벨 C의 답은 보기 번호이거나 잡음 수다).
       길이는 이름표가 빨강·파랑이라 크기를 말하지 않으므로 건드리지 않는다.
       구차도 같은 이름표를 쓰지만 위에서 이미 n1 > n2를 못 박아 둔다. */
    if (kind === '합병' && !isLen && n2 > n1) {
      const nx = n1; n1 = n2; n2 = nx;
      const tx = t1; t1 = t2; t2 = tx;
    }
