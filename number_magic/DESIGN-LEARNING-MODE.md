...
- [x] N-14 산가지·규칙 (nl14_pattern: read=미리 그려진 탤리를 세어 numpad로
      답 — tallyBuild에 interaction:'read' 모드 추가(비활성 보드+축소
      numpad) / arrow=화살표를 따라 ±1 늘고 주는 수열 빈칸, seqFill 재사용
      (신규 위젯 0개). N4 챕터 완성(N-04·N-10·N-14) → **수의 나라 15개 유아
      유닛(N-01~N-15) 전체 구현 완료**
      — 2000시드 퍼즈 + Playwright 전 구간(practice→discover→lab→stamp,
      +20코인) 검증 완료 2026-07-16
