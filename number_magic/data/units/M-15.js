...
window.NM_UNITS['M-15'] = {
  ...
  subtitle:{ ko:'제곱해서 그 수가 되는 수를 거꾸로 찾습니다', en:'Work backward to find the number whose square gives that value', zh:'反过来找出平方后等于那个数的数' },
  icon:'🌱',
  symbols:[
  ...
  practice:{
    ...
    params:{mode:'perfect'},
    intro:{
      ko:'√25는 "제곱해서 25가 되는 수"입니다. 5×5=25니까 √25=5!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'넓이가 49인 정사각형이 있습니다. 한 변의 길이는 얼마일까요? "제곱해서 49가 되는 수"를 찾아야 합니다 — 곱셈을 거꾸로 하는 거입니다.',
      ...
      history:{ ko:'제곱근 기호 √는 라틴어 radix(뿌리)의 첫 글자 r에서 변형됐다고 알려져 있습니다. "근(root)"이라는 이름처럼, 어떤 수의 "뿌리"가 되는 수를 찾는 셈입니다.',
    ...
    stages:[
      { tag:{ko:'① 루트는 진짜 "뿌리"입니다',en:'1) A root is literally a root',zh:'① 根号真的是"根"'},
        head:{ko:'\\sqrt{9} = 3',en:'\\sqrt{9} = 3',zh:'\\sqrt{9} = 3'},
        desc:{ko:'<svg viewBox="0 0 200 130" style="width:min(230px,70vw);display:block;margin:0 auto 8px" aria-hidden="true"><circle cx="100" cy="38" r="30" fill="#5FB876"/><rect x="95" y="60" width="10" height="24" fill="#8a6b4a"/><line x1="0" y1="84" x2="200" y2="84" stroke="#C9A063" stroke-width="2"/><path d="M100 84 L88 108 M100 84 L100 112 M100 84 L112 108" stroke="#8a6b4a" stroke-width="3" fill="none"/><text x="100" y="44" text-anchor="middle" font-size="20" font-weight="900" fill="#fff">9</text><text x="100" y="126" text-anchor="middle" font-size="16" font-weight="900" fill="#8a6b4a">3</text></svg>땅 위의 나무가 9라면, 땅속의 <b>뿌리</b>가 3입니다. 뿌리 3이 자라서(3×3) 나무 9가 된 것입니다. 루트(root)라는 이름이 진짜 "뿌리"라는 뜻 — 나무를 보고 뿌리를 찾는 게 제곱근입니다.',
        ...
        result:{ko:'나무(제곱)를 보고 뿌리(제곱근)를 찾습니다!',en:'Look at the tree (square), find the root!',zh:'看着树(平方)，找到根(平方根)！'} },
      ...
      { tag:{ko:'② 말의 방향 — "제곱"과 "제곱근"',en:'2) Word direction — square vs square root',zh:'② 词的方向——平方与平方根'},
        ...
        desc:{ko:'말의 방향이 반대입니다! <b>"9의 제곱"</b>은 나무를 키우는 것(81), <b>"9의 제곱근"</b>은 뿌리를 캐는 것(3과 -3, 둘 다!). 그리고 <b>"제곱근 9"</b>라고 하면 그중 양수 뿌리 √9=3 하나만 가리킵니다. 이 세 가지를 구분하면 중3 시험의 함정 절반이 사라집니다.',
        ...
        mathSteps:['9^2=81', {ko:'9\\text{의 제곱근}: \\pm3',en:'\\text{square roots of } 9: \\pm3',zh:'9\\text{的平方根}: \\pm3'}, '\\sqrt{9}=3'],
        result:{ko:'제곱은 키우기, 제곱근은 캐기 — 방향이 반대!',en:'Squaring grows, rooting digs — opposite directions!',zh:'平方是养大，开方是挖根——方向相反！'},
        book:{ko:'a>0일 때 a의 제곱근은 ±√a로 두 개, "제곱근 a"(√a)는 양수인 것 하나입니다.',
      ...
      { tag:{ko:'③ 뿌리 속에 음수가 들어가면?',en:'3) What if a negative hides under the root?',zh:'③ 根号里是负数会怎样？'},
        ...
        desc:{ko:'그림으로 그려 보면 답이 보입니다 — <b>넓이가 -9인 정사각형은 그릴 수가 없습니다.</b> 어떤 수든 제곱하면 0 이상이 되니까(3²=9, (-3)²도 9), 제곱해서 음수가 되는 수는 우리가 아는 수 중엔 없습니다. 그래서 √ 안에 음수가 있으면 "(실수 중엔) 없음"이 답입니다.',
        ...
        mathSteps:['3^2=9,\\;(-3)^2=9', {ko:'\\text{제곱은 항상 } \\ge 0',en:'\\text{a square is always} \\ge 0',zh:'\\text{平方总是} \\ge 0'}, {ko:'\\sqrt{-9}\\;\\text{는 없음}',en:'\\sqrt{-9}\\;\\text{does not exist}',zh:'\\sqrt{-9}\\;\\text{不存在}'}],
        result:{ko:'넓이가 음수인 정사각형은 못 그립니다 — 그래서 없음!',en:'No square can have negative area — so no answer!',zh:'画不出负面积的正方形——所以不存在！'},
        book:{ko:'√2처럼 딱 떨어지지 않는 뿌리를 끝까지 계산하려던 히파소스가 "분수로도 못 쓴다"는 걸 알게 됐고, 그 비밀을 말했다가 바다에 던져졌다는 이야기가 전해집니다 — 무리수(다 쓸 수 없는 수)의 탄생 전설입니다.',
      ...
      { tag:{ko:'④ 제곱을 거꾸로 — 완전제곱수',en:'4) Undo the square — perfect squares',zh:'④ 平方的逆运算——完全平方数'},
        ...
        desc:{ko:'6×6=36이니까 √36=6입니다. 이렇게 어떤 정수의 제곱으로 정확히 나오는 수(1,4,9,16,25,36…)를 <b>완전제곱수</b>라고 합니다. 완전제곱수의 제곱근은 항상 정수로 딱 떨어집니다.',
        ...
        result:{ko:'완전제곱수의 제곱근은 항상 정수입니다!',en:'The square root of a perfect square is always an integer!',zh:'完全平方数的平方根总是整数！'},
        book:{ko:'어떤 수 a의 제곱근 중 양수인 것을 √a로 나타냅니다. a가 완전제곱수면 √a는 정수입니다.',
      ...
      { tag:{ko:'⑤ 제곱근을 다시 제곱하면 원래대로',en:'5) Squaring a root undoes it',zh:'⑤ 平方根再平方，回到原数'},
        ...
        desc:{ko:'√7을 다시 제곱하면 근호가 사라지고 그냥 7이 됩니다 — 제곱과 제곱근은 서로 반대 동작이기 때문입니다. 그런데 (-7)²을 다시 제곱근으로 풀면 -7이 <b>아니라 7</b>이 나옵니다 — 결과는 늘 0 이상이기 때문입니다(절댓값).',
        ...
        result:{ko:'제곱근은 항상 0 이상 — 음수가 숨어 있어도 절댓값으로 나옵니다!',en:'A square root is always nonnegative — even a hidden negative comes out as its absolute value!',zh:'平方根永远不小于0——就算藏着负数，出来的也是绝对值！'},
        book:{ko:'(√a)²=a(a≥0), √(a²)=|a| — 두 식의 차이를 헷갈리지 않도록 주의합니다.',
  ...
  check:{
    fills:[
      ...
      { tex:'(\\sqrt{15})^2 = \\square', answer:15,
        hint:{ ko:'제곱근을 다시 제곱하면 근호가 사라집니다', en:'Squaring a root cancels the root', zh:'平方根再平方，根号就消失' } }
    ],
    open:{ ko:'√((-9)²)의 값은 무엇일까요? -9가 아닌 이유도 설명해봅니다.',
      en:'What is √((-9)²)? Explain why the answer isn\'t -9.',
      zh:'√((-9)²)是多少？说说为什么答案不是-9。' },
    openHint:{ ko:'(-9)²=81, √81=9. 결과는 항상 0 이상이라 -9가 될 수 없습니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 절댓값이 되는 제곱근 문제를 모두 풉니다!', en:'Solve all the absolute-value square-root problems in 5 minutes!', zh:'5分钟内解答所有变成绝对值的平方根题目！' }
  },
