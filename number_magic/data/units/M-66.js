...
window.NM_UNITS['M-66'] = {
  ...
  subtitle:{ ko:'곱해서 0이 되려면 둘 중 하나는 0이어야 합니다', en:'For a product to be zero, one of the factors must be zero', zh:'乘积要等于0，两个因式中必有一个是0' },
  icon:'🎯',
  ...
  practice:{
    ...
    params:{mode:'factor'},
    intro:{
      ko:'인수분해한 다음, 각 괄호를 0으로 만드는 x를 찾으면 됩니다 — 작은 수부터 적습니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'두 수를 곱해서 0이 나왔습니다. 그럼 두 수 중 적어도 하나는 반드시 0입니다 — 5×7도, 0.001×0.001도 절대 0이 될 수 없기 때문입니다. 너무 당연해 보이는 이 사실 하나가 이차방정식을 푸는 열쇠입니다.',
      ...
      history:{ ko:'이차방정식은 4000년 전 바빌로니아 점토판에도 있습니다. 다만 그때는 음수를 수로 치지 않아 답이 하나뿐이었고, 문제도 "넓이가 이만큼인 밭의 변의 길이"처럼 늘 양수로 나오게 만들어져 있었습니다. 두 근을 모두 인정하게 된 건 훨씬 나중 일입니다.',
    ...
    stages:[
      { tag:{ko:'① 곱해서 0이면 둘 중 하나가 0',en:'1) A product of zero means one factor is zero',zh:'① 乘积为0就有一个因式为0'},
        ...
        desc:{ko:'더해서 −5, 곱해서 6이 되는 두 수는 −2와 −3입니다. 그래서 (x−2)(x−3)=0. 두 괄호의 곱이 0이니 <b>둘 중 하나는 0</b>이고, 그때의 x가 2와 3입니다.',
        ...
        result:{ko:'이차방정식의 근은 보통 두 개입니다!',en:'A quadratic usually has two roots!',zh:'一元二次方程通常有两个根！'},
        book:{ko:'인수분해에서 식을 쪼개는 것까지 했다면, 여기서는 그 쪼갠 식을 <b>=0으로 놓고 읽는</b> 한 걸음만 더 가는 거입니다. 두 근이 같아질 때(중근)도 있습니다 — (x−3)²=0이면 근은 3 하나뿐입니다.',
      ...
      { tag:{ko:'② 인수분해가 안 되면 완전제곱꼴로',en:'2) When it will not factor, complete the square',zh:'② 不能因式分解时就配方'},
        ...
        desc:{ko:'더해서 6, 곱해서 2가 되는 정수는 없습니다. 그럴 땐 x의 계수 6의 <b>절반인 3을 제곱</b>해 9를 더하고 다시 뺍니다. x²+6x+9는 (x+3)²이니 (x+3)²=9−2=7이 됩니다.',
        ...
        result:{ko:'제곱꼴로 만들면 제곱근으로 바로 풀립니다!',en:'Once it is a perfect square, a square root finishes it!',zh:'变成平方的形式后，开平方就能解出来！'},
        book:{ko:'이 방법을 ax²+bx+c=0에 그대로 해 보면 바로 <b>근의 공식</b>이 나옵니다. 근의 공식은 외워야 하는 새 규칙이 아니라, 완전제곱을 문자로 한 번 해 둔 결과입니다.',
    ...
    rule:{ ko:'곱해서 0이면 둘 중 하나가 0 — 인수분해해서 근을 읽고, 안 되면 완전제곱꼴로 고칩니다!',
  ...
  check:{
    ...
    open:{ ko:'x²+2x−8=0을 인수분해로 푸는 과정을 말해봅니다.',
  ...
  lab:{
    ...
    intro:{
      ko:'x의 계수의 절반을 제곱해서 더하고 빼면 (x+□)²=□ 꼴이 됩니다!',
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 공통인수를 묶어내고 두 근을 모두 찾습니다!', en:'Within 6 minutes, pull out the common factor and find both roots!', zh:'6分钟内提取公因数并找出两个根！' }
  },
