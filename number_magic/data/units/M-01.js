...
window.NM_UNITS['M-01'] = {
  ...
  subtitle:{ ko:'해발과 해저, 득점과 실점 — 0을 기준으로 반대 방향에 이름을 붙입니다', en:'Above and below sea level, points scored and lost — naming the two directions from 0', zh:'海拔与海拔以下，得分与失分——给0两侧的方向起名字' },
  icon:'🌋',
  symbols:[
  ...
  practice:{
    ...
    intro:{
      ko:'절댓값은 원점(0)에서 그 수까지의 거리입니다. 부호는 떼고 거리만 재봐!',
  ...
  discover:{
    ...
    stages:[
      { tag:{ko:'① 0을 기준으로 반대 방향',en:'1) Opposite directions from 0',zh:'① 以0为基准的两个方向'},
        ...
        desc:{ko:'0(해수면)을 기준으로 위로 500m 올라가면 <b>+500</b>, 아래로 500m 내려가면 <b>−500</b>입니다. 두 수는 부호만 다르고 0에서 떨어진 거리(절댓값)는 똑같이 500입니다. 축구에서 3득점은 <b>+3</b>, 3실점은 <b>−3</b>인 것도 같은 원리입니다.',
        ...
        mathSteps:[{ko:'+500\\,(\\text{해발}) \\;\\; -500\\,(\\text{해저})',en:'+500\\,(\\text{above sea level}) \\;\\; -500\\,(\\text{below sea level})',zh:'+500\\,(\\text{海拔}) \\;\\; -500\\,(\\text{海底})'}, '|+500| = 500', '|-500| = 500'],
        result:{ko:'부호는 다르지만 절댓값은 같습니다 — 방향만 반대일 뿐입니다!',en:'Different signs, same absolute value — only the direction is opposite!',zh:'符号不同，绝对值相同——只是方向相反！'},
        book:{ko:'양의 정수, 0, 음의 정수를 통틀어 <b>정수</b>라고 합니다. 양의 부호 +는 흔히 생략해서 그냥 500이라고 씁니다.',
      ...
      { tag:{ko:'② 수직선 — 오른쪽일수록 큰 수',en:'2) The number line — bigger to the right',zh:'② 数轴——越往右越大'},
        ...
        desc:{ko:'수직선 위에서는 오른쪽으로 갈수록 큰 수입니다. 그래서 <b>−1이 −3보다 큽니다</b> — 마이너스 수는 숫자가 클수록(절댓값이 클수록) 오히려 더 작습니다! 두 점 사이의 칸 수를 세면 거리도 알 수 있습니다.',
        ...
        mathSteps:['-3,\\,-1,\\,0,\\,2', '-3 < -1 < 0 < 2', {ko:'|-1 - (-3)| = 2\\,(\\text{칸})',en:'|-1 - (-3)| = 2\\,(\\text{steps})',zh:'|-1 - (-3)| = 2\\,(\\text{格})'}],
        result:{ko:'수직선을 그려보면 대소 비교와 거리가 한눈에 보입니다!',en:'Draw the number line and both comparison and distance become obvious!',zh:'画出数轴，大小比较和距离一目了然！'},
        book:{ko:'0을 원점이라 하고, 원점에서 어떤 수까지의 거리를 그 수의 <b>절댓값</b>이라고 합니다. 절댓값 기호는 |  |입니다.',
  ...
  check:{
    ...
    openHint:{ ko:'3이 더 큽니다(오른쪽). 거리는 |3-(-7)| = 10칸.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 수직선 위 거리 문제를 모두 풉니다!', en:'Solve all number-line distance problems in 5 minutes!', zh:'5分钟内解答所有数轴距离题！' }
  },
