...
window.NM_UNITS['M-32'] = {
  ...
  subtitle:{ ko:'두 점을 정해진 비율로 나누는 점의 좌표를 찾습니다', en:'Find the coordinates of a point that splits two points in a given ratio', zh:'求按指定比例分割两点的点的坐标' },
  icon:'📍',
  ...
  practice:{
    ...
    intro:{
      ko:'A(2,1), B(6,7)의 중점은? x끼리, y끼리 각각 더해 2로 나누면 ((2+6)/2, (1+7)/2)=(4,4)입니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'두 점을 정확히 반씩 나누는 점(중점)은 이미 압니다. 그런데 절반이 아니라 "2:1"처럼 특정 비율로 나누는 점은 어떻게 찾을까요?',
      ...
      history:{ ko:'중점은 "1:1로 나누는 점"의 특별한 경우입니다. 내분점 공식은 그걸 일반화해서 m:n 어떤 비율이든 다룰 수 있게 만들어졌습니다 — 먼 쪽 점에 가까운 쪽 비율을 곱해서 가중치를 준다는 아이디어입니다.',
    ...
    stages:[
      { tag:{ko:'① 중점: 더해서 2로 나누기',en:'1) Midpoint: add and divide by 2',zh:'① 中点：相加除以2'},
        ...
        desc:{ko:'x좌표끼리 더해 2로: (2+6)/2=<b>4</b>. y좌표끼리 더해 2로: (1+7)/2=<b>4</b>. 두 점의 딱 중간입니다.',
        ...
        result:{ko:'중점은 x끼리, y끼리 각각 평균 내면 됩니다!',en:'The midpoint just averages the x\'s and the y\'s separately!',zh:'中点就是x、y各自求平均！'},
        book:{ko:'중점 M=((x₁+x₂)/2, (y₁+y₂)/2) — 1:1로 나누는 가장 단순한 경우입니다.',
      ...
      { tag:{ko:'② 내분점: m:n 비율로 가중',en:'2) Internal division: weight by m:n',zh:'② 内分点：按m:n加权'},
        ...
        desc:{ko:'A(1,1), B(7,4)를 2:1로 내분하면: x=(1×1+2×7)/(2+1)=15/3=<b>5</b>, y=(1×1+2×4)/3=9/3=<b>3</b> → P(5,3). A쪽엔 먼 쪽 비율(n=1), B쪽엔 먼 쪽 비율(m=2)이 곱해집니다.',
        ...
        result:{ko:'m:n 내분점은 먼 쪽 점에 가까운 쪽 비율을 곱해 가중치를 줍니다!',en:'The internal division point weights each far point by the ratio on its near side!',zh:'m:n内分点是把每个点按对侧比例加权！'},
        book:{ko:'m=n이면 내분점 공식이 정확히 중점 공식으로 돌아갑니다 — 중점은 1:1 내분의 특수한 경우입니다.',
  ...
  check:{
    ...
    open:{ ko:'A(1,2), B(7,5)를 1:2로 내분하는 점의 좌표를 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 외분점 문제를 모두 풉니다! 분모가 (m-n)으로 바뀐다는 걸 기억합니다.', en:'Solve all the external-division problems in 5 minutes! Remember the denominator becomes (m-n).', zh:'5分钟内解答所有外分点题！记住分母变成(m-n)。' }
  },
