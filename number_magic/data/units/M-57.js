...
window.NM_UNITS['M-57'] = {
  ...
  practice:{
    ...
    params:{mode:'maxmin'},
    intro:{
      ko:'y=a·sin(bx)+c는 c를 중심으로 위아래로 a만큼 흔들립니다 — 최댓값은 a+c, 최솟값은 c-a입니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'그네를 탈 때 가장 높이 올라갔다가 가장 낮게 내려오는 걸 반복하죠? 이걸 그래프로 그리면 sin·cos 곡선과 똑같은 모양입니다. 얼마나 높이 흔들리는지(진폭)와 한 번 왕복하는 데 얼마나 걸리는지(주기)가 궁금해집니다.',
      ...
      history:{ ko:'π(파이)는 원의 둘레와 지름의 비입니다. sin·cos 곡선이 한 바퀴(360°) 돌아 처음 모양으로 되돌아오는 데 걸리는 x의 길이(주기)를 2π로 재는 건, 각도를 "호의 길이"로 재는 라디안 단위 덕분입니다.',
    ...
    stages:[
      { tag:{ko:'① 최댓값=a+c, 최솟값=c-a',en:'1) Max = a+c, min = c-a',zh:'① 最大值=a+c，最小值=c-a'},
        ...
        desc:{ko:'sin값은 항상 −1과 1 사이를 오갑니다. 그래서 3sin(2x)는 −3과 3 사이, 여기에 +1을 더하면 <b>중심이 1인 채로 위아래 3만큼</b> 흔들려요: 최댓값 1+3=4, 최솟값 1-3=−2.',
        ...
        book:{ko:'b(괄호 안 x의 계수)는 최댓값·최솟값에는 영향을 주지 않습니다 — b는 오직 주기(속도)만 바꿉니다.',
      ...
      { tag:{ko:'② 주기 = 2π÷b(sin·cos), π÷b(tan)',en:'2) Period = 2π÷b (sin/cos), π÷b (tan)',zh:'② 周期 = 2π÷b(sin·cos)，π÷b(tan)'},
        ...
        desc:{ko:'sin(x)는 한 바퀴 도는 데 x가 2π만큼 필요합니다. 그런데 sin(2x)는 x가 <b>2배 빠르게</b> 움직이니, 원래 걸리던 시간의 절반이면 충분해요: 2π÷2=π.',
        ...
        result:{ko:'b가 클수록 더 빨리 반복해서 주기는 짧아집니다(2π÷b)!',en:'A bigger b repeats faster, so the period gets shorter (2π÷b)!',zh:'b越大重复得越快，周期就越短(2π÷b)！'},
        book:{ko:'tan은 sin·cos와 그래프 모양 자체가 달라서 반 바퀴(π)만 돌아도 같은 모양이 반복됩니다 — 그래서 주기가 π÷b로, sin·cos의 절반 규칙과 다릅니다.',
    ...
    rule:{ ko:'최댓값=a+c, 최솟값=c-a(진폭이 중심에서 위아래로). 주기는 sin·cos가 2π÷b, tan은 π÷b — π의 몇 배인지 기약분수로 나타냅니다!',
  ...
  lab:{
    ...
    intro:{
      ko:'주기는 2π를 b로 나눈 값 — 기약분수로 π의 몇 배인지 나타냅니다!',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 최대최소·주기·tan까지 섞어서 모두 풉니다!', en:'Solve a mix of max/min, period, and tan — all within 5 minutes!', zh:'5分钟内混合解出最大最小值·周期·tan！' }
  },
