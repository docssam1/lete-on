...
window.NM_UNITS['M-30'] = {
  ...
  subtitle:{ ko:'같은 자리끼리 더하고, 행과 열을 짝지어 곱합니다', en:'Add matching positions, multiply by pairing rows with columns', zh:'相同位置相加，行与列配对相乘' },
  icon:'🔲',
  ...
  practice:{
    ...
    params:{mode:'addSub'},
    intro:{
      ko:'행렬은 수를 네모 칸에 정리해 담은 표입니다. 덧셈·뺄셈은 같은 자리(성분)끼리만 계산합니다 — 아주 단순합니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'표 두 개를 한꺼번에 더하거나 곱하고 싶을 때가 있습니다. 숫자를 네모 칸(2×2)에 정리한 "행렬"이라는 게 있는데, 이걸 어떻게 계산할까요?',
      ...
      history:{ ko:'덧셈·뺄셈은 그냥 같은 자리끼리 계산하면 끝이라 직관적입니다. 곱셈은 다릅니다 — 행렬곱은 "앞 행렬의 행"과 "뒤 행렬의 열"을 하나씩 짝지어 곱하고 더하는 특별한 규칙을 따릅니다. 처음엔 낯설어도 규칙만 외우면 기계적으로 계산할 수 있습니다.',
    ...
    stages:[
      { tag:{ko:'① 덧셈·뺄셈: 같은 자리끼리',en:'1) Addition/subtraction: entry by entry',zh:'① 加减法：相同位置分别运算'},
        ...
        desc:{ko:'왼쪽 위끼리(1+5=6), 오른쪽 위끼리(2+0=2), 왼쪽 아래끼리(3+1=4), 오른쪽 아래끼리(4+2=6) — 딱 <b>같은 자리끼리만</b> 더합니다.',
        ...
        book:{ko:'두 행렬의 크기(행·열의 수)가 같아야 덧셈·뺄셈이 가능합니다.',
      ...
      { tag:{ko:'② 행렬곱: 행과 열을 짝지어',en:'2) Matrix multiplication: pair row with column',zh:'② 矩阵乘法：行与列配对'},
        ...
        desc:{ko:'왼쪽 위 자리는 "앞의 1행"과 "뒤의 1열"을 짝지어요: 1×5+2×7=19. 오른쪽 위는 1행과 2열: 1×6+2×8=22 — 자리마다 짝이 다 다릅니다.',
        ...
        result:{ko:'각 자리는 앞 행과 뒤 열을 짝지어 곱하고 더합니다!',en:'Each entry pairs a row from the first with a column from the second, multiplies, and adds!',zh:'每个位置都用前者的行配后者的列，乘完再加！'},
        book:{ko:'i행 j열 성분 = (앞 행렬의 i행)과 (뒤 행렬의 j열)을 순서대로 곱해 더한 값입니다.',
  ...
  check:{
    fills:[
      { tex:'\\begin{pmatrix}2&1\\\\0&3\\end{pmatrix} + \\begin{pmatrix}1&4\\\\2&1\\end{pmatrix} = \\begin{pmatrix}\\square&\\square\\\\\\square&\\square\\end{pmatrix}', answer:[3,5,2,4],
        hint:{ ko:'같은 자리끼리 더합니다', en:'add matching positions', zh:'相同位置相加' } },
      { tex:'\\begin{pmatrix}1&0\\\\0&1\\end{pmatrix} \\times \\begin{pmatrix}3&2\\\\5&7\\end{pmatrix} = \\begin{pmatrix}\\square&\\square\\\\\\square&\\square\\end{pmatrix}', answer:[3,2,5,7],
        hint:{ ko:'단위행렬을 곱하면 원래 그대로 나옵니다', en:'multiplying by the identity matrix leaves it unchanged', zh:'乘以单位矩阵结果不变' } }
    ],
    open:{ ko:'첫째 줄이 (2 0), 둘째 줄이 (1 3)인 행렬에 첫째 줄이 (1 2), 둘째 줄이 (0 1)인 행렬을 곱하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    params:{mode:'matMul'},
    rule:{ ko:'5분 안에 행렬곱 문제를 모두 풉니다! 행과 열을 짝짓는 걸 잊지 맙니다.', en:'Solve all the matrix multiplication problems in 5 minutes! Don\'t forget to pair rows with columns.', zh:'5分钟内解答所有矩阵乘法题！别忘了行配列。' }
  },
