...
window.NM_UNITS['M-20'] = {
  ...
  subtitle:{ ko:'곱셈공식을 거꾸로 — 더해서 b, 곱해서 c가 되는 두 수를 찾습니다', en:'Multiplication formulas in reverse — find two numbers that add to b and multiply to c', zh:'把乘法公式反过来——找相加得b、相乘得c的两个数' },
  icon:'🔍',
  ...
  practice:{
    ...
    intro:{
      ko:'x²+5x+6을 (x+□)(x+□)로 되돌리려면, 더해서 5·곱해서 6이 되는 두 수를 찾으면 됩니다 — 2와 3!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'이전 유닛에서 (x+2)(x+3)을 전개하면 x²+5x+6이 된다는 걸 배웠습니다. 그럼 거꾸로, x²+5x+6이 주어졌을 때 원래 곱셈식으로 되돌릴 수 있을까요?',
      ...
      history:{ ko:'"인수분해"의 영어 factor는 라틴어로 "만드는 사람"이라는 뜻입니다. 곱셈을 만드는 재료(인수)를 찾아내는 일이라, 곱셈공식을 정확히 알수록 인수분해가 쉬워집니다 — 외운 공식을 거꾸로 읽는 연습인 셈입니다.',
    ...
    stages:[
      { tag:{ko:'① 더해서 b, 곱해서 c가 되는 두 수',en:'1) Two numbers that add to b and multiply to c',zh:'① 相加得b、相乘得c的两个数'},
        ...
        desc:{ko:'곱셈공식 (x+a)(x+b)=x²+(a+b)x+ab을 거꾸로 읽습니다. x²+7x+12에서 b=7, c=12니까, <b>더해서 7, 곱해서 12</b>가 되는 두 수를 찾으면 됩니다 — 3과 4입니다(3+4=7, 3×4=12).',
        ...
        result:{ko:'곱셈공식을 거꾸로 읽으면 인수분해가 됩니다!',en:'Reading the multiplication formula backward gives you factoring!',zh:'把乘法公式反着读就是因式分解！'},
        book:{ko:'x²+bx+c = (x+p)(x+q) (단, p+q=b, pq=c). 곱해서 c인 짝을 먼저 나열해보고, 그중 더해서 b가 되는 걸 고르는 게 요령입니다.',
      ...
      { tag:{ko:'② 부호가 섞이면 곱해서 음수',en:'2) Mixed signs — the product is negative',zh:'② 符号混合时，乘积为负'},
        ...
        desc:{ko:'곱해서 -15가 되려면 두 수의 부호가 <b>반드시 반대</b>입니다(음수×양수=음수). -15를 만드는 짝(-15·1, -5·3, -3·5, -1·15…) 중 더해서 -2가 되는 건 -5와 3입니다.',
        ...
        result:{ko:'곱이 음수면 부호가 반대인 두 수를 찾습니다!',en:'If the product is negative, look for two numbers with opposite signs!',zh:'乘积为负，就找符号相反的两个数！'},
        book:{ko:'곱 c가 양수면 두 수는 같은 부호, c가 음수면 서로 다른 부호입니다. 그 뒤 합 b의 부호로 어느 쪽이 더 큰지 정합니다.',
  ...
  check:{
    ...
    open:{ ko:'x²-9x+20을 인수분해하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    params:{level:'mixed'},
    rule:{ ko:'5분 안에 부호가 섞인 인수분해를 모두 풉니다!', en:'Solve all the mixed-sign factoring problems in 5 minutes!', zh:'5分钟内解答所有符号混合的因式分解题！' }
  },
