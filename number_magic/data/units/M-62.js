...
window.NM_UNITS['M-62'] = {
  ...
  practice:{
    ...
    intro:{
      ko:'위치함수 s(t)를 미분하면 속도 s\'(t)가 됩니다 — MD44에서 배운 미분을 그대로 "속도"라는 이름으로 씁니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'자동차의 위치를 시간의 함수 s(t)로 나타낼 수 있습니다. 그럼 "지금 이 순간의 속도"는 위치가 얼마나 빠르게 변하는지를 뜻하니, 바로 <b>s(t)의 도함수</b> s\'(t)입니다! 미분이 "속도계" 역할을 하는 거입니다.',
      ...
      history:{ ko:'뉴턴이 미분을 발명한 이유가 바로 이거입니다 — 행성의 위치가 시시각각 바뀌는데 "그 순간의 속도"를 구하고 싶었습니다. 그래서 미적분학은 원래 "운동을 다루는 수학"으로 탄생했습니다.',
    ...
    stages:[
      { tag:{ko:'① 위치를 미분하면 속도',en:'1) Differentiating position gives velocity',zh:'① 位置求导得速度'},
        ...
        desc:{ko:"s(t)=t²+3t를 미분하면 s'(t)=2t+3입니다. t=2일 때 속도는 2(2)+3=<b>7</b> — 이건 MD44에서 배운 도함수 계산과 똑같아요, 이름만 '속도'로 바뀐 거입니다.",
        ...
        book:{ko:'속도가 양수면 앞으로, 음수면 뒤로 움직이고 있다는 뜻입니다.',
      ...
      { tag:{ko:'② 속도를 적분하면 이동 거리',en:'2) Integrating velocity gives distance',zh:'② 速度积分得移动距离'},
        ...
        desc:{ko:'미분의 반대인 <b>적분</b>을 쓰면, 속도로부터 거꾸로 이동 거리를 구할 수 있습니다(MD46의 정적분 그대로). 원시함수 S(t)=2t²+2t, S(3)-S(0)=(18+6)-0=24.',
        ...
        book:{ko:'속도가 도중에 음수가 되면(뒤로 갈 때) "이동 거리"와 "위치 변화량"이 달라집니다 — 이 유닛은 속도가 항상 0 이상인 경우만 다룹니다.',
  ...
  check:{
    ...
    open:{ ko:'v(t)=6t+2일 때 t=0부터 t=3까지 이동 거리를 구하는 과정을 설명해봅니다.',
  ...
  lab:{
    ...
    intro:{
      ko:'속도를 0부터 T까지 적분하면 이동 거리가 나옵니다!',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 구간 [t1,t2]의 이동 거리까지 모두 구합니다!', en:'Find the distance over the interval [t1,t2], all within 5 minutes!', zh:'5分钟内求出区间[t1,t2]的移动距离！' }
  },
