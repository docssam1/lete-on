...
window.NM_UNITS['M-67'] = {
  ...
  subtitle:{ ko:'제곱은 음수가 될 수 없어서 꺾이는 자리가 생깁니다', en:'A square can never be negative — that is where the curve turns', zh:'平方不可能为负，所以才有转折的地方' },
  icon:'⛰️',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'공을 위로 던지면 올라가다가 어느 순간 멈추고 내려옵니다. 그 "멈추는 한 점"이 꼭짓점입니다. 높이를 식으로 쓰면 언제나 이차식이고, (x−p)²이 0이 되는 자리 — 즉 x=p에서 가장 높습니다. 제곱은 절대 음수가 될 수 없기 때문입니다.',
      ...
      history:{ ko:'포물선은 기원전 3세기 아폴로니오스가 원뿔을 비스듬히 자른 단면으로 연구했습니다. 그로부터 1900년 뒤 갈릴레이가 던진 물체의 길이 정확히 그 포물선임을 밝혔고, 다시 포물면 거울이 빛을 한 점에 모은다는 성질이 오늘날 위성 안테나와 자동차 전조등에 쓰입니다.',
    ...
    stages:[
      { tag:{ko:'① 표준형에서 그대로 읽기 — 부호는 반대로',en:'1) Read it off the vertex form — flip the inner sign',zh:'① 从标准式直接读出——符号反过来'},
        ...
        desc:{ko:'(x−3)²은 절대 음수가 못 되고, x=3일 때 딱 0으로 가장 작아집니다. 그때 y는 남은 2입니다. 괄호 안이 <b>−3인데 꼭짓점의 x는 +3</b>이라는 점을 꼭 기억합니다.',
        ...
        result:{ko:'꼭짓점은 (3, 2), 최솟값은 2입니다!',en:'The vertex is (3, 2) and the minimum value is 2!',zh:'顶点是(3, 2)，最小值是2！'},
        book:{ko:'x²의 계수가 양수면 아래로 볼록해서 꼭짓점이 <b>최솟값</b>, 음수면 위로 볼록해서 <b>최댓값</b>입니다. 계수의 크기는 폭을 정합니다 — 클수록 좁아집니다.',
      ...
      { tag:{ko:'② 일반형은 완전제곱으로 고쳐서',en:'2) Complete the square to convert the general form',zh:'② 一般式要先配方'},
        ...
        desc:{ko:'x의 계수 −6의 절반은 −3, 제곱하면 9입니다. 9를 더하고 다시 빼면 (x−3)²+2가 됩니다. 결국 ①과 <b>똑같은 그래프</b>였던 거입니다 — 모양만 다르게 적혀 있었을 뿐입니다.',
        ...
        result:{ko:'일반형과 표준형은 같은 그래프의 두 가지 표기입니다!',en:'General form and vertex form are two ways of writing the same graph!',zh:'一般式和标准式是同一条图象的两种写法！'},
        book:{ko:'x²의 계수가 1이 아니면 <b>먼저 묶어냅니다</b>. y=2x²−12x+20은 2(x²−6x)+20 → 2(x−3)²+2입니다. 괄호 밖으로 나갈 때 9가 아니라 2×9=18이 빠져나간다는 데 주의합니다.',
    ...
    rule:{ ko:'y=a(x−p)²+q의 꼭짓점은 (p, q) — 괄호 안의 부호는 반대로 읽고, 일반형은 완전제곱으로 고칩니다!',
  ...
  check:{
    fills:[
      ...
      { tex:{ko:'y = x^2 - 4x + 7 \\quad\\Rightarrow\\quad \\text{꼭짓점의 } y = \\square',
        ...
        hint:{ ko:'(x−2)²+3으로 고칩니다', en:'Rewrite it as (x−2)²+3', zh:'改写成(x−2)²+3' } }
    ],
    open:{ ko:'y=x²+8x+19의 꼭짓점을 구하는 과정을 말해봅니다.',
  ...
  lab:{
    ...
    intro:{
      ko:'x의 계수의 절반이 p입니다 — 부호는 반대라는 걸 잊지 마세요!',
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 x²의 계수를 묶어내고 꼭짓점을 찾습니다!', en:'Within 6 minutes, factor out the leading coefficient and find the vertex!', zh:'6分钟内提取x²的系数并找出顶点！' }
  },
