...
window.NM_UNITS['M-40'] = {
  ...
  subtitle:{ ko:'같은 수를 계속 더해가는 수열입니다', en:'A sequence made by adding the same number again and again', zh:'不断加上同一个数得到的数列' },
  icon:'🪃',
  ...
  practice:{
    ...
    params:{mode:'nthTerm'},
    intro:{
      ko:'2,5,8,11,… 처럼 3씩 늘어나는 수열입니다. aₙ=a₁+(n-1)d에 대입만 하면 몇 번째 항인지 알 수 있습니다.',
  ...
  discover:{
    story:{
      hook:{ ko:'수를 일정한 간격으로 늘어놓으면 100번째 수를 하나씩 세지 않고 바로 말할 수 있습니다. 어떻게요?',
      ...
      history:{ ko:'가장 오래된 수열 문제는 약 4,000년 전 이집트의 린드 파피루스에 있습니다 — 곡물을 다섯 사람에게 일정한 차이로 나누는 문제였죠. 등차수열은 교과서보다 곡식 창고에서 먼저 쓰였습니다.',
    ...
    stages:[
      { tag:{ko:'① 일반항 aₙ=a₁+(n-1)d',en:'1) The general term aₙ=a₁+(n-1)d',zh:'① 通项公式aₙ=a₁+(n-1)d'},
        ...
        desc:{ko:'첫째항 2에서 5번째 항까지 가려면 3씩 <b>4번</b> 더해야 합니다(항번호-1번). 2+3×4=14. 이걸 공식으로 쓰면 aₙ=a₁+(n-1)d — "몇 번 더했는지(n-1)"만큼 공차 d를 곱해서 첫째항에 더하는 거입니다.',
        ...
        result:{ko:'몇 번째 항인지 알면 (항번호-1)번만큼 공차를 더합니다!',en:'For any term number, add the common difference (term number − 1) times!',zh:'知道项数，就把公差加(项数-1)次！'},
      ...
      { tag:{ko:'② 두 항으로 규칙 찾기, 그리고 합',en:'2) Finding the rule from two terms, and the sum',zh:'② 由两项求规律，以及求和'},
        ...
        desc:{ko:'2번째 항에서 5번째 항까지는 3칸 차이(5-2=3). 그동안 7에서 16으로 9만큼 늘었으니 한 칸당 9÷3=<b>3</b>이 공차입니다. a₁은 2번째 항 7에서 거꾸로 한 번 빼면 4. 합 Sₙ=n(2a₁+(n-1)d)÷2는 첫째항과 공차를 알면 그대로 대입해서 구합니다.',
        ...
        book:{ko:'무지개 덧셈법(1~n의 합=n(n+1)÷2)도 사실 a₁=1, d=1인 등차수열의 합이었던 거입니다 — 이미 알던 마법이 여기서 이름을 얻었습니다.',
    ...
    rule:{ ko:'aₙ=a₁+(n-1)d, Sₙ=n(2a₁+(n-1)d)÷2 — d=(뒤 항-앞 항)÷(항번호 차)로 거꾸로도 구합니다.',
  ...
  check:{
    ...
    open:{ ko:'a₃=10, a₆=19인 등차수열의 a₁과 d를 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 등차수열의 합을 모두 구합니다!', en:'Find all the arithmetic series sums in 5 minutes!', zh:'5分钟内求出所有等差数列的和！' }
  },
