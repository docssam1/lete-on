...
window.NM_UNITS['M-53'] = {
  ...
  subtitle:{ ko:'log의 정의와 성질로 x를 찾습니다', en:'Find x using the definition and properties of log', zh:'用log的定义和性质求x' },
  icon:'📜',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'log_a N = k는 a^k = N이라는 뜻 — 이 정의만 기억하면 x가 들어 있는 진수(안쪽 값)도 찾을 수 있습니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'log₂(2x+1)=3이라는 식은 얼핏 복잡해 보이지만, log의 정의를 떠올리면 간단합니다. "2를 3번 곱하면 (2x+1)이 된다"는 뜻이니, 2x+1=2³=8이 됩니다.',
      ...
      history:{ ko:'로그방정식은 log가 "지수를 구하는 함수"라는 것만 알면 다 지수방정식으로 바꿔 풀 수 있습니다. 실제로 log와 지수는 서로의 반대 방향(역함수) 관계입니다.',
    ...
    stages:[
      { tag:{ko:'① log의 정의로 x를 찾습니다',en:'1) Use the definition of log to find x',zh:'① 用log的定义求x'},
        ...
        desc:{ko:'log₂(2x+1)=3은 정의에 따라 <b>2x+1=2³=8</b>이라는 뜻입니다. 이제 일차방정식 2x+1=8만 풀면 됩니다. (실제 문항은 항상 정수 x가 나오도록 값이 골라져 있습니다.)',
        ...
        result:{ko:'log_a N=k는 항상 a^k=N으로 바꿔서 풉니다!',en:'Always rewrite log_a N=k as a^k=N to solve!',zh:'log_a N=k总是改写成a^k=N来解！'},
        book:{ko:'log 안의 값(진수)은 항상 0보다 커야 합니다 — 이걸 "진수 조건"이라고 부릅니다.',
      ...
      { tag:{ko:'② 로그의 덧셈 성질로 합쳐서 풉니다',en:'2) Combine with the addition property of logs',zh:'② 用对数的加法性质合并求解'},
        ...
        desc:{ko:'log_a X + log_a Y = log_a(XY) 성질을 쓰면 왼쪽이 log₂(3x)가 됩니다. 그러면 log₂(3x)=log₂12 — <b>양쪽 진수가 같아야</b> 하니 3x=12, x=4.',
        ...
        result:{ko:'같은 밑의 log끼리 값이 같으면, 진수끼리도 같습니다!',en:'When logs with the same base are equal, their arguments are equal too!',zh:'同底的log值相等，真数也相等！'},
        book:{ko:'로그의 뺄셈 성질(log_a X - log_a Y = log_a(X÷Y))도 같은 방식으로 나눗셈으로 바꿔서 풉니다.',
    ...
    rule:{ ko:'로그방정식: log_a N=k는 a^k=N으로, 로그끼리의 덧뺄셈은 곱나눗으로 바꿔서 x를 찾습니다!',
  ...
  check:{
    ...
    open:{ ko:'log₅(3x-1)=2를 log의 정의를 써서 설명해봅니다.',
      en:'Explain log₅(3x-1)=2 using the definition of log.',
      zh:'用log的定义说说log₅(3x-1)=2。' },
    openHint:{ ko:'3x-1=5^2=25, x=26/3은 예시일 뿐 — 실제 문항은 정수해가 나오도록 값이 골라져 있습니다',
  ...
  lab:{
    ...
    intro:{
      ko:'로그의 덧셈 성질로 두 로그를 하나로 합친 뒤 진수끼리 비교합니다!',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 로그의 뺄셈 성질까지 활용해서 모두 풉니다!', en:'Use the subtraction property too and solve them all within 5 minutes!', zh:'5分钟内连减法性质一起用上，全部解出！' }
  },
