...
window.NM_UNITS['M-38'] = {
  ...
  subtitle:{ ko:'로그의 덧뺄셈은 진수의 곱나눗이 됩니다', en:'Adding/subtracting logs multiplies/divides the arguments', zh:'对数的加减对应真数的乘除' },
  icon:'🔗',
  ...
  practice:{
    ...
    intro:{
      ko:'log₂4 + log₂8 = log₂(4×8) = log₂32! 두 로그를 더하면 안의 수를 곱합니다.',
  ...
  discover:{
    story:{
      hook:{ ko:'곱셈을 덧셈으로 바꿀 수 있다면 얼마나 편할까요? 로그가 바로 그 일을 합니다.',
        en:'What if multiplication could be turned into addition? That is exactly what a logarithm does.',
        zh:'如果乘法能变成加法该多方便？对数做的正是这件事。' },
      history:{ ko:'네이피어(1550~1617)는 곱셈과 나눗셈의 번거로움을 덜려고 로그를 만들었습니다. 오늘날에는 계산기로 곱하니 로그표를 쓸 일은 없지만, 로그 자체는 자연과학·사회과학까지 퍼져 지금도 계속 넓어지고 있습니다.',
    ...
    stages:[
      { tag:{ko:'① 로그의 덧셈 = 진수의 곱셈',en:'1) Adding logs = multiplying arguments',zh:'① 对数相加=真数相乘'},
        ...
        desc:{ko:'log₂4=2(2를 2번), log₂8=3(2를 3번)이니까 더하면 2+3=5번. 2를 5번 곱한 건 log₂32=5(2⁵=32)와 같습니다. 즉 <b>지수끼리 더하는 것</b>이 로그에서는 <b>진수끼리 곱하는 것</b>으로 보입니다.',
        ...
        book:{ko:'지수법칙 aᵐ×aⁿ=aᵐ⁺ⁿ이 로그의 옷을 입으면 이 덧셈 성질이 됩니다 — 같은 마법, 다른 표기입니다.',
      ...
      { tag:{ko:'② 뺄셈은 나눗셈, 밑변환은 정수만',en:'2) Subtracting is dividing; only clean change-of-base',zh:'② 相减是相除，只处理整数换底'},
        ...
        desc:{ko:'32÷4=8이니까 log₂32-log₂4=log₂8(5-2=3, log₂8=3와 일치!)이 됩니다. 밑이 다른 log₉81 같은 경우도 9=3²처럼 밑을 3의 거듭제곱으로 볼 수 있으면, 3을 몇 번 곱해야 81이 되는지 세고 2로 나눠 정수로 딱 떨어지게 만들 수 있습니다.',
        ...
        book:{ko:'로그의 곱셈·나눗셈은 다음 유닛(Σ)처럼 "이미 아는 걸 새 기호로 다시 쓰는 것"의 또 다른 예입니다.',
  ...
  check:{
    ...
    open:{ ko:'log₅25 + log₅5의 값을 두 가지 방법(각각 구해서 더하기 / 곱해서 한 번에 구하기)으로 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 밑변환이 정수로 떨어지는 문제를 모두 풉니다!', en:'Solve all the change-of-base problems that land on integers in 5 minutes!', zh:'5分钟内解答所有换底后为整数的题目！' }
  },
