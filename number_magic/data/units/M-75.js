...
window.NM_UNITS['M-75'] = {
  ...
  subtitle:{ ko:'두 직선이 만나는 점이 곧 연립방정식의 해입니다', en:'Where two lines meet is the solution of the system', zh:'两直线的交点就是方程组的解' },
  icon:'✖️',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'연립방정식을 배울 때는 식만 두 줄 썼습니다. 그런데 그 두 식은 각각 <b>직선 하나</b>입니다. 두 식을 동시에 만족하는 (x, y)란 두 직선이 <b>함께 지나는 점</b>, 곧 교점입니다. 그래서 해가 하나면 두 직선이 한 점에서 만나고, 해가 없으면 평행하고, 해가 무수히 많으면 아예 같은 직선입니다 — 대수와 기하가 여기서 만납니다.', en:'When you learned systems you just wrote two lines of algebra. But each of those equations is <b>a line</b>, and an (x, y) satisfying both is the point the two lines <b>share</b> - their intersection. One solution means they cross at a point, no solution means they are parallel, and infinitely many means they are the same line. Algebra and geometry meet right here.', zh:'学方程组时只是写了两行式子。但那两个式子各自就是<b>一条直线</b>，同时满足两式的(x, y)就是两直线<b>共有的点</b>，即交点。所以有唯一解就是相交于一点，无解就是平行，有无数解就是同一条直线——代数和几何在这里相遇。' },
      history:{ ko:'이 발상을 밀고 나가면 "식을 푸는 것"과 "그림에서 만나는 곳을 찾는 것"이 같은 일이 됩니다. 데카르트가 좌표를 만든 뒤 수학이 크게 달라진 이유가 이것입니다. 오늘날 컴퓨터 그래픽에서 두 선이 부딪히는지 검사하는 것도 결국 이 연립방정식을 푸는 일입니다.', en:'Follow the idea through and solving equations becomes the same task as finding where two drawings meet. That is why mathematics changed so much once Descartes introduced coordinates. Even today, checking whether two lines collide in computer graphics comes down to solving this same system.', zh:'把这个想法推下去，"解式子"和"找图形相交的地方"就成了同一件事。这正是笛卡儿引入坐标后数学大变样的原因。今天计算机图形学里检测两条线是否相撞，归根到底也是解这个方程组。' }
    },
    stages:[
      { tag:{ ko:'① 일반형은 사실 일차함수', en:'1) The general form really is a linear function', zh:'① 一般式其实就是一次函数' },
        ...
        desc:{ ko:'y에 대해 풀면 기울기와 y절편이 바로 보입니다. 모양만 다를 뿐 <b>같은 직선</b>입니다 — 일반형은 x와 y를 대등하게 쓴 표기이고, y=ax+b는 y를 주인공으로 세운 표기입니다.', en:'Solve for y and the slope and intercept appear. It is <b>the same line</b>, written differently: the general form treats x and y evenly, while y=ax+b puts y in the leading role.', zh:'解出y后斜率和y截距一目了然。只是写法不同，<b>是同一条直线</b>——一般式把x和y平等对待，y=ax+b则以y为主角。' },
        mathSteps:['-y = -2x + 3', 'y = 2x - 3'],
        result:{ ko:'식의 모양이 달라도 그리면 같은 직선!', en:'Different forms, same line when you draw it!', zh:'写法不同，画出来是同一条直线！' },
        book:{ ko:'by=... 에서 y의 계수 b로 나눌 때, b가 <b>음수면 모든 항의 부호가 바뀝니다</b>. 여기서 실수가 가장 많이 나옵니다 — 나눈 뒤 한 번 더 확인하세요.', en:'When you divide by the coefficient of y, a <b>negative</b> coefficient flips the sign of every term. This is where most slips happen - check once more after dividing.', zh:'除以y的系数时，系数为<b>负数</b>会让每一项都变号。这里最容易出错——除完后再核对一遍。' } },
      ...
      { tag:{ ko:'② 교점 = 연립방정식의 해', en:'2) The intersection is the solution', zh:'② 交点 = 方程组的解' },
        ...
        desc:{ ko:'교점에서는 x도 y도 두 식에서 같습니다. y끼리 같다고 놓으면 2x−3=−x+3이라 x=2, 그 x를 아무 식에나 넣으면 y=1입니다. 그림으로 보면 두 직선이 (2, 1)에서 <b>실제로 만납니다</b>.', en:'At the intersection both x and y agree in the two equations. Setting the y expressions equal gives 2x-3 = -x+3, so x = 2, and either equation then gives y = 1. On the graph the two lines <b>really do cross</b> at (2, 1).', zh:'在交点处x和y在两个式子里都相同。令两个y相等得2x−3=−x+3，x=2，代入任一式得y=1。画出来两条直线<b>确实相交</b>于(2, 1)。' },
        ...
        book:{ ko:'기울기가 같고 y절편이 다르면 <b>평행</b>이라 해가 없습니다. 기울기도 y절편도 같으면 같은 직선이라 해가 무수히 많습니다 — 연립방정식의 세 경우가 그림으로 전부 설명됩니다.', en:'Equal slopes with different intercepts means <b>parallel</b>, so there is no solution; equal slopes and equal intercepts means one line, so there are infinitely many. All three cases for a system are explained by the picture.', zh:'斜率相同而y截距不同就<b>平行</b>，无解；斜率和y截距都相同就是同一条直线，有无数解——方程组的三种情况全都能用图说明。' } }
    ],
    rule:{ ko:'일반형도 직선이고, 두 직선의 교점이 연립방정식의 해입니다 — 식과 그림이 같은 것을 말합니다!', en:'The general form is a line too, and where two lines meet is the solution of the system - the algebra and the picture say the same thing!', zh:'一般式也是直线，两直线的交点就是方程组的解——式子和图说的是同一件事！' }
  },
  ...
  check:{
    ...
    open:{ ko:'두 직선이 평행하면 연립방정식의 해가 왜 없는지 말해봅니다.', en:'Explain why parallel lines mean the system has no solution.', zh:'说说两直线平行时方程组为什么无解。' },
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 일반형을 고쳐 기울기를 맞춥니다!', en:'Within 6 minutes, rewrite the general form and match the slopes!', zh:'6分钟内改写一般式并配平斜率！' }
  },
