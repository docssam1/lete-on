/* Numbers of Magic — 유닛 C-14: 창살격자 곱셈 (중급 C 챕터6) */
(function(){
'use strict';
...
window.NM_UNITS['C-14'] = {
  ...
  title:{ ko:'창살격자 곱셈', en:'Lattice Multiplication', zh:'格子乘法' },
  subtitle:{ ko:'격자를 그리고 대각선으로 더하는 옛 마법사의 방법!', en:'Draw a lattice and add along the diagonals — the old wizards\' method!', zh:'画格子沿对角线相加——古代魔法师的方法！' },
  icon:'🏛️',
  ...
  practice:{
    ...
    params:{},
    intro:{
      ko:'큰 수 곱셈을 격자에 나눠 담고 대각선으로 더하는 창살격자법이야. 종이에 격자를 그리며 풀어봐! 준비됐지?',
      en:"Lattice multiplication stores each little product in a grid, then adds along diagonals. Draw the grid on paper as you solve! Ready?",
  ...
  discover:{
    ...
    stages:[
      { tag:{ko:'① 격자 그리기',en:'1) Drawing the lattice',zh:'① 画格子'},
        head:{ko:'234×56: 가로 3칸 × 세로 2칸 격자!',en:'234×56: a 3-wide, 2-tall lattice!',zh:'234×56：3列×2行的格子！'},
        desc:{ko:'수백 년 전 마법사(수학자)들이 쓰던 방법이에요! 234×56이면 <b>가로 3칸(2,3,4), 세로 2칸(5,6)</b> 격자를 그려요. 각 칸에 <b>대각선(창살)</b>을 긋고, 행×열의 곱을 써 넣어요: 십의 자리는 대각선 위, 일의 자리는 아래. 예: 3×5=15면 위에 1, 아래에 5.',
              en:'Mathematicians used this centuries ago! For 234×56, draw a lattice <b>3 columns wide (2,3,4) and 2 rows tall (5,6)</b>. Slash each cell with a <b>diagonal</b>, then fill in each row×column product: tens above the slash, ones below. E.g. 3×5=15 → 1 above, 5 below.',
      ...
      { tag:{ko:'② 대각선으로 더하기',en:'2) Adding along the diagonals',zh:'② 沿对角线相加'},
        ...
        book:{ko:'창살격자의 좋은 점: 곱하기 단계와 더하기 단계가 완전히 분리돼요. 곱할 때는 곱셈만, 더할 때는 덧셈만 — 머릿속이 한 번에 한 가지 일만 해요.',
              en:'The lattice\'s gift: multiplying and adding are fully separated. While multiplying you only multiply; while adding you only add — the brain does one job at a time.',
              zh:'格子法的好处：乘和加完全分开。乘的时候只乘，加的时候只加——大脑一次只做一件事。'} },

      { tag:{ko:'③ 234×56 완주하기',en:'3) Completing 234×56',zh:'③ 完成234×56'},
        head:{ko:'격자로 234×56 = 13104',en:'By lattice: 234×56 = 13104',zh:'用格子：234×56 = 13104'},
        ...
        result:{ko:'234×56=13104! 격자가 여섯 곱을 정리해 줬어요.',en:'234×56=13104! The lattice organised all six products.',zh:'234×56=13104！格子整理好了六个积。'},
        book:null }
    ],
    rule:{ ko:'① 자릿수만큼 격자 그리고 대각선 긋기  ② 칸마다 곱을 십/일로 나눠 적기  ③ 오른쪽 아래 대각선부터 더하고 올림',
      en:'① Draw the lattice with a slash in each cell  ② Fill each cell\'s product as tens/ones  ③ Add diagonals from the bottom-right, carrying',
  ...
  check:{
    ...
    open:{ ko:'창살격자 곱셈에서 대각선 줄이 왜 같은 자리끼리 모이는지 설명해 봐요.',
      en:'Explain why each diagonal band in lattice multiplication collects digits of the same place value.',
  ...
  lab:{
    ...
    params:{},
    intro:{
      ko:'창살격자 마법! 종이에 격자를 그리고 대각선으로 더해봐.',
      en:'Lattice magic! Draw the grid on paper and add the diagonals.',
  ...
  arena:{
    ...
    params:{},
    rule:{ ko:'7분 안에 격자 곱셈 문제를 모두 풀어요!', en:'Solve all lattice problems in 7 minutes!', zh:'7分钟内解答所有格子乘法题！' }
  },

  stamp:{ label:{ ko:'격자 마법사', en:'Lattice Wizard', zh:'格子魔法师' }, coins:30 },

  voice:{
    correct:[ {ko:'정답이야! ✨',en:'Correct!',zh:'答对了！'}, {ko:'격자 완성! 🏛️',en:'Lattice complete!',zh:'格子完成！'}, {ko:'대단해! 🌟',en:'Amazing!',zh:'太棒了！'} ],
    ...
    finish:{ ko:'완벽해! 격자 마법사! 🏛️✨', en:'Perfect! Lattice Wizard!', zh:'完美！格子魔法师！' }
  }
};
