...
window.NM_UNITS['M-27'] = {
  ...
  subtitle:{ ko:'근을 구하지 않고도 계수만 보면 두 근의 합과 곱을 압니다', en:'You don\'t need to solve — the coefficients alone reveal the sum and product', zh:'不用求根，只看系数就知道两根之和与积' },
  icon:'🌈',
  ...
  practice:{
    ...
    intro:{
      ko:'x²-5x+6=0의 두 근은 2와 3입니다. 2+3=5=-(-5), 2×3=6 — 계수 -5, 6과 정확히 연결돼 있습니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'무지개 덧셈법에서 두 수의 합과 곱만 알면 그 두 수를 다룰 수 있다는 걸 배웠습니다. 이차방정식의 두 근도 "어떤 두 수"인데, 근을 직접 구하지 않고 합과 곱만 바로 알 수 있을까요?',
      ...
      history:{ ko:'무지개 덧셈법 → 차가 2인 두 수의 곱 → 평균값 곱셈 → 합차공식으로 이어진 여정은 늘 "두 수를 합과 곱으로 다루는" 것이었습니다. 근과 계수의 관계는 그 여정의 마지막 걸음 — 이번엔 그 두 수가 방정식의 근이라는 것만 다를 뿐입니다.',
    ...
    stages:[
      { tag:{ko:'① α+β=-b, αβ=c(a=1일 때)',en:'1) α+β=-b, αβ=c (when a=1)',zh:'① α+β=-b，αβ=c(a=1时)'},
        ...
        desc:{ko:'x²-5x+6=(x-α)(x-β)로 전개하면 x²-(α+β)x+αβ가 됩니다. 원래 식과 비교하면 <b>α+β=5, αβ=6</b> — 실제 근 2, 3과 정확히 맞습니다(2+3=5, 2×3=6).',
        ...
        result:{ko:'x²+bx+c=(x-α)(x-β)를 전개해서 계수와 바로 비교합니다!',en:'Expand x²+bx+c=(x-α)(x-β) and compare with the coefficients directly!',zh:'展开x²+bx+c=(x-α)(x-β)，直接和系数比较！'},
        book:{ko:'x²+bx+c=0의 두 근 α,β는 α+β=-b, αβ=c를 항상 만족합니다.',
      ...
      { tag:{ko:'② 합·곱으로 다른 식도 구하기',en:'2) Use sum & product for other expressions',zh:'② 用和与积求其他式子'},
        ...
        result:{ko:'(α+β)²를 펼쳐 보면 α²+β²을 합·곱만으로 뽑아낼 수 있습니다!',en:'Expand (α+β)² and you can extract α²+β² from just the sum and product!',zh:'展开(α+β)²就能只用和与积求出α²+β²！'},
        book:{ko:'대칭식(α,β를 바꿔도 값이 같은 식)은 항상 합과 곱만으로 다시 쓸 수 있습니다.',
  ...
  check:{
    ...
    open:{ ko:'x²-4x+1=0의 두 근 α,β에 대해 α²+β²의 값을 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 α²+β² 구하는 문제를 모두 풉니다!', en:'Solve all the α²+β² problems in 5 minutes!', zh:'5分钟内解答所有求α²+β²的题！' }
  },
