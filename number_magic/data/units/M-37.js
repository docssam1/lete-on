...
window.NM_UNITS['M-37'] = {
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'2×2×2=8이라는 걸 압니다. 그럼 "2를 몇 번 곱해야 8이 될까?"라는 질문에는 어떻게 답을 적을까요? 지수 사다리를 거꾸로 오르는 새 이름표가 필요합니다 — 그게 바로 <b>log</b>입니다.',
      ...
      history:{ ko:'로그는 계산기가 없던 시절 천문학자들의 골칫거리(엄청 큰 수의 곱셈)를 덧셈으로 바꿔주는 발명품이었습니다. 네이피어가 1614년에 처음 만들었고, "logos(비율)"와 "arithmos(수)"를 합쳐 이름 붙였습니다.',
    ...
    stages:[
      { tag:{ko:'① log는 지수 사다리를 거꾸로 읽는 것',en:'1) log reads the exponent ladder backward',zh:'① log是反过来读指数梯子'},
        ...
        desc:{ko:'지수 사다리에서 2¹=2, 2²=4, 2³=8이었죠. "2를 3번 곱하면 8"이라는 이 사실을 <b>log₂8=3</b>이라고 적습니다. 밑(아래 작은 수) 2는 "몇을 곱했는지", 진수(오른쪽 수) 8은 "결과", 값 3은 "몇 번 곱했는지"입니다.',
        ...
        result:{ko:'log_a N = x는 aˣ=N을 다르게 적은 것뿐입니다!',en:'log_a N = x is just another way to write aˣ=N!',zh:'log_a N = x只是aˣ=N的另一种写法！'},
        book:{ko:'log_a N = x ⟺ a^x = N (a>0, a≠1, N>0) — 이 동치 관계가 로그의 정의입니다.',
      ...
      { tag:{ko:'② 세 자리의 이름 — 밑·진수·값',en:'2) Three positions: base, argument, value',zh:'② 三个位置——底数·真数·值'},
        ...
        desc:{ko:'5×5×5=125이니까 log₅125=3입니다. 여기서 <b>밑(5)</b>은 "반복해서 곱하는 수", <b>진수(125)</b>는 "곱해서 나온 결과", <b>값(3)</b>은 "몇 번 곱했는지"를 뜻합니다. 이 세 자리를 헷갈리지 않는 게 로그를 다루는 첫걸음입니다.',
        ...
        book:{ko:'a=1이거나 a≤0이면 로그를 정의할 수 없습니다 — 1을 아무리 곱해도 1이고, 음수는 사다리를 오를수록 부호가 튀기 때문입니다.',
    ...
    rule:{ ko:'log_a N = x ⟺ a^x = N — 밑(a)을 x번 곱하면 진수(N). 지수 사다리를 거꾸로 읽는 것뿐입니다!',
  ...
  check:{
    ...
    open:{ ko:'log₄64는 얼마인지, "몇 번 곱하는가"의 관점으로 설명해봅니다.',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 더 큰 범위의 로그값을 모두 구합니다!', en:'Find all the wider-range log values in 5 minutes!', zh:'5分钟内求出所有更大范围的对数值！' }
  },
