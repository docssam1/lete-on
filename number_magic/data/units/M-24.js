...
window.NM_UNITS['M-24'] = {
  ...
  subtitle:{ ko:'나눗셈을 다 하지 않아도 대입 한 번으로 나머지를 압니다', en:'One substitution finds the remainder — no long division needed', zh:'不用做完整除法，代入一次就知道余数' },
  icon:'🎯',
  ...
  practice:{
    ...
    params:{mode:'direct'},
    intro:{
      ko:'P(x)를 (x-2)로 나눈 나머지가 궁금하다면? 나눗셈을 안 하고 그냥 P(2)만 계산하면 바로 나옵니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'P(x)=x²+3x-1을 (x-2)로 나눈 나머지가 궁금합니다. 조립제법으로 나눌 수도 있지만, 더 빠른 지름길이 있다고 합니다 — 나눗셈을 아예 안 하고 나머지를 구할 수 있을까요?',
      ...
      history:{ ko:'"나머지 = P(2)"라는 사실은 나눗셈의 구조 자체에서 나옵니다. P(x)=(x-2)×(몫)+나머지인데, x에 2를 넣으면 (x-2) 부분이 0이 돼서 몫이 통째로 사라지고 나머지만 남기 때문입니다.',
    ...
    stages:[
      { tag:{ko:'① P(2)를 계산하면 그게 나머지',en:'1) P(2) is the remainder',zh:'① P(2)就是余数'},
        ...
        desc:{ko:'P(x)=x²+3x-1에 x=2를 대입하면 2²+3×2-1=4+6-1=<b>9</b>. 조립제법으로 직접 나눠도 나머지는 똑같이 9가 나옵니다 — 훨씬 빠르죠.',
        ...
        result:{ko:'P(a)를 계산하면 (x-a)로 나눈 나머지가 바로 나옵니다!',en:'Computing P(a) gives you the remainder of division by (x-a), instantly!',zh:'算出P(a)，就是除以(x-a)的余数！'},
        book:{ko:'P(x)를 (x-a)로 나눈 나머지 = P(a) — 이걸 나머지정리라고 합니다.',
      ...
      { tag:{ko:'② 왜 그럴까? 나눗셈 구조에서 확인',en:'2) Why? Check the division structure',zh:'② 为什么？看除法结构'},
        ...
        desc:{ko:'나눗셈은 항상 이 꼴로 쓸 수 있습니다. 여기에 x=2를 넣으면 (x-2)가 0이 되어 앞부분이 통째로 사라지고, <b>P(2)=R</b>만 남습니다.',
        ...
        result:{ko:'대입하면 몫은 사라지고 나머지만 남는 게 원리입니다!',en:'Substituting makes the quotient vanish, leaving only the remainder — that\'s the principle!',zh:'代入后商消失，只剩余数——这就是原理！'},
        book:{ko:'같은 원리로 (x-a)가 P(x)의 인수인지도 알 수 있습니다 — P(a)=0이면 (x-a)로 나누어떨어집니다(인수정리).',
  ...
  check:{
    ...
    open:{ ko:'P(x)=x²+kx-3을 (x-1)로 나눈 나머지가 2일 때 k를 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 나머지로 계수를 역산하는 문제를 모두 풉니다!', en:'Solve all the coefficient-from-remainder problems in 5 minutes!', zh:'5分钟内解答所有由余数反求系数的题！' }
  },
