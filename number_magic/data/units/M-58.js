...
window.NM_UNITS['M-58'] = {
  ...
  subtitle:{ ko:'켤레를 곱해 근호를 없앱니다', en:'Multiply by the conjugate to clear the root', zh:'乘以共轭式去掉根号' },
  icon:'√',
  ...
  practice:{
    ...
    intro:{
      ko:'근호가 있는 0/0 꼴은 켤레(부호만 반대인 짝)를 분모·분자에 곱합니다 — 그러면 근호가 사라지고 (x-a)가 약분됩니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'lim(x→0) (√(x+4)-2)/x를 보면 x=0을 넣는 순간 0/0이 됩니다. MD43에서 배운 인수분해로는 근호를 없앨 수가 없습니다 — 근호가 있을 땐 새로운 도구, "켤레"가 필요합니다.',
      ...
      history:{ ko:'"켤레(conjugate)"는 같은 두 항에서 부호만 반대인 짝을 말합니다. (√A - B)와 (√A + B)를 곱하면 (√A)²-B²=A-B²이 되어 근호가 깔끔하게 사라집니다 — 이건 곱셈공식 (x-y)(x+y)=x²-y²와 완전히 같은 원리입니다.',
    ...
    stages:[
      { tag:{ko:'① 켤레를 곱해 분자의 근호를 없앱니다',en:'1) Multiply by the conjugate to clear the root in the numerator',zh:'① 乘以共轭式去掉分子的根号'},
        ...
        desc:{ko:'분자·분모에 켤레 (√(x+4)+2)를 곱하면 분자가 (x+4)-4=<b>x</b>가 됩니다. 그럼 분자·분모 둘 다 x라서 약분되고, 남는 건 1/(√(x+4)+2) — x=0을 넣으면 1/(2+2)=1/4.',
        ...
        result:{ko:'분자에 근호가 있으면 켤레를 곱해 분자를 (x-a)로 만듭니다!',en:'When the root is in the numerator, multiply by the conjugate to turn it into (x-a)!',zh:'根号在分子时，乘以共轭式把分子变成(x-a)！'},
        book:{ko:'켤레를 곱하는 건 "1을 곱하는 것"과 같습니다(같은 걸 분모·분자에 곱하니까) — 그래서 식의 값은 바뀌지 않습니다.',
      ...
      { tag:{ko:'② 분모에 근호가 있으면 정수로 딱 떨어집니다',en:'2) A root in the denominator lands on an integer',zh:'② 根号在分母时会得到整数'},
        head:{ko:'\\lim_{x\\to 0} \\dfrac{x}{\\sqrt{x+4}-2} = 4',en:'\\lim_{x\\to 0} \\dfrac{x}{\\sqrt{x+4}-2} = 4',zh:'\\lim_{x\\to 0} \\dfrac{x}{\\sqrt{x+4}-2} = 4'},
        desc:{ko:'이번엔 근호가 분모에 있습니다. 켤레를 곱하면 분모가 x가 되어 분자의 x와 약분되고, 남는 건 <b>√(x+4)+2</b> — x=0을 넣으면 2+2=4(근호 없이 정수로 끝나요).',
        ...
        result:{ko:'근호가 분모에 있으면 약분 후 근호값 그대로 정수로 끝납니다!',en:'When the root is in the denominator, canceling leaves the root value itself — a clean integer!',zh:'根号在分母时，约分后剩下的根号值本身就是整数！'},
        book:{ko:'분자·분모 어느 쪽에 근호가 있는지에 따라 최종 답의 형태(정수 vs 분수)가 달라진다는 걸 기억합니다.',
    ...
    rule:{ ko:'근호가 있는 0/0 꼴은 켤레를 곱해 근호를 없앱니다 — 분모의 근호는 정수로, 분자의 근호는 분수(1/2m)로 끝납니다!',
  ...
  check:{
    ...
    open:{ ko:'lim(x→2) (x-2)/(√(x+7)-3)의 값을 켤레를 곱하는 과정과 함께 설명해봅니다.',
  ...
  lab:{
    ...
    params:{mode:'numRoot'},
    intro:{
      ko:'분자에 근호가 있으면 켤레를 곱한 뒤 분수로 답이 딱 떨어집니다!',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 분모·분자 근호를 섞어서 모두 풉니다!', en:'Solve a mix of roots in numerator and denominator, all within 5 minutes!', zh:'5分钟内混合解出根号在分子·分母的题！' }
  },
