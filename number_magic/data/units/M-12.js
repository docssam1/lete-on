...
window.NM_UNITS['M-12'] = {
  ...
  subtitle:{ ko:'같은 종류끼리만 모읍니다 — 사과는 사과끼리, x는 x끼리', en:'Only combine what matches — apples with apples, x with x', zh:'只把同类合在一起——苹果归苹果，x归x' },
  icon:'🍎',
  ...
  practice:{
    ...
    params:{mode:'linear'},
    intro:{
      ko:'사과 3개랑 배 2개는 "5개"로 합칠 수 없습니다. x와 상수도 마찬가지 — 문자가 같은 항끼리만 더합니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'사과 3개 + 배 2개는 몇 개일까요? "5개"라고 답하면 틀립니다 — 사과랑 배는 다른 것이기 때문입니다. 그럼 3x + 2y는 어떻게 정리해야 할까요?',
      ...
      history:{ ko:'다항식(多項式)이라는 말 자체가 "여러(多) 개의 항(項)으로 된 식"이라는 뜻입니다. 항을 종류별로 나누는 습관은 대수학이 발전하면서 자연스럽게 자리 잡았습니다.',
    ...
    stages:[
      { tag:{ko:'① 동류항 — 문자와 차수가 같은 항',en:'1) Like terms — same letter, same power',zh:'① 同类项——字母和次数都相同的项'},
        ...
        desc:{ko:'3x와 5x는 둘 다 "x가 1개씩인 항"이라 <b>동류항</b>입니다. 동류항끼리는 계수만 더하거나 뺍니다(3+5=8) — 문자 x는 그대로 둡니다. 하지만 3x와 5(상수)는 종류가 달라 더할 수 없습니다.',
        ...
        result:{ko:'동류항끼리만 계수를 더하거나 뺍니다!',en:'Only add or subtract the coefficients of like terms!',zh:'只有同类项才能把系数相加或相减！'},
        book:{ko:'문자와 차수가 같은 항을 <b>동류항</b>이라 하고, 동류항끼리만 계수를 계산해 하나로 정리할 수 있습니다.',
      ...
      { tag:{ko:'② 괄호 앞이 −이면 부호가 전부 바뀜',en:'2) A minus before the brackets flips every sign',zh:'② 括号前是−号，全部变号'},
        ...
        desc:{ko:'괄호 앞에 −가 있으면 괄호를 풀 때 <b>안의 모든 항의 부호가 바뀝니다</b> — (2x+5)가 -2x-5로. 그다음 동류항끼리(4x와 -2x, 3과 -5) 정리하면 끝입니다.',
        ...
        result:{ko:'괄호 앞 −는 안의 모든 항의 부호를 뒤집습니다!',en:'A minus before the brackets flips every term inside!',zh:'括号前的−会把里面每一项都变号！'},
        book:{ko:'−(a+b) = -a-b 입니다. 부호를 하나씩 다 바꿔야지, 첫 항만 바꾸면 틀립니다(자주 하는 실수).',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 괄호 앞 부호 바꾸기 문제를 모두 풉니다!', en:'Solve all the sign-flip-before-brackets problems in 5 minutes!', zh:'5分钟内解答所有括号前变号的题目！' }
  },
