...
window.NM_UNITS['M-16'] = {
  ...
  subtitle:{ ko:'같은 소인수가 두 번 만나면(짝) 근호 밖으로 나올 수 있습니다', en:'When the same prime factor appears twice (a pair), it can step outside the root', zh:'同一个质因数出现两次(配对)，就能走出根号' },
  icon:'🔓',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'√48 = √(16×3) = √16×√3 = 4√3. 근호 안에서 완전제곱수를 찾아 밖으로 꺼냅니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'√48은 완전제곱수가 아니라서 정수로 안 떨어집니다. 그런데 48 안에는 16(=4²)이 숨어 있습니다 — 근호를 계산기 없이 더 간단하게 쓸 방법이 있을까요?',
      ...
      history:{ ko:'2와 5가 만나 10이 되는 이야기, 기억하죠? 소인수분해에서 "숨은 짝"을 찾아내는 습관은 그때부터 계속 이어지고 있습니다 — 이번엔 짝이 "같은 수 두 번"으로 바뀐 것뿐입니다.',
    ...
    stages:[
      { tag:{ko:'① 완전제곱수 짝을 찾아 꺼내기',en:'1) Find the perfect-square pair and pull it out',zh:'① 找到完全平方数配对，提出来'},
        ...
        desc:{ko:'48 = 16×3이고 16은 4²입니다. √48 = √(4²×3) = √4²×√3 = <b>4√3</b>. 소인수분해하면 48=2×2×2×2×3 — 2가 네 번(짝 두 쌍)이라 2×2=4가 근호 밖으로 나올 수 있습니다.',
        ...
        result:{ko:'근호 안의 완전제곱수 인수를 찾아 밖으로 꺼냅니다!',en:'Find the perfect-square factor inside the root and step it outside!',zh:'找到根号内的完全平方因数，提到根号外！'},
        book:{ko:'a²×b (b는 제곱인 인수가 없는 수) 꼴이면 √(a²b) = a√b로 정리합니다. 이게 근호의 "가장 간단한 꼴"입니다.',
      ...
      { tag:{ko:'② 이미 계수가 있으면 곱해서 합침',en:'2) With an existing coefficient, multiply it in',zh:'② 已有系数时相乘合并'},
        ...
        desc:{ko:'근호 앞에 이미 5가 있어도 방법은 같습니다. √48을 먼저 4√3으로 정리한 뒤, 원래 있던 5와 새로 꺼낸 4를 곱합니다(5×4=20). <b>정리 → 곱하기</b> 순서만 지키면 됩니다.',
        ...
        result:{ko:'원래 있던 계수와 새로 꺼낸 수를 곱합니다!',en:'Multiply the original coefficient by the newly pulled-out number!',zh:'把原来的系数和新提出的数相乘！'},
        book:{ko:'c√(a²b) = ca√b — 원래 계수 c와 근호에서 꺼낸 a를 곱해 하나의 계수로 합칩니다.',
  ...
  check:{
    ...
    open:{ ko:'√200을 가장 간단한 꼴로 정리하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 계수가 있는 근호 정리를 모두 풉니다!', en:'Solve all the radicals-with-a-coefficient problems in 5 minutes!', zh:'5分钟内解答所有带系数的根号化简题！' }
  },
