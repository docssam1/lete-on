...
window.NM_UNITS['M-68'] = {
  ...
  subtitle:{ ko:'두 수로 평면 위의 한 자리를 꼭 집습니다', en:'Two numbers pin down one spot on the plane', zh:'用两个数就能确定平面上的一个位置' },
  icon:'📍',
  ...
  discover:{
    ...
    stages:[
      { tag:{ ko:'① 가로 먼저, 세로 나중', en:'1) Across first, then up', zh:'① 先横后纵' },
        ...
        desc:{ ko:'좌표는 <b>순서가 있는 짝</b>입니다. (3, 2)와 (2, 3)은 다른 점입니다 — 앞의 수가 가로, 뒤의 수가 세로라는 약속을 모두가 지키기 때문에 통합니다.', en:'Coordinates are an <b>ordered</b> pair: (3, 2) and (2, 3) are different points. It works because everyone keeps the same agreement - across first, up second.', zh:'坐标是<b>有顺序的</b>一对：(3, 2)和(2, 3)是不同的点。之所以能通用，是因为大家都遵守同一个约定——先横后纵。' },
        mathSteps:['x = 3', 'y = 2', '(3,\\, 2)'],
        result:{ ko:'앞이 가로, 뒤가 세로 — 순서가 뜻을 정합니다!', en:'Across then up - the order carries the meaning!', zh:'先横后纵——顺序决定含义！' },
        book:{ ko:'원점 O는 (0, 0)입니다. x축 위의 점은 세로가 0이라 (a, 0), y축 위의 점은 (0, b) 꼴입니다 — 축 위의 점은 어느 사분면에도 들어가지 않습니다.', en:'The origin O is (0, 0). A point on the x-axis has zero height, so it is (a, 0), and one on the y-axis is (0, b). Points on the axes belong to no quadrant at all.', zh:'原点O是(0, 0)。x轴上的点纵坐标为0，写成(a, 0)；y轴上的点是(0, b)。轴上的点不属于任何象限。' } },
      ...
      { tag:{ ko:'② 부호 두 개가 사분면을 정한다', en:'2) Two signs decide the quadrant', zh:'② 两个符号决定象限' },
        ...
        desc:{ ko:'x가 음수면 <b>왼쪽</b>, y가 양수면 <b>위쪽</b>이니 왼위 — 제2사분면입니다. 오른위부터 시계 반대 방향으로 1, 2, 3, 4입니다.', en:'A negative x means <b>left</b> and a positive y means <b>up</b>, so this is the upper left - quadrant 2. Numbering runs counter-clockwise from the top right: 1, 2, 3, 4.', zh:'x为负在<b>左</b>，y为正在<b>上</b>，所以是左上——第2象限。从右上开始逆时针依次是1、2、3、4。' },
        mathSteps:['x = -4 < 0', 'y = 5 > 0', {ko:'\\text{제} 2 \\text{사분면}',en:'\\text{quadrant } 2',zh:'\\text{第} 2 \\text{象限}'}],
        result:{ ko:'부호만 봐도 어느 구역인지 알 수 있습니다!', en:'The signs alone tell you which region it is in!', zh:'只看符号就知道在哪个区域！' },
        book:{ ko:'x축에 비추면 y의 부호만 바뀌고, y축에 비추면 x의 부호만, 원점에 대해 비추면 둘 다 바뀝니다. 제2사분면의 점을 원점에 비추면 제4사분면으로 갑니다.', en:'Reflecting in the x-axis flips only y, in the y-axis only x, and through the origin flips both. A point in quadrant 2 lands in quadrant 4 after a reflection through the origin.', zh:'关于x轴对称只变y的符号，关于y轴对称只变x的符号，关于原点对称两个都变。第2象限的点关于原点对称后落在第4象限。' } }
    ],
    rule:{ ko:'가로 먼저 세로 나중, 부호 둘이 사분면을 정합니다 — 이 약속 하나로 평면 위 어디든 가리킬 수 있습니다!', en:'Across first and up second, and the two signs give the quadrant - one agreement lets you point anywhere on the plane!', zh:'先横后纵，两个符号决定象限——有了这一个约定，就能指向平面上的任何地方！' }
  },
  ...
  check:{
    fills:[
      { tex:{ ko:'(5,\\, -2) \\quad\\Rightarrow\\quad \\text{제} \\square \\text{사분면}', en:'(5,\\, -2) \\quad\\Rightarrow\\quad \\text{quadrant } \\square', zh:'(5,\\, -2) \\quad\\Rightarrow\\quad \\text{第} \\square \\text{象限}' }, answer:4,
        hint:{ ko:'오른쪽 아래입니다', en:'Right and down', zh:'在右下方' } },
      { tex:{ ko:'(3,\\, 4) \\text{의 x축 대칭} \\quad\\Rightarrow\\quad \\left(3,\\, \\square\\right)', en:'(3,\\, 4) \\text{ reflected in the x-axis} \\quad\\Rightarrow\\quad \\left(3,\\, \\square\\right)', zh:'(3,\\, 4) \\text{关于x轴对称} \\quad\\Rightarrow\\quad \\left(3,\\, \\square\\right)' }, answer:-4,
        hint:{ ko:'y의 부호만 바뀝니다', en:'Only y changes sign', zh:'只有y变号' } }
    ],
    open:{ ko:'(−2, −5)가 제몇사분면인지, 그 까닭을 말해봅니다.', en:'Say which quadrant (-2, -5) is in and why.', zh:'说说(−2, −5)在第几象限，为什么。' },
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 x축·y축·원점 대칭점을 정확히 구합니다!', en:'Within 6 minutes, reflect points in the x-axis, the y-axis and the origin!', zh:'6分钟内准确求出关于x轴、y轴、原点的对称点！' }
  },
