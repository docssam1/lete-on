...
window.NM_UNITS['M-21'] = {
  ...
  subtitle:{ ko:'괄호를 분배해서 곱하고, 조립제법으로 나눕니다', en:'Distribute to multiply, use synthetic division to divide', zh:'分配展开来相乘，用综合除法来相除' },
  icon:'📦',
  ...
  practice:{
    ...
    params:{mode:'mul'},
    intro:{
      ko:'(2x+3)(4x+1)을 곱할 때, 앞항끼리·바깥끼리·안쪽끼리·뒷항끼리 — 네 번 곱해서 더합니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'단항식 하나를 다항식에 곱하는 건 이미 할 줄 압니다. 그런데 (2x+3)(4x+1)처럼 괄호 두 개를 곱하면 어떻게 될까요? 나누는 것도 거꾸로 할 수 있을까요?',
      ...
      history:{ ko:'괄호 두 개를 곱하는 방법은 예로부터 "네 부분을 빠짐없이 곱한다"는 원칙 하나로 정리돼 왔습니다. 나눗셈은 그 반대 방향 — 몫을 하나씩 맞춰가며 나머지를 줄여가는 조립제법으로 다듬어졌습니다.',
    ...
    stages:[
      { tag:{ko:'① 괄호 두 개는 네 번 곱해서 더하기',en:'1) Two brackets: four products, then add',zh:'① 两个括号：四次相乘后相加'},
        ...
        desc:{ko:'2x×4x=8x², 2x×1=2x, 3×4x=12x, 3×1=3. 가운데 두 항(2x와 12x)을 더하면 14x — <b>8x²+14x+3</b>이 됩니다.',
        ...
        result:{ko:'네 부분을 빠짐없이 곱한 뒤 같은 차수끼리 모읍니다!',en:'Multiply all four parts, then collect like-degree terms!',zh:'四个部分都乘到，再合并同类项！'},
        book:{ko:'(ax+b)(cx+d) = acx² + (ad+bc)x + bd — 항상 이 네 항의 합으로 정리됩니다.',
      ...
      { tag:{ko:'② 조립제법으로 나누기',en:'2) Divide with synthetic division',zh:'② 用综合除法相除'},
        ...
        desc:{ko:'맨 앞 계수 2를 그대로 내리고, 1을 곱해 다음 계수에 더해요: 2×1+5=7. 다시 7×1+(-3)=4가 나머지입니다. 몫은 <b>2x+7</b>, 나머지는 <b>4</b>.',
        ...
        result:{ko:'k를 곱하고 다음 계수를 더하는 걸 반복합니다!',en:'Multiply by k and add the next coefficient, again and again!',zh:'乘以k再加上下一个系数，反复进行！'},
        book:{ko:'(x-k)로 나눌 때: 몫의 계수를 왼쪽부터 차례로 만들며, 마지막에 남는 수가 나머지입니다. 검산은 (x-k)×몫+나머지=원래 식.',
  ...
  check:{
    ...
    open:{ ko:'(2x-1)(3x+4)를 전개하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    params:{mode:'mulTri'},
    rule:{ ko:'5분 안에 이항식×삼항식 전개를 모두 풉니다! 이번엔 답이 네 칸입니다.', en:'Solve all the binomial×trinomial expansions in 5 minutes! The answer has four slots this time.', zh:'5分钟内解答所有二项式×三项式的展开题！这次答案有四格。' }
  },
