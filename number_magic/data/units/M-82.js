...
window.NM_UNITS['M-82'] = {
  ...
  subtitle:{ ko:'0을 가운데 두면 모든 수에 제 자리가 생깁니다', en:'Put zero in the middle and every number gets a place', zh:'把0放在中间，每个数就都有了位置' },
  icon:'📏',
  ...
  discover:{
    ...
    stages:[
      { tag:{ ko:'① 0에서 몇 칸인가', en:'1) How many steps from zero', zh:'① 离0几格' },
        ...
        desc:{ ko:'점이 어디 있든 <b>0에서 세기 시작</b>합니다. 오른쪽으로 세면 양수, 왼쪽으로 세면 음수. 눈금 하나가 1이라 세 칸 왼쪽이면 −3입니다.', en:'Wherever the dot is, <b>start counting at zero</b>: right for positives, left for negatives. One mark is one, so three to the left is -3.', zh:'不管点在哪里，都要<b>从0开始数</b>：向右是正数，向左是负数。一格是1，往左三格就是−3。' },
        mathSteps:['-3 < 0 < 3', '\\text{P} = -3'],
        result:{ ko:'수직선은 수에게 자리를 주는 줄입니다!', en:'A number line gives every number a place!', zh:'数轴给每个数一个位置！' },
        book:{ ko:'오른쪽으로 갈수록 <b>큰 수</b>입니다. 그래서 −5는 −3보다 왼쪽이고 더 작습니다 — 절댓값(0에서 떨어진 거리)은 −5가 더 큰데도 그렇습니다. 이 둘을 헷갈리지 않는 것이 이 단원의 고비입니다.', en:'Further right means <b>bigger</b>. So -5 sits left of -3 and is the smaller number, even though -5 is further from zero. Keeping those two ideas apart is the hurdle in this topic.', zh:'越往右<b>越大</b>。所以−5在−3的左边，是更小的数——尽管−5离0更远。分清这两件事，正是本单元的难点。' } },
      ...
      { tag:{ ko:'③ 절댓값 — 0에서의 거리라 둘이다', en:'3) Absolute value - a distance, so there are two', zh:'③ 绝对值——是距离，所以有两个' },
        ...
        desc:{ ko:'절댓값은 그 수가 <b>0에서 얼마나 떨어져 있는가</b>입니다. 거리니까 방향은 상관없고, 그래서 5칸 떨어진 자리는 오른쪽 5와 왼쪽 −5 <b>둘</b>입니다.', en:'An absolute value is <b>how far a number is from zero</b>. Distance has no direction, so two places are five steps away: 5 on the right and -5 on the left.', zh:'绝对值是这个数<b>离0有多远</b>。距离不分方向，所以离5格的位置有<b>两个</b>：右边的5和左边的−5。' },
        mathSteps:['|-5| = 5', '|5| = 5', 'x = -5 \\;\\text{or}\\; 5'],
        result:{ ko:'거리는 방향이 없어서 답이 둘입니다!', en:'A distance has no direction, so there are two answers!', zh:'距离没有方向，所以答案有两个！' },
        book:{ ko:'그래서 절댓값은 <b>절대 음수가 되지 않습니다</b>. |0|=0 이고, 0 말고는 절댓값이 0인 수가 없습니다. 두 수의 절댓값이 같다면 두 수는 같거나 부호만 반대입니다.', en:'That is why an absolute value is <b>never negative</b>. Zero is the only number whose absolute value is zero, and if two numbers share an absolute value they are either equal or opposite in sign.', zh:'所以绝对值<b>永远不会是负数</b>。|0|=0，除0以外没有绝对值为0的数。两数绝对值相同，它们要么相等，要么只是符号相反。' } }
    ],
    rule:{ ko:'0에서 몇 칸인지 세면 자리가 나옵니다 — 오른쪽은 양수, 왼쪽은 음수, 그리고 절댓값이 같은 수는 언제나 둘!', en:'Count the steps from zero and you have the place - right is positive, left is negative, and a shared absolute value always means two numbers!', zh:'数出离0几格就知道位置——右正左负，而绝对值相同的数总是有两个！' }
  },
  ...
  check:{
    ...
    open:{ ko:'−5와 −3 중 어느 쪽이 큰지, 절댓값은 어느 쪽이 큰지 말해봅니다.', en:'Say which of -5 and -3 is bigger, and which has the bigger absolute value.', zh:'说说−5和−3哪个更大，哪个的绝对值更大。' },
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 절댓값이 같은 두 자리를 찾아 왼쪽 수를 답합니다!', en:'Within 6 minutes, find the two places with the same absolute value and give the left one!', zh:'6分钟内找出绝对值相同的两个位置，答出左边那个！' }
  },
