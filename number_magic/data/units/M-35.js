...
window.NM_UNITS['M-35'] = {
  ...
  subtitle:{ ko:'x항·y항을 완전제곱으로 묶으면 중심과 반지름이 보입니다', en:'Complete the square on x and y to reveal the center and radius', zh:'把x、y项配成完全平方，中心和半径就显现出来' },
  icon:'⭕',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'x²+y²-4x-6y+9=0을 (x-2)²+(y-3)²=4로 바꾸면, 중심(2,3)·반지름 2인 원이라는 게 한눈에 보입니다.',
  ...
  discover:{
    ...
    story:{
      ...
      history:{ ko:'원은 원래 (x-a)²+(y-b)²=r² — 중심에서 거리가 항상 r인 점들의 모임입니다(MD31 거리 공식 그대로). 이걸 전개하면 x²+y²+Ax+By+C=0 꼴이 되니, 거꾸로 완전제곱식으로 묶으면 중심과 반지름이 다시 드러납니다.',
    ...
    stages:[
      { tag:{ko:'① x항·y항을 각각 완전제곱으로',en:'1) Complete the square on x and y separately',zh:'① x项、y项分别配成完全平方'},
        ...
        result:{ko:'완전제곱으로 묶으면 중심(2,3), 반지름 2가 바로 보입니다!',en:'Complete the square and the center (2,3) and radius 2 appear directly!',zh:'配方后中心(2,3)、半径2直接显现！'},
      ...
      { tag:{ko:'② 공통계수가 있으면 먼저 나누기',en:'2) Divide out a common coefficient first',zh:'② 有公共系数先除掉'},
        ...
        desc:{ko:'모든 항이 2의 배수입니다. 전체를 2로 나누면 앞서 본 x²+y²-4x-6y+9=0으로 돌아가고, 그 다음은 똑같이 완전제곱으로 묶으면 됩니다.',
        ...
        result:{ko:'x²,y² 앞의 공통계수는 먼저 나눠서 없앱니다!',en:'Divide out any common coefficient in front of x², y² first!',zh:'先除掉x²、y²前的公共系数！'},
        book:{ko:'x²,y²의 계수가 같은 수 m이면 mx²+my²+Ax+By+C=0을 m으로 나눈 뒤 완전제곱을 진행합니다.',
  ...
  check:{
    ...
    open:{ ko:'x²+y²-2x+4y-4=0을 표준형으로 바꾸는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 공통계수가 있는 원의 방정식을 모두 풉니다! 먼저 나누는 걸 잊지 맙니다.', en:'Solve all the circle equations with a common coefficient in 5 minutes! Don\'t forget to divide first.', zh:'5分钟内解答所有带公共系数的圆方程！别忘了先除。' }
  },
