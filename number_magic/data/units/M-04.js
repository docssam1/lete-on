...
window.NM_UNITS['M-04'] = {
  ...
  subtitle:{ ko:'음수 개수가 짝이면 +, 홀이면 − — 부호부터 정하고 시작합니다', en:'Even count of negatives = +, odd = − : decide the sign first', zh:'负数个数为偶得正，为奇得负——先定符号' },
  icon:'🎲',
  ...
  discover:{
    ...
    stages:[
      { tag:{ko:'① 부호를 먼저 정합니다',en:'1) Decide the sign first',zh:'① 先确定符号'},
        head:{ko:'(−4) × (−6) = +24',en:'(−4) × (−6) = +24',zh:'(−4) × (−6) = +24'},
        desc:{ko:'음수가 <b>2개(짝수)</b>면 곱은 +, <b>1개(홀수)</b>면 −입니다. (−4)×(−6)은 음수가 2개니까 부호는 +, 절댓값끼리 곱해서 4×6=24 → <b>+24</b>. 나눗셈도 똑같은 규칙입니다.',
        ...
        result:{ko:'음수 개수만 세면 부호는 저절로 정해집니다!',en:'Just count the negatives and the sign decides itself!',zh:'数一数负数个数，符号自然就定了！'},
        book:{ko:'어떤 수와 0의 곱은 항상 0입니다 — 부호를 셀 필요도 없습니다.',
      ...
      { tag:{ko:'② 세 개 이상도 개수만 세면 끝',en:'2) Three or more — just count',zh:'② 三个以上也只数个数'},
        ...
        desc:{ko:'몇 개를 곱하든 규칙은 같습니다 — 음수가 <b>3개(홀수)</b>니까 부호는 −, 절댓값끼리 곱해 2×3×5=30 → <b>−30</b>. 나눗셈이 섞여 있어도(앞에서부터 차례로) 같은 방식으로 부호를 정합니다.',
        ...
        result:{ko:'음수를 세는 습관만 들이면 아무리 길어도 두렵지 않습니다!',en:'Once you build the habit of counting negatives, long chains stop being scary!',zh:'养成数负数的习惯，再长的算式也不怕！'},
        book:null }
    ],
  ...
  check:{
    ...
    open:{ ko:'(−2) × (−3) × 5를 계산하고, 부호를 어떻게 정했는지 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 곱셈·나눗셈 혼합 문제를 모두 풉니다!', en:'Solve all × and ÷ mixed problems in 5 minutes!', zh:'5分钟内解答所有乘除混合题！' }
  },
