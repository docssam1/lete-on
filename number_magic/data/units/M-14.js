...
window.NM_UNITS['M-14'] = {
  ...
  subtitle:{ ko:'등호를 건너가면 부호가 옷을 갈아입습니다', en:'Cross the equals sign and the sign changes its coat', zh:'跨过等号，符号就要换件衣服' },
  icon:'⚖️',
  ...
  practice:{
    ...
    params:{mode:'add'},
    intro:{
      ko:'저울이 평형이면(=), 한쪽에서 뭔가를 빼면 반대쪽도 똑같이 빼야 평형이 유지됩니다. 이걸 식으로 나타낸 게 "이항"입니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'양팔저울이 평형을 이루고 있습니다. 한쪽 접시에서 추 5개를 덜어내면, 평형을 유지하려면 반대쪽은 어떻게 해야 할까요?',
      ...
      history:{ ko:'"이항"의 영어 transposition은 "자리를 바꾼다"는 뜻의 라틴어에서 왔습니다. 9세기 수학자 알콰리즈미의 책 제목에 있는 "al-jabr"(복원하다, 맞추다)가 바로 지금의 "algebra"(대수학)의 어원입니다 — 등식을 맞추는 이 기술 자체가 대수학의 이름이 된 거입니다.',
    ...
    stages:[
      { tag:{ko:'① 등호를 넘어가면 부호가 바뀜',en:'1) Cross the equals sign and the sign flips',zh:'① 跨过等号，符号就变'},
        ...
        desc:{ko:'양변에서 7을 똑같이 빼면 x + 7 - 7 = 12 - 7, 즉 x = 12 - 7이 됩니다. 결과만 보면 <b>좌변의 +7이 우변으로 넘어가며 -7로 바뀐 것</b>과 똑같습니다 — 이게 "이항"입니다.',
        ...
        book:{ko:'등식의 성질: 양변에 같은 수를 더하거나 빼도 등식은 유지됩니다. 그 결과를 "이항(부호를 바꿔 반대쪽으로 옮김)"으로 간단히 나타냅니다.',
      ...
      { tag:{ko:'② 계수가 있으면 나눗셈까지 완주',en:'2) With a coefficient, finish by dividing',zh:'② 有系数时，最后再除一次'},
        ...
        desc:{ko:'먼저 상수항 4를 이항해요: 3x = 19 - 4 = 15. 그다음 x의 계수 3으로 양변을 나눠요: x = 15÷3 = 5. <b>이항 → 나눗셈</b>, 이 두 단계면 x가 완전히 혼자 남습니다.',
        ...
        result:{ko:'상수항을 이항하고, x의 계수로 나누면 x만 남습니다!',en:'Transpose the constant, then divide by the coefficient of x, and x stands alone!',zh:'移项常数，再除以x的系数，x就单独留下了！'},
        book:{ko:'ax+b=c 꼴의 등식은 ①b를 이항(ax=c-b) ②양변을 a로 나누기(x=(c-b)/a) 두 단계로 x를 구합니다.',
  ...
  arena:{
    ...
    params:{mode:'solve'},
    rule:{ ko:'5분 안에 이항→나눗셈까지 완주하는 문제를 모두 풉니다!', en:'Solve all the transpose-then-divide problems in 5 minutes!', zh:'5分钟内解答所有先移项再除法的题目！' }
  },
