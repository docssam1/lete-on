...
window.NM_UNITS['M-51'] = {
  ...
  practice:{
    ...
    params:{mode:'direct'},
    intro:{
      ko:'정비례 y=ax는 x가 늘어나면 y도 같은 비율로 늘어납니다 — 비율 a를 먼저 구하고 시작합니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'한 자루에 500원인 연필을 x자루 사면 값은 y=500x원입니다 — 자루 수가 2배면 값도 2배! 반대로, 일정한 거리를 갈 때 속력을 2배로 하면 걸리는 시간은 절반이 됩니다 — 이건 반비례입니다.',
      ...
      history:{ ko:'정비례·반비례는 고대 그리스의 "비례론"에서 나왔습니다. 유클리드의 『원론』 5권 전체가 비(比)와 비례를 다루는데, 이는 오늘날의 함수 개념의 뿌리가 됐습니다.',
    ...
    stages:[
      { tag:{ko:'① 정비례 — 비율(a)이 항상 일정합니다',en:'1) Direct proportion — the ratio(a) is always constant',zh:'① 正比例——比值(a)恒定'},
        ...
        desc:{ko:'점(2,6)을 지나니 y÷x=6÷2=3, 그래서 a=3(비율은 항상 y÷x). x=5일 때 y=ax=3×5=<b>15</b>가 됩니다.',
        ...
        result:{ko:'정비례는 y÷x(비율)가 항상 같습니다!',en:'In direct proportion, y÷x (the ratio) is always the same!',zh:'正比例中y÷x(比值)始终相同！'},
        book:{ko:'정비례 그래프는 원점(0,0)을 지나는 직선입니다 — x가 0이면 y도 항상 0.',
      ...
      { tag:{ko:'② 반비례 — 곱(a)이 항상 일정합니다',en:'2) Inverse proportion — the product(a) is always constant',zh:'② 反比例——乘积(a)恒定'},
        ...
        desc:{ko:'점(2,6)을 지나니 x×y=2×6=12, 그래서 a=12(곱은 항상 x×y). x=4일 때 y=a÷x=12÷4=<b>3</b>이 됩니다.',
        ...
        result:{ko:'반비례는 x×y(곱)가 항상 같습니다!',en:'In inverse proportion, x×y (the product) is always the same!',zh:'反比例中x×y(乘积)始终相同！'},
        book:{ko:'반비례 그래프는 직선이 아니라 곡선(쌍곡선)입니다 — x가 커질수록 y는 점점 작아집니다(0에는 닿지 않아요).',
    ...
    rule:{ ko:'정비례 y=ax는 비율(y÷x)이 일정, 반비례 y=a/x는 곱(x×y)이 일정 — 한 점을 알면 a를 구하고, 새 x로 y를 찾습니다!',
  ...
  check:{
    ...
    open:{ ko:'반비례 y=a/x가 점(4,3)을 지날 때 x=2일 때의 y를 구하는 과정을 설명해봅니다.',
  ...
  lab:{
    ...
    params:{mode:'inverse'},
    intro:{
      ko:'반비례는 곱(x×y)이 항상 같습니다 — 그 값의 약수 중에서 새 x를 골라 나눗셈이 딱 떨어지게 합니다!',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 정비례·반비례를 섞어서 빠르게 구합니다!', en:'Solve a mix of direct and inverse proportion, all within 5 minutes!', zh:'5分钟内快速求出正比例·反比例混合题！' }
  },
