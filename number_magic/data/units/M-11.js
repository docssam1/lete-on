...
window.NM_UNITS['M-11'] = {
  ...
  practice:{
    ...
    params:{mode:'mul'},
    intro:{
      ko:'3x²는 "3과 x²이 곱해진 것"입니다. 두 단항식을 곱할 땐 숫자는 숫자끼리, 문자는 지수법칙으로!',
  ...
  discover:{
    story:{
      hook:{ ko:'a와 −a 중에 어느 쪽이 더 큰 수일까요?',
        ...
        zh:'a和−a，哪个更大？' },
      history:{ ko:'정할 수 없습니다. a가 −3이면 −a는 3이라 −a가 더 크죠. 문자는 숫자를 담는 상자라서, 안에 무엇이 들었는지 모르면 크기도 정해지지 않습니다. 단항식을 계산할 때 숫자는 숫자끼리, 문자는 문자끼리 묶는 것도 같은 이유입니다 — 상자는 상자끼리 세는 겁니다.',
    ...
    stages:[
      { tag:{ko:'① 계수는 계수끼리, 문자는 문자끼리',en:'1) Coefficients with coefficients, letters with letters',zh:'① 系数归系数，字母归字母'},
        ...
        desc:{ko:'단항식은 "숫자 부분(계수)"과 "문자 부분"으로 나뉩니다. 곱할 땐 계수는 계수끼리(3×4=12), 문자는 지수법칙으로(x²×x³=x⁵) — <b>두 계산을 각각 따로</b> 하면 끝입니다.',
        ...
        result:{ko:'두 계산을 각각 따로 하면 됩니다 — 섞이지 않습니다!',en:'Do the two calculations separately — they never mix!',zh:'两个计算分开做——不会混在一起！'},
        book:{ko:'단항식끼리의 곱셈은 계수의 곱과 문자의 곱(지수법칙)을 각각 구해 합칩니다.',
      ...
      { tag:{ko:'② 나눗셈도 같은 방식',en:'2) Division works the same way',zh:'② 除法也是一样的方式'},
        ...
        desc:{ko:'나눗셈도 계수는 계수끼리 나누고(8÷2=4), 문자는 지수법칙으로 나눕니다(x⁵÷x²=x³). 세 개 이상이 곱셈·나눗셈으로 이어지면 <b>앞에서부터 차례로</b> 계산합니다.',
        ...
        book:{ko:'단항식끼리의 나눗셈은 계수의 나눗셈과 문자의 나눗셈(지수법칙)을 각각 구합니다.',
  ...
  arena:{
    ...
    params:{mode:'chain'},
    rule:{ ko:'5분 안에 세 단항식 곱나눗 혼합을 모두 풉니다!', en:'Solve all the three-monomial × and ÷ mixes in 5 minutes!', zh:'5分钟内解答所有三个单项式的乘除混合题！' }
  },
