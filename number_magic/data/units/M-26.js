...
window.NM_UNITS['M-26'] = {
  ...
  practice:{
    ...
    params:{mode:'value'},
    intro:{
      ko:'x²-5x+6=0의 근을 구하기 전에, D=b²-4ac=25-24=1부터 계산해봅니다. D>0이니 서로 다른 두 실근이 있다는 걸 미리 알 수 있습니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'이차방정식은 근이 2개, 1개, 또는 0개일 수 있습니다. 근의 공식으로 직접 풀어보지 않고도 몇 개인지 미리 알 방법이 있을까요?',
      ...
      history:{ ko:'근의 공식 x=(-b±√(b²-4ac))/(2a) 안을 자세히 보면, 근호 안의 b²-4ac라는 값이 근의 운명을 결정합니다. 이 값이 양수·0·음수 중 무엇이냐에 따라 √ 안이 실수가 되는지, 근이 겹치는지가 갈립니다 — 그래서 "판별식"이라는 이름이 붙었습니다.',
    ...
    stages:[
      { tag:{ko:'① D=b²-4ac를 계산',en:'1) Compute D=b²-4ac',zh:'① 计算D=b²-4ac'},
        ...
        desc:{ko:'a=1, b=-5, c=6이니 D=(-5)²-4×1×6=25-24=<b>1</b>. D>0이므로 서로 다른 두 실근이 있습니다(실제로는 x=2,3).',
        ...
        book:{ko:'D>0: 서로 다른 두 실근, D=0: 중근(1개), D<0: 실근 없음 — 근을 구하지 않고도 미리 알 수 있습니다.',
      ...
      { tag:{ko:'② D=0이면 근이 겹친다',en:'2) D=0 means the roots coincide',zh:'② D=0说明根重合'},
        ...
        desc:{ko:'D=(-4)²-4×1×4=16-16=<b>0</b>. 근의 공식에서 √D=0이 되어 x=(-b)/(2a)=2 하나로 합쳐집니다. 실제로 x²-4x+4=(x-2)²이라 근이 정확히 2에서 겹칩니다.',
        ...
        result:{ko:'D=0이면 중근 하나로 합쳐집니다!',en:'D=0 collapses the roots into one repeated root!',zh:'D=0时根合并成一个重根！'},
        book:{ko:'중근 조건 D=0은 완전제곱식(x-p)²=0과 같은 뜻입니다 — 인수분해로도 확인할 수 있습니다.',
  ...
  check:{
    ...
    open:{ ko:'x²+kx+9=0이 중근을 갖도록 하는 양수 k를 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 중근 조건으로 계수를 역산하는 문제를 모두 풉니다!', en:'Solve all the repeated-root coefficient problems in 5 minutes!', zh:'5分钟内解答所有由重根条件反求系数的题！' }
  },
