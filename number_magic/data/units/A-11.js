  ...
  discover:{
    ...
    stages:[
      { tag:{ ko:'① 몇 십까지만 먼저 빼요', en:'1) Subtract just enough to reach a ten', zh:'① 先减到整十' },
        ...
        desc:{ ko:'43-8처럼 받아내림이 생기는 뺄셈은, <b>먼저 40이 되도록 3만 빼고</b>, 남은 5는 잠깐 쉬었다가 마저 빼요.',
               en:'For subtraction like 43-8 that needs borrowing, <b>first subtract just 3 to reach 40</b>, rest a moment, then subtract the remaining 5.',
               zh:'像43-8这样需要退位的减法，<b>先减3变成40</b>，歇一下，再减剩下的5。' },
      ...
      { tag:{ ko:'② 세 자리 수도 똑같아요', en:'2) Works for 3-digit numbers too', zh:'② 三位数也一样' },
        ...
        desc:{ ko:'큰 수여도 방법은 같아요. <b>가까운 몇 백(또는 몇 십)이 되도록</b> 먼저 조금 빼고, 나머지를 마저 빼요.',
               en:'Even with bigger numbers, the method is the same: subtract just enough to <b>reach the nearest hundred (or ten)</b>, then subtract the rest.',
               zh:'数再大，方法也一样：<b>先减到最近的整百（或整十）</b>，再减剩下的。' },
      ...
      { tag:{ ko:'③ 언제 쓰면 좋을까요?', en:'3) When is this handy?', zh:'③ 什么时候好用？' },
        ...
        desc:{ ko:'앞 수의 일의 자리보다 <b>빼는 수가 더 클 때</b>(예: 43-8) 이 방법이 좋아요. 이미 받아내림 없이 뺄 수 있는 경우(예: 48-12)엔 그냥 빼도 충분해요.',
               en:'This works great when the number you\'re subtracting is <b>bigger than the ones digit</b> (like 43-8). If no borrowing is needed (like 48-12), just subtract directly.',
               zh:'当<b>减数比个位数大时</b>（如43-8），这个方法很好用。不需要退位的（如48-12）直接减就行。' },
