...
window.NM_UNITS['M-74'] = {
  ...
  subtitle:{ ko:'두 점만 찾으면 직선은 그려져요 — x에 대입하거나, 절편을 구하거나', en:'Two points draw the line - substitute for x, or find the intercepts', zh:'找到两个点就能画出直线——代入x，或者求截距' },
  icon:'📐',

  practice:{
    generator:'md74_lineGraph', level:'practice', count:5,
    params:{mode:'twoPoints'},
    intro:{ ko:'x에 수를 넣어 y를 구하면 점 하나 — 두 번 하면 두 점이에요!', en:'Put a number in for x and work out y - one point. Do it twice and you have two!', zh:'把一个数代入x求出y就是一个点——做两次就有两个点！' }
  },

  discover:{
    title:{ ko:'누미의 마법 노트', en:"Numi's Magic Note", zh:'努米的魔法笔记' },
    story:{
      hook:{ ko:'y=2x+1의 그래프를 그려 볼까요? 직선은 두 점만 있으면 하나로 정해져요. x=1을 넣으면 y=3, x=2를 넣으면 y=5 — 점 (1,3)과 (2,5)가 나왔어요. 이 둘을 찍고 자를 대고 이으면 끝이에요. 점을 몇 개 더 찍어 봐도 전부 그 직선 위에 있어요.', en:'Shall we draw y=2x+1? A straight line is fixed by just two points. Put x=1 and y=3; put x=2 and y=5 - the points (1,3) and (2,5). Plot them, lay a ruler across, done. Plot a few more if you like: every one lands on that same line.', zh:'来画y=2x+1的图象吧？直线只要两个点就能确定。代入x=1得y=3，代入x=2得y=5——得到点(1,3)和(2,5)。描出这两点，用尺子连起来就完成了。再多描几个点，全都落在这条直线上。' },
      history:{ ko:'절편(intercept)은 "가로채다"라는 뜻의 라틴어에서 왔어요. 직선이 축을 가로채는 자리라는 뜻이죠. x절편과 y절편도 두 점이라, 이 둘을 찍으면 자로 직선을 그릴 수 있어요 — 그래서 손으로 그릴 때 가장 빠른 방법으로 쓰여요.', en:'The word intercept comes from Latin for "to catch between" - the place where the line catches an axis. The x-intercept and y-intercept are two points too, so plotting them and using a ruler is the quickest way to draw a line by hand.', zh:'截距(intercept)来自拉丁语"截取"的意思，就是直线截住坐标轴的地方。x截距和y截距也是两个点，标出它们用尺子就能画出直线——所以这是手画图象最快的办法。' }
    },
    stages:[
      { tag:{ ko:'① 두 점 찾기 — x에 대입', en:'1) Two points - substitute for x', zh:'① 找两个点——代入x' },
        ...
        desc:{ ko:'x=1을 넣으면 y=2×1+1=3, x=2를 넣으면 y=2×2+1=5예요. 점 (1,3)과 (2,5)를 찍고 <b>자로 이으면</b> 그래프예요. x에 어떤 수를 넣어도 되지만 계산이 쉬운 작은 정수가 좋아요.', en:'Put x=1 and y=2×1+1=3; put x=2 and y=2×2+1=5. Plot (1,3) and (2,5) and <b>join them with a ruler</b>. Any x will do, but small integers keep the arithmetic easy.', zh:'代入x=1得y=2×1+1=3，代入x=2得y=2×2+1=5。描出(1,3)和(2,5)，<b>用尺子连起来</b>就是图象。x代什么都行，但小整数算起来最方便。' },
        ...
        book:{ ko:'직선은 두 점으로 하나가 정해져요. 세 번째 점을 넣어 보면 <b>검산</b>이 돼요 — 그 점이 직선 위에 없으면 앞의 계산이 틀린 거예요.', en:'Two points fix one straight line. A third point is a <b>check</b>: if it misses the line, an earlier calculation was wrong.', zh:'两点确定一条直线。再代第三个点就是<b>检验</b>——如果它不在直线上，说明前面算错了。' } },
      ...
      { tag:{ ko:'③ 기울기로 두 번째 점', en:'3) The slope gives the second point', zh:'③ 用斜率找第二个点' },
        ...
        book:{ ko:'y=ax+b의 그래프는 y=ax의 그래프를 y축의 방향으로 b만큼 <b>평행이동</b>한 거예요. 기울기가 같으니 평행하고, b는 얼마나 위에서 출발하느냐만 정해요 — 그리는 방법이 아니라 그린 뒤에 보이는 성질이에요.', en:'The graph of y=ax+b is the graph of y=ax <b>translated</b> b along the y-axis. Same slope means parallel; b only sets how high the line starts - a property you notice after drawing, not a way to draw.', zh:'y=ax+b的图象是把y=ax的图象沿y轴方向<b>平移</b>b得到的。斜率相同所以平行，b只决定从多高出发——这是画完之后看到的性质，不是画法。' } }
    ],
    rule:{ ko:'직선은 두 점이면 그려져요 — x에 대입해서 두 점, 또는 y=0·x=0으로 두 절편, 또는 y절편에서 기울기만큼 한 걸음!', en:'Two points draw the line - substitute two values of x, or take the two intercepts (y=0, x=0), or step from the y-intercept by the slope!', zh:'两个点就能画直线——代入两个x值，或者用y=0、x=0求两个截距，或者从y截距按斜率走一步！' }
  },
  ...
  check:{
    ...
    open:{ ko:'y=2x+3의 그래프를 그리려면 어떤 두 점을 찾으면 되는지 말해봐요.', en:'Which two points would you find to draw y=2x+3?', zh:'说说画y=2x+3的图象要找哪两个点。' },
    openHint:{ ko:'x=0이면 (0,3), x=1이면 (1,5) — 아무 x 두 개면 돼요. 절편 (0,3)과 (−1.5,0)도 좋아요', en:'x=0 gives (0,3), x=1 gives (1,5) - any two x values work. The intercepts (0,3) and (-1.5,0) do too', zh:'x=0得(0,3)，x=1得(1,5)——任意两个x都行。截距(0,3)和(−1.5,0)也可以' }
  },

  lab:{
    generator:'md74_lineGraph', level:'main', count:4,
    params:{mode:'intercepts'},
    intro:{ ko:'y=0을 넣어 x절편을, x=0을 넣어 y절편을 구해 보세요 — 이 둘이 두 점이에요!', en:'Put y=0 for the x-intercept and x=0 for the y-intercept - those are your two points!', zh:'令y=0求x截距，令x=0求y截距——这就是两个点！' }
  },
