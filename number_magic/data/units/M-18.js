...
window.NM_UNITS['M-18'] = {
  ...
  subtitle:{ ko:'분모에서 근호를 쫓아냅니다 — 분자·분모에 같은 근호를 곱해서', en:'Chase the root out of the denominator — multiply top and bottom by the same root', zh:'把根号从分母赶出去——分子分母同乘一个相同的根号' },
  icon:'🚪',
  ...
  practice:{
    ...
    params:{level:'plain'},
    intro:{
      ko:'1/√2는 분모에 근호가 있어 계산이 불편합니다. 분자·분모에 √2를 곱하면 √2/2로 바뀝니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'1/√2를 계산기 없이 어림해야 한다면 어떨까요? 분모에 근호가 있으면 크기를 가늠하기 어렵습니다 — 분모를 "깨끗한 정수"로 만들 방법이 있을까요?',
      ...
      history:{ ko:'분모의 유리화는 계산기가 없던 시절 특히 중요했습니다. 손으로 근삿값을 계산할 때, 근호가 분모에 있으면 나눗셈이 훨씬 번거로웠습니다 — 분자로 옮겨두면 계산이 한결 쉬워졌습니다.',
    ...
    stages:[
      { tag:{ko:'① 같은 근호를 분자·분모에 곱하기',en:'1) Multiply top and bottom by the same root',zh:'① 分子分母同乘一个相同的根号'},
        ...
        desc:{ko:'√2×√2=2(제곱근을 다시 제곱하면 근호가 사라짐)를 이용합니다. 분자와 분모에 <b>똑같이 √2를 곱하면</b> 분수의 값은 그대로인데 분모의 근호만 사라져요: 1×√2/(√2×√2) = √2/2.',
        ...
        result:{ko:'분모·분자에 같은 근호를 곱하면 분모의 근호가 사라집니다!',en:'Multiply top and bottom by the same root, and the denominator\'s root disappears!',zh:'分子分母同乘相同根号，分母的根号就消失了！'},
        book:{ko:'분모에 근호가 있는 분수를, 분자·분모에 같은 수를 곱해 분모를 유리수(근호 없는 수)로 만드는 것을 <b>분모의 유리화</b>라고 합니다.',
      ...
      { tag:{ko:'② 분모의 근호부터 정리해야 할 때',en:'2) When the denominator\'s root needs simplifying first',zh:'② 分母的根号需要先化简的时候'},
        ...
        result:{ko:'근호 정리 → 유리화, 두 기술을 순서대로 이어 씁니다!',en:'Simplify the radical, then rationalize — chain the two skills in order!',zh:'先化简根号，再有理化——按顺序衔接这两项技能！'},
  ...
  check:{
    fills:[
      { tex:'\\dfrac{1}{\\sqrt3} = \\dfrac{\\sqrt{\\square}}{\\square}', answer:[3,3],
        hint:{ ko:'분자·분모에 √3을 곱합니다', en:'Multiply top and bottom by √3', zh:'分子分母同乘√3' } },
      { tex:'\\dfrac{4}{\\sqrt5} = \\dfrac{4\\sqrt{\\square}}{\\square}', answer:[5,5],
        hint:{ ko:'계수 4는 그대로 두고 근호만 옮깁니다', en:'Keep the coefficient 4 as is, only move the root', zh:'系数4不变，只处理根号' } }
    ],
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 분모부터 정리해야 하는 유리화를 모두 풉니다!', en:'Solve all the rationalize-after-simplifying problems in 5 minutes!', zh:'5分钟内解答所有需要先化简分母的有理化题！' }
  },
