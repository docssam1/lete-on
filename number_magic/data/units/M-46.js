...
window.NM_UNITS['M-46'] = {
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'미분은 f(x)에서 f\'(x)를 만들었습니다. 그럼 <b>거꾸로</b> f\'(x)만 알 때 원래 f(x)를 되찾을 수 있을까요? 그리고 울퉁불퉁한 곡선 아래 넓이는 어떻게 구할까요 — 답이 같은 기호에서 나옵니다. 바로 <b>∫</b>입니다.',
      ...
      history:{ ko:'라이프니츠는 1675년에 "합(summa)"이라는 라틴어의 첫 글자 S를 위아래로 길게 늘여 <b>∫</b> 모양을 만들었습니다. "잘게 쪼갠 조각들을 다 더한다"는 뜻을 글자 모양에 그대로 담은 거입니다.',
    ...
    stages:[
      { tag:{ko:'① ∫는 미분을 거꾸로 — 부정적분',en:'1) ∫ undoes differentiation — the antiderivative',zh:'① ∫是求导的逆过程——不定积分'},
        ...
        desc:{ko:'앞서 3x²을 미분하면 6x가 됐죠? ∫는 그 <b>반대 방향</b>입니다 — 6x를 적분하면 다시 3x²이 나옵니다. 그런데 3x²+1, 3x²+7도 미분하면 똑같이 6x가 되니, "어떤 상수인지는 알 수 없다"는 뜻으로 <b>+C</b>(적분상수)를 꼭 붙입니다.',
        ...
        result:{ko:'∫는 미분의 반대 — 계수를 (n+1)로 나누고 지수를 하나 늘립니다!',en:'∫ reverses differentiation — divide the coefficient by (n+1) and raise the exponent by one!',zh:'∫是求导的逆过程——系数除以(n+1)，指数加1！'},
        book:{ko:'∫axⁿdx = (a÷(n+1))xⁿ⁺¹ + C — 미분 규칙 naxⁿ⁻¹을 거꾸로 뒤집은 것뿐입니다.',
      ...
      { tag:{ko:'② 정적분은 구간의 넓이 — F(끝)-F(처음)',en:'2) A definite integral is an area — F(end)-F(start)',zh:'② 定积分是面积——F(终点)-F(起点)'},
        ...
        desc:{ko:'∫ 아래 1, 위 3은 "1부터 3까지의 구간"을 뜻합니다. 원시함수 F(x)=3x²를 구한 뒤, F(3)-F(1)=27-3=24를 계산합니다. +C는 뺄셈에서 저절로 사라지니 정적분에는 붙이지 않습니다.',
        ...
        result:{ko:'정적분은 원시함수 F로 F(끝)-F(처음)을 계산합니다!',en:'A definite integral is F(end)-F(start) using the antiderivative F!',zh:'定积分是用原函数F计算F(终点)-F(起点)！'},
        book:{ko:'구간을 무한히 잘게 쪼갠 좁은 직사각형들의 넓이를 다 더한 것이 정적분의 원래 뜻입니다 — ∫라는 글자 모양(길게 늘인 S) 그대로입니다.',
  ...
  check:{
    ...
    open:{ ko:'∫[0,2] 6x dx의 값을 원시함수를 이용해 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 정적분 값을 모두 구합니다!', en:'Find all the definite integral values in 5 minutes!', zh:'5分钟内求出所有定积分的值！' }
  },
