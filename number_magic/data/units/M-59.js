...
window.NM_UNITS['M-59'] = {
  ...
  subtitle:{ ko:'끊어지지 않으려면 극한값과 함숫값이 같아야 합니다', en:'To avoid a break, the limit must equal the function value', zh:'不断开，就要极限值等于函数值' },
  icon:'🔗',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'함수가 어떤 점에서 연속이려면, 그 점에서의 극한값과 그 점의 함숫값이 정확히 같아야 합니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'연필을 떼지 않고 그래프를 그릴 수 있으면 "연속"입니다. 그런데 x=2에서만 값이 원래와 다른 이상한 점(k)이 있다면 어떻게 될까요? k가 딱 맞는 값이 아니면 그 점에서 그래프가 뚝 끊어지거나 튑니다.',
      ...
      history:{ ko:'"연속"의 정확한 수학적 정의(극한값=함숫값)는 19세기 독일 수학자 바이어슈트라스가 엄밀하게 정리했습니다. 그전까지는 "연속"이 "손을 떼지 않고 그린다"는 직관적인 설명뿐이었습니다.',
    ...
    stages:[
      { tag:{ko:'① k를 그 점의 극한값과 같게 두면 이어집니다',en:'1) Setting k equal to the limit there connects the pieces',zh:'① 把k设成该点的极限值就能接上'},
        ...
        desc:{ko:'x≠3일 땐 (x²-9)/(x-3)=(x-3)(x+3)/(x-3)=x+3입니다. x=3 근처의 극한값은 3+3=6. 이 함수가 <b>x=3에서 연속이려면</b> k도 정확히 6이어야 합니다.',
        ...
        book:{ko:'만약 k가 6이 아닌 다른 값이면, x=3인 그 한 점만 그래프에서 살짝 떨어져 나갑니다(불연속점).',
      ...
      { tag:{ko:'② 구간별 함수는 경계에서 값이 같아야 합니다',en:'2) Piecewise functions must match at the boundary',zh:'② 分段函数在边界处值要相等'},
        ...
        result:{ko:'구간별 함수는 경계에서 왼쪽 식과 오른쪽 식의 값이 같아야 이어집니다!',en:'A piecewise function connects only if the left and right expressions agree at the boundary!',zh:'分段函数只有在边界处左右两式相等才能连接！'},
        book:{ko:'이 유형은 극한 계산이 아니라 "대입해서 값 맞추기"라서 다항식 대입만 있으면 됩니다(MD43 factorCancel과는 다른 접근).',
    ...
    rule:{ ko:'함수가 한 점에서 연속이려면 그 점의 함숫값과 극한값이 같아야 합니다 — 미정계수는 그 값에 맞춰서 정합니다!',
  ...
  check:{
    ...
    open:{ ko:'f(x)=(x²-25)/(x-5)(x≠5), k(x=5)가 연속이 되도록 k를 구하는 과정을 설명해봅니다.',
  ...
  lab:{
    ...
    intro:{
      ko:'분자를 인수분해해서 분모와 같은 인수를 약분한 뒤, 남은 값에 x=a를 대입합니다!',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 구간별 일차함수의 경계를 모두 맞춥니다!', en:'Match every piecewise-linear boundary within 5 minutes!', zh:'5分钟内匹配所有分段一次函数的边界！' }
  },
