...
window.NM_UNITS['M-45'] = {
  ...
  subtitle:{ ko:'곡선에 살짝 닿는 직선의 기울기는 도함수입니다', en:'The slope of a line just touching a curve is its derivative', zh:'轻轻碰到曲线的直线，其斜率就是导数' },
  icon:'📏',
  ...
  practice:{
    ...
    params:{mode:'slope'},
    intro:{
      ko:'곡선 위 한 점에서 그 곡선에 살짝 닿는 직선(접선)의 기울기는, 바로 그 점에서의 미분계수 f\'(x₀)입니다!',
  ...
  discover:{
    story:{
      hook:{ ko:'곡선 위 한 점에서 기울기를 어떻게 잴까요? 직선이 아닙니다.',
        en:'How do you measure slope at a single point on a curve, when it is not a straight line?',
        zh:'曲线不是直线，怎样测量它某一点处的斜率？' },
      history:{ ko:'페르마는 뉴턴보다 13년, 라이프니츠보다 17년 먼저 곡선의 꼭대기와 바닥을 찾는 방법을 고안했습니다. 뉴턴은 흐르는 양의 변화를 유율(fluxion)이라 부르며 점 기호를 썼고, 라이프니츠는 dy/dx와 ∫를 만들었죠. 같은 개념인데 기호가 달랐고, 오늘 우리가 쓰는 쪽은 라이프니츠의 기호입니다.',
    ...
    stages:[
      { tag:{ko:'① 접선의 기울기 = 미분계수',en:'1) Tangent slope = derivative value',zh:'① 切线斜率=导数值'},
        ...
        desc:{ko:'앞 유닛에서 배운 미분계수 f\'(x₀)는 사실 "<b>곡선 위 그 점에서 접선이 얼마나 가파른가</b>"였습니다. f(x)=x²의 도함수는 f\'(x)=2x이니, x=3에서는 f\'(3)=6 — 그 점에 살짝 닿는 직선의 기울기가 바로 6입니다.',
        ...
        result:{ko:'접선의 기울기를 구하는 건 곧 미분계수를 구하는 거입니다!',en:'Finding a tangent slope is exactly finding a derivative value!',zh:'求切线斜率其实就是求导数值！'},
        book:{ko:'접선은 그 점 근처에서 곡선과 거의 겹쳐 보일 만큼 딱 맞게 스치는 직선입니다.',
      ...
      { tag:{ko:'② 접선의 방정식 y=mx+n',en:'2) The tangent line equation y=mx+n',zh:'② 切线方程y=mx+n'},
        ...
        desc:{ko:'접선의 기울기 m=f\'(x₀)=6을 구했으면, 그 점의 y좌표 f(3)=9를 이용해 y=m(x-x₀)+f(x₀)에 대입해요: y=6(x-3)+9=6x-18+9=6x-9. 정리하면 y=mx+n 꼴에서 n=f(x₀)-m·x₀입니다.',
        ...
        book:{ko:'접선의 방정식은 "그 점을 지나는 직선 중 기울기가 f\'(x₀)인 것"이라는 뜻입니다 — 두 조건(점, 기울기)만 있으면 직선이 하나로 정해집니다.',
  ...
  check:{
    fills:[
      { tex:{ko:'f(x)=x^2+1,\\;x_0=2 \\;\\Rightarrow\\; \\text{기울기}=\\square',en:'f(x)=x^2+1,\\;x_0=2 \\;\\Rightarrow\\; \\text{slope}=\\square',zh:'f(x)=x^2+1,\\;x_0=2 \\;\\Rightarrow\\; \\text{斜率}=\\square'}, answer:4,
    ...
    open:{ ko:'f(x)=x²-2x, x₀=2에서 접선의 방정식을 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 더 큰 범위의 접선의 방정식을 모두 구합니다!', en:'Find all the wider-range tangent line equations in 5 minutes!', zh:'5分钟内求出所有更大范围的切线方程！' }
  },
