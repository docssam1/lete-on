...
window.NM_UNITS['M-31'] = {
  ...
  subtitle:{ ko:'가로·세로 차를 제곱해 더하고 제곱근을 씌웁니다(피타고라스 정리)', en:'Square the horizontal and vertical differences, add, then take the root (Pythagorean theorem)', zh:'横纵坐标差平方后相加再开方(勾股定理)' },
  icon:'📏',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'(0,0)과 (3,4) 사이의 거리는? 가로로 3, 세로로 4 떨어졌으니 3²+4²=25, √25=5 — 직각삼각형의 빗변입니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'좌표평면 위 두 점 사이의 거리를 구하고 싶습니다. 자로 재지 않고, 좌표만 알면 계산으로 정확히 구할 방법이 있을까요?',
      ...
      history:{ ko:'두 점을 잇는 선분은 항상 가로 변화량과 세로 변화량으로 만든 직각삼각형의 빗변입니다. 2500년 전부터 알려진 피타고라스 정리(빗변²=밑변²+높이²)를 좌표에 그대로 적용하면 거리 공식이 나옵니다.',
    ...
    stages:[
      { tag:{ko:'① 가로·세로 차를 구해 직각삼각형',en:'1) The differences form a right triangle',zh:'① 横纵差构成直角三角形'},
        ...
        desc:{ko:'가로로 3-0=3, 세로로 4-0=4만큼 떨어져 있습니다. 이 3과 4가 직각삼각형의 두 변, AB가 빗변이니 <b>3²+4²=25, √25=5</b>.',
        ...
        result:{ko:'가로·세로 차를 제곱해 더한 뒤 제곱근을 씌웁니다!',en:'Square the differences, add, then take the square root!',zh:'把横纵差平方后相加，再开平方！'},
      ...
      { tag:{ko:'② 완전제곱수가 아니면 근호 정리',en:'2) Simplify the radical if it isn\'t a perfect square',zh:'② 不是完全平方数就化简根号'},
        ...
        desc:{ko:'1²+2²=5는 완전제곱수가 아니라 √5 그대로 남습니다. 더 큰 수라면 √48=4√3처럼 근호 정리(MD16)로 계수와 근호 안을 나눠 답합니다.',
        ...
        result:{ko:'답은 [계수,근호안] 두 정수 — 정리가 안 되면 계수는 1입니다!',en:'The answer is [coefficient, radicand] — if it doesn\'t simplify, the coefficient is just 1!',zh:'答案是[系数,根号内]两个整数——不能化简时系数就是1！'},
        book:{ko:'근호 정리 규약을 그대로 재사용합니다 — 거리 공식만의 새로운 규약을 만들지 않습니다.',
  ...
  check:{
    ...
    open:{ ko:'A(-1,2), B(2,-2) 사이의 거리를 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    params:{mode:'signed'},
    rule:{ ko:'5분 안에 음수 좌표가 섞인 거리 문제를 모두 풉니다!', en:'Solve all the distance problems with negative coordinates in 5 minutes!', zh:'5分钟内解答所有带负坐标的距离题！' }
  },
