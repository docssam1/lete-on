...
window.NM_UNITS['M-54'] = {
  ...
  subtitle:{ ko:'증가함수는 부등호 방향을 그대로 넘겨줍니다', en:'An increasing function passes the inequality direction straight through', zh:'增函数会原样传递不等号方向' },
  icon:'📶',
  ...
  practice:{
    ...
    intro:{
      ko:'밑이 1보다 크면 지수함수·로그함수 둘 다 커질수록 값도 커지는 증가함수입니다 — 등호가 성립하는 경계값부터 찾습니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'2ˣ은 x가 커질수록 값도 커집니다(2¹=2, 2²=4, 2³=8…) — 이런 함수를 "증가함수"라고 합니다. 그래서 2ˣ>2³이면, x도 3보다 커야 해요: x>3.',
      ...
      history:{ ko:'"증가함수는 부등호 방향을 유지한다"는 원리는 함수의 단조성(單調性) 개념에서 나옵니다. 반대로 밑이 1보다 작으면(예: (1/2)ˣ) 감소함수가 되어 부등호 방향이 뒤집힙니다 — 이 유닛에서는 밑이 항상 1보다 큰 경우만 다룹니다.',
    ...
    stages:[
      { tag:{ko:'① 지수부등식 — 등호가 성립하는 경계부터',en:'1) Exponential inequality — start from where equality holds',zh:'① 指数不等式——从等号成立处开始'},
        ...
        desc:{ko:'밑 3은 1보다 크니 3ˣ은 증가함수입니다. <b>등호가 성립하는 자리</b>(x=4)를 먼저 찾고, 부등호(≥)는 원래 방향 그대로 x≥4가 됩니다.',
        ...
        result:{ko:'증가함수는 지수(또는 진수)의 부등호 방향을 그대로 넘겨줍니다!',en:'An increasing function passes the inequality direction of the exponent (or argument) straight through!',zh:'增函数会把指数(或真数)的不等号方向原样传递！'},
        book:{ko:'경계값 자체가 부등식의 답에 포함되는지(≥,≤)와 포함되지 않는지(>,<)를 꼭 확인합니다.',
      ...
      { tag:{ko:'② 로그부등식도 방향은 그대로',en:'2) The log inequality also keeps the direction',zh:'② 对数不等式方向也不变'},
        ...
        desc:{ko:'밑 2가 1보다 크니 log₂도 증가함수입니다. <b>등호가 성립하는 경계</b>: x+1=2³=8, x=7. 부등호(≤)는 그대로 유지되어 x≤7이 됩니다.',
        ...
        result:{ko:'로그부등식도 log의 정의로 경계값을 찾고, 방향은 그대로 넘깁니다!',en:'For log inequalities too, find the boundary with the definition of log, and the direction carries through!',zh:'对数不等式也是用log的定义找边界，方向原样传递！'},
        book:{ko:'로그부등식은 진수가 0보다 커야 한다는 조건도 잊지 않아야 합니다(진수 조건).',
              en:'For log inequalities, don\'t forget the argument must stay positive (the argument condition).',
              zh:'对数不等式也别忘了真数必须大于0(真数条件)。'} }
    ],
    rule:{ ko:'밑이 1보다 크면 지수·로그 부등식 둘 다 증가함수 — 등호가 성립하는 경계값을 찾으면 부등호 방향은 그대로입니다!',
  ...
  check:{
    ...
    open:{ ko:'3^(x-1)≥27을 경계값 찾기부터 설명해봅니다.',
  ...
  lab:{
    ...
    intro:{
      ko:'지수에 계수가 붙어도 경계값 찾는 방법은 같습니다!',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 로그부등식의 경계값을 모두 찾습니다!', en:'Find the boundary value of every log inequality within 5 minutes!', zh:'5分钟内找出所有对数不等式的边界值！' }
  },
