...
window.NM_UNITS['M-61'] = {
  ...
  subtitle:{ ko:'두 교점을 구간 삼아 정적분합니다', en:'Take the two intersection points as the interval and integrate', zh:'以两个交点为区间求定积分' },
  icon:'🏔️',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'곡선이 x축과 만나는 두 점 사이의 넓이는 그 구간의 정적분값입니다 — 원시함수 F를 구해 F(q)-F(p)만 계산하면 됩니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'포물선 f(x)=-(x-1)(x-4)를 그리면, x=1과 x=4에서 x축을 만나고 그 사이에서는 곡선이 x축 위로 볼록 솟아 있습니다. 이 볼록한 부분과 x축 사이의 넓이는 얼마일까요? MD46에서 배운 정적분이 답을 줍니다.',
      ...
      history:{ ko:'"곡선 아래 넓이를 구하는 문제"는 고대 그리스 아르키메데스가 포물선 조각의 넓이를 구하면서 처음 다뤘습니다(기원전 3세기, "소진법"이라는 방법으로). 뉴턴·라이프니츠의 미적분학이 이걸 F(q)-F(p)라는 간단한 계산으로 바꿔놓았습니다.',
    ...
    stages:[
      { tag:{ko:'① 두 교점을 정적분 구간으로 삼습니다',en:'1) Take the two intersection points as the integral\'s interval',zh:'① 以两个交点为定积分的区间'},
        ...
        desc:{ko:'f(x)=−(x-1)(x-4)를 전개하면 −x²+5x−4입니다. x=1과 x=4에서 x축과 만나고, 그 사이에서 f(x)≥0이니 <b>[1,4] 구간의 정적분</b>이 곧 넓이입니다.',
        ...
        book:{ko:'이 유닛의 문항은 항상 두 교점(p,q)과 볼록한 정도(계수)를 먼저 정해두고 거꾸로 f(x)를 만듭니다 — 그래서 넓이가 항상 정수로 딱 떨어집니다.',
  ...
  check:{
    ...
    open:{ ko:'f(x)=-x²+2x가 x축과 만나는 두 점을 찾고, 그 사이 넓이를 구하는 과정을 설명해봅니다.',
  ...
  lab:{
    ...
    params:{mode:'wide'},
    intro:{
      ko:'교점이 음수여도 방법은 같습니다 — 원시함수를 구해 F(q)-F(p)를 계산합니다!',
  ...
  arena:{
    ...
    params:{mode:'wider'},
    rule:{ ko:'5분 안에 더 큰 범위의 넓이까지 모두 구합니다!', en:'Find every wider-range area within 5 minutes!', zh:'5分钟内求出所有更大范围的面积！' }
  },
