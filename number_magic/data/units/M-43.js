...
window.NM_UNITS['M-43'] = {
  ...
  subtitle:{ ko:'x가 다가갈 때 식이 어디로 향하는지 알아봅니다', en:'Find where an expression heads as x approaches a value', zh:'看看当x靠近某个值时式子会趋向哪里' },
  icon:'🧭',
  symbols:[
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'f(x)=(x²-4)/(x-2)에 x=2를 넣으면 0/0이 돼서 계산할 수가 없습니다. 그런데 x=1.9, 1.99, 1.999…처럼 2에 아주 가까운 수를 넣으면 어떤 값에 점점 다가갈까요? "딱 그 값은 아니지만 한없이 가까이 갈 때"를 나타내는 기호가 필요합니다 — 그게 <b>lim</b>입니다.',
      ...
      history:{ ko:'lim은 "한계(limit)"라는 영어 단어를 줄인 거입니다. "가까워진다"는 뜻의 화살표(→) 표기는 여러 수학자들이 조금씩 다듬다가 20세기 초에 지금 모습으로 자리 잡았습니다.',
    ...
    stages:[
      { tag:{ko:'① 다항식은 그냥 대입하면 끝',en:'1) For polynomials, just substitute',zh:'① 多项式直接代入就行'},
        ...
        desc:{ko:'다항식은 <b>끊어지는 지점이 없습니다</b>(연속함수). 그래서 x가 3에 가까워질 때 2x+1이 어디로 가는지는, 그냥 x=3을 넣어보면 알 수 있어요: 2×3+1=7. "한없이 가까워지는 값"과 "그 값을 직접 대입한 값"이 다항식에서는 항상 같습니다.',
        ...
        book:{ko:'연속함수는 lim(x→a) f(x) = f(a)가 항상 성립합니다 — 극한값과 함숫값이 일치한다는 뜻입니다.',
      ...
      { tag:{ko:'② 0/0 꼴은 인수분해로 뚫습니다',en:'2) Break through a 0/0 form by factoring',zh:'② 0/0型用因式分解突破'},
        head:{ko:'\\lim_{x\\to 2} \\dfrac{x^2-4}{x-2} = 4',en:'\\lim_{x\\to 2} \\dfrac{x^2-4}{x-2} = 4',zh:'\\lim_{x\\to 2} \\dfrac{x^2-4}{x-2} = 4'},
        desc:{ko:'x=2를 그대로 넣으면 0/0이 돼서 막힙니다. 그런데 분자 x²-4=(x-2)(x+2)로 인수분해하면 분모 (x-2)와 <b>약분</b>할 수 있습니다. 남는 건 (x+2)뿐이고, 여기에 x=2를 넣으면 4 — 막혔던 길이 뚫립니다.',
        ...
        result:{ko:'0/0이면 인수분해해서 같은 인수를 약분합니다!',en:'For a 0/0 form, factor and cancel the matching factor!',zh:'0/0型就因式分解，约去相同的因式！'},
        book:{ko:'약분은 x=2 그 자리에서는 원래 정의되지 않지만, "그 근처에서" 무슨 일이 일어나는지는 알려줍니다 — 그게 극한의 힘입니다.',
  ...
  check:{
    ...
    open:{ ko:'lim(x→3) (x²-9)/(x-3)의 값을 인수분해 과정과 함께 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 더 큰 범위의 극한값을 모두 구합니다!', en:'Find all the wider-range limits in 5 minutes!', zh:'5分钟内求出所有更大范围的极限值！' }
  },
