...
window.NM_UNITS['M-60'] = {
  ...
  subtitle:{ ko:"f'(x)=0인 자리가 산꼭대기와 골짜기입니다", en:"Where f'(x)=0 are the hilltops and valleys", zh:"f'(x)=0的位置就是山顶和谷底" },
  icon:'⛰️',
  ...
  practice:{
    ...
    params:{mode:'points'},
    intro:{
      ko:"곡선이 올라가다가 잠깐 멈추고(기울기 0) 다시 내려가는 지점 — 그게 극값입니다. f'(x)=0을 풀면 그 자리를 찾을 수 있습니다!",
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'롤러코스터가 산꼭대기에 도착하는 순간, 아주 잠깐 수평이 됩니다(기울기=0) — 그 직후엔 다시 내려가죠. 이 "잠깐 수평이 되는 지점"이 바로 극댓값입니다. 골짜기의 가장 낮은 지점도 마찬가지로 기울기가 0입니다(극솟값).',
      ...
      history:{ ko:'"기울기가 0인 곳에서 극값이 생긴다"는 원리는 17세기 페르마가 처음 발견했습니다(미분이 발명되기도 전입니다!). 뉴턴과 라이프니츠의 미적분학이 이 원리를 f\'(x)=0이라는 정확한 방정식으로 다듬었습니다.',
    ...
    stages:[
      { tag:{ko:"① f'(x)=0을 풀면 극값의 자리가 나옵니다",en:"1) Solving f'(x)=0 finds where extrema occur",zh:"① 解f'(x)=0就能找到极值的位置"},
        ...
        desc:{ko:"먼저 도함수를 구해요: f'(x)=3x²-6x. 이걸 0으로 두면 3x(x-2)=0이니 <b>x=0 또는 x=2</b> — 이 두 자리가 곡선이 잠깐 수평이 되는 지점입니다.",
        ...
        book:{ko:'x=0에서는 부호가 +에서 −로 바뀌어(극대), x=2에서는 −에서 +로 바뀝니다(극소) — 부호가 안 바뀌면 극값이 아닙니다.',
      ...
      { tag:{ko:'② 그 x를 f(x)에 다시 넣으면 극값(높이)이 나옵니다',en:'2) Substituting that x back into f(x) gives the extreme value',zh:'② 把那个x代回f(x)就得到极值(高度)'},
        ...
        result:{ko:'극값(높이)을 구하려면 그 x를 원래 함수 f(x)에 대입합니다!',en:'To find the extreme value (height), substitute that x back into the original f(x)!',zh:'求极值(高度)要把那个x代回原函数f(x)！'},
        book:{ko:'x가 작은 쪽(x=0)이 극대, 큰 쪽(x=2)이 극소가 된 건 이 함수의 최고차항 계수가 양수(x³)이기 때문입니다.',
    ...
    rule:{ ko:"f'(x)=0을 풀어 극값의 x좌표를 찾고, 그 x를 f(x)에 다시 대입하면 극댓값·극솟값(높이)이 나옵니다!",
  ...
  check:{
    ...
    open:{ ko:'f(x)=x³-12x의 극값을 구하는 전체 과정을 설명해봅니다.',
  ...
  lab:{
    ...
    params:{mode:'values'},
    intro:{
      ko:"f'(x)=0인 x를 찾은 뒤, f(x)에 다시 대입해서 극댓값·극솟값을 구합니다!",
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 더 큰 범위의 극값까지 모두 구합니다!', en:'Find every wider-range extremum within 5 minutes!', zh:'5分钟内求出所有更大范围的极值！' }
  },
