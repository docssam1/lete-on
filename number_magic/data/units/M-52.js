...
window.NM_UNITS['M-52'] = {
  ...
  subtitle:{ ko:'밑을 통일하면 지수끼리 등식이 됩니다', en:'Unify the base, and the exponents form an equation', zh:'统一底数，指数就构成等式' },
  icon:'🪜',
  ...
  practice:{
    ...
    intro:{
      ko:'밑이 같은 지수식이 등호로 이어져 있으면, 지수끼리도 똑같이 등호로 이어집니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'2ˣ=8이라는 식을 봤습니다. 8을 2의 거듭제곱으로 바꿔보면 8=2³이니 2ˣ=2³이 됩니다. 밑이 똑같이 2라면, 남은 건 지수뿐입니다 — x=3이라는 게 바로 보입니다!',
      ...
      history:{ ko:'지수함수 aˣ가 x값 하나마다 결과가 딱 하나씩 대응(일대일 대응)한다는 성질은 17세기 로그의 발명과 함께 명확해졌습니다. 이 성질 덕분에 밑만 같으면 지수방정식은 복잡한 계산 없이 눈으로 풀 수 있습니다.',
    ...
    stages:[
      { tag:{ko:'① 밑이 이미 같으면 지수끼리 비교',en:'1) When the bases already match, compare exponents',zh:'① 底数已经相同就比较指数'},
        ...
        desc:{ko:'양변의 밑이 똑같이 3입니다. 지수함수는 <b>일대일 대응</b>이라, 밑이 같으면 지수도 같아야 해요: x+1=4. 이건 그냥 일차방정식이니 x=3.',
        ...
        result:{ko:'밑이 같으면 지수끼리 그대로 등식이 됩니다!',en:'With equal bases, the exponents themselves form the equation!',zh:'底数相同，指数本身就构成等式！'},
        book:{ko:'밑이 음수이거나 0, 1이면 이 성질이 성립하지 않습니다 — 지수방정식은 밑이 1이 아닌 양수일 때만 다룹니다.',
      ...
      { tag:{ko:'② 우변이 아직 수라면, 먼저 밑을 통일',en:'2) If the right side is still a plain number, unify the base first',zh:'② 右边还是个数就先统一底数'},
        ...
        desc:{ko:'16은 2의 거듭제곱(2⁴)으로 바꿀 수 있습니다. 그러면 2ˣ=2⁴가 되고, 밑이 같아졌으니 <b>지수끼리 비교</b>해서 x=4.',
        ...
        result:{ko:'수로 남아 있으면 먼저 같은 밑의 거듭제곱으로 바꿉니다(밑 통일)!',en:'If a side is still a plain number, first rewrite it as a power of the same base (unify the base)!',zh:'还是个数就先改写成同底的幂(统一底数)！'},
        book:{ko:'32=2⁵, 27=3³처럼 밑 통일에 필요한 거듭제곱 표를 눈에 익혀두면 훨씬 빨라집니다.',
              en:'Memorizing powers like 32=2⁵ or 27=3³ makes unifying the base much faster.',
              zh:'熟记32=2⁵、27=3³这样的幂表，统一底数会快得多。'} }
    ],
    rule:{ ko:'지수방정식: 밑이 같으면 지수끼리 등식 — 우변이 수로 남아 있으면 먼저 같은 밑의 거듭제곱으로 바꿉니다(밑 통일)!',
  ...
  check:{
    fills:[
      { tex:'5^{x} = 5^3 \\;\\Rightarrow\\; x = \\square', answer:3,
        hint:{ ko:'밑이 같으니 지수도 같습니다', en:'Same base, so same exponent', zh:'底数相同，指数也相同' } },
      { tex:'2^{x} = 32 \\;\\Rightarrow\\; x = \\square', answer:5,
        hint:{ ko:'32=2^5', en:'32=2^5', zh:'32=2^5' } }
    ],
    open:{ ko:'3^(2x-1)=81을 밑 통일부터 시작해 설명해봅니다.',
      en:'Explain 3^(2x-1)=81, starting from unifying the base.',
      zh:'从统一底数开始说说3^(2x-1)=81。' },
    openHint:{ ko:'81=3^4, 2x-1=4, x=2.5는 정수가 아니니 문제를 다시 확인 — 실제로는 정수해가 되도록 값을 고릅니다',
  ...
  lab:{
    ...
    intro:{
      ko:'지수에 계수가 붙어도 방법은 같습니다 — 지수끼리 등식을 세우고 풉니다!',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 밑 통일부터 시작하는 지수방정식을 모두 풉니다!', en:'Solve every exponential equation, starting from unifying the base, within 5 minutes!', zh:'5分钟内解出所有需先统一底数的指数方程！' }
  },
