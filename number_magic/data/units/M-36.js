...
window.NM_UNITS['M-36'] = {
  ...
  subtitle:{ ko:'근호 속 지수를 분수 지수로 옮겨 적습니다', en:'Rewrite a radical\'s exponent as a fraction', zh:'把根号里的指数写成分数指数' },
  icon:'🪜',
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'⁵√(2³)은 2^(3/5)와 같습니다 — 근호의 지수(5)는 분모, 거듭제곱의 지수(3)는 분자로!',
  ...
  discover:{
    story:{
      ...
      history:{ ko:'16세기 후반, 항해술과 천문학은 자릿수가 어마어마한 수를 곱하고 나누어야 했습니다. 계산은 전부 손으로 했으니 그 일은 차라리 고통에 가까웠죠. 지수와 로그는 그 고통을 덜려는 아주 현실적인 이유에서 태어났습니다.',
    ...
    stages:[
      { tag:{ko:'① 근호 지수를 분수 지수로',en:'1) A root\'s index becomes a fraction exponent',zh:'① 把根号的指数写成分数指数'},
        ...
        desc:{ko:'³√(2²)을 지수로 옮겨 쓰면 밑은 그대로 2, 지수는 분수 2/3가 됩니다. 근호의 작은 숫자(3, 몇 제곱근인지)는 분모로, 근호 안의 지수(2)는 분자로 갑니다 — <b>"근호는 분모, 거듭제곱은 분자"</b>로 외웁니다.',
        ...
        book:{ko:'ⁿ√(aᵐ) = a^(m/n) — 이 식이 유리수 지수의 정의입니다.',
              en:'ⁿ√(aᵐ) = a^(m/n) — this is the definition of a rational exponent.',
              zh:'ⁿ√(aᵐ) = a^(m/n)——这就是有理数指数的定义。'} },

      { tag:{ko:'② 지수를 항상 기약분수로 줄입니다',en:'2) Always reduce the exponent to lowest terms',zh:'② 指数一定要约成最简分数'},
        head:{ko:'\\sqrt[6]{2^4} = 2^{\\frac{4}{6}} = 2^{\\frac{2}{3}}',en:'\\sqrt[6]{2^4} = 2^{\\frac{4}{6}} = 2^{\\frac{2}{3}}',zh:'\\sqrt[6]{2^4} = 2^{\\frac{4}{6}} = 2^{\\frac{2}{3}}'},
        desc:{ko:'⁶√(2⁴)을 그대로 옮기면 2^(4/6)인데, 4/6은 아직 기약분수가 아닙니다. 분자·분모를 2로 나누면 2/3 — 방금 본 것과 <b>똑같은 수</b>입니다. 분수처럼 지수도 항상 끝까지 약분해서 답해야 합니다.',
        ...
        result:{ko:'지수 분수도 기약분수로 끝까지 줄입니다!',en:'Reduce the exponent fraction all the way, just like any fraction!',zh:'指数分数也要约到最简为止！'},
        book:{ko:'m,n의 공약수로 먼저 나누면 항상 같은 기약분수 지수가 나옵니다 — 나누는 순서는 결과에 영향이 없습니다.',
  ...
  check:{
    fills:[
      ...
      { tex:'\\sqrt[3]{5^6} = 5^{\\square}', answer:2,
        hint:{ ko:'6/3=2, 정수로 딱 떨어집니다', en:'6/3=2, comes out as a whole integer', zh:'6/3=2，正好是整数' } }
    ],
    open:{ ko:'⁴√(2⁶)을 유리수 지수로 나타내고, 기약분수로 줄이는 과정을 설명해봅니다.',
      en:'Express ⁴√(2⁶) as a rational exponent and explain how you reduce it.',
      zh:'把⁴√(2⁶)写成有理数指数，并说说约分的过程。' },
    openHint:{ ko:'⁴√(2⁶)=2^(6/4). 6과 4의 최대공약수는 2이므로 6÷2=3, 4÷2=2 → 2^(3/2)',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 더 큰 범위의 유리수 지수 문제를 모두 풉니다!', en:'Solve all the wider-range rational exponent problems in 5 minutes!', zh:'5分钟内解答所有更大范围的有理数指数题！' }
  },
