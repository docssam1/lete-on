...
window.NM_UNITS['M-23'] = {
  ...
  subtitle:{ ko:'모든 x에서 성립하려면 양변의 계수가 같아야 합니다', en:'To hold for every x, the coefficients on both sides must match', zh:'要对任意x都成立，两边的系数必须相等' },
  icon:'⚖️',
  ...
  practice:{
    ...
    params:{mode:'direct'},
    intro:{
      ko:'ax+b가 항상 5x-3과 같으려면? x가 어떤 값이든 성립해야 하니 a=5, b=-3일 수밖에 없습니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'"어떤 x를 넣어도 항상 성립하는 식"이 있습니다. 방정식은 특정 x에서만 참이지만, 항등식은 x가 뭐든 항상 참입니다 — 이걸 이용하면 모르는 계수를 어떻게 찾을 수 있을까요?',
      ...
      history:{ ko:'"항상 같다(恒等)"는 말 그대로, 미지수의 값과 무관하게 성립하는 식을 뜻합니다. 이 성질 덕분에 계수끼리 직접 비교하는 것만으로 방정식 없이도 미지수를 구할 수 있습니다 — 이게 미정계수법입니다.',
    ...
    stages:[
      ...
      { tag:{ko:'② 전개한 뒤 비교',en:'2) Expand, then compare',zh:'② 展开后再比较'},
        ...
        result:{ko:'괄호가 있으면 먼저 펼쳐서 x계수와 상수항으로 정리합니다!',en:'With brackets, expand first and sort into the x-coefficient and the constant!',zh:'有括号就先展开，整理成x系数和常数项！'},
        book:{ko:'전개 후 x계수끼리, 상수항끼리 각각 방정식을 세우면 두 미지수를 모두 구할 수 있습니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 더 큰 범위의 항등식 문제를 모두 풉니다!', en:'Solve all the wider-range identity problems in 5 minutes!', zh:'5分钟内解答所有更大范围的恒等式题！' }
  },
