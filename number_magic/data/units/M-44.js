...
window.NM_UNITS['M-44'] = {
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'평균 속도(전체 거리÷전체 시간)는 압니다. 그런데 "바로 이 순간"의 속도는 어떻게 구할까요? 시간 간격을 점점 0에 가깝게 줄여가면(바로 앞 유닛의 lim!) "순간의 기울기"가 나옵니다 — 이걸 나타내는 두 가지 기호가 <b>f′</b>와 <b>d/dx</b>입니다.',
      ...
      history:{ ko:'같은 것을 두 사람이 각자 다른 옷으로 표기했습니다. 라이프니츠는 d(아주 작은 차이)로, 라그랑주는 프라임(′)으로 썼습니다. 오늘날은 상황에 맞게 두 표기를 섞어 씁니다 — 같은 뜻, 다른 옷입니다.',
    ...
    stages:[
      { tag:{ko:'① 같은 것의 두 표기 — f′(x)와 d/dx',en:'1) Two notations for the same thing — f′(x) and d/dx',zh:'① 同一件事的两种写法——f′(x)和d/dx'},
        ...
        desc:{ko:'왼쪽 f\'(x)(프라임)와 오른쪽 d/dx·f(x)(디엑스분의디)는 완전히 <b>같은 것</b>을 가리킵니다 — "f(x)의 순간 기울기(도함수)"요. 라그랑주는 짧게 프라임(′)을, 라이프니츠는 "아주 작은 변화의 비율"이 보이도록 d/dx를 썼을 뿐입니다.',
        ...
        result:{ko:'f′과 d/dx는 같은 뜻, 다른 표기입니다!',en:'f′ and d/dx mean the same thing, written differently!',zh:'f′和d/dx意思相同，写法不同！'},
        book:{ko:'f\'(a)처럼 괄호에 숫자가 있으면 "그 점에서의 값"(미분계수), f\'(x)처럼 x가 있으면 "식 전체"(도함수)입니다.',
      ...
      { tag:{ko:'② axⁿ의 도함수는 naxⁿ⁻¹',en:'2) The derivative of ax^n is n·a·x^(n-1)',zh:'② ax^n的导数是n·a·x^(n-1)'},
        ...
        desc:{ko:'3x²을 미분할 땐 지수 2를 <b>앞으로 곱해 내리고</b>, 지수를 1 줄여요: 2×3=6, x²→x¹. 그래서 f\'(x)=6x. 여기에 x=2를 넣으면(미분계수) f\'(2)=12 — "x=2에서 곡선이 얼마나 가파른지"를 뜻합니다.',
        ...
        book:{ko:'상수항은 미분하면 사라집니다(기울기가 0이니까) — f(x)=3x²+5의 도함수도 6x로 똑같습니다.',
    ...
    rule:{ ko:"axⁿ의 도함수는 naxⁿ⁻¹(지수를 앞으로 곱해 내리고 1 줄이기). f'(x)와 d/dx는 같은 뜻, 다른 표기입니다.",
  ...
  check:{
    ...
    open:{ ko:"f(x)=2x²-x의 도함수를 구하고, f'(3)의 값을 설명해봅니다.",
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 미분계수 f\'(x₀) 값을 모두 구합니다!', en:'Find all the derivative values f\'(x₀) in 5 minutes!', zh:'5分钟内求出所有导数值f\'(x₀)！' }
  },
