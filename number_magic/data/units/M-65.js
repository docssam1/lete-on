...
window.NM_UNITS['M-65'] = {
  ...
  subtitle:{ ko:'두 점만 있으면 직선 하나가 정해집니다', en:'Two points are enough to pin down one straight line', zh:'只要两个点，就能确定一条直线' },
  icon:'📈',
  ...
  practice:{
    ...
    intro:{
      ko:'직선을 따라 오른쪽으로 한 칸 간 뒤, 위나 아래로 몇 칸 움직였는지 세어 보세요 — 그게 기울기입니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'도로의 "경사 8%" 표지판은 100m를 가는 동안 8m 올라간다는 뜻입니다. 8÷100 = 0.08 — 이게 바로 그 도로의 기울기입니다. 정비례(y=ax)에서 배운 a가 원점을 떠나 어디서든 출발할 수 있게 된 게 일차함수 y=ax+b입니다.',
      ...
      history:{ ko:'점을 두 수의 짝으로 적는 방법은 1637년 데카르트의 『방법서설』 부록에서 나왔습니다. 침대에 누워 천장의 파리 위치를 두 벽에서 잰 거리로 적을 수 있다는 걸 깨달았다는 이야기가 전해집니다. 이 덕분에 도형 문제를 식으로, 식을 그래프로 바꿔 볼 수 있게 됐습니다.',
    ...
    stages:[
      { tag:{ko:'① 기울기 = (y의 증가량) ÷ (x의 증가량)',en:'1) Slope is the change in y divided by the change in x',zh:'① 斜率 = (y的增量) ÷ (x的增量)'},
        ...
        desc:{ko:'x가 1에서 4로 3만큼 갈 동안 y는 3에서 12로 9만큼 올라갔습니다. 9÷3=3 — 오른쪽 1칸마다 3칸씩 오른다는 뜻입니다. <b>어느 두 점을 골라도 같은 값</b>이 나오는 게 직선의 성질입니다.',
        ...
        result:{ko:'기울기는 "얼마나 가파른가"를 수 하나로 적은 거입니다!',en:'The slope writes "how steep" as a single number!',zh:'斜率就是把"有多陡"写成一个数！'},
        book:{ko:'기울기가 음수면 오른쪽으로 갈수록 <b>내려가는</b> 직선입니다. 0이면 수평선입니다. 분모가 되는 (x의 증가량)이 0이면 수직선인데, 이건 함수가 아닙니다(x 하나에 y가 여럿).',
      ...
      { tag:{ko:'② y절편 — 한 점을 식에 넣어 구하기',en:'2) The y-intercept — put one point into the equation',zh:'② y截距——把一个点代入式子求出'},
        ...
        desc:{ko:'점이 그래프 위에 있다는 건 그 좌표를 넣으면 <b>등식이 성립한다</b>는 뜻입니다. y=3x+b에 (1,5)를 넣으면 5=3+b, 그래서 b=2입니다.',
        ...
        result:{ko:'기울기와 점 하나면 직선의 식이 완성됩니다!',en:'One slope plus one point completes the equation of the line!',zh:'有了斜率和一个点，直线的式子就完成了！'},
        book:{ko:'y절편 b는 <b>x=0일 때의 y값</b>, 즉 그래프가 y축과 만나는 높이입니다. 두 점 중 아무 점을 넣어도 b는 같게 나옵니다 — 다르게 나왔다면 기울기를 잘못 구한 거입니다.',
      ...
      { tag:{ko:'③ 그래프에서 식을 거꾸로 읽기',en:'3) Read the equation backward off the graph',zh:'③ 从图象反过来读出式子'},
        ...
        desc:{ko:'그림만 있어도 식이 나옵니다. 그래프가 y축을 지나는 높이가 <b>b</b>, 거기서 오른쪽으로 한 칸 갔을 때 오르내린 칸 수가 <b>a</b>입니다. 내려가면 a가 음수입니다 — 그래서 오른쪽으로 갈수록 낮아지는 직선은 기울기가 음수입니다.',
        ...
        result:{ko:'격자 한 칸이 1이라 눈으로 셀 수 있습니다!',en:'One square of the grid is 1, so you can simply count!',zh:'格子一格就是1，所以可以直接数出来！'},
        book:{ko:'기울기가 <b>클수록 가파르고</b>, 0에 가까울수록 눕습니다. a가 같고 b만 다른 두 직선은 <b>서로 평행</b>합니다 — 기울기가 그 직선의 방향을 통째로 정하기 때문입니다.',
    ...
    rule:{ ko:'기울기는 (y의 증가량)÷(x의 증가량), y절편은 x=0일 때의 높이 — 이 둘이면 직선 하나가 정해집니다!',
  ...
  check:{
    ...
    open:{ ko:'(1,5)와 (3,11)을 지나는 직선의 식을 구하는 과정을 말해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 두 점만 보고 직선의 식을 완성합니다!', en:'Within 6 minutes, build the equation of the line from just two points!', zh:'6分钟内仅凭两个点写出直线的式子！' }
  },
