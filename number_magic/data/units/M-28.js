...
window.NM_UNITS['M-28'] = {
  ...
  practice:{
    ...
    params:{mode:'basic'},
    intro:{
      ko:'x²-3x+1=0은 인수분해가 안 됩니다. 이럴 때 근의 공식 x=(-b±√(b²-4ac))/(2a)에 그대로 대입하면 답이 나옵니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'x²+bx+c=(x+p)(x+q) 인수분해는 p,q가 딱 맞아떨어질 때만 통합니다. x²-3x+1=0처럼 정수로 안 떨어지는 방정식은 어떻게 풀까요?',
      ...
      history:{ ko:'판별식(MD26)에서 본 b²-4ac가 사실 근의 공식 안에 그대로 들어 있습니다. 완전제곱식으로 바꾸는 과정을 한 번만 미리 해 두면, 어떤 이차방정식이든 대입만으로 풀리는 만능 열쇠가 완성됩니다.',
    ...
    stages:[
      { tag:{ko:'① 공식에 그대로 대입',en:'1) Substitute straight into the formula',zh:'① 直接代入公式'},
        ...
        desc:{ko:'x²-3x+1=0은 a=1,b=-3,c=1. D=9-4=5이니 x=(3±√5)/2. <b>근호 안이 완전제곱수가 아니어도 그대로가 정답</b>입니다.',
        ...
        result:{ko:'D=b²-4ac를 먼저 구한 뒤 공식에 통째로 넣습니다!',en:'Find D=b²-4ac first, then drop it straight into the formula!',zh:'先求D=b²-4ac，再整体代入公式！'},
        book:{ko:'답은 [p,q,r] 세 정수(x=(p±√q)/r)로 받습니다 — 근호를 더 정리하지 않은 형태 그대로가 정답입니다.',
      ...
      { tag:{ko:'② 공식이 나오는 이유(완전제곱식)',en:'2) Why the formula works (completing the square)',zh:'② 公式从何而来(配方)'},
        ...
        desc:{ko:'양변을 a로 나누고 완전제곱식으로 묶으면 이 꼴이 됩니다. 양변에 제곱근을 씌우고 x를 정리하면 <b>근의 공식이 그대로 나옵니다</b>.',
        ...
        result:{ko:'근의 공식은 완전제곱식을 한 번만 일반적으로 풀어 둔 결과입니다!',en:'The quadratic formula is just completing the square done once, in general!',zh:'求根公式就是把配方的过程一次性做好的结果！'},
        book:{ko:'D<0이면 근호 안이 음수라 실근이 없습니다 — 판별식과 근의 공식은 한 몸입니다.',
  ...
  check:{
    ...
    open:{ ko:'x²+2x-4=0을 근의 공식으로 풀어봅니다.',
  ...
  arena:{
    ...
    params:{mode:'wide'},
    rule:{ ko:'5분 안에 더 큰 범위의 근의 공식 문제를 모두 풉니다!', en:'Solve all the wider-range quadratic-formula problems in 5 minutes!', zh:'5分钟内解答所有更大范围的求根公式题！' }
  },
