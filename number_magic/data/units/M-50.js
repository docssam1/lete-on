...
window.NM_UNITS['M-50'] = {
  ...
  subtitle:{ ko:'저울처럼 양변에 같은 조작을 해서 x를 찾습니다', en:'Like a balance scale, do the same thing to both sides to find x', zh:'像天平一样对两边做相同的操作来找出x' },
  icon:'⚖️',
  ...
  practice:{
    ...
    params:{mode:'oneStep'},
    intro:{
      ko:'등식은 저울과 같습니다 — 양변에 같은 수를 더하거나 빼거나 곱하거나 나눠도 균형이 유지됩니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'양팔저울에 왼쪽엔 상자 하나와 추 3개, 오른쪽엔 추 9개가 있고 저울이 평형입니다. 상자 안에는 무게가 얼마인 물건이 들었을까요? 이게 바로 방정식이 하는 일입니다 — 모르는 값(x)을 찾는 거입니다.',
      ...
      history:{ ko:'"방정식(equation)"이라는 말은 라틴어 "aequare"(같게 하다)에서 왔습니다. 9세기 페르시아 수학자 알콰리즈미가 쓴 책 제목 『al-jabr』(복원)에서 오늘날의 "algebra"(대수)라는 단어가 나왔습니다.',
    ...
    stages:[
      { tag:{ko:'① 상수항을 반대쪽으로 이항',en:'1) Transpose the constant to the other side',zh:'① 把常数项移到另一边'},
        ...
        desc:{ko:'양변에서 3을 <b>똑같이</b> 빼면 2x=8이 됩니다(저울의 양쪽에서 같은 무게를 덜어내도 평형은 유지돼요). 그다음 양변을 2로 나누면 x=4.',
        ...
        result:{ko:'양변에 같은 조작을 해서 x만 남깁니다!',en:'Apply the same operation to both sides until only x remains!',zh:'两边做相同的操作，最后只留下x！'},
        book:{ko:'"이항"이란 등식의 한쪽 항을 부호를 바꿔 반대쪽으로 옮기는 걸 말합니다(+3을 옮기면 −3이 되는 것도 사실은 양변에서 3을 뺀 것과 같아요).',
      ...
      { tag:{ko:'② 양변에 x가 있으면 x항끼리 모읍니다',en:'2) With x on both sides, gather the x-terms',zh:'② 两边都有x就把x项集中在一起'},
        head:{ko:'5x+2=2x+11 \\;\\Rightarrow\\; x=3',en:'5x+2=2x+11 \\;\\Rightarrow\\; x=3',zh:'5x+2=2x+11 \\;\\Rightarrow\\; x=3'},
        desc:{ko:'양변에서 2x를 빼면 3x+2=11이 됩니다(x항을 한쪽으로 모으는 것). 그다음 상수항 2를 이항하면 3x=9, 마지막으로 3으로 나누면 x=3.',
        ...
        result:{ko:'x항은 한쪽으로, 상수항은 다른 쪽으로 모읍니다!',en:'Gather the x-terms on one side, constants on the other!',zh:'x项集中一边，常数项集中另一边！'},
        book:{ko:'등식의 성질(양변에 같은 수를 더하기·빼기·곱하기·나누기)만 지키면, 어떤 순서로 풀어도 답은 항상 같습니다.',
    ...
    rule:{ ko:'등식의 성질로 양변에 같은 조작을 해서 x만 남깁니다 — 상수항은 이항, x항은 한쪽으로 모으고, 마지막엔 계수로 나눕니다!',
  ...
  lab:{
    ...
    intro:{
      ko:'상수항부터 이항하고, 마지막에 x의 계수로 나눕니다!',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 양변에 x가 있는 방정식도 빠르게 풉니다!', en:'Solve equations with x on both sides quickly, within 5 minutes!', zh:'5分钟内快速解出两边都有x的方程！' }
  },
