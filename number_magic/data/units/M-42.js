...
window.NM_UNITS['M-42'] = {
  ...
  subtitle:{ ko:'무지개 덧셈법이 새 기호 옷을 입었습니다', en:'The rainbow-sum trick, wearing a new symbol', zh:'彩虹加法法换上了新符号的外衣' },
  icon:'🌈',
  symbols:[
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'1부터 100까지 더하는 걸 매번 "1+2+3+…+100"이라고 길게 쓰긴 힘듭니다. 이미 무지개 덧셈법으로 답(n(n+1)÷2)까지 아는데, 식 자체를 짧게 쓰는 방법은 없을까요? <b>Σ</b>가 그 줄임말입니다.',
      ...
      history:{ ko:'오일러는 1755년에 합(Sum)을 나타내려고 그리스 대문자 S에 해당하는 <b>시그마(Σ)</b>를 골랐습니다. 알파벳 S를 그리스 글자로 바꿔 쓴 것뿐이라, 사실 낯선 기호가 아니라 "합"이라는 익숙한 뜻의 새 옷입니다.',
    ...
    stages:[
      { tag:{ko:'① Σ는 "k에 하나씩 넣어서 더하라"',en:'1) Σ means "plug in k, one by one, and add"',zh:'① Σ的意思是"把k依次代入再相加"'},
        ...
        desc:{ko:'Σ 아래의 k=1은 "1부터 시작", 위의 4는 "4에서 끝"이라는 뜻입니다. k자리에 1,2,3,4를 차례로 넣고(이 경우는 k 자체가 식이라 그대로) 다 더하면 10. <b>Σ는 계산 방법이 아니라 "무엇을 하라"는 지시문</b>입니다 — 계산은 그다음입니다.',
        ...
        book:{ko:'Σ(k=a부터 b까지) f(k) = f(a)+f(a+1)+⋯+f(b) — 이게 Σ 기호의 완전한 뜻입니다.',
      ...
      { tag:{ko:'② 이미 아는 마법이 공식으로',en:'2) Magic you already know, now as a formula',zh:'② 早就会的魔法变成了公式'},
        ...
        desc:{ko:'Σk(k=1~n)는 무지개 덧셈법 그 공식(n(n+1)÷2)이고, Σk²(k=1~n)은 제곱수의 합 공식(n(n+1)(2n+1)÷6)입니다. <b>새로 외울 건 없습니다</b> — 예전에 배운 두 마법에 Σ라는 짧은 이름표를 붙인 것뿐입니다.',
        ...
        result:{ko:'Σ는 새 계산이 아니라, 이미 아는 공식에 붙는 새 이름입니다!',en:'Σ isn\'t new math — it\'s a new name for formulas you already know!',zh:'Σ不是新的计算，而是给已知公式贴上的新名字！'},
        book:{ko:'Σ(pk+q)처럼 항이 여러 조각이면 p·Σk + q·Σ1로 쪼개서 각각 계산한 뒤 더합니다.',
    ...
    rule:{ ko:'Σ는 "k에 시작부터 끝까지 하나씩 넣어 더하라"는 지시문. Σk=n(n+1)÷2(무지개 덧셈법), Σk²=n(n+1)(2n+1)÷6(제곱수의 합) — 이미 아는 공식입니다!',
  ...
  check:{
    ...
    open:{ ko:'Σ(k=1~5) k가 왜 무지개 덧셈법과 같은 계산인지 설명해봅니다.',
      en:'Explain why Σ(k=1 to 5) k is the same calculation as the rainbow-sum trick.',
      zh:'说说为什么Σ(k=1到5) k和彩虹加法法是同一种计算。' },
    openHint:{ ko:'Σ(k=1~5)k=1+2+3+4+5 — 무지개 덧셈법이 구하던 바로 그 합입니다. n=5로 n(n+1)÷2=15',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 Σ(pk+q) 문제를 모두 쪼개서 풉니다!', en:'Split and solve all the Σ(pk+q) problems in 5 minutes!', zh:'5分钟内拆解并解答所有Σ(pk+q)题目！' }
  },
