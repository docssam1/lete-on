...
window.NM_UNITS['M-48'] = {
  ...
  subtitle:{ ko:'문자 자리에 수를 넣어 계산합니다', en:'Substitute a number for the letter and calculate', zh:'把数代入字母的位置计算' },
  icon:'🎯',
  ...
  practice:{
    ...
    params:{mode:'linear'},
    intro:{
      ko:'문자 자리에 수를 넣을 땐 곱셈 기호를 다시 살려서 계산합니다. x=4면 3x는 3×4입니다!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'문자식 "3x+2"는 x가 어떤 수인지에 따라 값이 달라집니다. x=1이면 5, x=10이면 32입니다. 문자에 실제 수를 넣어서 값을 구하는 걸 "식의 값을 구한다"고 합니다.',
      ...
      history:{ ko:'문자에 수를 대입해 값을 구하는 방법은 방정식·함수 등 수학 전체의 기초입니다. 컴퓨터 프로그램의 "변수"도 사실 이 문자식의 아이디어에서 나왔습니다.',
    ...
    stages:[
      { tag:{ko:'① 곱셈 기호를 살려서 대입',en:'1) Restore the × before substituting',zh:'① 补回乘号再代入'},
        ...
        desc:{ko:'"3x"는 사실 "3×x"입니다. x=4를 넣을 땐 지워졌던 곱셈 기호를 <b>다시 살려서</b> 3×4로 계산합니다. 그다음 +2를 더하면 14가 나옵니다.',
        ...
        book:{ko:'식의 값은 문자에 넣는 수가 바뀌면 매번 달라집니다 — 그래서 "변수"라고 부릅니다.',
      ...
      { tag:{ko:'② 음수를 대입할 땐 괄호로 감쌉니다',en:'2) Wrap a negative value in parentheses',zh:'② 代入负数要用括号括起来'},
        ...
        desc:{ko:'음수 x=−2를 대입할 땐 <b>괄호로 감싸서</b> (−2)로 넣어야 부호 실수가 줄어듭니다. (−2)²=4(짝수 번 곱하면 +), −5×(−2)=+10이 됩니다.',
        ...
        result:{ko:'음수를 대입할 땐 괄호로 감싸서 부호 실수를 줄입니다!',en:'Wrap negative substitutions in parentheses to avoid sign errors!',zh:'代入负数用括号括起来，减少符号错误！'},
        book:{ko:'거듭제곱이 있으면 지수부터 계산하고, 그다음 곱셈, 마지막에 덧뺄셈 순서입니다(연산 순서는 그대로!).',
    ...
    rule:{ ko:'식의 값 = 문자에 수를 넣어 곱셈 기호를 살려 계산. 음수는 반드시 괄호로 감싸서 대입합니다!',
  ...
  check:{
    ...
    open:{ ko:'x=−1일 때 3x²−2x의 값을 계산 과정과 함께 설명해봅니다.',
  ...
  lab:{
    ...
    intro:{
      ko:'이차식에 음수를 대입할 땐 꼭 괄호로 감싸서 계산합니다!',
  ...
  arena:{
    ...
    params:{mode:'twoVars'},
    rule:{ ko:'5분 안에 문자가 두 개인 식의 값을 모두 구합니다!', en:'Evaluate all the two-letter expressions within 5 minutes!', zh:'5分钟内求出所有含两个字母的代数式的值！' }
  },
