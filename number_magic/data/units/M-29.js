...
window.NM_UNITS['M-29'] = {
  ...
  subtitle:{ ko:'두 인수의 부호가 다를 때만 곱이 음수가 됩니다', en:'The product is negative only when the two factors disagree in sign', zh:'只有两因式符号相反时乘积才是负' },
  icon:'🚦',
  ...
  practice:{
    ...
    params:{mode:'between'},
    intro:{
      ko:'(x-2)(x-5)<0이 되려면? 두 인수 중 하나는 양수, 하나는 음수여야 합니다 — 그건 x가 2와 5 사이일 때뿐입니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'이차방정식은 등호(=)로 특정 x만 찾았습니다. 그런데 (x-2)(x-5)<0처럼 부등호가 있으면 답은 특정 값이 아니라 범위입니다 — 어떤 x들이 이 부등식을 만족할까요?',
      ...
      history:{ ko:'부호를 다루는 규칙은 아주 단순해요: 양수×양수=양수, 음수×음수=양수, 양수×음수=음수. 이 규칙 하나만 있으면 두 인수의 부호 조합을 따져서 부등식의 해를 구간으로 정리할 수 있습니다.',
    ...
    stages:[
      { tag:{ko:'① 두 인수의 부호가 다를 때만 곱이 음수',en:'1) The product is negative only when the signs differ',zh:'① 只有符号不同乘积才是负'},
        ...
        desc:{ko:'x=3(2와 5 사이)이면 (3-2)(3-5)=1×(-2)=<b>음수</b> — 부등식을 만족합니다. x=6(5보다 큼)이면 (6-2)(6-5)=4×1=<b>양수</b> — 만족 안 합니다. 그래서 해는 두 근 사이입니다.',
        ...
        result:{ko:'곱이 음수인 구간은 항상 두 근 사이입니다!',en:'The negative-product interval is always between the two roots!',zh:'乘积为负的区间总是在两根之间！'},
        book:{ko:'(x-p)(x-q)<0(p<q)의 해는 p<x<q — 두 근이 경계가 되는 열린 구간입니다.',
      ...
      { tag:{ko:'② 전개된 식은 먼저 인수분해',en:'2) Factor an expanded expression first',zh:'② 展开式先因式分解'},
        ...
        desc:{ko:'더해서 7, 곱해서 10인 두 수는 2와 5 — <b>(x−2)(x−5) &lt; 0</b>으로 인수분해되고, 앞서 배운 대로 해는 2 &lt; x &lt; 5입니다.',
        ...
        result:{ko:'전개된 식은 먼저 인수분해해서 두 근을 찾습니다!',en:'For an expanded expression, factor first to find the two roots!',zh:'展开式先因式分解，找出两根！'},
        book:{ko:'x²의 계수가 1이 아니면 먼저 그 수로 나눠(양수일 때 부등호 방향 유지) 인수분해합니다.',
  ...
  check:{
    ...
    open:{ ko:'2x²-2x-24<0의 해를 구하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 x²계수가 1이 아닌 이차부등식을 모두 풉니다!', en:'Solve all the quadratic inequalities with a leading coefficient ≠1 in 5 minutes!', zh:'5分钟内解答所有x²系数不是1的二次不等式！' }
  },
