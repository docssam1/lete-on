...
window.NM_UNITS['M-64'] = {
  ...
  practice:{
    ...
    intro:{
      ko:'부등식도 방정식처럼 이항하고 나누면 돼요 — 양수로 나눌 때는 방향이 그대로예요!',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'3 < 5는 누가 봐도 맞아요. 그런데 양변에 −1을 곱하면 −3과 −5가 되는데, 이번엔 −3이 더 커요. 수직선에서 두 수를 원점 반대쪽으로 옮기면 왼쪽·오른쪽이 통째로 뒤바뀌기 때문이에요. 부등식에서 조심할 건 이 한 가지뿐이에요.',
      ...
      history:{ ko:'"<"와 ">" 기호는 1631년 영국의 토머스 해리엇이 남긴 유고에서 처음 인쇄됐어요. 그보다 앞서 등호 "="를 만든 로버트 레코드는 "평행한 두 선분보다 더 같은 것은 없으니까"라고 이유를 적어 뒀는데, 부등호도 같은 발상이에요 — 벌어진 쪽이 큰 수예요.',
    ...
    stages:[
      { tag:{ko:'① 양수로 나눌 때 — 방향 그대로',en:'1) Dividing by a positive — direction unchanged',zh:'① 除以正数时——方向不变'},
        ...
        desc:{ko:'2를 이항해 3x>9, 양변을 3으로 나눠 x>3. 방정식을 풀 때와 <b>한 글자도 다르지 않아요</b> — 부등호만 그대로 따라 내려와요.',
        ...
        result:{ko:'답은 하나가 아니라 3보다 큰 수 전부예요!',en:'The answer is not one number but every number greater than 3!',zh:'答案不是一个数，而是所有大于3的数！'},
        book:{ko:'방정식의 답은 점 하나, 부등식의 답은 <b>범위</b>예요. 수직선에 그리면 3에서 시작해 오른쪽으로 뻗은 반직선이 돼요(3 자리는 비운 동그라미).',
      ...
      { tag:{ko:'② 음수로 나눌 때 — 방향 뒤집기',en:'2) Dividing by a negative — flip the direction',zh:'② 除以负数时——方向反转'},
        ...
        desc:{ko:'1을 이항해 −2x≥6까지는 그대로예요. 마지막에 −2로 나누는 순간 <b>≥가 ≤로</b> 뒤집혀 x≤−3이 돼요. x=−4를 넣어 확인해 보면 −2(−4)+1=9로 정말 7 이상이에요.',
        ...
        result:{ko:'음수로 곱하거나 나눌 때만 뒤집어요 — 더하기·빼기는 그대로!',en:'Flip only when multiplying or dividing by a negative — adding and subtracting never flip!',zh:'只有乘除负数时才反转——加减永远不变！'},
        book:{ko:'헷갈리면 항상 <b>수를 하나 넣어 확인</b>해요. 답의 범위 안에 있는 수를 원래 식에 넣었을 때 참이 되면 맞게 푼 거예요.',
    ...
    rule:{ ko:'부등식은 방정식과 똑같이 풀어요 — 단, 음수로 곱하거나 나눌 때만 부등호의 방향을 뒤집어요!',
  ...
  check:{
    fills:[
      { tex:'2x + 1 > 9 \\quad\\Rightarrow\\quad x > \\square', answer:4,
        hint:{ ko:'2x>8이니 양변을 2로 나눠요', en:'2x>8, so divide both sides by 2', zh:'2x>8，两边除以2' } },
      { tex:'-3x \\ge 12 \\quad\\Rightarrow\\quad x \\le \\square', answer:-4,
        hint:{ ko:'−3으로 나누며 방향을 뒤집어요', en:'Divide by −3 and flip the direction', zh:'除以−3并反转方向' } }
    ],
    open:{ ko:'−x+5<2를 풀고, 왜 부등호가 뒤집히는지 설명해봐요.',
  ...
  lab:{
    ...
    intro:{
      ko:'x의 계수가 음수예요 — 마지막에 나눌 때 방향을 뒤집는 걸 잊지 마세요!',
  ...
  arena:{
    ...
    rule:{ ko:'6분 안에 범위를 구하고, 그 범위의 가장 큰 정수까지 답해요!', en:'Within 6 minutes, find the range and then the largest integer in it!', zh:'6分钟内求出范围，再答出范围内最大的整数！' }
  },
