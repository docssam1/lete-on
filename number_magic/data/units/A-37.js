...
window.NM_UNITS['A-37'] = {
  ...
  discover:{
    ...
    stages:[
      { tag:{ ko:'① 소수 부분을 먼저 빼요', en:'1) Subtract the decimal part first', zh:'① 先减小数部分' },
        head:{ ko:'소수점 아래끼리, 자연수끼리 나눠 빼요', en:'Subtract decimal parts separately, then integer parts', zh:'小数部分单独相减，整数部分也单独相减' },
        desc:{ ko:'3.7-1.4=? 소수: 0.7-0.4=0.3, 자연수: 3-1=2. 결과: 2+0.3=2.3!',
               en:'3.7−1.4=? Decimal: 0.7−0.4=0.3, Integer: 3−1=2. Result: 2+0.3=2.3!',
               zh:'3.7-1.4=？小数：0.7-0.4=0.3，整数：3-1=2。结果：2+0.3=2.3！' },
        mathSteps:['3.7 - 1.4 = □',{ko:'소수: 0.7 - 0.4 = 0.3',en:'\\text{decimal parts: } 0.7 - 0.4 = 0.3',zh:'小数部分：0.7 - 0.4 = 0.3'},{ko:'자연수: 3 - 1 = 2',en:'\\text{whole parts: } 3 - 1 = 2',zh:'整数部分：3 - 1 = 2'},'2 + 0.3 = 2.3'],
      ...
      { tag:{ ko:'② 받아내림이 있을 때', en:'2) When borrowing is needed', zh:'② 需要借位时' },
        head:{ ko:'소수 부분이 부족하면 자연수에서 빌려요', en:'If the decimal part is too small, borrow from the integer', zh:'小数部分不够减时从整数部分借位' },
        desc:{ ko:'4.2-1.7=? 소수: 0.2-0.7 불가 → <b>자연수에서 1 빌려서</b> 1.2-0.7=0.5. 자연수: 4-1-1=2. 답: 2.5!',
               en:'4.2−1.7=? Decimal: 0.2−0.7 impossible → <b>borrow 1 from integer</b>: 1.2−0.7=0.5. Integer: 4−1−1=2. Answer: 2.5!',
               zh:'4.2-1.7=？小数：0.2-0.7不够→<b>从整数借1</b>：1.2-0.7=0.5。整数：4-1-1=2。答案：2.5！' },
        mathSteps:['4.2 - 1.7 = □',{ko:'소수: 0.2-0.7 → 자연수에서 1 빌림',en:'\\text{decimals: } 0.2-0.7 → \\text{borrow 1 from the wholes}',zh:'小数部分：0.2-0.7 → 向整数借1'},'(10+2)-7 = 5 → 0.5',{ko:'자연수: 4-1-1 = 2 → 2.5',en:'\\text{wholes: } 4-1-1 = 2 → 2.5',zh:'整数部分：4-1-1 = 2 → 2.5'}],
        result:{ ko:'4.2-1.7=2.5 ✓', en:'4.2−1.7=2.5 ✓', zh:'4.2-1.7=2.5 ✓' },
        book:null }
    ],
    rule:{ ko:'① 소수 부분(십분의 자리) 빼기  ② 부족하면 자연수에서 1 빌리기  ③ 자연수 부분 빼기 (빌렸으면 1 더 빼기)',
  ...
  check:{
    ...
    openHint:{ ko:'4.2-1.7처럼 소수 부분(0.2)이 빼는 수의 소수 부분(0.7)보다 작을 때 자연수에서 1을 빌려요.', en:'When the decimal part (0.2) is smaller than what you subtract (0.7), borrow 1 from the integer (like 4.2−1.7).', zh:'当小数部分（0.2）小于被减的小数（0.7）时，从整数部分借1（如4.2-1.7）。' }
  },
  ...
  voice:{
    ...
    wrong:[ {ko:'소수 부분끼리 먼저 빼봐!',en:'Subtract decimal parts first!',zh:'先减小数部分！'}, {ko:'부족하면 자연수에서 1 빌려야 해!',en:'Borrow 1 from integer if needed!',zh:'不够减就从整数借1！'} ],
