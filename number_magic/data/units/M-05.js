...
window.NM_UNITS['M-05'] = {
  ...
  subtitle:{ ko:'(−2)³과 −2³, 괄호 하나가 답을 완전히 바꿔놓기도 합니다', en:'(−2)³ vs −2³ — one pair of parentheses can change everything', zh:'(−2)³和−2³，一个括号可能改变一切' },
  icon:'💥',
  ...
  practice:{
    ...
    params:{mode:'paren'},
    intro:{
      ko:'(−a)ⁿ은 밑이 통째로 음수입니다 — (−a)를 n번 곱해봐!',
  ...
  discover:{
    story:{
      hook:{ ko:'같은 수를 두 번 곱해서 음수를 만들 수 있을까요?',
        ...
        zh:'同一个数自乘两次，能得到负数吗？' },
      history:{ ko:'실수 안에서는 불가능합니다. 그래서 수학자들은 제곱하면 −1이 되는 새 수 i를 만들어 버렸습니다. 처음엔 상상의 수라 놀림받았지만, 지금은 전기 신호를 다루는 공학과 3D 게임의 회전 계산(사원수)에서 매일 쓰입니다. 없는 수를 만들어 세상을 계산한 셈이죠.',
    ...
    stages:[
      { tag:{ko:'① 괄호가 있으면 밑 전체가 음수',en:'1) With brackets, the whole base is negative',zh:'① 有括号，底数整体为负'},
        ...
        desc:{ko:'(−2)⁴은 (−2)를 <b>4번 곱하는 것</b>: (−2)×(−2)×(−2)×(−2). 음수 4개(짝수)를 곱하니까 부호는 +, 절댓값은 2⁴=16 → <b>+16</b>입니다.',
        ...
        result:{ko:'괄호 안 음수를 지수만큼 곱하면 부호는 홀짝으로 결정됩니다!',en:'Multiply the bracketed negative by its exponent count — the sign follows odd/even!',zh:'把括号里的负数连乘指数次，符号看奇偶数决定！'},
        book:null },
      ...
      { tag:{ko:'② 괄호가 없으면 지수는 밑에만',en:'2) Without brackets, the exponent hugs the base only',zh:'② 没有括号，指数只贴着底数'},
        ...
        desc:{ko:'−2⁴은 "마이너스, 2의 4제곱"으로 읽습니다 — 지수는 <b>2에만</b> 걸리고 맨 앞의 −는 마지막에 붙여요: 2⁴=16을 먼저 구하고 −를 붙이면 <b>−16</b>. (−2)⁴=16과 완전히 다른 값이죠!',
        ...
        result:{ko:'지수는 딱 붙어 있는 밑에만 걸립니다 — 괄호가 없으면 부호는 그대로!',en:'The exponent only reaches the base it touches — no brackets, the leading sign stays put!',zh:'指数只作用于紧贴着的底数——没有括号，前面的符号原地不动！'},
        book:{ko:'지수가 <b>홀수</b>일 땐 (−a)ⁿ과 −aⁿ이 우연히 같은 값이 됩니다(둘 다 음수) — 하지만 <b>짝수</b>일 땐 반드시 값이 갈립니다. 헷갈리면 항상 괄호 유무부터 확인하세요.',
    ...
    rule:{ ko:'① (−a)ⁿ: 괄호 안 음수를 n번 곱하기  ② −aⁿ: aⁿ을 먼저, −는 마지막에  ③ 짝수 지수일 때만 두 값이 달라집니다',
  ...
  check:{
    fills:[
      { tex:'(-3)^3 = \\square', answer:-27,
        hint:{ ko:'(-3)를 3번 곱합니다', en:'Multiply (-3) three times', zh:'把(-3)连乘3次' } },
      { tex:'-3^3 = \\square', answer:-27,
        hint:{ ko:'3³=27을 구하고 -를 붙입니다(홀수라 같은 값!)', en:'Find 3³=27, attach - (odd exponent, same value!)', zh:'先求3³=27再加负号(奇数指数值相同!)' } }
    ],
    open:{ ko:'(−3)⁴과 −3⁴을 각각 계산하고, 왜 값이 다른지 설명해봅니다.',
      en:'Compute (−3)⁴ and −3⁴ separately, and explain why they differ.',
      zh:'分别计算(−3)⁴和−3⁴，说说为什么不一样。' },
    openHint:{ ko:'(−3)⁴=81(괄호 안 음수 4번 곱), −3⁴=−81(3⁴=81 먼저, −는 나중). 지수 4가 짝수라서 값이 갈립니다.',
  ...
  arena:{
    ...
    params:{mode:'mixed'},
    rule:{ ko:'5분 안에 괄호형·비괄호형을 구분하며 다 풉니다!', en:'Solve all — telling bracketed and bare forms apart — in 5 minutes!', zh:'5分钟内区分带括号和不带括号，全部解答！' }
  },
