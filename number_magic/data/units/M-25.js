...
window.NM_UNITS['M-25'] = {
  ...
  practice:{
    ...
    params:{mode:'sumCube'},
    intro:{
      ko:'x³+27을 인수분해하고 싶다면? 27=3³이니 합의 세제곱 공식 x³+a³=(x+a)(x²-ax+a²)을 거꾸로 쓰면 됩니다.',
  ...
  discover:{
    ...
    story:{
      hook:{ ko:'x²+bx+c 인수분해는 이제 익숙합니다. 그런데 x³+27이나 x⁴+5x²+6처럼 차수가 더 높은 식은 어떻게 인수분해할까요? 아는 공식으로 되돌릴 방법이 있을까요?',
      ...
      history:{ ko:'인수분해 심화는 두 가지 무기를 새로 꺼냅니다. 하나는 곱셈공식을 거꾸로 읽는 것(합·차의 세제곱), 다른 하나는 낯선 문자를 새 문자 t로 바꿔치기하는 것(치환) — 둘 다 "이미 아는 모양으로 바꾸기"라는 같은 전략입니다.',
    ...
    stages:[
      { tag:{ko:'① 합의 세제곱 공식을 거꾸로',en:'1) The sum-of-cubes formula, backward',zh:'① 反过来用立方和公式'},
        ...
        desc:{ko:'27=3³이니 a=3. 공식 x³+a³=(x+a)(x²-ax+a²)에 그대로 넣으면 <b>(x+3)(x²-3x+9)</b>. 전개해서 검산하면 가운데 항들이 다시 사라지고 x³+27로 돌아옵니다.',
        ...
        result:{ko:'세제곱수를 찾으면 합·차의 세제곱 공식을 거꾸로 쓸 수 있습니다!',en:'Spot the cube, and you can run the sum/difference-of-cubes formula backward!',zh:'找到立方数，就能把立方和差公式反过来用！'},
        book:{ko:'x³+a³=(x+a)(x²-ax+a²), x³-a³=(x-a)(x²+ax+a²) — 곱셈공식(MD22)의 짝꿍 인수분해입니다.',
      ...
      { tag:{ko:'② 치환으로 낯선 식을 익숙하게',en:'2) Substitute to make it familiar',zh:'② 用换元变熟悉'},
        ...
        desc:{ko:'x²를 t로 바꿔 보면 t²+5t+6 — 더해서 5, 곱해서 6인 두 수는 2와 3입니다. t=x²를 다시 대입하면 <b>(x²+2)(x²+3)</b>.',
        ...
        result:{ko:'x²를 하나의 문자 t로 보면 이미 아는 인수분해로 돌아갑니다!',en:'Treat x² as a single letter t, and it turns back into factoring you already know!',zh:'把x²看成一个字母t，就变回你已经会的因式分解！'},
        book:{ko:'복잡한 식은 반복되는 덩어리를 새 문자로 치환하면 낯익은 모양이 되는 경우가 많습니다.',
  ...
  check:{
    ...
    open:{ ko:'x⁴+7x²+12을 치환으로 인수분해하는 과정을 설명해봅니다.',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 치환을 이용한 사차식 인수분해를 모두 풉니다!', en:'Solve all the quartic-by-substitution factorings in 5 minutes!', zh:'5分钟内解答所有换元法四次式因式分解！' }
  },
