  ...
  discover:{
    ...
    stages:[
      { tag:{ ko:'① 수를 둘로 나눠요', en:'1) Split the number in two', zh:'① 把数字分成两半' },
        ...
        desc:{ ko:'80-47처럼 두 자리 수를 뺄 땐, 빼는 수(47)를 <b>십의 자리(40)와 일의 자리(7)로 나눠서</b> 순서대로 빼면 훨씬 쉬워요.',
               en:'When subtracting like 80-47, split the number you\'re subtracting (47) <b>into tens (40) and ones (7)</b>, then subtract each in turn.',
               zh:'像80-47这样的减法，把要减的数(47)<b>分成十位(40)和个位(7)</b>，依次相减就容易多了。' },
      ...
      { tag:{ ko:'② 어떤 자리를 먼저 빼도 괜찮아요', en:'2) Either order works', zh:'② 先减哪一位都可以' },
        ...
        desc:{ ko:'십의 자리를 먼저 빼든, 일의 자리를 먼저 빼든 <b>답은 똑같아요</b>. 다만 십의 자리부터 빼는 게 더 편할 때가 많아요.',
               en:'Whether you subtract tens or ones first, <b>the answer is the same</b> — but starting with tens is usually easier.',
               zh:'不管先减十位还是先减个位，<b>答案都一样</b>，但先减十位通常更方便。' },
      ...
      { tag:{ ko:'③ 세 자리 수도 똑같아요', en:'3) Works for 3-digit numbers too', zh:'③ 三位数也一样' },
        ...
        desc:{ ko:'세 자리, 네 자리 수도 마찬가지예요. 백의 자리, 십의 자리, 일의 자리 순서로 <b>하나씩 따로 빼주면 돼요</b>.',
               en:'The same works for 3- and 4-digit numbers — subtract hundreds, then tens, then ones, <b>one place at a time</b>.',
               zh:'三位数、四位数也一样，按百位、十位、个位<b>依次分别相减</b>即可。' },
