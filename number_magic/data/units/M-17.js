...
window.NM_UNITS['M-17'] = {
  ...
  subtitle:{ ko:'근호끼리는 안에 있는 수를 먼저 곱하거나 나눕니다', en:'Between two roots, combine what\'s inside first', zh:'两个根号之间，先把里面的数相乘或相除' },
  icon:'💧',
  ...
  practice:{
    ...
    params:{mode:'mul'},
    intro:{
      ko:'√2×√8 = √(2×8) = √16 = 4. 근호 안의 수끼리 먼저 곱하고 나서 정리합니다!',
  ...
  discover:{
    story:{
      ...
      history:{ ko:'피타고라스 학파는 모든 길이를 분수로 나타낼 수 있다고 믿었습니다. 그런데 한 변이 1인 정사각형의 대각선을 제곱하면 2가 되고, 그 길이는 어떤 분수로도 나타낼 수 없었습니다. 이 비밀을 밖으로 알린 히파소스가 바다에 던져졌다는 이야기가 전해집니다 — 사실인지는 아무도 모릅니다.',
    ...
    stages:[
      { tag:{ko:'① 곱셈은 안의 수끼리 먼저',en:'1) Multiply what\'s inside first',zh:'① 相乘先合并根号内的数'},
        ...
        desc:{ko:'근호끼리의 곱셈은 <b>근호 안의 수를 먼저 곱한 뒤</b> 하나의 근호로 합쳐요: √a×√b=√(ab). 3×12=36이고 36은 완전제곱수라 √36=6으로 정리됩니다.',
        ...
        result:{ko:'근호끼리의 곱셈은 안의 수부터 합칩니다!',en:'Multiplying roots — combine what\'s inside first!',zh:'根号相乘，先合并根号内的数！'},
      ...
      { tag:{ko:'② 완전제곱수가 안 되면 근호 정리로',en:'2) If it\'s not a perfect square, simplify the radical',zh:'② 不是完全平方数就化简根号'},
        ...
        desc:{ko:'2×6=12는 완전제곱수가 아닙니다. 이럴 땐 √12를 그대로 두지 말고, 이전 유닛에서 배운 <b>근호 정리</b>를 이어서 써요: 12=4×3 → √12=2√3.',
        ...
        result:{ko:'곱한 결과가 완전제곱수가 아니면 근호 정리까지 이어서 합니다!',en:'If the product isn\'t a perfect square, continue on to simplify the radical!',zh:'相乘的结果不是完全平方数，就接着化简根号！'},
        book:{ko:'제곱근의 곱셈·나눗셈은 "안의 수 계산 → (필요하면) 근호 정리" 두 단계로 완성합니다.',
  ...
  arena:{
    ...
    params:{mode:'mixed'},
    rule:{ ko:'5분 안에 근호가 남는 곱셈 문제를 모두 풉니다!', en:'Solve all the multiplications that leave a radical in 5 minutes!', zh:'5分钟内解答所有会剩下根号的乘法题！' }
  },
