...
window.NM_UNITS['M-03'] = {
  ...
  subtitle:{ ko:'분모만 맞추면 정수와 똑같은 규칙이 그대로 통합니다', en:'Match the denominators, and the integer rules work exactly the same', zh:'只要分母对齐，整数的规则原封不动地适用' },
  icon:'🧩',
  ...
  discover:{
    story:{
      hook:{ ko:'산을 시속 10 km로 올라가고 시속 30 km로 내려왔습니다. 평균 속력은 20 km/h일까요?',
        en:'You climb a hill at 10 km/h and come down at 30 km/h. Is your average speed 20 km/h?',
        zh:'上山时速10公里，下山时速30公里。平均速度是20公里每小时吗？' },
      history:{ ko:'아니에요, 15 km/h입니다. 편도 30 km라면 올라갈 때 3시간, 내려올 때 1시간이라 60 km를 4시간에 간 셈이죠. 느린 쪽에 시간을 훨씬 오래 썼으니 그냥 더해서 2로 나눌 수 없습니다. 분수를 더할 때 분모부터 맞추는 것과 똑같은 조심성이 필요합니다.',
    ...
    stages:[
      { tag:{ko:'① 분모가 같으면 분자만',en:'1) Same denominator — just the numerators',zh:'① 分母相同就只管分子'},
        ...
        desc:{ko:'분모가 같은 유리수는 분모는 그대로 두고 <b>분자끼리 정수 덧뺄셈 규칙 그대로</b> 계산합니다. −2와 −1은 같은 부호니까 절댓값을 더하고 공통 부호 −를 붙여 −3, 분모는 그대로 5입니다.',
        ...
        result:{ko:'분모가 같으면 정수 규칙을 분자에 그대로 쓰면 됩니다!',en:'Same denominator — just reuse the integer rule on the numerators!',zh:'分母相同——分子直接套用整数规则！'},
        book:null },
      ...
      { tag:{ko:'② 분모가 다르면 통분부터',en:'2) Different denominators — convert first',zh:'② 分母不同先通分'},
        ...
        desc:{ko:'분모가 다르면 <b>최소공배수</b>로 통분해서 분모를 맞춥니다. 2와 3의 최소공배수는 6이니까 1/2 = 3/6, −1/3 = −2/6 — 이제 분모가 같아졌으니 분자만 계산: 3+(−2)=1.',
              en:'Different denominators? Convert to the <b>LCM</b> (least common multiple) first. LCM of 2 and 3 is 6, so 1/2=3/6 and -1/3=-2/6 — now the denominators match, so just combine numerators: 3+(−2)=1.',
              zh:'分母不同就先通分到<b>最小公倍数</b>。2和3的最小公倍数是6，所以1/2=3/6，-1/3=-2/6——现在分母一样了，只算分子：3+(−2)=1。'},
        mathSteps:[{ko:'\\text{최소공배수}(2,3)=6',en:'\\text{lcm}(2,3)=6',zh:'\\text{最小公倍数}(2,3)=6'}, '\\dfrac{3}{6} + \\dfrac{-2}{6}', '\\dfrac{1}{6}'],
        result:{ko:'통분만 마치면 그다음은 이미 아는 규칙입니다!',en:'Once converted, the rest is a rule you already know!',zh:'通分完成之后，剩下的就是已经会的规则了！'},
        book:{ko:'계산 결과가 약분이 되면 기약분수로 나타내는 게 원칙입니다.',
  ...
  check:{
    ...
    open:{ ko:'1/4 + (−1/6)을 통분부터 해서 계산해봅니다.',
      en:'Compute 1/4 + (−1/6) by converting to a common denominator first.',
      zh:'先通分，再计算1/4 + (−1/6)。' },
    openHint:{ ko:'4와 6의 최소공배수는 12. 3/12+-2/12=1/12.',
  ...
  arena:{
    ...
    params:{mode:'chain3'},
    rule:{ ko:'5분 안에 세 유리수 혼합 계산을 모두 풉니다!', en:'Solve all three-term rational mixes in 5 minutes!', zh:'5分钟内解答所有三数有理数混合题！' }
  },
