  ...
  discover:{
    ...
    stages:[
      { tag:{ko:'① 두 자리 수는 계단 두 칸',en:'1) A 2-digit number is two steps',zh:'① 两位数是两个台阶'},
        ...
        desc:{ko:'두 자리 수는 언제나 십의 자리와 일의 자리로 쪼갤 수 있어요. 24는 20(십의 자리)과 4(일의 자리)예요.',
              en:'Any 2-digit number splits into a tens part and a ones part. 24 is 20 (tens) and 4 (ones).',
              zh:'两位数总能拆成十位和个位。24就是20（十位）和4（个位）。'},
        mathSteps:['24 = 20 + 4'],
      ...
      { tag:{ko:'② 십의 자리부터 계단을 올라요',en:'2) Climb the tens step first',zh:'② 先爬十位的台阶'},
        ...
        desc:{ko:'13을 10과 3으로 나눠서, 먼저 10을 더하고(첫 계단), 그다음 3을 더해요(둘째 계단). 한 칸씩 밟아 올라가는 거예요.',
              en:'Split 13 into 10 and 3. Add the 10 first (first step), then the 3 (second step) — climbing one step at a time.',
              zh:'把13拆成10和3。先加10（第一级台阶），再加3（第二级台阶）——一级一级往上爬。'},
      ...
      { tag:{ko:'③ 수가 늘어도 계단은 계속돼요',en:'3) More numbers, more steps',zh:'③ 数字变多，台阶继续爬'},
        ...
        desc:{ko:'32+15+21처럼 수가 늘어나도, 각 수를 십의 자리와 일의 자리로 나눠 순서대로 계단을 오르면 돼요.',
              en:'Even with three numbers like 32+15+21, split each into tens and ones and keep climbing in order.',
              zh:'即使是32+15+21这样三个数，只要把每个都拆成十位和个位，按顺序继续爬台阶就行。'},
