...
window.NM_UNITS['M-41'] = {
  ...
  subtitle:{ ko:'같은 수를 계속 곱해가는 수열입니다', en:'A sequence made by multiplying by the same number again and again', zh:'不断乘以同一个数得到的数列' },
  icon:'🌀',
  ...
  practice:{
    ...
    params:{mode:'nthTerm'},
    intro:{
      ko:'2,6,18,54,… 처럼 3배씩 커지는 수열입니다. aₙ=a₁×r^(n-1)에 대입해봅니다.',
  ...
  discover:{
    story:{
      ...
      history:{ ko:'전해지는 이야기 속 답은 18,446,744,073,709,551,615알입니다. 한 칸씩 두 배씩만 늘렸는데 나라의 곳간으로도 감당할 수 없는 수가 되죠. 두 배는 처음엔 느리게, 나중엔 무섭게 빨라집니다.',
    ...
    stages:[
      { tag:{ko:'① 일반항 aₙ=a₁×r^(n-1)',en:'1) The general term aₙ=a₁×r^(n-1)',zh:'① 通项公式aₙ=a₁×r^(n-1)'},
        ...
        desc:{ko:'첫째항 2에서 4번째 항까지 가려면 3을 <b>3번</b> 곱해야 합니다(항번호-1번). 2×3×3×3=2×27=54. 등차수열이 "더하기"를 반복했다면, 등비수열은 <b>같은 수를 곱하기</b>를 반복합니다 — aₙ=a₁×r^(n-1).',
        ...
        result:{ko:'공비 r을 (항번호-1)번 곱합니다!',en:'Multiply by the common ratio r (term number − 1) times!',zh:'把公比r乘(项数-1)次！'},
      ...
      { tag:{ko:'② 항을 나열해 더하면 합',en:'2) Add the terms one by one for the sum',zh:'② 依次相加就是求和'},
        ...
        desc:{ko:'등비수열의 합은 공식을 외우기 전에 <b>그냥 다 나열해서 더해도</b> 돼요: 1,2,4,8을 더하면 15. r이 정수면 각 항이 항상 정수이기 때문에, 나열해서 더하는 방법이 나눗셈 공식보다 오히려 더 안전합니다(나눗셈 없이 답이 딱 나와요).',
        ...
        result:{ko:'항을 하나씩 나열해서 더해도 등비수열의 합이 나옵니다!',en:'Listing and adding the terms one by one gives the geometric sum too!',zh:'把各项一一列出相加，也能得到等比数列的和！'},
        book:{ko:'등비수열의 합 공식 Sₙ=a₁(rⁿ-1)÷(r-1)은 정수 r에서 항상 정수가 나옵니다 — 나열해서 더한 값과 완전히 같습니다.',
    ...
    rule:{ ko:'aₙ=a₁×r^(n-1) — 공비 r을 (항번호-1)번 곱합니다. 합은 항을 하나씩 나열해서 더해도 똑같습니다.',
  ...
  check:{
    ...
    open:{ ko:'a₂=6, a₄=24인 등비수열의 a₁과 r을 구하는 과정을 설명해봅니다(r>0).',
  ...
  arena:{
    ...
    rule:{ ko:'5분 안에 등비수열의 합을 모두 구합니다!', en:'Find all the geometric series sums in 5 minutes!', zh:'5分钟内求出所有等比数列的和！' }
  },
