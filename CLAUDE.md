...
| ~~rp·ws·sl 보기 길이 미보정~~ | ✅ 해소됨 (2026-08-30) | rp 108/131(82%) · ws 162/240(68%) · sl 231/320(72%) → **세 책 모두 0%**. 오답에 지문 근거가 있는 구체성을 주고(rp), 정답에 붙은 군더더기를 걷어내(ws·sl) 해결. 작업 중 **정답만 깎으면 이번엔 "최단=정답"이 된다**는 것을 확인해(ws가 한때 39%) 검사기에 반대 방향을 넣었다. 기준은 **정답이 최장도 최단도 아닌 중간** — 이미 고쳐 둔 CARS 3책·Bricks도 양쪽 0~1%였다. 도구: `check-choice-length.js`(양방향) · `check-choice-integrity.js`(보기 중복·분포) · `show-set.js`(지문+문항 펼치기) · `apply-choice-fixes.js`(보기만 교체, 정답 letter 불변) |
| ~~sl 정답 키 오류 7건~~ | ✅ 수정됨 (2026-08-30) | 보기 길이를 보다가 **정답 letter가 가리키는 보기가 해설·지문과 어긋나는** 문항을 발견(sl5 추가 Q3·Q5·Q8, sl5 유사 Q3·Q5, sl15 유사 Q5, sl4 추가 Q4). 보기 순서만 바뀌고 letter가 따라가지 않은 흔적이다. 길이·분포·중복 검사는 이 종류를 전혀 못 잡는다 → `check-key-explanation.js` 신규(해설과 보기의 낱말 겹침으로 **후보**를 낸다, 오탐 많음). 840문항 중 37건이 걸려 7건이 진짜였다. 전부 보기 내용을 맞바꿔 고쳤으므로 정답 letter·3/3/3/3 분포는 불변. 곁가지로 sl5 추가 Q7·Q10이 **지문에 없는 사실**('salary'의 라틴어 어원)을 근거로 삼고 있어 지문에 있는 낱말·사실로 다시 씀 |
| ~~br15~br18 보기 길이 편향~~ | ✅ 해소됨 (2026-08-30) | Bricks는 **책 평균 12%**라 이미 우연 기대값 아래로 보였는데, 레슨별로 보니 br1~14·br19~20은 전부 0%이고 **br15·16·17·18만 40·65·53·80%**였다. 연속된 4개 레슨이 다른 방식으로 쓰인 채 남아 있던 것 — **책 단위 평균이 블록 하나를 통째로 가린다.** 앞으로 레슨별로도 볼 것. Bricks는 보기가 짧아(20~60자) 정답만 줄이면 곧바로 최단이 되므로 "가장 긴 오답 이하 · 그 오답보다 19자 이내"로 맞췄다. 함께 cd14·cd15 유사 Q6도 보정 → **git의 전 교재 1,740문항이 최장 0% · 최단 0%** |
| ~~br17 정답 키 오류 4건~~ | ✅ 수정됨 (2026-08-30) | `reveal`의 정답이 "hide from view", `trapped`의 정답이 "asked to inspect a bridge"였다(둘 다 뜻이 반대). 추가 Q6·Q7·Q11, 유사 Q7. **검사기는 이 중 2건만 후보로 올렸다** — 보기가 짧으면 낱말 겹침 신호가 약해진다. 그래서 낱말 뜻·비유 문항 356개를 전 책 전수 대조했고 나머지는 전부 정상이었다. 정답 키 검사기는 탐지기가 아니라 **후보 생성기**로만 쓸 것 |
...
## 티처 콘솔 (print.html) — 3건 전부 완료 (2026-08-10)

제품명은 **`{학원명} 티처 콘솔`**. 학원 미설정 시에만 GFIELD(기본 브랜드).
설정 후에는 화면·인쇄물 어디에도 GFIELD가 안 나온다(프리셋 칩은 예외 — 선택 버튼이라 무관).

1. ✅ **로고 누락 버그** — 원인은 `checkName()`이 **`brand.name !== p.name`일 때만** `usePreset()`을
   불렀던 것. 학원 이름을 **정확히 그대로** 입력(또는 붙여넣기)하면 이름이 이미 같아서
   "이미 적용됨"으로 보고 프리셋을 통째로 건너뛰었다. 로고는 저장된 필드가 아니라 **파일에서
   비동기로 오는 값**이라 이게 눈에 띄는 피해자였고, **설정은 저장돼 있는데 로고 캐시만 없는
   재방문 브라우저**는 영영 로고를 못 받았다(재현·수정 전후 브라우저로 검증).
   → 프리셋 적용 여부는 **텍스트 4개(name·sub·color·color2)로만** 판정하고, 로고는 없으면
   따로 가져온다. 가져오기는 프리셋별 1회로 메모(`logoTried`) — 파일이 없거나 로딩 중일 때
   키 입력마다 재요청하는 것을 막는다. 칩을 직접 누르면 메모를 지워 재시도한다.
2. ✅ **분석지 드래그 강조** — 형광펜·★·밑줄. `marks[]`에 `{b: 블록 index, s, e: 오프셋,
   k: 종류, t: 덮은 텍스트}`로 저장하고 매 렌더 후 다시 입힌다. **오프셋은 빠른 경로, 텍스트가
   진짜 앵커** — 위쪽이 길어져 블록이 밀려도 같은 낱말을 다시 잡는다(코멘트 수정 검증됨).
   - 텍스트 노드는 **가장 가까운 블록 조상 하나에만** 귀속시킨다. 블록은 중첩되므로
     (`.rp-verdict`가 `<b>`와 `<p>`를 같이 가짐) 이걸 안 하면 한 번 드래그에 표시가 2개 겹쳐 붙었다.
   - 표시는 **자기 텍스트를 추가하지 않는다**(★는 `::before`). 그래야 표시를 입혀도 저장된
     오프셋이 밀리지 않는다.
   - 표시를 **클릭하면 지워진다.** 툴바의 지우기는 선택 범위에 걸친 것을 전부 지운다.
   - 툴바는 스크롤 시 숨지 않고 **다시 붙는다**(문장 뒷부분 읽으려고 스크롤하다 선택을 잃는 문제).
   - ⚠️ **채점을 바꾸면 코칭 문구 자체가 새로 생성돼** 그 표시는 사라진다(설계상). 매뉴얼에
     "채점을 끝낸 뒤에 표시하세요"로 안내함.
   - 인쇄: `print-color-adjust:exact`로 컬러 유지, 흑백 프린터는 회색 띠 + 밑줄/★ 검정.
3. ✅ **매뉴얼 8장 갱신** — 글자 크기(지문·문항 각각 4단계, 실제 pt값), 여러 단원 선택 시
   지문 배치(각 단원 뒤 / 책 뒤), 분석지 언어(학습지와 별개), 선생님 코멘트(위치·비우면 미출력),
   드래그 강조 사용법까지 추가. 수치는 전부 코드에서 읽어 적음.

---

## 음성 재합성 캐시 (2026-09-21) — 같은 글을 두 번 사지 않는다

**측정치부터.** `generate-audio.js` 는 `type === 'original'` 만 건너뛰고 나머지는 **매 실행
전부 다시 합성**했다. 한 번에 약 **208,811자** — 창작 지문 110편 179,143자 + 라이팅 빌리지
4,668자 + 라이브러리 36쪽 ≈ 25,000자. 그런데 워크플로는 `lc*.js`·`lesson*.js`·`ws*.js`·
`sl*.js` 중 **하나라도 건드린 푸시마다** 돈다. 레슨 한 줄을 고쳐도 전 교재가 다시 과금됐고,
달에 다섯 번만 푸시하면 Neural2 무료 한도(월 100만 자)를 넘는다.
`generate-cars-d-audio.js` 는 아예 건너뛰기가 없었다.

- **`scripts/tts-cache.js`** — 글 + 목소리의 해시를 명세에 적어 두고, 같으면 건너뛴다.
  "지문을 고치면 음성도 새로" 라는 원래 의도는 그대로다(글이 바뀌면 해시가 바뀐다).
- 명세는 **Supabase Storage `audio/_tts-<이름>.json`** 에 둔다 — git 에 커밋하지 않으므로
  워크플로에 쓰기 권한을 더 줄 필요가 없고, 음성 파일과 같은 자리라 둘이 따로 놀 수 없다.
- **첫 실행은 명세가 비어 전부 다시 만든다(약 20만 자, 한 번).** 그 뒤로는 바뀐 것만.
  이미 올라간 음성이 현재 글과 맞다고 확신하면 `SEED_TTS_CACHE=1` 로 돌려 **합성 없이
  해시만 기록**해 그 한 번도 건너뛸 수 있다.
  ⚠ 씨앗 모드는 "스토리지의 MP3 = 지금 데이터 파일의 글"이라고 **믿는** 것이다. 앱의
  `AUDIO_STALE` 이 비어 있는 동안(= 앱이 저장된 음성을 그대로 믿는 동안)만 쓸 것.
- **검사기 `node scripts/check-tts-cache.js`** — 돈이 걸린 판단이라 눈으로 믿지 않는다.
  너무 잘 건너뛰면 아이가 고친 지문 대신 옛 음성을 듣고, 너무 안 건너뛰면 고치려던 그
  문제가 그대로다. 양쪽을 다 확인한다(13항목).
- `generate-share-reader-audio.js` 는 이미 `timingsMatchStory`(글·목소리·속도 비교)로
  건너뛰고 있어 손대지 않았다. `generate-nm-audio.js` 도 2026-09-14 에 같은 일을 이미 했다.


## OmniVoice 검토 — 비용이 아니라 목소리 (2026-09-21)

> 원장: "https://github.com/k2-fsa/OmniVoice 이거 쓰면 우리 음성비용 저렴해지나"

**직접 확인한 것** — k2-fsa(next-gen Kaldi 팀), **Apache 2.0**(상업적 사용 가능),
600+ 언어 zero-shot TTS, 목소리 복제 + 목소리 설계, GPU 에서 실시간의 40배.
PyPI `omnivoice` 0.2.1(Python ≥3.10, torch ≥2.4, transformers ≥5.3.0), 가중치는
HuggingFace 에서 받는다.

**결론: 지금 갈아타는 건 이득이 아니다.**
1. 과금 방식이 바뀔 뿐 공짜가 아니다 — 글자당 요금 → GPU/CPU 시간.
   우리 오디오 워크플로는 전부 `ubuntu-latest`(GPU 없음)라 GPU 를 새로 빌려야 한다.
2. 엔진을 바꾸면 **전곡을 한 번 다시 만들어야** 한다 — 그게 바로 우리가 줄이려던 그
   비용이다. 목소리도 전부 다시 오디션해야 한다(Chirp3-HD-Leda 는 원장이 A/B/C/D
   들어 보고 고른 것).
3. 돈이 새던 곳은 엔진이 아니라 **매 푸시마다 전량 재합성**이었고, 그건 위의
   "음성 재합성 캐시" 절에서 고쳤다.

**대신 OmniVoice 가 진짜 값어치를 하는 자리는 따로 있다 — 비용이 아니라 기능이다.**
누미·독쌤·할아버지·기호 캐릭터마다 다른 목소리. Google 은 목소리 목록이 고정이라
애초에 못 하고, 캐릭터가 늘수록 글자당 요금도 는다.

⚠ **한국어 주의 두 가지**(공식 문서 확인)
- 학습량이 **한국어 8,609시간 / 영어 206,061 / 중국어 111,343** — 한국어는 영어의 1/24.
- **목소리 설계(instruct)는 중국어·영어 데이터로만 학습**됐다. 다른 언어에서는 불안정할
  수 있다고 문서가 명시한다. 한국어 캐릭터 목소리는 **복제(clone)** 쪽이 안정적이다.
- 복제 참조 음성은 **권리가 깨끗한 것**만 쓴다. 다른 TTS 업체가 만든 음성을 복제하는
  것은 약관 위반이 될 수 있다. 선생님이 직접 3~10초 읽는 게 가장 깨끗하고,
  그러면 **독쌤이 진짜 선생님 목소리**가 된다 — 이게 이 기술의 진짜 쓸모다.

**들어 보고 정하도록 시청 장치를 만들어 뒀다** (이 세션에서는 직접 못 들었다 —
클라우드 컨테이너라 GPU 가 없고 huggingface.co 도 막혀 있다).
- **`scripts/local/omnivoice-audition.cmd`** — **원장 PC 에서 더블클릭.** 원장 확인:
  **"우리 GPU 있잖아, 지금 실행 PC"** — GPU 가 있으면 글자당 요금이 **0**이 된다(전기값만).
  전용 가상환경 → torch(CUDA)+omnivoice 설치 → 생성 → 폴더 열기까지 한 번에.
  **키를 하나도 안 쓰고 아무것도 안 올린다.** 쓰는 법은 `scripts/local/README.md`.
- `scripts/omnivoice-audition.py` — 앱에 실제로 들어 있는 대사(누미 3줄 + 영·중 1줄씩)를
  캐릭터 설계안 4종으로 읽는다. `--device auto` 가 GPU 를 알아서 잡고(있으면 float16,
  없으면 CPU+float32) 무엇을 쓰는지·VRAM 이 얼마인지 찍는다. 참조 음성을 주면 복제 모드.
- `.github/workflows/omnivoice-audition.yml` — PC 가 없을 때의 대안(러너 CPU). 느리다.
- 로그에 **RTF**(음성 1초를 만드는 데 몇 초)가 찍힌다 — 이 숫자가 운영에 쓸 수 있는지를
  정한다.

**원장 결정(2026-09-21): "그냥 우리 나온 목소리 복제해."** — 아이들이 이미 아는 누미
목소리가 바뀌면 안 되니까. 설계(instruct)는 잘 나와도 *다른* 목소리가 되고, 중국어·영어로만
학습돼 한국어에서 불안정하다. 복제가 그 둘을 한 번에 푼다.
- **`scripts/local/omnivoice-clone.cmd`** ← **이것부터.** `scripts/omnivoice-clone.py` 가
  `number_magic/data/tts-map.js`(ko 260 · en 254 · zh 251줄)에서 언어마다 **3~10초짜리**
  참조를 고른다. 글자 수로 후보를 추린 뒤 **실제로 받아서 길이를 잰다** — 한/중/영은 글자당
  길이가 달라 글자 수만으로는 못 맞춘다. **전사는 지어내지 않는다**: 우리가 합성에 쓴 바로
  그 문장을 쓴다(자동 전사를 쓰면 거기서 오차가 생긴다).
- ffmpeg 이 없어도 된다 — omnivoice 가 이미 의존하는 librosa 로 mp3 를 읽는다.
- 결과는 `clone-ref/`(원본)와 `clone/`(복제). **번갈아 들어 같은 사람으로 들리면 성공.**
- **원격 데스크톱이면 소리가 안 넘어온다**(원장: "원격이라 소리를 어떻게 들어"). 그래서
  `clone/듣기.html` 을 함께 쓴다 — **wav 를 base64 로 파일 안에 박아 넣어 그 한 장에 소리가
  전부 들어 있다.** 메일·메신저·USB 무엇으로든 옮기면 휴대폰에서도 열린다(서버·인터넷·
  다른 파일 불필요). 원본과 복제를 한 화면에 나란히 놓아 번갈아 누르게 했다.
  먼저 원격 프로그램의 오디오 전송 설정부터 볼 것 — 대개 그걸로 끝난다(README 에 4종 안내).
- ⚠ TTS 업체 음성을 복제해 자체 음성을 만드는 것은 일반적으로 약관이 다루는 영역이고, 그
  목소리들은 실제 성우를 계약해 만든 것이다. **구글 약관의 해당 조항은 확인하지 못했다**
  (약관 페이지가 자바스크립트로 그려진다). 원장이 위험을 알고 진행하기로 했다. 깨끗한
  대안은 선생님이 직접 3~10초 읽는 것이고, 그러면 독쌤이 진짜 선생님 목소리가 된다.

**아직 안 만든 것: 운영 생성기.** 시청 결과가 좋으면 그때 로컬 GPU 로 MP3 를 만들어
Supabase 에 올리는 스크립트를 만든다. 먼저 만들지 않는 이유는 한국어 품질이 기대에
못 미치면 그게 통째로 헛일이기 때문이다(한국어 8,609시간 = 영어의 1/24).


## sprite-gen 검토 — 안 쓰기로 함 (2026-09-21)

> 원장이 링크만 보냈고(https://github.com/aldegad/sprite-gen), 검토 뒤 **"4"(지금은 안 씀)** 로 결정.
> 다음 세션이 같은 조사를 반복하지 않도록 판단 근거만 남긴다.

**무엇인가** — Apache 2.0, Codex/Claude 스킬 + 파이썬 CLI. 그림 한 장 → 스프라이트 아틀라스
(`manifest.json.frame_layout`) 또는 투명 배경 모션 루프. 생성은 `codex`/`grok` 구독 또는
`openai`(호출당 과금), 영상 루프는 `XAI_API_KEY`. **공짜가 아니다.**

**왜 안 쓰나 — 우리 그림 체계가 이미 다른 길로 갔고, 그쪽이 우리한테는 낫다.**
- 마을 걷는 캐릭터는 `number_magic/app/walker.js` **SVG 리그**(400줄, 정면·후면·측면 3뷰,
  관절을 CSS 키프레임으로). 해상도 독립이고 옷장에서 색이 바뀐다 — **고정 PNG 프레임은 후퇴**다.
- 만화 캐릭터 12종은 네 컷 만화용 **정지 초상화**라 움직일 필요가 없다.
- science-lab docssam 은 이미 **몸통 포즈 5장 + 얼굴 오버레이 4장**을 런타임에 겹친다
  (`science-lab/v2/v2.js` BODY/FACE). 얼굴만 22 KB — 아틀라스보다 가볍다.
- 매거진 그림·스토리 삽화 35장·개념 애니는 전부 원본 SVG 도형이다.
- PNG 아틀라스를 들이면 **그림 체계가 둘로 쪼개진다**(SVG 리그 + 스프라이트 아틀라스).

**다시 볼 만한 때** — ①외주 PNG 아트를 또 받아서 알파·그림자·색 변형을 일관되게 정리해야 할 때
(`cutout`·`recolor`·`shadow`·`slice-sheet`·`compose-layers`·`background-tile` 은 **Pillow/NumPy
만 쓰는 로컬 처리라 API 비용 0**) ②docssam 을 말하는 얼굴이 아니라 **걷거나 손짓하게** 만들고
싶을 때(영상→루프). 그때도 `grok` 구독 여부부터 확인할 것.

**곁가지 발견(미처리)** — `science-lab/assets/docssam.png` 1.2 MB 가 **코드 어디서도 참조되지
않는다.** webp 프레임을 잘라낸 원본으로 보이는데 배포 폴더에 그대로 있다(science-lab 이미지
1.9 MB 중 1.2 MB). 원장이 이번엔 손대지 않기로 했으므로 그대로 둔다.


## Bricks Reading 250 — 구조 파악 완료, 콘텐츠 미착수 (2026-08-10)

**상세는 저장소 루트 `BRICKS-READING-250.md`. 다음 세션은 그 파일부터 읽을 것.**

출판사 원본 PDF(Drive의 L1 Word List · SB Answer Key)를 직접 읽어 확인한 것:
- **Level 1 = 20유닛**, 10개 주제 × 2유닛 쌍(홀수=논픽션, 짝수=같은 주제 이야기).
- **핵심 낱말 = 유닛당 10개**(Word Box). Word List PDF엔 보조 10개를 더해 20개, L1 총 400개.
- **문항 = 유닛당 9개**, 선택지 **3개(a/b/c)** — 20유닛 전부 d 없음. True/False 3문항 포함.
  구성: `A 객관식1 · B T/F 3 · C 객관식4 · D 객관식1`.
- **CARS식 문항별 전략명이 없다.** 출판사 분류는 사실적/추론적/평가적 3분류.
  대신 Sum Up Step 1에 유닛별 스킬명 8종(Cause & Effect, Sequence, Searching for
  Information, Grouping/Categorizing Information, Problem & Solution, Finding Details,
  Finding Reasons, Classifying Details).
- 지문 ~250단어(L1 220~240), G4~G6 / CEFR A2+. **AR·Lexile은 출판사가 제공 안 함 — 지어내지 말 것.**

사용자 지시: **창작 콘텐츠는 CARS와 같은 형태**(12전략·4지선다·3/3/3/3)로 가되
**낱말 수와 지문 수만** 다르게.

### 엔진 영향 — 이미 되는 것 / 고쳐야 하는 것 (코드 확인 완료)
- 되는 것: 낱말 개수 가변(`main.js:890` 주석에 이미 명시), 선택지 3개·T/F 렌더
  (`nOpt=(q[2]||[]).length`), 문항 수 가변(`thresholds(n)`), 검사기 어휘 규칙.
- **고쳐야 하는 것 — `cumulativeStrategyStats()` (main.js:1221)**: 책·문항수와 무관하게
  **모든 시도에 12전략을 전부 카운트**한다. Bricks 원본 9문항을 STEP 3에 넣으면 index 0~8이
  CARS 전략 1~9번으로 잘못 귀속되고 없는 10~12번까지 `seen`이 올라가 **부모 대시보드 약점
  랭킹이 오염된다.**
- 티처 콘솔(`print.html`)은 현재 **CARS B·C·D만** 싣는다. rp·ws·sl도 안 들어가 있음.

### 막힌 곳
**영어 원문 지문은 Drive zip 3개(125·145·394MB)에만 있고 이 세션에서 열 수 없다** —
Drive 다운로드가 base64로 대화 문맥에 실려 오는 구조라 100MB급은 불가(디스크는 여유 있음).
정답지에 **지문의 한국어 번역 전문**은 있으나 영어 원문은 없어서, 번역만 보고 창작 지문을
쓰는 것은 "가장 확실하게만" 지시에 어긋난다고 판단해 착수하지 않음.
→ **유닛 1개 분량이라도 텍스트로 풀어서 Drive에 올려주면** 나머지는 같은 파이프라인으로 진행.
   ※ 매뉴얼 본문에서 "인쇄 센터"는 이미 "티처 콘솔"로 일괄 치환됨.

### 분석지 현재 구성 (완료)
**유지보수 기준 문서:** `skills/cars-analysis-report/SKILL.md`. 판정·처방·추천·강조·인쇄 검증의
상세 규칙은 해당 스킬과 `references/`를 먼저 읽고, 이 절은 현재 기능의 요약으로만 사용한다.

게이지 · 전략 구성 원그래프 · 레벨별 막대(2개 이상 응시 시) · 전략별 정답률 막대(약한 것부터) ·
선생님 코멘트 · 전략별 코칭(놓친 문항 수와 어느 레벨에서 무너졌는지 명시) · 코칭 우선순위 3 ·
...
- 3D: 둥근 모서리·환경맵·라벨은 흰 카드(화면 폭 안으로 자동 축소·밀어 넣기, 시선 깊이로 계산), 「가로로 크게」 전체 화면.
- 채점: `science-lab/bank/written-score.mjs`(모델은 요소 충족·근거만, 점수는 서버). 오개념표 4단원(부정 발문도 매핑 — Codex와 정책 다름).
- **MSG 초·과·심**(`docssam1/msg-science-lab`)을 이 수준으로 끌어올리는 작업 착수(2026-09-26) — 그쪽 저장소 `HANDOFF-CLAUDE.md` 참조.
