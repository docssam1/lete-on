...
## 티처 콘솔 (print.html) — 미해결 3건 (2026-08-11)

제품명은 **`{학원명} 티처 콘솔`**. 학원 미설정 시에만 GFIELD(기본 브랜드).
설정 후에는 화면·인쇄물 어디에도 GFIELD가 안 나온다(프리셋 칩은 예외 — 선택 버튼이라 무관).

1. **로고 누락 버그 (상)** — 등록된 학원을 **이름으로 입력**하면 이름·부제·색은 들어오는데
   **로고만 안 딸려온다**. 같은 학원을 **칩으로 누르면 정상**. `usePreset()` 안의 비동기
   `loadLogo()`가 타이핑 경로에서 유실되는 것으로 보임. 당장은 칩 선택으로 우회 가능.
2. **분석지 드래그 강조 (중)** — 선택 영역에 형광펜·★·밑줄. 분석지는 상태 변경마다 통째로
   재렌더되므로 표시가 날아간다. `{블록 id, 시작·끝 오프셋, 종류}`로 저장했다가 매 렌더 후
   다시 입히는 구조 필요. 인쇄 CSS에서 회색조 프린터 대응도.
3. **매뉴얼 8장 갱신 (중)** — 「인쇄 센터 사용법」이 신기능 미반영: 지문·문항 글자 크기 개별
   조절, 여러 단원 선택 시 지문 배치(단원 뒤/책 뒤), 분석지 언어 선택, 선생님 코멘트.
