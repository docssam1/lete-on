...
- 그래머는 기본 문항 위주.
- **6세와 7세 시험이 비슷하다.**
- **리스닝 섹션이 따로 있다.** 대치 학부모 대화 기준, 프렙 학원들이 **브릭스 리스닝 300-2**로
  대비시킨다("브릭스 리스닝 300 풀려보세요"). 즉 브릭스 300은 난이도 비유가 아니라
  **리스닝 대비 교재 그 자체**다. 위 15/15/20 표는 리딩·라이팅만 확인된 것이고,
  리스닝·그래머의 시간 배분은 아직 모른다 — 확인 필요.
...
| 리스닝 | **음성 파이프라인은 이미 있음** — `scripts/generate-audio.js`(Google Neural2 → Supabase Storage)로 지문 MP3를 만들어 쓰는 중 | 리스닝 **문항**이 없음. 스크립트+문항을 쓰면 기존 파이프라인으로 음성까지 바로 나옴 |
| 시간 훈련 | **없음** | 섹션별 시간 박스 |
