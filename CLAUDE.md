...
이 환경은 huggingface.co 가 막혀 있어 가중치를 받을 수 없다).
- `.github/workflows/omnivoice-audition.yml` — 손으로 돌리는 워크플로.
  **비밀키를 하나도 안 쓰고 Supabase 에 아무것도 안 올린다.** 결과는 artifact 로만.
- `scripts/omnivoice-audition.py` — 앱에 실제로 들어 있는 대사(누미 3줄 + 영·중 1줄씩)를
  캐릭터 설계안 4종으로 읽는다. 참조 음성을 주면 복제 모드로 바뀐다.
- 로그에 **RTF**(실시간 대비 배속)가 찍힌다 — 1 보다 크면 CPU 로는 실시간보다 느리다는
  뜻이고, 운영에 쓰려면 GPU 가 필요하다는 신호다.
