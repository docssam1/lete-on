...
test("program-track scopes preserve their evidence status instead of inferring a generic transfer range", () => {
  ...
  assert.deepEqual(Array.from(transfer.evidenceRefs), ["PUBLIC:DP-TIMETABLE-OFFICIAL-2026-09"]);
  assert.equal(data.programTrackBindings.some(item => item.trackId === "high-transfer"), false);
});

test("DP middle2-2 entry is rebuilt from originals within the current cutoff", () => {
  ...
  assert.equal(exam.scopeLabel, "중1-1~중2-1 연립일차방정식까지 · 30문항");
  ...
  assert.equal(resolved.binding.scopeKey, "middle1-1-to-middle2-1-systems");
});
