...
- 관리자 화면은 운영 준비 상태를 표시한다. 프로그램별 검증 후보 수가 0개이거나 저장소가 메모리 전용이면, 원문·정답·경로를 드러내지 않고 설정 누락만 경고한다.
...
- 새 초안은 목표 문항 수·총점·유형군당 최대 문항 수를 반드시 설정한다. 이 제약을 만족하지 못하면 출력 구성은 잠긴다.
- 이전 초안에 목표 제약이 빠져 있으면 관리자 화면의 **출제 목표**에서 보완할 수 있다. 기존 배치는 지우지 않으며 재검토 상태로 전환된다.
- 출력 구성은 최종 검수 확정 전까지 잠긴다. 초안 확정은 배치 계약 확인일 뿐 원문 시험지·정답지 공개 권한을 부여하지 않는다.
