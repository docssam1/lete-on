...
![적용 후 — 짝을 이어 100으로 접는다](assets/after-a05-animated.png)

초록 호가 `67`과 `33`을, 주황 호가 `59`와 `41`을 잇습니다. 그 아래 줄에서 짝끼리 괄호로 묶이고,
다시 각각 `100`으로 접힙니다. **어느 수가 왜 짝인지, 어떤 순서로 찾는지가 화면에 있습니다.**

수는 매번 새로 생성되므로 이 그림은 한 예시입니다 — 장면은 유닛의 산문이 아니라 **생성기가
내보낸 값**에서 만들어집니다.

[▶ 18초 시연 영상 (MP4)](assets/demo-a05.mp4)
