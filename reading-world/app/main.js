...
function availableBooks(){const books=new Map();
 const add=(bid)=>{if(books.has(bid))return;const c=bookCatalogEntry(bid);if(c){if(c.available===false)return;const title=(c.subtitle?`${c.title} · ${c.subtitle}`:c.title);books.set(bid,{id:bid,title,shortLabel:c.subtitle||c.title});}else{const r=Object.values(window.LESSONS||{}).find(x=>(x.bookId||'cars-level-b')===bid);const lvl=(r&&r.levelId)||bid.split('-').pop().toUpperCase();books.set(bid,{id:bid,title:`CARS · Level ${lvl}`,shortLabel:`Level ${lvl}`});}};
 add('cars-level-b');
 Object.values(window.LESSONS||{}).forEach(r=>add(r.bookId||'cars-level-b'));
 return [...books.values()];}
function lessonNum(id){const match=String(id).match(/(\d+)$/);return match?parseInt(match[1],10):0;}
function currentBookLabel(){return(availableBooks().find(b=>b.id===currentBookId)||{shortLabel:'Level B'}).shortLabel;}
function bookIsVocabularyOnly(bookId=currentBookId){return !!bookCatalogEntry(bookId)?.vocabularyOnly;}
// The lesson chrome used to hardcode "CARS" even for the non-CARS books, so the
// breadcrumb and sidebar now name whichever series the open lesson belongs to.
function currentBookBrand(){const c=bookCatalogEntry(currentBookId);return (c&&(c.title||c.series))||'CARS';}
// Spell the book out in full ("Reading Prime Level 1"), not the bare level that
// reads the same across every series.
// Title + level, not fullTitle — for CARS that field holds the expanded acronym
// ("Comprehensive Assessment of Reading Strategies"), which drops the level and is
// far too long for a breadcrumb.
function currentBookFullName(){const c=bookCatalogEntry(currentBookId);
 if(c)return c.subtitle?`${c.title} ${c.subtitle}`:(c.title||c.fullTitle);
 return `CARS ${currentBookLabel()}`;}
...
function hasOriginalPassage(){return Array.isArray(L.originalPassage)&&L.originalPassage.length>0;}
function hasOriginal(){return hasOriginalPassage()&&Array.isArray(L.originalQuestions)&&L.originalQuestions.length>0;}
function lessonUsesOriginalAssessment(id){const row=(window.LESSONS&&window.LESSONS[id])||{};const bookId=row.bookId||(id==='lesson1'?'cars-level-b':currentBookId);const meta=bookCatalogEntry(bookId);return !meta||meta.originalAssessment!==false;}
function usesOriginalAssessment(){return lessonUsesOriginalAssessment(currentLessonId);}
function offersOriginalPassage(){const meta=bookCatalogEntry(currentBookId);return !meta||(!meta.vocabularyOnly&&meta.originalPassage!==false);}
...
function fetchOriginal(id){if(bookIsVocabularyOnly()&&(L.words||[]).length)return;if(!bookIsVocabularyOnly()&&hasOriginalPassage()&&(!usesOriginalAssessment()||hasOriginal()))return;if(!(window.Store&&Store.pullOriginal))return;const bookId=(window.LESSONS&&window.LESSONS[id])?window.LESSONS[id].bookId:'cars-level-b';const run=++originalReqRun;Store.pullOriginal(bookId,id).then(o=>{if(run!==originalReqRun||currentLessonId!==id||!o)return;let changed=false;if(Array.isArray(o.passage)&&o.passage.length){L.originalPassage=o.passage;changed=true;}if(Array.isArray(o.questions)&&o.questions.length){L.originalQuestions=o.questions;changed=true;}else if(o.questions&&o.questions.kind==='vocabulary-pack'&&Array.isArray(o.questions.words)){L.words=o.questions.words.map(w=>[w[0],w[1],w[2]||w[1]]);L._zh={};o.questions.words.forEach(w=>{L._zh[w[0]]=w[3]||w[1];});st.wordIndex=Math.min(st.wordIndex,Math.max(0,L.words.length-1));st.gameIndex=Math.min(st.gameIndex,Math.max(0,L.words.length-1));st.gameChoices=[];changed=true;}if(changed)render();}).catch(()=>{});}
...
function setView(view){stopSpeak();st.view=view;if(view==='originalRead')st.originalRead=true; if(view==='questions')st.qMode='original'; if(view==='originalExtra')st.qMode='originalExtra'; if(view==='similar')st.qMode='similar';save();render();window.scrollTo({top:0,behavior:'smooth'});}
...
function nav(){const n=lessonNum(currentLessonId);const wc=(L.words||[]).length;const qn=(mode)=>String((qs(mode)||[]).length||12);const items=[['home','⌂','home','Lesson '+n],['words','▣','words',wc+' words']];if(!bookIsVocabularyOnly()){if(offersOriginalPassage())items.push(['originalRead','📖','read','Book']);if(usesOriginalAssessment())items.push(['questions','✎','questions',qn('original')]);items.push(['originalExtra','＋','originalExtra',qn('originalExtra')],['similar','↻','similar',qn('similar')],['report','▥','report','Results']);}const townLabel=st.lang==='ko'?'마을로':st.lang==='zh'?'返回小镇':'Reading Town';return `<aside class="side"><div class="logo">${esc(currentBookBrand())}<small>G-FIELD ONLINE READING</small></div><button class="town-btn" data-act="go-town"><span class="nav-icon">🏘</span><b>${townLabel}</b></button><div class="profile"><b>${esc(currentBookFullName())} · Lesson ${n}</b><span>${esc(L.title||'')}</span></div><div class="menu">${items.map(([view,icon,label,small])=>`<button class="nav-btn ${st.view===view?'active':''}" data-view="${view}"><span class="nav-icon">${icon}</span><span><b>${t(label)}</b><small>${small}</small></span></button>`).join('')}</div><div class="side-bottom"><b>💡 ${t('learnFirst')}</b><br>${bookIsVocabularyOnly()?(st.lang==='ko'?'카드와 게임을 번갈아 반복해요.':st.lang==='zh'?'交替使用单词卡和游戏进行复习。':'Alternate word cards and games for spaced review.'):(st.lang==='ko'?'문제를 풀기 전에 단어를 먼저 익혀요.':st.lang==='en'?'Learn words before solving reading questions.':'先学习单词，再完成阅读练习。')}</div></aside>`}
function top(){const oc=(qs('original')||[]).length||12;const o=st.original?`${st.original.score}/${oc}`:'—';const wc=(L.words||[]).length||12;const originalPill=usesOriginalAssessment()?`<span class="progress-pill">${t('original')} ${o}</span>`:'';return `<div class="topbar"><div class="crumb"><button class="crumb-link" data-act="go-town">${st.lang==='ko'?'리딩 타운':st.lang==='zh'?'阅读小镇':'Reading Town'}</button><span class="crumb-sep">›</span><button class="crumb-link" data-act="open-shelf">${esc(currentBookFullName())}</button><span class="crumb-sep">›</span><b>Lesson ${lessonNum(currentLessonId)}</b></div><div class="status"><span class="progress-pill points-pill">★ ${(profile&&profile.points)||0}P</span><span class="progress-pill">${t('words')} ${Object.keys(st.known).filter(k=>st.known[k]).length}/${wc}</span>${originalPill}<div class="top-language language-toggle" role="group" aria-label="Language"><button data-lang="ko" class="${st.lang==='ko'?'active':''}">한국어</button><button data-lang="en" class="${st.lang==='en'?'active':''}">English</button><button data-lang="zh" class="${st.lang==='zh'?'active':''}">中文</button></div></div></div>`}
function stepDone(view){const wc=(L.words||[]).length;switch(view){case 'words':return wc>0&&(st.gameDone||Object.keys(st.known).filter(k=>st.known[k]).length>=wc);case 'originalRead':return !!st.originalRead||!!st.original;case 'questions':return !!st.original;case 'originalExtra':return !!st.originalExtra;case 'similar':return !!st.similar;case 'report':return (!usesOriginalAssessment()||!!st.original)&&!!st.originalExtra&&!!st.similar;default:return false;}}
function flow(){const items=[['words',t('words')]];if(!bookIsVocabularyOnly()){if(offersOriginalPassage())items.push(['originalRead',t('read')]);if(usesOriginalAssessment())items.push(['questions',t('questions')]);items.push(['originalExtra',t('originalExtra')],['similar',t('similar')],['report',t('report')]);}return `<div class="flow">${items.map((x,i)=>{const done=stepDone(x[0]),active=st.view===x[0];return `${i?'<span class="arrow">→</span>':''}<button class="flow-step ${active?'active':''} ${done?'done':''}" data-view="${x[0]}"><small>STEP ${i+1}${done?' <span class="flow-check">✓</span>':''}</small><b>${x[1]}</b></button>`;}).join('')}</div>`}
// Falling back to a fixed CARS B picture would show the wrong book's illustration
// on any lesson without its own art, so render nothing instead when image is empty.
function art(){if(!L.image)return '';return `<div class="art-stack"><div class="original-figure"><img class="bw-art" src="${esc(L.image)}" alt="${esc(L.title||'Lesson illustration')}" onerror="this.closest('.art-stack').remove()"></div></div>`}
...
function home(){const vocabOnly=bookIsVocabularyOnly(),nextView=offersOriginalPassage()?'originalRead':'originalExtra',nextLabel=offersOriginalPassage()?t('read'):t('originalExtra');const count=(L.words||[]).length;const intro=vocabOnly?(st.lang==='ko'?'카드, 암기, 뜻 맞히기를 번갈아 반복해 단어를 익혀요.':st.lang==='zh'?'交替使用单词卡、记忆卡和释义游戏来掌握词汇。':'Build mastery by alternating cards, memory, and meaning games.'):(st.lang==='ko'?'단어를 먼저 익히고, 준비된 지문과 문제를 한 단계씩 풀어 보세요.':st.lang==='en'?'Learn the words, then work through each available passage and question set.':'先学单词，再逐步完成已准备的文章和题目。');const readingAction=vocabOnly?'':`<button class="action pink" data-view="${nextView}"><span class="action-step">STEP 2</span><h3>${nextLabel}</h3><p>${st.lang==='ko'?'지문을 읽고 전략 문제를 풀어요.':'Read and solve strategy questions.'}</p><span class="go">${nextLabel}</span></button>`;return `<div class="work-grid"><section class="card reading-card"><div class="headrow"><div><div class="section-tag"><i>❧</i> Lesson ${lessonNum(currentLessonId)}</div><h1 class="title">${esc(L.title||t('title'))}</h1><p class="sub">${intro}</p></div><button class="btn primary" data-view="words">${t('start')} →</button></div><div class="story">${art()}<div class="passage"><p><b>${esc(L.title||'')}</b>${L.theme?` · ${esc(L.theme)}`:''}</p><p>${count?`${count} ${st.lang==='ko'?'개 핵심 단어':st.lang==='zh'?'个重点单词':'focus words'}`:(st.lang==='ko'?'단어 데이터를 불러오는 중입니다.':st.lang==='zh'?'正在加载词汇数据。':'Loading vocabulary data.')}</p></div></div></section></div><section class="card learning"><h2>${t('learnFirst')} <span style="color:#2f8a5c;font-size:15px">(${count})</span></h2><div class="action-grid ${vocabOnly?'vocabulary-only':''}"><button class="action purple" data-wordmode="cards"><span class="action-step">STEP 1</span><h3>${t('cards')}</h3><p>${st.lang==='ko'?'뜻과 한국어를 확인해요.':'Meanings and Korean support.'}</p><span class="go">${t('start')}</span></button><button class="action yellow" data-wordmode="memory"><span class="action-step">STEP 1</span><h3>${t('memory')}</h3><p>${st.lang==='ko'?'카드를 뒤집어 외워요.':'Flip and remember.'}</p><span class="go">${t('start')}</span></button><button class="action blue" data-wordmode="game"><span class="action-step">STEP 1</span><h3>${t('game')}</h3><p>${st.lang==='ko'?'뜻 맞히기로 복습해요.':'Match the meaning.'}</p><span class="go">${t('start')}</span></button>${readingAction}</div></section>`}
function words(){let body='';const wc=(L.words||[]).length;const myWordsLbl=st.lang==='ko'?'나만의 단어':st.lang==='zh'?'我的单词':'My Words';if(!wc)return `<section class="card panel"><div class="panel-head"><div><div class="section-tag"><i>▣</i> ${t('stepWords')}</div><h2>${st.lang==='ko'?'단어를 불러오는 중입니다':st.lang==='zh'?'正在加载词汇':'Loading vocabulary'}</h2></div></div><div class="mywords-empty"><div class="mw-star">···</div><p>${st.lang==='ko'?'잠시 후 자동으로 표시됩니다.':st.lang==='zh'?'稍后会自动显示。':'The word pack will appear automatically.'}</p></div></section>`;
 if(st.wordMode==='cards'){body=`<div class="word-grid">${L.words.map((w,i)=>`<article class="word-card ${st.known[w[0]]?'known':''} ${st.lang==='ko'?'ko':''}"><strong>${esc(w[0])}</strong><p class="meaning">${esc(wordMeaning(w))}</p>${wordDetail(w)?`<p class="word-detail">${esc(wordDetail(w))}</p>`:''}<div class="word-actions"><button class="btn tiny" data-act="word-speak" data-i="${i}">🔊</button><button class="btn tiny star ${isSaved(w[0])?'on':''}" data-act="save-word" data-i="${i}" title="${myWordsLbl}">${isSaved(w[0])?'★':'☆'}</button><button class="btn tiny ${st.known[w[0]]?'known':''}" data-act="known" data-i="${i}">${st.known[w[0]]?t('knownDone'):t('known')}</button></div></article>`).join('')}</div>`}
  ...
  if(st.gameDone){const lessons=availableLessons(),at=lessons.indexOf(currentLessonId),nextLesson=lessons[at+1];const nextView=offersOriginalPassage()?'originalRead':'originalExtra',nextLabel=offersOriginalPassage()?t('read'):t('originalExtra');const nextButton=bookIsVocabularyOnly()?(nextLesson?`<button class="btn violet" data-act="open-lesson" data-lesson="${esc(nextLesson)}">${st.lang==='ko'?'다음 유닛':st.lang==='zh'?'下一单元':'Next Unit'} →</button>`:`<button class="btn violet" data-act="open-shelf">${st.lang==='ko'?'책장으로':st.lang==='zh'?'返回书架':'Bookshelf'} →</button>`):`<button class="btn violet" data-view="${nextView}">${nextLabel} →</button>`;body=`<div class="game-stage"><div class="score-orb">${st.gameScore}/${wc}</div><h3>${st.lang==='ko'?'단어 게임을 마쳤어요!':st.lang==='en'?'Word game finished!':'单词游戏完成！'}</h3><div class="tools center"><button class="btn primary" data-act="restart-game">${st.lang==='ko'?'다시 하기':st.lang==='zh'?'再玩一次':'Play Again'}</button>${nextButton}</div></div>`;}
...
function originalRead(){const text=passageBlock(L.originalPassage,'original',true);const questions=usesOriginalAssessment()?`<div class="tools"><button class="btn violet" data-act="open-original-questions">${t('startQuestions')}</button></div>${st.questionOpen?`<section class="question-drawer" id="question-drawer">${questionCard('original',true)}</section>`:''}`:`<div class="tools"><button class="btn violet" data-view="originalExtra">${t('originalExtra')} →</button></div>`;return `<div class="work-grid"><section class="card reading-card"><div class="headrow"><div><div class="section-tag"><i>▤</i> ${t('stepRead')}</div><h1 class="title">${esc(L.title||t('title'))}</h1><p class="sub">${t('originalSubtitle')}</p></div><div class="header-actions">${listenButtons('original')}<button class="btn print-btn" data-act="print-study" data-passage="original">🖨 ${t('printStudy')}</button></div></div>${text}${questions}</section></div>`}
...
function questionScreen(mode){
 ...
 // The "same N words" line has to count the lesson's own vocabulary — non-CARS books
 // (Reading Prime, WonderSkills, Subject Link, CARS D) are not all 12 words.
 const vocabCount=(L.words||[]).length||12;
 const extraSub=t('extraSubtitle').replace(/12(?=\s*단어|\s*words|\s*个单词)/,String(vocabCount));
 const subtitle=isReview?(st.lang==='ko'?'틀린 문제만 다시 풀어요. 힌트를 보고 스스로 고쳐 봐요.':st.lang==='zh'?'只重做错题，看提示自己改正。':'Re-solve only the ones you missed. Use the hint and fix them.'):(isOriginal?t('originalSubtitle'):(isExtra?t('originalExtraSubtitle'):extraSub));
...
function report(){const threeStage=usesOriginalAssessment();if((threeStage&&!st.original)||!st.originalExtra||!st.similar)return `<section class="card panel"><div class="section-tag"><i>▥</i> ${t('report')}</div><h2>${st.lang==='ko'?'준비된 두 종류의 연습 문제까지 풀면 비교 리포트가 나와요.':st.lang==='en'?'Complete both practice modes to see your comparison report.':'完成两种练习后可查看对比报告。'}</h2></section>`;
 const T=(ko,en,zh)=>st.lang==='ko'?ko:st.lang==='zh'?zh:en;
 const baselineMode=threeStage?'original':'originalExtra',baseline=threeStage?st.original:st.originalExtra;
 const old=baseline.missed,now=st.similar.missed;
 ...
 const recMode=still.length?baselineMode:'similar',rec=(still.length?still:fresh).slice(0,3);
 const tile=(label,score,cls,mode)=>`<div class="rp-tile ${cls}"><span class="rp-score">${score}<i>/${(qs(mode)||[]).length||12}</i></span><small>${esc(label)}</small></div>`;
 const tiles=threeStage?`${tile(t('original'),st.original.score,'a','original')}${tile(t('originalExtra'),st.originalExtra.score,'b','originalExtra')}${tile(t('similar'),st.similar.score,'c','similar')}`:`${tile(t('originalExtra'),st.originalExtra.score,'b','originalExtra')}${tile(t('similar'),st.similar.score,'c','similar')}`;
 return `<section class="card rp-card"><div class="rp-head"><div class="section-tag"><i>▥</i> ${t('report')}</div><h2 class="rp-title">${t('reportTitle')}</h2><p class="rp-sub">${threeStage?T('교재 학습 → 추가 학습 → 새 지문 연습, 세 단계의 변화를 비교해요.','Compare your three stages: Book Study → Extra → New Passage.','对比三个阶段的变化。'):T('추가 학습 → 새 지문 연습, 두 단계의 전략 변화를 비교해요.','Compare strategy growth from Extra Learning to New Passage Practice.','比较追加学习到新文章练习的策略变化。')}</p></div>
  <div class="rp-tiles">${tiles}</div>
  <div class="rp-grid">
   <div class="rp-box good"><div class="rp-lab">✅ ${T('해결된 약점','Improved','已改进')}</div><div class="rp-chips">${chips(solved,baselineMode)}</div></div>
   <div class="rp-box warn"><div class="rp-lab">⚠️ ${T('계속 어려운 유형','Still tricky','仍薄弱')}</div><div class="rp-chips">${chips(still,baselineMode)}</div></div>
  ...
  <div class="tools">${threeStage&&(still.length||old.length)?`<button class="btn primary" data-act="coach-start">🎯 ${coachT('스킬 코치 (적응형)','Skill Coach (adaptive)','技能教练（自适应）')}</button>`:''}<button class="btn violet" data-act="retry-similar">${t('retry')}</button></div></section>`}
function modal(){if(!st.modal)return '';const {mode}=st.modal;const blanks=Array.from({length:qs(mode).length},(_,i)=>i).filter(i=>!st.answers[mode][i]);return `<div class="modal-backdrop"><div class="modal"><button class="modal-x" data-act="close-modal">×</button><h2>${t('unanswered')}</h2><p><b>${blanks.length}</b>${t('unansweredCount')}</p><div class="blank-list">${blanks.map(i=>`<button class="num empty" data-act="go-q-modal" data-mode="${mode}" data-i="${i}">${i+1}</button>`).join('')}</div><div class="tools"><button class="btn" data-act="close-modal">${t('close')}</button><button class="btn primary" data-act="grade-anyway" data-mode="${mode}">${t('gradeAnyway')}</button></div></div></div>`}
...
function enterStudent(student){
 ...
 currentBookId=(profile&&profile.currentBookId)||'cars-level-b';
 const initialLesson=handoff?firstLessonOf(currentBookId):lastStudiedLesson();currentLessonId=initialLesson;loadLesson(initialLesson);
 st=stForLesson(initialLesson,lang);atTown=true;townView=(profile.avatar&&profile.avatar.picked)?landingView():'pick';save();render();window.scrollTo({top:0});flushLevelUp();
...
function openLesson(id,view){stopSpeak();saveCurrentToProfile();const _olr=window.LESSONS&&window.LESSONS[id];if(_olr&&_olr.bookId)currentBookId=_olr.bookId;else if(id==='lesson1')currentBookId='cars-level-b';if(profile)profile.currentBookId=currentBookId;loadLesson(id);st=stForLesson(currentLessonId,st.lang);st.view=view||'home';if(view==='words')st.wordMode='cards';
 if(view==='originalRead')st.originalRead=true;
...
function lessonDone(id){const l=profile&&profile.lessons?profile.lessons[id]:null;const row=(window.LESSONS||{})[id]||{};const bookId=row.bookId||(id==='lesson1'?'cars-level-b':currentBookId);if(bookIsVocabularyOnly(bookId))return !!(l&&(l.gameDone||l.awarded?.words));return !!(l&&(!lessonUsesOriginalAssessment(id)||l.original)&&l.originalExtra&&l.similar);}
...
function lessonMastery(id){const l=profile&&profile.lessons?profile.lessons[id]:null;
 ...
 const done=!!((!lessonUsesOriginalAssessment(id)||l.original)&&l.originalExtra&&l.similar);
...
function town(){
 ...
 const alphaName=st.lang==='ko'?'알파 인터뷰':st.lang==='zh'?'Alpha 面试':'Alpha Interview';
 const alphaBtn=`<a class="btn alpha-prep-entry" href="alpha-prep/">🎙 ${alphaName}</a>`;
 const avatarCard=`<section class="avatar-card"><div class="ava-wrap">${renderAvatar(null,132)}</div><div class="ava-info"><div class="ava-hello">${st.lang==='ko'?'안녕,':st.lang==='zh'?'你好，':'Hi,'} <b>${esc(currentStudent?currentStudent.name:'')}</b>!</div><div class="ava-pts"><span class="points-pill">★ ${pts}P</span></div><div class="ava-btns">${exploreBtn}${sqBtn}${alphaBtn}<button class="btn primary" data-act="town-closet">${customize}</button></div></div></section>`;
 ...
 el.querySelectorAll('.book-card').forEach(card=>{
  card.addEventListener('click',()=>{
   ...
   startBookCopy(bk,()=>{
    ...
    const firstView=bk.vocabularyOnly?'words':(lessonUsesOriginalAssessment(firstLesson)?'questions':(bk.originalPassage!==false?'originalRead':'originalExtra'));
    openLessonAt(firstLesson,firstView);
   });
  });
 });}
...
function parentStats(){const lessons=availableLessons();
 const rows=lessons.map(id=>{const ls=(profile.lessons||{})[id]||null;const rt=buildRuntimeLesson(id)||{};const orig=ls&&ls.original,ext=ls&&ls.originalExtra,sim=ls&&ls.similar;
  ...
  const qc=(arr)=>(arr||[]).length||12;
  return {id,num:lessonNum(id),title:rt.title||'',done:lessonDone(id),started:lessonStarted(id),orig:orig?orig.score:null,ext:ext?ext.score:null,sim:sim?sim.score:null,
   origTotal:qc(rt.originalQuestions),extTotal:qc(rt.originalExtraQuestions),simTotal:qc(rt.extraQuestions),wc,known};});
 return {rows};}
// Cumulative strategy-weakness ranking across every lesson and book attempted.
// New attempts carry their own strategy list. Older attempts are reconstructed
// from bundled lesson data, with the canonical CARS order used only for CARS
// originals that live in Supabase. A non-CARS original must never be guessed as
// CARS merely because its question indexes happen to overlap.
...
function attemptStrategyNames(lessonId,mode,att){
 if(Array.isArray(att.strategies)&&att.strategies.length)return att.strategies.map(s=>s||'Unclassified Reading Skill');
 const rt=buildRuntimeLesson(lessonId)||{};
 const local=mode==='original'?rt.originalQuestions:mode==='originalExtra'?rt.originalExtraQuestions:rt.extraQuestions;
 if(Array.isArray(local)&&local.length)return local.map(q=>(q&&q[0])||'Unclassified Reading Skill');
 const row=(window.LESSONS&&window.LESSONS[lessonId])||{};
 const bookId=att.bookId||row.bookId||(lessonId==='lesson1'?'cars-level-b':'');
 if(String(bookId).startsWith('cars-level-')){
  const total=Number(att.total)||Number(att.score)+(att.missed||[]).length||CANON_STRATS.length;
  return CANON_STRATS.slice(0,total);
 }
 return [];
}
function cumulativeStrategyStats(){
 const strat={};const lessons=(profile&&profile.lessons)||{};let attempts=0,unclassifiedAttempts=0;
 Object.entries(lessons).forEach(([lessonId,ls])=>{if(!ls)return;
  ['original','originalExtra','similar'].forEach(mode=>{const att=ls[mode];if(!att||!Array.isArray(att.missed))return;attempts++;
   const names=attemptStrategyNames(lessonId,mode,att);if(!names.length){unclassifiedAttempts++;return;}
   names.forEach((s,i)=>{if(!strat[s])strat[s]={seen:0,miss:0};strat[s].seen++;if(att.missed.includes(i))strat[s].miss++;});});});
 const ranked=Object.keys(strat).map(s=>({name:s,seen:strat[s].seen,miss:strat[s].miss,rate:strat[s].seen?strat[s].miss/strat[s].seen:0}))
  .filter(r=>r.seen>0).sort((a,b)=>b.rate-a.rate||b.miss-a.miss);
 return {ranked,attempts,unclassifiedAttempts};}
function parentDashboard(){const t=(ko,en,zh)=>st.lang==='ko'?ko:st.lang==='zh'?zh:en;const pts=window.GameStore?GameStore.coins(profile):(profile&&profile.points)||0;
 ...
 const sc=(v,total)=>v==null?'—':v+'/'+(total||12);
 const rowsHtml=S.rows.map(r=>{const status=r.done?t('완료','Done','完成'):r.started?t('학습 중','In progress','进行中'):t('시작 전','Not started','未开始');return `<div class="pd-row ${r.done?'done':''}"><span class="pd-lnum">${r.num}</span><div class="pd-linfo"><b>Lesson ${r.num}</b><small>${esc(r.title)}</small></div><span class="pd-stat ${r.done?'ok':r.started?'mid':''}">${status}</span><span class="pd-scv">${sc(r.orig,r.origTotal)}</span><span class="pd-scv">${sc(r.ext,r.extTotal)}</span><span class="pd-scv">${sc(r.sim,r.simTotal)}</span><span class="pd-scv">${r.known}/${r.wc||12}</span></div>`;}).join('');
...
function noticeBoardScreen(){const t=(ko,en,zh)=>st.lang==='ko'?ko:st.lang==='zh'?zh:en;
 ...
 return `<div class="town notice-board"><div class="town-top"><div class="town-brand">📢 ${t('공지사항','Notices','公告栏')}</div><div class="town-tools"><a class="btn tiny" href="manual.html?lang=${st.lang}" target="_blank" rel="noopener">📖 ${t('사용법','How to use','使用说明')}</a><button class="btn tiny" data-act="nb-compose">✏️ ${t('새 공지','New','新公告')}</button><button class="btn tiny" data-act="town-map">← ${t('마을','Town','小镇')}</button><div class="language-toggle town-lang">${langBtns}</div></div></div><div class="nb-wrap">${compose}<div class="nb-list">${list}</div></div></div>`;}
...
function render(){if(!currentStudent){clearTownFit();if(window.TownGame)TownGame.destroy();if(!introSeen()){app.innerHTML=introScreen();bindIntro();return;}app.innerHTML=gate();bindGate();return;}
 ...
 if(atTown){if(townView==='game'&&gameAvailable()){app.innerHTML=gameTownShell();bindGameTown();return;}if(townView==='screen'&&sqAvailable()){app.innerHTML=screenQuestScreen();bind();return;}if(townView==='notice'){app.innerHTML=noticeBoardScreen();bind();return;}if(townView==='diagnostic'){app.innerHTML=diagnosticScreen();bind();return;}if(townView==='parent'){app.innerHTML=parentDashboard();bind();return;}if(townView==='library'){app.innerHTML=libraryScreen();bind();bindLibraryAudio();bindLibraryPdf();return;}if(townView==='writing'){app.innerHTML=writingScreen();bind();bindWritingInputs();return;}app.innerHTML=town();bind();return;}let content=st.view==='home'?home():st.view==='words'?words():st.view==='originalRead'&&!hasOriginalPassage()?missingOriginal():st.view==='questions'&&!hasOriginal()?missingOriginal():st.view==='originalRead'?originalRead():st.view==='questions'?questionScreen('original'):st.view==='questionOriginalExtra'?questionScreen('originalExtra'):st.view==='questionSimilar'?questionScreen('similar'):st.view==='review'?questionScreen('review'):st.view==='coach'?coachScreen():st.view==='originalExtra'?originalExtra():st.view==='similar'?extra():report();app.innerHTML=`<div class="shell">${nav()}<main class="main">${top()}${flow()}${content}</main></div>${modal()}`;bind();}
...
function grade(mode){const list=qs(mode),miss=[];for(let i=0;i<list.length;i++)if(st.answers[mode][i]!==list[i][3])miss.push(i);st[mode]={score:list.length-miss.length,missed:miss,total:list.length,strategies:list.map(q=>(q&&q[0])||'Unclassified Reading Skill'),bookId:currentBookId,lessonId:currentLessonId};st.modal=null;
...
function printStudy(passage){
 ...
 // Question count varies by book (CARS = 12, but Reading Prime / WonderSkills /
 // Subject Link / CARS D sets differ), so split the sheet in half instead of at a
 // fixed 6, and build the answer line from the actual count.
 const qArr=cfg.questions||[];const half=Math.ceil(qArr.length/2);
 const qBlock=(list,offset)=>list.map((q,i)=>`<div class="q"><b>${offset+i+1}. ${esc(q[1])}</b>${q[2].map((c,j)=>`<div class="choice">${letters[j]}. ${esc(c)}</div>`).join('')}</div>`).join('');
 const answerLine=qArr.map((_,i)=>`${i+1} ___`).join(' ');
 const html=`<!doctype html><html><head><meta charset="utf-8"><title>${cfg.title}</title><style>@page{size:A4;margin:15mm}body{font-family:Arial,sans-serif;color:#111;font-size:11pt;line-height:1.45}.page{break-after:page;min-height:250mm}.page:last-child{break-after:auto}h1{font-size:22pt;margin:0 0 5mm}.meta{margin:0 0 7mm}.print-art{display:block;max-width:100%;max-height:72mm;margin:0 auto 7mm;object-fit:contain}.passage p{margin:0 0 4mm}.q{break-inside:avoid;margin:0 0 5mm}.q b{display:block;margin-bottom:2mm}.choice{margin-left:4mm}</style></head><body><section class="page"><h1>${esc(cfg.title)}</h1><div class="meta">Name: ____________________　Date: ____________________</div><img class="print-art" src="${artUrl}" alt="Lesson illustration"><div class="passage">${cfg.text.map(x=>`<p>${esc(x)}</p>`).join('')}</div></section><section class="page">${qBlock(qArr.slice(0,half),0)}</section><section class="page">${qBlock(qArr.slice(half),half)}<p style="margin-top:10mm">Answers: ${answerLine}</p></section></body></html>`;
