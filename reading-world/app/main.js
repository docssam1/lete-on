...
function offersOriginalPassage(){const meta=bookCatalogEntry(currentBookId);return !meta||meta.originalPassage!==false;}
...
function home(){const nextView=offersOriginalPassage()?'originalRead':'originalExtra',nextLabel=offersOriginalPassage()?t('read'):t('originalExtra');return `<div class="work-grid"><section class="card reading-card"><div class="headrow"><div><div class="section-tag"><i>❧</i> Lesson ${lessonNum(currentLessonId)}</div><h1 class="title">${esc(L.title||t('title'))}</h1><p class="sub">${st.lang==='ko'?'단어를 먼저 익히고, 준비된 지문과 문제를 한 단계씩 풀어 보세요.':st.lang==='en'?'Learn the words, then work through each available passage and question set.':'先学单词，再逐步完成已准备的文章和题目。'}</p></div><button class="btn primary" data-view="words">${t('start')} →</button></div><div class="story">${art()}<div class="passage"><p><b>${esc(L.title||'')}</b>${L.theme?` · ${esc(L.theme)}`:''}</p><p>${st.lang==='ko'?`${(L.words||[]).length}개 핵심 단어를 익힌 뒤 준비된 지문으로 읽기 전략을 연습해요.`:st.lang==='en'?`Learn ${(L.words||[]).length} focus words, then practise reading strategies with the available passages.`:`先学习 ${(L.words||[]).length} 个重点单词，再用配套文章练习阅读策略。`}</p></div></div></section></div><section class="card learning"><h2>${t('learnFirst')} <span style="color:#2f8a5c;font-size:15px">(${(L.words||[]).length})</span></h2><div class="action-grid"><button class="action purple" data-wordmode="cards"><span class="action-step">STEP 1</span><h3>${t('cards')}</h3><p>${st.lang==='ko'?'뜻과 한국어를 확인해요.':'Meanings and Korean support.'}</p><span class="go">${t('start')}</span></button><button class="action yellow" data-wordmode="memory"><span class="action-step">STEP 1</span><h3>${t('memory')}</h3><p>${st.lang==='ko'?'카드를 뒤집어 외워요.':'Flip and remember.'}</p><span class="go">${t('start')}</span></button><button class="action blue" data-wordmode="game"><span class="action-step">STEP 1</span><h3>${t('game')}</h3><p>${st.lang==='ko'?'뜻 맞히기로 복습해요.':'Match the meaning.'}</p><span class="go">${t('start')}</span></button><button class="action pink" data-view="${nextView}"><span class="action-step">STEP 2</span><h3>${nextLabel}</h3><p>${st.lang==='ko'?'지문을 읽고 전략 문제를 풀어요.':'Read and solve strategy questions.'}</p><span class="go">${nextLabel}</span></button></div></section>`}
function words(){let body='';const wc=(L.words||[]).length;const myWordsLbl=st.lang==='ko'?'나만의 단어':st.lang==='zh'?'我的单词':'My Words';
 if(st.wordMode==='cards'){body=`<div class="word-grid">${L.words.map((w,i)=>`<article class="word-card ${st.known[w[0]]?'known':''} ${st.lang==='ko'?'ko':''}"><strong>${esc(w[0])}</strong><p class="meaning">${esc(wordMeaning(w))}</p>${wordDetail(w)?`<p class="word-detail">${esc(wordDetail(w))}</p>`:''}<div class="word-actions"><button class="btn tiny" data-act="word-speak" data-i="${i}">🔊</button><button class="btn tiny star ${isSaved(w[0])?'on':''}" data-act="save-word" data-i="${i}" title="${myWordsLbl}">${isSaved(w[0])?'★':'☆'}</button><button class="btn tiny ${st.known[w[0]]?'known':''}" data-act="known" data-i="${i}">${st.known[w[0]]?t('knownDone'):t('known')}</button></div></article>`).join('')}</div>`}
  ...
  if(st.gameDone){const nextView=offersOriginalPassage()?'originalRead':'originalExtra',nextLabel=offersOriginalPassage()?t('read'):t('originalExtra');body=`<div class="game-stage"><div class="score-orb">${st.gameScore}/${wc}</div><h3>${st.lang==='ko'?'단어 게임을 마쳤어요!':st.lang==='en'?'Word game finished!':'单词游戏完成！'}</h3><div class="tools center"><button class="btn primary" data-act="restart-game">${st.lang==='ko'?'다시 하기':st.lang==='zh'?'再玩一次':'Play Again'}</button><button class="btn violet" data-view="${nextView}">${nextLabel} →</button></div></div>`;}
...
function lessonDone(id){const l=profile&&profile.lessons?profile.lessons[id]:null;return !!(l&&(!lessonUsesOriginalAssessment(id)||l.original)&&l.originalExtra&&l.similar);}
 ...
 el.querySelectorAll('.book-card').forEach(card=>{
  card.addEventListener('click',()=>{
   ...
   startBookCopy(bk,()=>{
    ...
    const firstView=lessonUsesOriginalAssessment(firstLesson)?'questions':(bk.originalPassage!==false?'originalRead':'originalExtra');
