...
- 오개념 진단 파일(`*.misc.js`): **4단원 전부**(s41-u01·s41-u02·s41-u03·s42-u01). 검사기 `bank/misc-audit.mjs`. 서술형 부분점수 규칙(`written`)도 4단원에 있다(32개, `bank/written-audit.mjs`).
- 3D 실험실: 고리 자석 탑, 흙 언덕, 화산, 얼음(freeze), 연못.
- 살아 있는 교재(A4 학생용·강사용 11쪽 + 수업 화면): **5권 전부** — s41-u01 「둥실 고리 자석 탑」·s41-u02 「얼었다 녹는 물기둥」·s41-u03 「흙 언덕 물길」·s41-u03b 화산·s42-u01 「둥둥 부레옥잠의 비밀」 (`data/book/*.book.js`). 검사 `node science-lab/bank/tools/book-check.mjs <단원…>`(서버 8765 필요).
...
2. **[완료 2026-09-25]** 오개념 진단을 나머지 두 단원으로 확장했다: `s41-u02.misc.js`, `s42-u01.misc.js`. 17차 패턴을 그대로 반복한다(오개념 정의 → distractors/typed/cloze/cells → remedy → 검증 스크립트 → `v2/v2.js` 로더에 `misc` 한 줄 추가 → 화면 확인). 주의: "틀린 것 고르기" 부정 발문 문항은 오답 인덱스 방향이 반대다.
