...
- **라벨**: Gaegu 700 **종이 꼬리표**(크림 종이 · 남색 잉크 · 손으로 그은 떨리는 테두리 · 연필 겹선). 흰 배경을 준 라벨은 종이로, 색 배경을 준 라벨은 그 색 유지. 구글 폰트가 한글을 조각으로 보내므로 **그 글자 조각이 도착하면 같은 텍스처에 다시 그린다**. `fitLabels()`가 매 프레임 ①화면 글자 높이 최소 보장 ②**화면 폭을 넘지 않게 축소** ③가장자리에 걸리면 `sprite.center`를 옮겨 안으로 밀어 넣음(휴대폰에서 긴 라벨 잘림 해결). v1 페이지에는 Gaegu 링크가 없어 킷이 스스로 붙인다.
...
- **80년대 광택 단추**(`v2.css`): `.btn`·`.modes button`·`.icon-btn` — 위쪽 반사 띠 + 두꺼운 아래 옆면, 누르면 3px 들어감. 기본=남색 사탕, 보조=아이보리 플라스틱. 다크 모드 토큰 별도, 인쇄에서는 평면.
