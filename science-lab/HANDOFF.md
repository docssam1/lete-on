...
- 아직: 다른 단원(u01·u02·s42-u01)엔 misc 파일이 없어 진단 버튼이 안 뜸다(같은 형식으로 만들면 자동). 기록은 기기(localStorage)에만 — 강사가 여러 학생을 보려면 Supabase 테이블 필요(별도 제안). 서술형은 기록하지 않는다(사용자 지시).
...
- 소리 버튼이 상태를 보여 준다: `🔊 소리 켜기` → 재생되면 `🔊 독쌤 음성`(MP3) 또는 `🔊 기기 음성`, 꺼면 `🔇 소리 꺼짐`.
...
- 영상이 안 나온다는 지적 → 위키미디어 자료를 **webm 480p → mp4(360p mov, 사파리·아이폰) → 원본** 3단으로 주고, 재생 실패 시 "새 창에서 영상 보기" 링크로 바뀜다(`v2/live.js`, `v2/v2.js`, `data/media/*.media.js`의 `V()`에 `mp4` 추가).
...
- 사용자 지적: 땅의 변화 5단계 수업(흙 언덕 물길)이 소단원 1·2만 다루고 화산(3~6)은 개념 없이 문제만 있었다 → **두 번째 5단계 수업 「화산 실험실」** `s41-u03b`.
  ...
  - 3D 장면 `scenes/volcano-erupt.js`: 지형 높이맵 화산, 땅속 단면(반투명 X-ray)에 마그마 방·통로, 분출 = 용암 분수·화산재 기둥·암석 조각·용암류 3줄기·주황 점광, 식어 현무암 / 땅속 화강암. **InstancedMesh는 `frustumCulled=false` 필수**(count 0으로 첫 렌더 되면 바운딩 구가 0이라 영영 안 보임 — 이번에 한참 헤맸).
...
1. **성취기준 문장 미대조.** 교육부 고시 제2022-33호 [별책 9] 원문을 보려 했으나 이 컨테이너의 이그레스
   ...
   막는다.** 추측하지 않고 `{code, text:null, verified:false}`로 됗다.
...
- ※ 폰트 CDN 2건(`cdn.jsdelivr.net`·`fonts.googleapis.com`)은 이 컨테이너의 이그레스 차단이라 집계에서 뻔다.
...
- 진도 저장 localStorage `sciLab.v2`. 검증: Playwright 390·320px × 12경로 콘솔 에러 0·가로 넘침 0, 실험실 기록·빈칸·관문·재도전·인쇄 PDF 확인. `bank/audit.mjs`는 `*.lesson.js`를 건너띄.
...
- docssam 표정 9장 반입(GPT 생성, 사용자 G드라이브 `docssam 표정/`): A1 몸 고정 + A2~A5 얼굴 조각(말하기 3·깜밥임 1, SIFT 정렬 오차 <1px), B1~B4 반응. 웹용 webp는 `docssam 표정/web/` 9개(≈560KB). 말하기 = 음절 모음(ㅏㅓ→크게, ㅗㅜ→동그랗게, 그 밖→반쯤).
