...
- 광고 책에 **CHAPTER Ⅳ 집에서도**(준비물 목록·집에 있음/QR로 구매·준비물 QR·수업 전/수업/수업 후 활용) + **FAQ** 쪽 추가 → 광고 7쪽(표지 포함 8) + 교재 11쪽 + 뒤표지 = 20쪽. 교재 첫 쪽이 오른쪽에 오도록 광고 쪽 수를 짝수로 맞춘 것임(쪽 수를 바꿀 땀 `state.pages` 슬라이스와 `narrate()`의 `c0`도 같이 고쳐야 한다).
...
- **카카오톡 인앱 브라우저 탈출** `v2/inapp.js`: KAKAOTALK·FB·인스타·라인·네이버 인앱을 감지해 `kakaotalk://web/openExternal`(안드로이드는 intent://, iOS는 googlechrome://·x-safari-https://)로 바깥 브라우저에서 다시 열다. 자동 전환이 막히면 아래 띠에 "브라우저에서 열기" 버튼. v2 앱과 광고 페이지 둘 다 적용.
...
- `v2/book.js` `renderChapter(..., { live, media })`: live면 첫 쪽 그림 자리에 실제 영상(누르면 쪽 안에서 재생), QR·실험 순서 그림에 주황 `[data-pop]` 버튼, 빈칸 `data-a`(누르면 답), 확인 문제 `data-key`(누르면 채점). 인쇄 CSS는 `.bk-web`·버튼을 숨기고 `.bk-print`(원래 그림)·QR만 남긴다. 웹 교재 `#/<u>/lab-book/*`는 live로 열다.
...
- **나레이션**: `intro/narration.json`(docssam 12줄, 목소리 `ko-KR-Chirp3-HD-Puck`, 실패 시 `ko-KR-Neural2-C`). `scripts/generate-audio.js`가 읽어 `audio/science-lab/<id>-<sha1(voice|text) 앞10자>.mp3`로 올린다(있으면 건너뜀). 페이지는 같은 해시로 주소를 만들어 재생, 없으면 기기 음성. **generate-audio.js를 고치면 main 푸시 때 Generate Audio 워크플로가 돌다** — narration.json만 고친 경우는 워크플로를 수동 실행(Actions → Generate Audio → Run workflow).
- 지도 화면 머리에 "교재 소개 ›" 링크.
- **디자인(사용자 요구: 만화책 말고 판타지 고서, 두꺼운 마법서 넘기는 느낌, 표지도 살아 있게)**: 촛불 켜 서재 배경 + 떠오르는 불씨(canvas), 가죽 장정 보드·금박 이중 테두리·모서리 덩굴, 제목 금박 반짝임(SVG gradient animate), 표지 가운데 로드맵 = 8학기 별자리(체험 가능한 학기 빛남)·회전하는 룬 고리 2겹(영문/한글 탐구 단계)·플라스크 문장, 빛줄기 훑기, 닫힌 책은 마우스 따라 3D 기울기. 넘길 때 책장 두께가 좌우로 옮겨 가고(--tl/--tr), 넘기는 장에 그림자, WebAudio로 합성한 종이 넘김 소리(표지는 묵직한 울림). 소개 장은 양피지·명조(Nanum Myeongjo)·붉은 밀랍 인장. 테스트 스크립트는 넘김 애니메이션(1.25s)이 끝난 뒤 조작해야 한다.
...
- 사용자 지적: 땅의 변화 5단계 수업(흙 언덕 물길)이 소단원 1·2만 다루고 화산(3~6)은 개념 없이 문제만 있었다 → **두 번째 5단계 수업 「화산 실험실」** `s41-u03b`.
  ...
  - 3D 장면 `scenes/volcano-erupt.js`: 지형 높이맵 화산, 땅속 단면(반투명 X-ray)에 마그마 방·통로, 분출 = 용암 분수·화산재 기둥·암석 조각·용암류 3줄기·주황 점광, 식어 현무암 / 땅속 화강암. **InstancedMesh는 `frustumCulled=false` 필수**(count 0으로 첫 렌더 되면 바운딩 구가 0이라 영영 안 보임 — 이번에 한참 헤맵).
  - 체험 실험실 `v2/lab-volcano3d.js`: **단계 띠**(①포일 화산 올리기 → ②불 붙이기 → ③가열하기(누르고 있기) → ④불 끄고 굳히기 → ⑤표에 적기 / 식히기: 백반 물 담기 → 식힐 곳 고르기 → 식히기 → 결정 살펴보기 → 표에 적기). 주 버튼이 단계마다 바뀜(누르기/누르고 있기). 마시멀로 방울이 포일 비탈을 타고 흘러 받침에 고이고 식으면 갈색으로 굳음, 백반 결정은 팔면체 인스턴스(얼음물 150개 작게 / 상자 12개 크게). 기록 행 `{exp, cond, result}`. WebGL 없으면 글 실험실.
- **실제 사진·영상**(사용자 요구: 돌하르방·화강암 등 실물): `data/media/s41-u03b.media.js` — 위키미디어 공용 자유 이용 자료를 **원본 주소로 핫링크**(저장소에 안 넣음), 출처·라이선스를 화면에 표시. ①궁금 3D 아래 USGS 킬라우에아 분출 영상(PD, 480p 트랜스코드), ②3D 보기 아래 용암 흘름 영상, ③개념에 사진 11장 갤러리(분출·용암·화산재·현무암·화강암·천지·백록담·돌하르방+주상절리·온천), ④읽기 옆 돌하르방. 조회는 Commons API를 **사용자 컴퓨터의 내장 브라우저**로 열어서 했다(컨테이너·device_bash 모두 위키미디어 차단).
...
- 사용자 결정: 단원평가 유형·이론편·실험2·CMS 영재원 자료를 합친 **우리만의 교과 연계 실험 영재 교재**. 예 책 구조는 각색하고 그림은 새로 그린다. CMS·하늘교육 등 상용 자료는 형식 참고만(문장·그림 복제 금지).
  웹과 A4 인쇄 둘 다. 품질 기준 = 사용자 제공 줄기과학 샘플(색 머리띄·옆날개 플러스 노트·용어풀이·빈칸 답·단원 탭·꼭지 띠).
...
- **체험형 3D 실험실(사용자 요청 "체험할 수 있게 움직이는 형태")**: `v2/lab-hill3d.js` + 순수 계산 `v2/hill-sim.js`(물방울 침식 — 물방울이 비탈을 따라 흘르며 깎고 옮기고 쌓음, 흙 양 보존, 흔어짐은 고정 수열이라 같은 조건=같은 결과). 경사·물의 양 선택 → 언덕을 눌러 컵 자리 → '물 부기' 누르고 있기 → 컵이 비면 표에 적기. WebGL 없으면 2D `lab-hill.js`로 대체. 같은 컵 자리에서 가파름>완만, 많이>적게가 항상 성립(node로 확인).
...
- **문제은행**: Supabase `science_bank_source` `s41-u02` 세트1~4 = 80행(원문·정답, 분류 태그). git엠 원문 없음.
...
- **유사문항** `data/units/s41-u02.similar.js` 80개(선택41·단답28·서쉀8·복수선택3, 정답 위치 9·8·8·8·8). `bank/audit.mjs` 통과.
...
이미 푸시도에 있었는데, 뒤 차례는 그것을 모르고 같은 일을 다시 했다(14유형 JSON + `s41-u01.js` 안의 `v001~v080`
+ 별도 교재 섹션 + 창작 그림 4개).

**합칠 때 앞 차례 것을 남겨다.** 이미 앱에 물려 있었기 때문이다 — 소단원 화면 `#/<단원>/sub/<E>`,
...
1. **`bank/taxonomy/s41-u01.json`** — 작업 지시서가 지정한 경로·형식의 공개 산출물. 내용은 14유형에서
   ...
   **`bank/taxonomy/build.mjs`가 거기서 생성**한다(유형별 원문 수·유사문항 수·형식 집계·원문 세트번호 목록을
   얹어서). 직접 고치지 말 것.
2. **`bank/audit.mjs`에 동기화 검사** — `build.mjs --check`로 JSON이 `.taxonomy.js`와 어깼나면 실패시킨다.
...
1. **성취기준 문장 미대조.** 교육부 고시 제2022-33호 [별책 9] 원문을 보려 했으나 이 컨테이너의 이그레스
   ...
   막는다.** 추측하지 않고 `{code, text:null, verified:false}`로 둥다.
2. **원본 그림 PNG 없음.** `crop_blocks.py`는 PDF를 **디스크에서** 읽는데 `drive.google.com`이 막혔 있고(403),
...
- ※ 폰트 CDN 2건(`cdn.jsdelivr.net`·`fonts.googleapis.com`)은 이 컨테이너의 이그레스 차단이라 집계에서 뻐다.
...
- docssam 표정 9장 반입(GPT 생성, 사용자 G드라이브 `docssam 표정/`): A1 몸 고정 + A2~A5 얼굴 조각(말하기 3·깜빡임 1, SIFT 정렬 오차 <1px), B1~B4 반응. 웹용 webp는 `docssam 표정/web/` 9개(≈560KB). 말하기 = 음절 모음(ㅡㅏㅓ→크게, ㅗㅜ→동그랬야게, 그 밖→반오른켐).
