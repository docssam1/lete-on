...
- 사용자 지적: 땅의 변화 5단계 수업(흙 언덕 물길)이 소단원 1·2만 다루고 화산(3~6)은 개념 없이 문제만 있었다 → **두 번째 5단계 수업 「화산 실험실」** `s41-u03b`.
  ...
  - 3D 장면 `scenes/volcano-erupt.js`: 지형 높이맵 화산, 땅속 단면(반투명 X-ray)에 마그마 방·통로, 분출 = 용암 분수·화산재 기둥·암석 조각·용암류 3줄기·주황 점광, 식어 현무암 / 땅속 화강암. **InstancedMesh는 `frustumCulled=false` 필수**(count 0으로 첫 렌더 되면 바운딩 구가 0이라 영영 안 보임 — 이번에 한참 헤맴).
