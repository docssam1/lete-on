...
export async function mountVolcano3D(el, opts = {}) {
  ...
  const gauze = new THREE.Mesh(new THREE.PlaneGeometry(1.3, 1.3), new THREE.MeshStandardMaterial({ map: gauzeTex, alphaTest: 0.35, alphaToCoverage: true, side: THREE.DoubleSide, roughness: 0.55, metalness: 0.5 }));
  ...
  // 흘러나온 마시멜로: 분화구 가장자리에서 포일 비탈을 타고 내려와 받침에 고이는 두 줄기.
  // 단면이 반원(끈적한 녹은 설탕이 표면에 붙어 흐르는 모양)인 띠를 미리 만들어 두고, 녹은 양만큼 앞에서부터 그린다(drawRange).
  // 뜨거울 땐 윤기 나는 흰색 + 속에서 비치는 주황 빛, 식으면 캐러멜색으로 굳고 광택이 죽는다.
  const FOIL_Y = ringY + 0.02, CONE = (y) => 0.55 - 0.45 * (y / 0.75);    // 포일 원뿔 반지름(높이 y에서)
  const flowM = new THREE.MeshPhysicalMaterial({ color: 0xfff3ea, roughness: 0.22, clearcoat: 1, clearcoatRoughness: 0.12, emissive: 0xff8a4a, emissiveIntensity: 0.3, sheen: 0.4, sheenColor: new THREE.Color(0xffe2cf) });
  const ROWS = 48, COLS = 10;
  const makeFlow = (a0, wob, len) => {
    const pos = new Float32Array(ROWS * (COLS + 1) * 3), idx = [], P = [], N = [];
    const at = (s) => { const onCone = s < 0.78, y = onCone ? 0.73 - (s / 0.78) * 0.71 : 0.015, a = a0 + wob * Math.sin(s * 7.5) + wob * 0.4 * Math.sin(s * 17 + a0);
      const r = onCone ? CONE(y) + 0.012 : CONE(0) + 0.03 + (s - 0.78) / 0.22 * len; return [Math.cos(a) * r, FOIL_Y + y, Math.sin(a) * r, a, onCone]; };
    for (let i = 0; i < ROWS; i++) { const s = i / (ROWS - 1), [x, y, z, a, onCone] = at(s); P.push(new THREE.Vector3(x, y, z));
      N.push(onCone ? new THREE.Vector3(Math.cos(a), 0.6, Math.sin(a)).normalize() : new THREE.Vector3(0, 1, 0)); }
    const T3 = new THREE.Vector3(), B3 = new THREE.Vector3(), N3 = new THREE.Vector3(), v = new THREE.Vector3();
    for (let i = 0; i < ROWS; i++) {
      const s = i / (ROWS - 1); T3.subVectors(P[Math.min(ROWS - 1, i + 1)], P[Math.max(0, i - 1)]).normalize();
      B3.crossVectors(T3, N[i]).normalize(); N3.crossVectors(B3, T3).normalize();
      const w = (0.045 + 0.03 * s + (s > 0.78 ? (s - 0.78) * 0.35 : 0)) * (1 + 0.18 * Math.sin(s * 23 + a0 * 5)), h = 0.028 + 0.012 * Math.sin(s * 11 + a0);
      for (let j = 0; j <= COLS; j++) { const th = (j / COLS) * Math.PI; v.copy(P[i]).addScaledVector(B3, Math.cos(th) * w).addScaledVector(N3, Math.sin(th) * h - 0.004); pos.set([v.x, v.y, v.z], (i * (COLS + 1) + j) * 3); }
    }
    for (let i = 0; i < ROWS - 1; i++) for (let j = 0; j < COLS; j++) { const q = i * (COLS + 1) + j; idx.push(q, q + COLS + 1, q + 1, q + 1, q + COLS + 1, q + COLS + 2); }
    const geo = new THREE.BufferGeometry(); geo.setAttribute('position', new THREE.BufferAttribute(pos, 3)); geo.setIndex(idx); geo.computeVertexNormals(); geo.setDrawRange(0, 0);
    const m = new THREE.Mesh(geo, flowM); m.castShadow = true; m.receiveShadow = true; m.frustumCulled = false;
    const head = new THREE.Mesh(new THREE.SphereGeometry(1, 20, 14), flowM); head.castShadow = true; head.visible = false;
    m.userData.set = (f) => { const rows = Math.round(Math.max(0, Math.min(1, f)) * (ROWS - 1)); geo.setDrawRange(0, rows * COLS * 6); head.visible = rows > 0;
      if (rows > 0) { const i = rows, s = i / (ROWS - 1), w = 0.05 + 0.03 * s; head.position.copy(P[i]).addScaledVector(N[i], 0.006); head.scale.set(w * 1.05, 0.03, w * 1.05); head.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), N[i]); } };
    A.add(m, head); return m;
  };
  const flows = [makeFlow(0.12, 0.09, 0.2), makeFlow(1.5, 0.07, 0.14), makeFlow(-1.35, 0.06, 0.1)];
  // 분화구에서 부풀어 오르는 녹은 마시멜로(뜨거울수록 크게, 숨 쉬듯)
  const dome = new THREE.Mesh(new THREE.SphereGeometry(0.1, 24, 14, 0, Math.PI * 2, 0, Math.PI / 2), flowM); dome.position.y = FOIL_Y + 0.73; dome.visible = false; A.add(dome);
  const cHot = new THREE.Color(0xfff3ea), cSet = new THREE.Color(0xc98f58);
  const setBlobs = (melt, set, t = 0) => {
    flows[0].userData.set(melt * 1.05); flows[1].userData.set((melt - 0.18) * 1.3); flows[2].userData.set((melt - 0.45) * 1.6);
    flowM.color.copy(cHot).lerp(cSet, set); flowM.roughness = 0.22 + 0.4 * set; flowM.clearcoat = 1 - 0.7 * set; flowM.sheen = 0.4 * (1 - set);
    const hot = Math.max(0, Math.min(1, (S.temp - 40) / 120)) * (1 - set);
    flowM.emissiveIntensity = 0.06 + 0.4 * hot * (0.9 + 0.1 * Math.sin(t * 5));
    dome.visible = melt > 0.02 || S.temp > MELT; const sw = Math.min(1, melt * 3 + Math.max(0, S.temp - MELT) / 60);
    dome.scale.set(0.8 + sw * 0.5, (0.3 + sw * 0.9) * (1 + 0.06 * Math.sin(t * 3.1) * (1 - set)), 0.8 + sw * 0.5);
  };
  // 연기(화산 가스): 밝은 배경에서도 보이는 회갈색 뭉게 입자. 올라가며 커지고 흐려진다.
  const smoke = K.puffCloud(K.particleBudget(canvas, 120), { color: 0x857c72, opacity: 0.55, soft: 0.7 }); A.add(smoke);
  // 불꽃 빛무리(더해지는 빛) — 불이 흔들리면 같이 일렁인다
  const halo = K.puffCloud(2, { color: 0xffa24a, opacity: 0.8, additive: true, soft: 1 }); lamp.add(halo);
  const puffs = [];
  ...
  const steam = K.puffCloud(K.particleBudget(canvas, 40), { color: 0xc4cfd9, opacity: 0.45, soft: 0.8 }); beaker.add(steam);
  ...
  const refresh = () => {
    if (A.visible) {
      ...
      setBlobs(S.melt, S.set, performance.now() / 1000); flame.visible = flameCore.visible = S.lit; const fl = S.lit ? (holding ? (heat === '강하게' ? 1.3 : 0.85) : 0.6) : 0; flame.scale.set(fl, fl, fl); flameCore.scale.set(fl, fl, fl); if (!S.lit) flameLight.intensity = 0;
  ...
  const resetAll = () => { Object.assign(S, { temp: 20, melt: 0, set: 0, smoke: 0, lit: false, foilY: 4, temp2: 80, grow: 0, beakerY: 3 }); puffs.length = 0; smoke.userData.commit(0); steam.userData.commit(0); setCrystals(0, cool); holding = false; $main.classList.remove('on'); goStep(0); refresh(); };
  stage.update = (dt, t) => {
    ...
    if (A.visible) {
      ...
      if (S.smoke > 0.05 && puffs.length < smoke.userData.max && Math.random() < S.smoke * 0.9) puffs.push({ a: Math.random() * Math.PI * 2, r: Math.random() * 0.05, y: 0, s: 0.4 + Math.random() * 0.5, v: 0.35 + S.smoke * 0.5, sw: Math.random() * 6 });
      let k = 0; for (let i = puffs.length - 1; i >= 0; i--) { const p = puffs[i]; p.y += p.v * dt; p.r += dt * 0.16; p.v *= 1 - dt * 0.25; if (p.y > 2.2) { puffs.splice(i, 1); continue; }
        const drift = p.y * p.y * 0.12, f = p.y / 2.2;   // 위로 갈수록 옆바람에 기울고 퍼진다
        smoke.userData.set(k++, Math.cos(p.a) * p.r + drift + Math.sin(t * 0.9 + p.sw) * 0.04 * p.y, FOIL_Y + 0.78 + p.y, Math.sin(p.a) * p.r - drift * 0.4, p.s * (0.22 + p.y * 0.5), Math.min(1, p.y * 6) * (1 - f) * (1 - f) * (0.5 + 0.5 * S.smoke)); }
      smoke.userData.commit(k);
      if (S.lit) { const fl = (holding ? (heat === '강하게' ? 1.3 : 0.9) : 0.6) * (0.9 + 0.12 * Math.sin(t * 17) + 0.06 * Math.sin(t * 29)); halo.userData.set(0, 0, 0.82, 0, 0.55 * fl, 0.55); halo.userData.set(1, 0, 0.72, 0, 1.1 * fl, 0.16); halo.userData.commit(2); flameLight.intensity = (holding ? 4 : 1.5) * (0.85 + 0.2 * Math.sin(t * 23)); }
      else halo.userData.commit(0);
      ...
      let k = 0; if (S.temp2 > 45 && S.beakerY <= 0) { const hot = Math.min(1, (S.temp2 - 45) / 30); for (let i = 0; i < steam.userData.max; i++) { const life = ((t * 0.45 + hash(i)) % 1);
        steam.userData.set(k++, (hash(i + 40) - 0.5) * 0.4 + Math.sin(t + i) * 0.03 * life, 0.62 + life * 0.8, (hash(i + 80) - 0.5) * 0.4, 0.1 + life * 0.28, Math.sin(Math.PI * life) * hot); } }
      steam.userData.commit(k);
