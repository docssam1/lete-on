...
export function renderChapter(ch, art, similar, { teacher = false, live = false, media = null } = {}) {
  ...
  const page = (body, { cls = '', band = '', full = false, say = '' } = {}) => {
    const n = ++pageNo, side = n % 2 ? 'odd' : 'even';
    // 독립 읽을거리 쪽을 끼워도 기존 쪽의 용어·답 노트가 밀리지 않는다.
    const notes = full ? '' : rail(++noteNo);
    const html = `<section class="bk-page ${side} ${cls}"${say ? ` data-say="${say}"` : ''}>
  ...
  out.push(page(`
    ...
    <div class="bk-road"><div class="bk-road-steps">${['가설', '설계', '실험', '결과·결론', '개념 정리', '창의·토의', '영재성', '확인 문제'].map((t, i) => `<span><b>${i + 1}</b>${t}</span>`).join('')}</div>${qr(ch.qr.scene, '3D로 먼저 보기', 'scene')}</div>`, { cls: 'first', band: `
      <div class="bk-open"><div class="bk-no">${String(ch.no).padStart(2, '0')}</div><div><p class="bk-kicker"><span class="bk-exp">EXPERIMENT ${String(ch.no).padStart(2, '0')}</span>${esc(ch.link.topics[0])}</p><h2>${esc(ch.title)}</h2></div>
      <div class="bk-vol">${esc(ch.book)}<b>${esc(ch.vol)}</b></div></div>`, say: 'cover' }));
  ...
  out.push(page(`${banner('탐구 설계하기', 'design')}
    ...
    ${step(2, '실험 설계하기')}
    <table class="bk-tbl design"><tbody>${[d.change, d.same, d.measure].map((r) => `<tr><th>${esc(r.q)}</th><td>${teacher ? `<span class="bk-ans">${esc(r.a)}</span>` : ''}</td></tr>`).join('')}</tbody></table>`, { say: 'design' }));
  ...
  out.push(page(`${step(4, '결과 기록하기')}<ol class="bk-ol">${res}</ol>
    ${step(5, '결론 내리기')}<ol class="bk-ol">${ch.conclusion.map((c) => `<li>${esc(c.q)}${ans(c.a, 2)}</li>`).join('')}</ol>`, { say: 'result' }));
  ...
  out.push(page(`${banner('영재성 기르기', 'gift')}
    ...
    <div class="bk-box self"><h4>나의 탐구 돌아보기</h4><table class="bk-tbl chk"><tbody>${self.map((s) => `<tr><td>${esc(s)}</td><td class="st">☆ ☆ ☆</td></tr>`).join('')}</tbody></table></div>`, { say: 'gifted' }));
  ...
  if (ch.report) {
    ...
    out.push(page(`${banner('탐구보고서', 'report')}
      ...
      <div class="bk-box self tight"><h4>스스로 점검</h4><table class="bk-tbl chk"><tbody>${R.checks.map((c) => `<tr><td>${esc(c)}</td><td class="st">☆ ☆ ☆</td></tr>`).join('')}</tbody></table></div>`, { say: 'report' }));
  }
  ...
  if (ch.formative) {
    ...
    out.push(page(`${banner('형성평가', 'check')}<p class="bk-hint">수업을 마치며 5분 동안 풀어요. 맞은 개수로 다음 공부를 정해요.</p>
      ...
      <div class="bk-box self"><h4>성취기준 확인</h4><table class="bk-tbl chk"><tbody>${F.standards.map((c) => `<tr><td>${esc(c)}</td><td class="st">○ △ ✕</td></tr>`).join('')}</tbody></table>
        <p class="bk-hint">○ 잘 안다 · △ 조금 더 · ✕ 다시 배우기 — △·✕는 개념 정리 쪽으로 돌아가 다시 읽어요.</p></div>`, { cls: 'check', say: 'formative' }));
  }
