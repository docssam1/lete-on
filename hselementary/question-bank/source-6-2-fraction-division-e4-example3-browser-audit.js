...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e4-example3-browser-audit");
const sourceItemId = "6-2-u1-e4-example-3";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["13:1:8:5:23:3", { burnPerHour: "10:3", remainingTime: "23:10", minutes: "138", answer: "2시간 18분" }],
  ["12:1:3:2:15:2", { burnPerHour: "3:1", remainingTime: "5:2", minutes: "150", answer: "2시간 30분" }],
  ["14:1:9:4:67:8", { burnPerHour: "5:2", remainingTime: "67:20", minutes: "201", answer: "3시간 21분" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 10000) fail(`${label}: ${name}가 비었거나 양초 길이·시간 계산이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-candle-time-board.is-solved,#problemView .source62-candle-time-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-candle-time-board");
        ...
        const evidence = item.querySelector("[data-source62-fraction-e4-example3-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E4Example3Structure || "",
          values: board?.dataset.source62E4Example3Values || "",
          burnPerHour: board?.dataset.burnPerHour || "",
          remainingTime: board?.dataset.remainingTime || "",
          minutes: board?.dataset.answerMinutes || "",
          rowCount: item.querySelectorAll(".source61-math-row").length,
          solvedCount: item.querySelectorAll(".source62-candle-time-board.is-solved").length,
          solutionCount: item.querySelectorAll(".source62-candle-time-solution").length,
          ...
          kind: evidence?.dataset.source62FractionE4Example3Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e4-example3-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedRows) fail(`${label}/${index + 1}: 양초 길이·시간 계산판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "constant-candle-burn" || !expectedItem || item.burnPerHour !== expectedItem?.burnPerHour || item.remainingTime !== expectedItem?.remainingTime || item.minutes !== expectedItem?.minutes) fail(`${label}/${index + 1}: 문제와 답의 양초 길이·시간 자료가 다릅니다.`);
    if (item.fractionCount < 4 || item.thinBars || item.expressionCount < 6 || item.brokenExpressions || item.unitCount < 6 || !item.text.includes("시간")) fail(`${label}/${index + 1}: 세로 분수·시간 단위 또는 줄바꿈되지 않는 수식 묶음이 빠졌습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "constant-candle-burn" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.rowCount !== 10 || item.answerSource !== sourceItemId || item.fractionCount < 12 || item.unitCount < 10 || !item.text.includes(expectedItem.answer)) fail(`${label}/${index + 1}: 답의 탄 길이·1시간에 타는 길이·남은 시간·전체 시간 계산이 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.rowCount !== 3 || item.answerSource || state.answerLeak || item.text.includes(expectedItem.answer) || !item.text.includes("일정한 빠르기")) fail(`${label}/${index + 1}: 문제 조건이 빠졌거나 풀이·답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.minutes !== answer.items[index]?.minutes) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 양초 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 4-3 원문 양초·세로 분수·시간 단위·독립 계산·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 4-3 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
