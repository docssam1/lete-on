...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(os.tmpdir(), "source-6-1-ratio-e4-browser-audit");
const sourceIds = [
...
async function inspect(page, generated, sourceId, difficulty, view, viewportName) {
  ...
  const state = await page.evaluate(() => {
    ...
    const diagrams = [...document.querySelectorAll("svg.source61-ratio-e4-diagram")];
    ...
    return {
      ...
      marker: document.querySelector("[data-source61-ratio-e4-kind]")?.dataset.sourceItem || ""
    };
  });
...
async function captureA4(page, filename, label, view) {
  ...
  const printState = await page.evaluate(expectedView => {
    ...
    items.forEach((item, index) => {
      ...
      item.querySelectorAll("svg.source61-ratio-e4-diagram").forEach(svg => {
...
async function inspectActualRoute(page, sourceId, viewportName, view) {
  const state = await page.evaluate(expected => {
    ...
    const diagrams = items.flatMap(item => [...item.querySelectorAll("svg.source61-ratio-e4-diagram")]);
    ...
    const diagramBoxes = diagrams.map(svg => {
      ...
      return {
        ...
        height: box.height,
        bbox,
        structure: svg.dataset.source61RatioE4Structure || "",
        values: svg.dataset.source61RatioE4Values || "",
    ...
    const answerNodes = items.flatMap(item => [...item.querySelectorAll(".source61-ratio-e4-answer")]);
    return {
  ...
  if (view === "answer" && sourceId === sourceIds[1] && state.answerMathFractionCount !== 3) fail(`${label}: 대분수 정답 3개가 공통 세로 분수로 표시되지 않았습니다.`);
...
(async () => {
  ...
  try {
    ...
    for (let variant = 0; variant < sourceIds.length; variant += 1) for (const difficulty of difficulties) {
      const generated = api.generate({ sourceItemId: sourceIds[variant], generatorKey: "sourceGrade6RatioE4", reviewLocked: false }, 0, difficulty, 614000 + variant * 10000 + (difficulty + 1) * 100 + 17, variant);
  ...
  if (screenshots !== 192 || pdfs !== 24 || expectedPdfPages < pdfs || renderedPdfPages !== expectedPdfPages) fail(`산출물 수 ${screenshots}/${pdfs}/${renderedPdfPages}/${expectedPdfPages}, 화면 192장·PDF 24개·PDF 전쪽 렌더여야 합니다.`);
  const summary = `${failures.length ? "실패" : "통과"}: 실제 주소 12유형×PC1440/mobile390 문제·답, 실제 주소 A4 문제·답, 12유형×3난이도 직접 생성 문제·답, 문제·답 SVG 자료 일치·정답 표시·겹침·글꼴·넘침·A4 전쪽 렌더 검사; 화면 ${screenshots}장, A4 ${pdfs}개, 렌더 ${renderedPdfPages}/${expectedPdfPages}쪽\n${failures.join("\n")}\n`;
