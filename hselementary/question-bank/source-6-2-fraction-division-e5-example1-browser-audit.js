...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e5-example1-browser-audit");
const sourceItemId = "6-2-u1-e5-example-1";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["13:19:24", { totalParts: 32, onePart: "3:4", dayMinutes: 585, nightMinutes: 855, answer: "14시간 15분" }],
  ["7:9:24", { totalParts: 16, onePart: "3:2", dayMinutes: 630, nightMinutes: 810, answer: "13시간 30분" }],
  ["11:13:24", { totalParts: 24, onePart: "1:1", dayMinutes: 660, nightMinutes: 780, answer: "13시간" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 15000 || inkPixels(file) < 5000) fail(`${label}: ${name}가 비었거나 낮·밤 관계가 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-day-night-board.is-solved,#problemView .source62-day-night-solution,#problemView .source62-day-night-strip").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-day-night-board");
        const boardBox = board?.getBoundingClientRect();
        const panels = [...item.querySelectorAll(".source62-day-night-board,.source62-day-night-solution")];
        const fractions = [...item.querySelectorAll(".math-fraction")];
        const evidence = item.querySelector("[data-source62-fraction-e5-example1-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E5Example1Structure || "", values: board?.dataset.source62E5Example1Values || "", totalParts: Number(board?.dataset.totalParts || 0), onePart: board?.dataset.onePartHours || "", dayMinutes: Number(board?.dataset.dayMinutes || 0), nightMinutes: Number(board?.dataset.nightMinutes || 0),
          rowCount: item.querySelectorAll(".source61-math-row").length, solvedCount: item.querySelectorAll(".source62-day-night-board.is-solved").length, solutionCount: item.querySelectorAll(".source62-day-night-solution").length, stripCount: item.querySelectorAll(".source62-day-night-strip").length,
          fractionCount: fractions.length, fractionOverlap: fractions.filter(fraction => { const top = fraction.firstElementChild?.getBoundingClientRect(); const bottom = fraction.lastElementChild?.getBoundingClientRect(); return !top || !bottom || top.bottom > bottom.top + 0.5; }).length,
          source: evidence?.dataset.sourceItem || "", kind: evidence?.dataset.source62FractionE5Example1Kind || "", candidateCount: Number(evidence?.dataset.candidateCount || 0), difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e5-example1-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 260 || item.panelOverflow) fail(`${label}/${index + 1}: 낮·밤 관계표가 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "day-night-fraction-of-whole-day" || !expectedItem || item.totalParts !== expectedItem?.totalParts || item.onePart !== expectedItem?.onePart || item.dayMinutes !== expectedItem?.dayMinutes || item.nightMinutes !== expectedItem?.nightMinutes) fail(`${label}/${index + 1}: 문제와 답의 낮·밤 비 또는 시간이 다릅니다.`);
    if (item.fractionCount < 1 || item.fractionOverlap || !item.fontsValid) fail(`${label}/${index + 1}: 세로 분수 또는 공통 글꼴이 빠졌습니다.`);
    if (item.source !== sourceItemId || item.kind !== "night-duration-from-day-fraction" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.stripCount !== 1 || item.rowCount !== 10 || item.answerSource !== sourceItemId || !item.text.includes(expectedItem.answer) || !item.text.includes("24시간")) fail(`${label}/${index + 1}: 답의 낮·밤 띠와 독립 계산표가 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.stripCount || item.rowCount !== 2 || item.answerSource || state.answerLeak || !item.text.includes("하루") || !item.text.includes("밤의 길이")) fail(`${label}/${index + 1}: 원문 조건이 빠졌거나 풀이·답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.dayMinutes !== answer.items[index]?.dayMinutes || problem.items[index]?.nightMinutes !== answer.items[index]?.nightMinutes) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 낮·밤 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 5-1 원문 밤 질문·세로 분수·단일 답·문제/답 자료 일치·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 5-1 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
