...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-example3-browser-audit");
const sourceItemId = "6-2-u1-e2-example-3";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["280", "350", "380"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      items: items.map(item => {
        ...
        const evidence = item.querySelector("[data-source62-fraction-e2-example3-kind]");
        ...
        const expressions = values.length === 14 ? `${values[0]}:${values[1]}:${values[2]}:${values[3]}:${values[4]}:${values[5]}` : "";
        return {
          ...
          clippedDescendants,
          structure: board?.dataset.source62E2Example3Structure || "",
          expression: board?.dataset.source62E2Example3Expression || "",
          ...
          kind: evidence?.dataset.source62FractionE2Example3Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e2-example3-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (item.structure !== "two-weighings-after-drinking" || !item.expression || item.expression !== item.expressions || !item.fillPart || !item.drinkPart || !item.firstWaterMass || !item.emptyBottleMass) fail(`${label}/${index + 1}: 같은 물병의 두 무게 자료 모델이 빠졌습니다.`);
    if (item.bottleStates !== 2 || item.waterFills !== 2 || item.waterLevels.some(level => !Number.isFinite(level) || level < 4) || item.waterLevels[0] <= item.waterLevels[1] || item.fractionCount < 2) fail(`${label}/${index + 1}: 처음과 마신 뒤의 물 높이 또는 분수 표시가 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 물병 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 2-3 같은 물병·두 물 높이·빈 병 무게·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 2-3 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
