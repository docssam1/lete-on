...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-mission2-browser-audit");
const sourceItemId = "6-2-u1-e2-mission-2";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["380", "450", "480"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-bottle-weight-solution,#problemView .source62-bottle-weight-board.is-solved").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-bottle-weight-board");
        const evidence = item.querySelector("[data-source62-fraction-e2-mission2-kind]");
        const flow = item.querySelector(".source62-bottle-weight-flow");
        const boardBox = board?.getBoundingClientRect();
        const clippedDescendants = boardBox ? [...board.querySelectorAll(":scope .source62-bottle-weight-flow,:scope .source62-bottle-state,:scope .source62-bottle-drink,:scope .source62-bottle,:scope > p")].filter(element => {
          const box = element.getBoundingClientRect();
          return box.left < boardBox.left - 1 || box.right > boardBox.right + 1;
        }).length : 1;
        const values = (evidence?.dataset.values || "").split(",").map(Number);
        const waterLevels = [...item.querySelectorAll(".source62-bottle-water")].map(water => parseFloat(getComputedStyle(water).height));
        const expression = values.length === 14 ? `${values[0]}:${values[1]}:${values[2]}:${values[3]}:${values[4]}:${values[5]}` : "";
        return {
          ...
          clippedDescendants,
          structure: board?.dataset.source62E2Mission2Structure || "",
          boardExpression: board?.dataset.source62E2Mission2Expression || "",
          fillPart: board?.dataset.fillPart || "",
          drinkPart: board?.dataset.drinkPart || "",
          firstWaterMass: board?.dataset.firstWaterMass || "",
          emptyBottleMass: board?.dataset.emptyBottleMass || "",
          expression,
          bottleStates: item.querySelectorAll(".source62-bottle-state").length,
          arrows: item.querySelectorAll(".source62-bottle-drink > span").length,
          waterFills: waterLevels.length,
          waterLevels,
          waterMassLabels: item.querySelectorAll(".source62-bottle-state small").length,
          solvedCount: item.querySelectorAll(".source62-bottle-weight-board.is-solved").length,
          solutionRows: item.querySelectorAll(".source62-bottle-weight-solution .source61-math-row").length,
          ...
          kind: evidence?.dataset.source62FractionE2Mission2Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e2-mission2-answer")?.dataset.answerSource || "",
          answerValue: values.length === 14 && values[13] === 1 ? String(values[12]) : "",
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.flowOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 문항이나 물병 자료판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "two-weighings-after-drinking" || !item.boardExpression || item.boardExpression !== item.expression || !item.fillPart || !item.drinkPart || !item.firstWaterMass || !item.emptyBottleMass) fail(`${label}/${index + 1}: 같은 물병의 두 무게 자료 모델이 빠졌습니다.`);
    if (item.bottleStates !== 2 || item.arrows !== 1 || item.waterFills !== 2 || item.waterLevels.some(level => !Number.isFinite(level) || level < 4) || item.waterLevels[0] <= item.waterLevels[1] || item.fractionCount < 2) fail(`${label}/${index + 1}: 처음과 마신 뒤의 물 높이·화살표 또는 분수 표시가 다릅니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 물병 자료판의 공통 글꼴이 아닙니다.`);
    if (item.source !== sourceItemId || item.kind !== "two-weighings-after-drinking" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 후보·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.waterMassLabels !== 2 || item.solvedCount !== 1 || item.solutionRows !== 3 || item.answerSource !== sourceItemId || !expectedAnswers.has(item.answerValue)) fail(`${label}/${index + 1}: 답 자료판의 물 무게와 세 계산 단계가 다릅니다.`);
    } else if (item.waterMassLabels || item.solvedCount || item.solutionRows || state.answerLeak) fail(`${label}/${index + 1}: 문제에 물 무게나 풀이가 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.boardExpression !== answer.items[index]?.boardExpression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 물병 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 2 같은 물병·두 물 높이·빈 병 무게·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 Mission 2 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
