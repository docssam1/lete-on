...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(os.tmpdir(), "source-6-1-ratio-e2-browser-audit");
const sourceIds = [
...
async function inspect(page, generated, sourceId, difficulty, view, viewportName) {
  ...
  const state = await page.evaluate(() => {
    ...
    const diagrams = [...document.querySelectorAll("svg.source61-ratio-e2-diagram")];
    ...
    return {
      ...
      marker: document.querySelector("[data-source61-ratio-e2-kind]")?.dataset.sourceItem || ""
    };
  });
...
async function captureA4(page, filename, label, view) {
  ...
  const printState = await page.evaluate(expectedView => {
    ...
    items.forEach((item, index) => {
      ...
      item.querySelectorAll("svg.source61-ratio-e2-diagram").forEach(svg => {
...
async function inspectActualRoute(page, sourceId, viewportName, view) {
  const state = await page.evaluate(expected => {
    ...
    const diagrams = items.flatMap(item => [...item.querySelectorAll("svg.source61-ratio-e2-diagram")]);
    ...
    const diagramBoxes = diagrams.map(svg => {
      ...
      return {
        ...
        height: box.height,
        bbox,
        structure: svg.dataset.source61RatioE2Structure || "",
        values: svg.dataset.source61RatioE2Values || "",
    ...
    const answerNodes = items.flatMap(item => [...item.querySelectorAll(".source61-ratio-e2-answer")]);
    return {
...
(async () => {
  ...
  try {
    ...
    for (let variant = 0; variant < sourceIds.length; variant += 1) for (const difficulty of difficulties) {
      const generated = api.generate({ sourceItemId: sourceIds[variant], generatorKey: "sourceGrade6RatioE2", reviewLocked: false }, 0, difficulty, 612000 + variant * 10000 + (difficulty + 1) * 100 + 17, variant);
