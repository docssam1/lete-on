...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e3-mission2-browser-audit");
const sourceItemId = "6-2-u1-e3-mission-2";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["2:17:15", { answer: "111/64", count: 8, sum: 64 }],
  ["3:19:17", { answer: "224/81", count: 9, sum: 81 }],
  ["2:23:19", { answer: "177/100", count: 10, sum: 100 }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      items: items.map(item => {
        const board = item.querySelector(".source62-odd-sum-board");
        const equation = item.querySelector(".source62-odd-sum-equation");
        ...
        const outerBar = outer?.querySelector(":scope > .source62-series-reciprocal__top");
        const evidence = item.querySelector("[data-source62-fraction-e3-mission2-kind]");
        ...
        return {
          ...
          outerWithinEquation: equationBox && outerBox ? outerBox.left >= equationBox.left - 1 && outerBox.right <= equationBox.right + 1 && outerBox.top >= equationBox.top - 1 && outerBox.bottom <= equationBox.bottom + 1 : false,
          outerBarWidth: outerBar?.getBoundingClientRect().width || 0, outerBarThickness: outerBar ? getComputedStyle(outerBar).borderBottomWidth : "",
          structure: board?.dataset.source62E3Mission2Structure || "", expressionSignature: board?.dataset.source62E3Mission2Expression || "", termCount: Number(board?.dataset.termCount || 0), oddSum: Number(board?.dataset.oddSum || 0), boardAnswer: board?.dataset.answerFraction || "",
          smallFractions: board?.querySelectorAll(".source62-series-reciprocal__bottom .math-fraction").length || 0, dots: board?.querySelectorAll(".source62-odd-sum-dots").length || 0,
          solvedCount: item.querySelectorAll(".source62-odd-sum-board.is-solved").length, solutionRows: item.querySelectorAll(".source62-odd-sum-solution .source61-math-row").length,
          source: evidence?.dataset.sourceItem || "", kind: evidence?.dataset.source62FractionE3Mission2Kind || "", evidenceExpression: evidence?.dataset.expression || "", evidenceAnswer: evidence?.dataset.answer || "", candidateCount: Number(evidence?.dataset.candidateCount || 0), difficulty: evidence?.dataset.difficultyDesign || "", answerSource: item.querySelector(".source62-e3-mission2-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    const expectedItem = expected.get(item.expressionSignature);
    if (!item.itemVisible || !item.boardVisible || !item.equationVisible || !item.outerVisible || item.boardOutside || item.equationOutside || item.boardWidth < 280 || item.boardOverflow || item.equationOverflow || !item.outerWithinEquation) fail(`${label}/${index + 1}: 홀수 분수 합이 들어 있는 겹분수가 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "reciprocal-of-odd-fraction-sum" || !expectedItem || item.boardAnswer !== expectedItem.answer || item.termCount !== expectedItem.count || item.oddSum !== expectedItem.sum || item.evidenceExpression !== item.expressionSignature || item.evidenceAnswer !== item.boardAnswer) fail(`${label}/${index + 1}: 문제와 답의 홀수 항·합·계산 자료가 다릅니다.`);
    if (item.smallFractions !== 4 || item.dots !== 1 || item.outerBarWidth < 150 || item.outerBarThickness !== "2px") fail(`${label}/${index + 1}: 작은 분수 네 개·줄임표·큰 분수선이 정확하지 않습니다.`);
    if (item.source !== sourceItemId || item.kind !== "reciprocal-of-odd-fraction-sum" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid || !item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 원문·단일 답·난이도·공통 글꼴 또는 분수 표시가 다릅니다.`);
    if (answerView) {
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expressionSignature !== answer.items[index]?.expressionSignature || problem.items[index]?.boardAnswer !== answer.items[index]?.boardAnswer || problem.items[index]?.oddSum !== answer.items[index]?.oddSum) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 2 홀수 분수 합·큰 분수선·단일 답·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
