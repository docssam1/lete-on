...
check(readiness.integrity.publicCandidateCount === readiness.items.filter(item => item.publicDecision === "public").length && readiness.integrity.lockedCount === readiness.items.filter(item => item.publicDecision === "locked").length && readiness.integrity.allImplementationLocked === false, "6-2 1단원 검토표의 공개·잠금 요약이 다릅니다.");
