...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-example3-browser-audit");
const sourceItemId = "6-2-u1-e1-example-3";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["32/21", "13/12", "95/66"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspectView(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ isAnswer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: isAnswer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-sequence-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-sequence-board");
        const solution = item.querySelector(".source62-sequence-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-example3-kind]");
        ...
        return {
          ...
          sequence: board?.dataset.source62E1Example3Sequence || "",
          structure: board?.dataset.source62E1Example3Structure || "",
          ...
          kind: evidence?.dataset.source62FractionE1Example3Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-example3-answer")?.dataset.answerSource || "",
          mathFractions: item.querySelectorAll(".math-fraction").length,
          keepsTwoFourths: Boolean(item.querySelector('[aria-label="4분의 2"]')),
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || item.itemClipped || !item.boardVisible || item.boardOutside || item.boardClipped || item.boardWidth < 250) fail(`${label} ${index + 1}: 문항이나 수열이 비었거나 화면 밖·잘림 상태입니다.`);
    if (item.structure !== "growing-denominator-groups" || item.sequence !== "1/1:1/2:2/2:1/3:2/3:3/3:1/4:2/4:3/4:4/4" || !item.keepsTwoFourths) fail(`${label} ${index + 1}: 원본 수열의 항과 순서가 다릅니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "growing-denominator-sequence" || item.difficulty !== difficultyNames[String(difficulty)]) fail(`${label} ${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (item.rawFraction || item.invalid || item.mathFractions < (answerView ? 15 : 6)) fail(`${label} ${index + 1}: 가로 분수·깨진 값이 있거나 세로 분수 표시가 부족합니다.`);
    if (answerView) {
      const values = item.values.split(",").map(Number);
      if (!item.solutionVisible || item.solutionOutside || item.solutionClipped || item.solutionRows !== 3 || item.answerSource !== sourceItemId || !expectedAnswers.has(`${values[8]}/${values[9]}`)) fail(`${label} ${index + 1}: 답 계산표의 배치·원문 연결·계산 결과가 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.sequence !== answer.items[index]?.sequence) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 수열 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 1-3 3난이도 PC/모바일 문제·답, 수열·세로 분수·넘침·답 누출 검사; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 1-3 브라우저·인쇄 감사 통과: 3난이도×PC/모바일×문제/답 · 화면 12장 · A4 2개 전 ${renderedPages}쪽`);
})().catch(error => {
