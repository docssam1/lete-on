...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-mission4-browser-audit");
const sourceItemId = "6-2-u1-e1-mission-4";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["4/5", "3/4", "5/6"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ answer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-ratio-comparison-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-ratio-comparison-board");
        const operations = [...item.querySelectorAll(".source62-ratio-operation")];
        const solution = item.querySelector(".source62-ratio-comparison-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-mission4-kind]");
        return {
          ...
          structure: board?.dataset.source62E1Mission4Structure || "",
          expression: board?.dataset.source62E1Mission4Expression || "",
          operations: operations.length,
          operationOverflow: operations.some(operation => operation.scrollWidth > operation.clientWidth + 1),
          symbols: operations.map(operation => operation.querySelector(".source62-ratio-symbol")?.textContent.trim()).join(""),
          mixed: item.querySelectorAll(".math-mixed-number").length,
          ...
          kind: evidence?.dataset.source62FractionE1Mission4Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-mission4-answer")?.dataset.answerSource || "",
          ratioQuestion: item.querySelector(".source62-ratio-question")?.innerText || "",
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 250 || item.operationOverflow) fail(`${label}/${index + 1}: 문항이나 ■·▲ 계산판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "two-mixed-number-divisions-and-ratio" || !item.expression || item.operations !== 2 || item.symbols !== "■▲" || item.mixed !== 4) fail(`${label}/${index + 1}: 원본의 두 대분수 나눗셈 구조가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "two-division-results-ratio" || item.difficulty !== difficultyNames[String(difficulty)]) fail(`${label}/${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (!item.ratioQuestion.includes("■는 ▲의 몇 배") || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 물음이나 분수 표시가 깨졌습니다.`);
    if (answerView) {
      const values = item.values.split(",").map(Number);
      const answer = `${values[12]}/${values[13]}`;
      if (!item.solutionVisible || item.solutionOutside || item.solutionRows !== 3 || item.answerSource !== state.sourceId || !expectedAnswers.has(answer) || !item.ratioQuestion.includes("■÷▲")) fail(`${label}/${index + 1}: 답 풀이표나 최종 몇 배 값이 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 ■·▲ 수가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 4 ■·▲·대분수·몇 배 순서·넘침·답 누출; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
