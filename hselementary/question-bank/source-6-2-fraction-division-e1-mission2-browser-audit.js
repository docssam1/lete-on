...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-mission2-browser-audit");
const sourceItemId = "6-2-u1-e1-mission-2";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["256/1", "81/1", "729/1"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspectView(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ isAnswer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: isAnswer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-defined-operation-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-defined-operation-board");
        const rule = board?.querySelector(".source62-defined-operation-rule");
        const target = board?.querySelector(".source62-defined-operation-target");
        const solution = item.querySelector(".source62-defined-operation-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-mission2-kind]");
        ...
        return {
          ...
          expression: board?.dataset.source62E1Mission2Expression || "",
          structure: board?.dataset.source62E1Mission2Structure || "",
          ruleParts: rule?.children.length || 0,
          symbolTokens: rule?.querySelectorAll(".source62-symbol-token").length || 0,
          targetParts: target?.children.length || 0,
          ruleOverflow: rule ? rule.scrollWidth > rule.clientWidth + 1 : true,
          targetOverflow: target ? target.scrollWidth > target.clientWidth + 1 : true,
          ...
          kind: evidence?.dataset.source62FractionE1Mission2Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-mission2-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || item.itemClipped || !item.boardVisible || item.boardOutside || item.boardClipped || item.boardWidth < 250 || item.ruleOverflow || item.targetOverflow) fail(`${label} ${index + 1}: 문항이나 ◆ 규칙판이 비었거나 화면 밖·잘림 상태입니다.`);
    if (item.structure !== "defined-operation-two-divisions" || !item.expression || item.ruleParts !== 5 || item.symbolTokens !== 2 || item.targetParts !== 3) fail(`${label} ${index + 1}: 원본의 ◆ 규칙 또는 계산할 두 수가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "defined-operation-two-divisions" || item.difficulty !== difficultyNames[String(difficulty)]) fail(`${label} ${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (item.rawFraction || item.invalid || item.mathFractions < (answerView ? 5 : 2)) fail(`${label} ${index + 1}: 가로 분수·깨진 값이 있거나 세로 분수 표시가 부족합니다.`);
    if (answerView) {
      const values = item.values.split(",").map(Number);
      const answerSignature = `${values[8]}/${values[9]}`;
      if (!item.solutionVisible || item.solutionOutside || item.solutionClipped || item.solutionRows !== 3 || item.answerSource !== state.sourceId || !expectedAnswers.has(answerSignature)) fail(`${label} ${index + 1}: 답 풀이표의 배치·원문 연결·계산 결과가 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 ◆ 규칙 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 2 3난이도 PC/모바일 문제·답, ◆ 규칙·세로 분수·넘침·답 누출 검사; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 Mission 2 브라우저·인쇄 감사 통과: 3난이도×PC/모바일×문제/답 · 화면 12장 · A4 2개 전 ${renderedPages}쪽`);
})().catch(error => {
