...
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-example-3")?.independentAnswer === "14m", "흙을 옮긴 뒤의 독립 높이가 14m가 아닙니다.");
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-example-3")?.candidateAnswerCount === 1 && items.find(item => item.sourceItemId === "6-1-u6-e2-example-3")?.singleAnswer === true, "흙 고르기 유형의 단일 정답 검증이 없습니다.");
const bundledRopeCandidates = [];
for (let width = 1; width <= 40; width += 1) for (let depth = 1; depth <= 40; depth += 1) for (let height = 1; height <= 40; height += 1) {
  if (2 * (depth + height) === 28 && 2 * (width + height) === 14 && 2 * (width + height) + 2 * (width + depth) === 40) bundledRopeCandidates.push([width, depth, height]);
}
assert(JSON.stringify(bundledRopeCandidates) === JSON.stringify([[3, 10, 4]]), `두 고리를 함께 묶은 끈 조건의 자연수 해가 하나가 아닙니다: ${JSON.stringify(bundledRopeCandidates)}`);
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-example-4")?.independentAnswer === "120cm³", "세 그림의 끈 길이로 구한 독립 답이 120cm³가 아닙니다.");
const mission4WholeVolume = 25 * 11 * 1;
const mission4CubeSide = 11 - 6;
const mission4RemainingVolume = mission4WholeVolume - mission4CubeSide ** 3;
assert(mission4WholeVolume === 275 && mission4CubeSide === 5 && mission4RemainingVolume === 150 && mission4RemainingVolume * 2 === 10 * 6 * 5, "흙을 평평하게 편 Mission 4의 대분수 높이 계산이 맞지 않습니다.");
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-mission-4")?.independentAnswer === "2 1/2cm", "흙을 평평하게 편 Mission 4의 독립 답이 2와 2분의 1cm가 아닙니다.");
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-mission-4")?.candidateAnswerCount === 1 && items.find(item => item.sourceItemId === "6-1-u6-e2-mission-4")?.singleAnswer === true, "흙 부피 보존 Mission 4의 단일 정답 검증이 없습니다.");
...
for (let width = 1; width < 26; width += 1) for (let height = 1; height < 26 - width; height += 1) {
  const depth = 26 - width - height;
  if (2 * (depth + height) === 36 && 2 * (width + height) === 28) threeRopeCandidates.push([width, height, depth]);
}
assert(JSON.stringify(threeRopeCandidates) === JSON.stringify([[8, 6, 12]]), `세 끈 조건의 자연수 해가 하나가 아닙니다: ${JSON.stringify(threeRopeCandidates)}`);
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-mission-5")?.independentAnswer === "576cm³", "세 끈 길이로 구한 독립 답이 576cm³가 아닙니다.");
const wrappedPaperCandidates = [];
for (let edge = 1; edge <= 100; edge += 1) if (4 * edge * (edge + 3 * edge) / 2 === 200) wrappedPaperCandidates.push(edge);
assert(JSON.stringify(wrappedPaperCandidates) === JSON.stringify([5]), `삼각형 종이 넓이 조건의 자연수 모서리가 하나가 아닙니다: ${JSON.stringify(wrappedPaperCandidates)}`);
assert(items.find(item => item.sourceItemId === "6-1-u6-e2-mission-6")?.independentAnswer === "1500cm³", "삼각형 종이 넓이로 구한 직육면체 부피가 1500cm³가 아닙니다.");
