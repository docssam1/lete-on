...
const compactMathText = text => String(text || "").replace(/\s+/g, "").replace(/㎠|cm2/g, "cm²");
  ...
  const state = await page.evaluate(({ sourceId, kind, isAnswer }) => {
    ...
    const svgState = svg => {
      ...
      return {
        ...
        netBase: svg?.querySelectorAll('[data-net-face="base"]').length || 0,
        netLateral: svg?.querySelectorAll('[data-net-face="lateral"]').length || 0,
        foldEdges: svg?.querySelectorAll("[data-fold-edge]").length || 0,
        ...
        netArea: svg?.dataset.netAreaCm2 || "",
        netLateralFill: svg?.querySelector(".source61-e4-net-lateral") ? getComputedStyle(svg.querySelector(".source61-e4-net-lateral")).fill : "",
        netBaseFill: svg?.querySelector(".source61-e4-net-base") ? getComputedStyle(svg.querySelector(".source61-e4-net-base")).fill : "",
        complementFill: svg?.querySelector(".source61-e4-complement") ? getComputedStyle(svg.querySelector(".source61-e4-complement")).fill : "",
        foldDash: svg?.querySelector(".source61-e4-fold-edge") ? getComputedStyle(svg.querySelector(".source61-e4-fold-edge")).strokeDasharray : "",
        diagonalDash: svg?.querySelector(".source61-e4-base-diagonal") ? getComputedStyle(svg.querySelector(".source61-e4-base-diagonal")).strokeDasharray : "",
        edgeCrossings,
        font,
        overlaps,
...
function checkTopology(source, problem, answer, label) {
  ...
  for (const [index, item] of answer.answerVisuals.entries()) {
    ...
    if (source.kind === "cube-six-pyramid-assembly") {
    ...
    } else if (source.kind === "square-pyramid-net-area-in-square") {
      ...
      const diagonalCandidates = Array.from({ length: Math.max(0, outerSide - 1) }, (_, candidateIndex) => candidateIndex + 1).filter(candidate => candidate * candidate / 2 === baseArea);
      if (svg.netBase !== 1 || svg.netLateral !== 4 || svg.foldEdges !== 4 || svg.complementRegions !== 4 || svg.baseDiagonals !== 1 || svg.netFrames !== 1) fail(`${label}: 정사각뿔 전개도의 밑면·네 옆면·접는 선·네 빈 삼각형 표시가 다릅니다.`);
      if (Number(svg.outerSide) !== outerSide || Number(svg.baseDiagonal) !== diagonal || Number(svg.baseArea) !== baseArea || Number(svg.netArea) !== netArea) fail(`${label}: 정사각뿔 전개도 답 그림의 길이·넓이 자료가 문항 자료와 다릅니다.`);
...
async function inspectType(browser, baseUrl, source, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (const [index, item] of answer.answerVisuals.entries()) {
      ...
      const answerText = answer.items[index]?.text || "";
      if (!Number.isInteger(pool) || !expected || !compactMathText(answerText).includes(compactMathText(expected))) fail(`${label}: pool ${pool}의 공식 답 표시가 예상 답과 다릅니다.`);
    }
