...
check(readiness.integrity.publicCandidateCount === readiness.items.filter(item => item.publicDecision === "public").length && readiness.integrity.lockedCount === readiness.items.filter(item => item.publicDecision === "locked").length && readiness.integrity.calculationCheckedCount === 64 && readiness.integrity.calculationNeedsClarificationCount === 2 && readiness.integrity.singleAnswerTrueCount === 62 && readiness.integrity.singleAnswerFalseCount === 4, "6-2 1단원 검토표 요약이 실제 교정 결과와 다릅니다.");
