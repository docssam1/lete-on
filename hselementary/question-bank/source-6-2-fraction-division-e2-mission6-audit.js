...
check(readiness.integrity.publicCandidateCount === readiness.items.filter(item => item.publicDecision === "public").length && readiness.integrity.lockedCount === readiness.items.filter(item => item.publicDecision === "locked").length && readiness.integrity.calculationCheckedCount === 62 && readiness.integrity.calculationNeedsClarificationCount === 4 && readiness.integrity.singleAnswerTrueCount === 60 && readiness.integrity.singleAnswerFalseCount === 6, "6-2 1단원 검토표 요약이 실제 교정 결과와 다릅니다.");
