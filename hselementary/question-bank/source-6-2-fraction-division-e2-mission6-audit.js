...
check(readiness.integrity.publicCandidateCount === readiness.items.filter(item => item.publicDecision === "public").length && readiness.integrity.lockedCount === readiness.items.filter(item => item.publicDecision === "locked").length && readiness.integrity.calculationCheckedCount === 66 && readiness.integrity.calculationNeedsClarificationCount === 0 && readiness.integrity.singleAnswerTrueCount === 64 && readiness.integrity.singleAnswerFalseCount === 2, "6-2 1단원 검토표 요약이 실제 교정 결과와 다릅니다.");
...
check(!wrongSlot?.reviewLocked && wrongSlot?.generatorKey === "sourceGrade6SecondFractionDivisionE4Mission6" && wrongSlot?.typeLabel === "3일 동안 한 시간에 평균 몇 문제를 풀었는지 구하기", "개념탐구 4 Mission 6이 교정된 평균 문제 유형에 연결되지 않았습니다.");
check(correctedLockedItem?.sourceVerified && correctedLockedItem?.answerCandidates?.[0] === "12문제" && correctedLockedItem?.candidateAnswerCount === 1 && correctedLockedItem?.calculationStatus === "checked-independent" && correctedLockedItem?.implementationStatus === "fixed-verified-pool" && correctedLockedItem?.publicDecision === "public" && correctedLockedItem?.releaseStatus === "verified", "개념탐구 4 Mission 6의 원문·답·공개 상태가 다릅니다.");
check(correctedLockedLedgerItem?.sourceVerified && correctedLockedLedgerItem?.implementationStatus === "fixed-verified-pool" && correctedLockedLedgerItem?.answerContract === "single-whole-number" && correctedLockedLedgerItem?.publicSourceItemId === "6-2-u1-e4-mission-6", "개념탐구 4 Mission 6 원자료 장부의 교정 상태가 다릅니다.");
