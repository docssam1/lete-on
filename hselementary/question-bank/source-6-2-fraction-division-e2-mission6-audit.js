...
check(readiness.integrity.publicCandidateCount === readiness.items.filter(item => item.publicDecision === "public").length && readiness.integrity.lockedCount === readiness.items.filter(item => item.publicDecision === "locked").length && readiness.integrity.calculationCheckedCount === 65 && readiness.integrity.calculationNeedsClarificationCount === 1 && readiness.integrity.singleAnswerTrueCount === 63 && readiness.integrity.singleAnswerFalseCount === 3, "6-2 1단원 검토표 요약이 실제 교정 결과와 다릅니다.");
