...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-mission6-browser-audit");
const sourceItemId = "6-2-u1-e1-mission-6";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["26500/729", "39", "3136/125"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ answer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-three-value-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-three-value-board");
        const equations = [...item.querySelectorAll(".source62-three-value-equation")];
        const solution = item.querySelector(".source62-three-value-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-mission6-kind]");
        const values = (evidence?.dataset.values || "").split(",").map(Number);
        const answerValue = values.length === 14 ? `${values[12]}${values[13] === 1 ? "" : `/${values[13]}`}` : "";
        return {
          ...
          equationOverflow: equations.some(equation => equation.scrollWidth > equation.clientWidth + 1),
          structure: board?.dataset.source62E1Mission6Structure || "",
          expression: board?.dataset.source62E1Mission6Expression || "",
          ...
          kind: evidence?.dataset.source62FractionE1Mission6Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerValue,
          answerSource: item.querySelector(".source62-e1-mission6-answer")?.dataset.answerSource || "",
          symbolsVisible: ["㉠", "㉡", "㉢"].every(symbol => (item.innerText || "").includes(symbol)),
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 250 || item.equationOverflow) fail(`${label}/${index + 1}: 문항이나 세 조건식이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "difference-ratio-sum-three-values" || !item.expression || item.equationCount !== 3 || item.fractionCount < 1 || !item.symbolsVisible) fail(`${label}/${index + 1}: 원본의 차·비·합 세 식 구조가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "difference-ratio-sum-three-values" || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·유형·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (!item.solutionVisible || item.solutionOutside || item.solutionRows !== 4 || item.answerSource !== state.sourceId || !expectedAnswers.has(item.answerValue)) fail(`${label}/${index + 1}: 답의 세 수 풀이표나 독립 답 자료가 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 세 식 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 6 차·비·합 세 식·세 수 풀이표·넘침·답 누출; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
