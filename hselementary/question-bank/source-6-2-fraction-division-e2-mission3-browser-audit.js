...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-mission3-browser-audit");
const sourceItemId = "6-2-u1-e2-mission-3";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["1:6:1:10:4:15:5:2:12", { days: "60", target: "7:1:12", answer: "7월 1일 낮 12시", angle: "0deg" }],
  ["1:8:1:12:1:6:3:14:9", { days: "48", target: "5:1:9", answer: "5월 1일 오전 9시", angle: "270deg" }],
  ["1:5:1:15:1:5:6:16:15", { days: "45", target: "7:31:15", answer: "7월 31일 오후 3시", angle: "90deg" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-clock-drift-solution,#problemView .source62-clock-drift-board.is-solved").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-clock-drift-board");
        const evidence = item.querySelector("[data-source62-fraction-e2-mission3-kind]");
        const pair = item.querySelector(".source62-clock-drift-pair");
        const timeline = item.querySelector(".source62-clock-timeline");
        const boardBox = board?.getBoundingClientRect();
        const clippedDescendants = boardBox ? [...board.querySelectorAll(":scope .source62-clock-drift-pair,:scope .source62-clock-drift-card,:scope .source62-clock-timeline,:scope .source62-clock-timeline > span")].filter(element => {
          const box = element.getBoundingClientRect();
          return box.left < boardBox.left - 1 || box.right > boardBox.right + 1;
        }).length : 1;
        const clockAngles = [...item.querySelectorAll(".source62-clock-face")].map(clock => clock.getAttribute("style") || "");
        ...
        return {
          ...
          pairOverflow: pair ? pair.scrollWidth > pair.clientWidth + 1 : true,
          timelineOverflow: timeline ? timeline.scrollWidth > timeline.clientWidth + 1 : true,
          clippedDescendants,
          structure: board?.dataset.source62E2Mission3Structure || "",
          expression: board?.dataset.source62E2Mission3Expression || "",
          dailyGap: board?.dataset.dailyGap || "",
          targetMinutes: board?.dataset.targetMinutes || "",
          elapsedDays: board?.dataset.elapsedDays || "",
          targetDate: board?.dataset.targetDate || "",
          clockCards: item.querySelectorAll(".source62-clock-drift-card").length,
          clockFaces: item.querySelectorAll(".source62-clock-face").length,
          clockAngles,
          timelineStates: item.querySelectorAll(".source62-clock-timeline > span").length,
          timelineArrows: item.querySelectorAll(".source62-clock-timeline > i").length,
          solvedCount: item.querySelectorAll(".source62-clock-drift-board.is-solved").length,
          solutionRows: item.querySelectorAll(".source62-clock-drift-solution .source61-math-row").length,
          ...
          kind: evidence?.dataset.source62FractionE2Mission3Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e2-mission3-answer")?.dataset.answerSource || "",
          text,
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.pairOverflow || item.timelineOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 문항이나 시계 자료판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "opposite-clock-drift-calendar" || !row || item.elapsedDays !== row?.days || item.targetDate !== row?.target || !item.dailyGap || !item.targetMinutes) fail(`${label}/${index + 1}: 두 시계와 날짜의 같은 자료 모델이 빠졌습니다.`);
    if (item.clockCards !== 2 || item.clockFaces !== 2 || item.timelineStates !== 2 || item.timelineArrows !== 1 || item.fractionCount < 3 || item.clockAngles.some(angle => !angle.includes(`--hour-angle:${row?.angle}`))) fail(`${label}/${index + 1}: 두 시계, 시작 시각의 시곗바늘, 날짜 흐름 또는 분수 표시가 다릅니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 시계 자료판의 공통 글꼴이 아닙니다.`);
    if (item.source !== sourceItemId || item.kind !== "opposite-clock-drift-calendar" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 후보·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionRows !== 4 || item.answerSource !== sourceItemId || !item.text.includes(row.answer) || !item.text.includes(`${row.days}일 뒤`)) fail(`${label}/${index + 1}: 답 자료판의 날짜와 네 계산 단계가 다릅니다.`);
    } else if (item.solvedCount || item.solutionRows || item.answerSource || state.answerLeak || item.text.includes(row.answer) || item.text.includes(`${row.days}일 뒤`)) fail(`${label}/${index + 1}: 문제에 지난 날 수나 답 날짜가 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 두 시계 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 3 두 시계·시곗바늘·날짜 이동·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
