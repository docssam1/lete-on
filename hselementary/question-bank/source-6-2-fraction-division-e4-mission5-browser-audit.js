...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e4-mission5-browser-audit");
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 15000 || inkPixels(file) < 5000) fail(`${label}: ${name}가 비었거나 일의 양과 답이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-work-rate-board.is-solved,#problemView .source62-work-rate-solution,#problemView .source62-work-strip").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-work-rate-board");
        const boardBox = board?.getBoundingClientRect();
        const panels = [...item.querySelectorAll(".source62-work-rate-board,.source62-work-rate-solution")];
        const strip = item.querySelector(".source62-work-strip");
        const stripBox = strip?.getBoundingClientRect();
        const fractions = [...item.querySelectorAll(".math-fraction")];
        const evidence = item.querySelector("[data-source62-fraction-e4-mission5-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E4Mission5Structure || "", values: board?.dataset.source62E4Mission5Values || "",
          firstRate: board?.dataset.firstRate || "", secondRate: board?.dataset.secondRate || "", firstDone: board?.dataset.firstDone || "", remaining: board?.dataset.remaining || "",
          wholeUnits: Number(board?.dataset.wholeUnits || 0), answerDays: Number(board?.dataset.answerDays || 0), rowCount: item.querySelectorAll(".source61-math-row").length,
          solvedCount: item.querySelectorAll(".source62-work-rate-board.is-solved").length, solutionCount: item.querySelectorAll(".source62-work-rate-solution").length,
          stripCount: item.querySelectorAll(".source62-work-strip").length, stripVisible: visible(strip), stripOutside: strip ? outside(strip) : false, stripWidth: stripBox?.width || 0,
          ...
          source: evidence?.dataset.sourceItem || "", kind: evidence?.dataset.source62FractionE4Mission5Kind || "", candidateCount: Number(evidence?.dataset.candidateCount || 0), difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e4-mission5-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.panelOverflow) fail(`${label}/${index + 1}: 일의 양 자료판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "remaining-work-days" || !expectedItem || item.firstRate !== expectedItem?.firstRate || item.secondRate !== expectedItem?.secondRate || item.firstDone !== expectedItem?.firstDone || item.remaining !== expectedItem?.remaining || item.wholeUnits !== expectedItem?.wholeUnits || item.answerDays !== expectedItem?.answerDays) fail(`${label}/${index + 1}: 문제와 답의 기간·분수·남은 일 자료가 다릅니다.`);
    if (item.fractionCount < (answerView ? 4 : 2) || item.thinBars || !item.fontsValid) fail(`${label}/${index + 1}: 세로 분수 또는 공통 글꼴이 빠졌습니다.`);
    if (item.source !== sourceItemId || item.kind !== "remaining-work-days" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.stripCount !== 1 || !item.stripVisible || item.stripOutside || item.stripWidth < 250 || item.rowCount !== 10 || item.answerSource !== sourceItemId || !item.text.includes(`${expectedItem.answerDays}일`) || !item.text.includes("남은 일")) fail(`${label}/${index + 1}: 답의 전체 막대와 두 계산 방법이 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.stripCount || item.rowCount !== 3 || item.answerSource || state.answerLeak || !item.text.includes("나머지 일") || !item.text.includes("며칠")) fail(`${label}/${index + 1}: 문제 조건이 빠졌거나 풀이·답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.answerDays !== answer.items[index]?.answerDays) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 일 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 5 원문 일의 양·세로 분수·전체 막대·독립 계산·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
