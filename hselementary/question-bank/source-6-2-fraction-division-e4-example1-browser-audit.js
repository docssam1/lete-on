...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e4-example1-browser-audit");
const sourceItemId = "6-2-u1-e4-example-1";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["4:5:14:15:48:49", { referenceArea: "16:25", answerData: "10:7", answer: "1 3/7kg" }],
  ["3:4:9:10:25:36", { referenceArea: "9:16", answerData: "10:9", answer: "1 1/9kg" }],
  ["5:6:7:12:7:6", { referenceArea: "25:36", answerData: "49:50", answer: "49/50kg" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 10000) fail(`${label}: ${name}가 비었거나 철판 넓이·무게 계산이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-sheet-weight-board.is-solved,#problemView .source62-sheet-weight-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-sheet-weight-board");
        ...
        const evidence = item.querySelector("[data-source62-fraction-e4-example1-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E4Example1Structure || "",
          values: board?.dataset.source62E4Example1Values || "",
          referenceArea: board?.dataset.referenceArea || "",
          answerData: board?.dataset.targetWeight || "",
          rowCount: item.querySelectorAll(".source61-math-row").length,
          solvedCount: item.querySelectorAll(".source62-sheet-weight-board.is-solved").length,
          solutionCount: item.querySelectorAll(".source62-sheet-weight-solution").length,
          ...
          kind: evidence?.dataset.source62FractionE4Example1Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e4-example1-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedRows) fail(`${label}/${index + 1}: 철판 넓이·무게 계산판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "same-thickness-sheet-weight" || !expectedItem || item.referenceArea !== expectedItem?.referenceArea || item.answerData !== expectedItem?.answerData) fail(`${label}/${index + 1}: 문제와 답의 철판 길이·넓이·무게 자료가 다릅니다.`);
    if (item.fractionCount < 6 || item.thinBars || item.expressionCount < 6 || item.brokenExpressions || item.unitCount < 6 || !item.text.includes("m²")) fail(`${label}/${index + 1}: 세로 분수·넓이 단위 또는 줄바꿈되지 않는 수식 묶음이 빠졌습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "same-thickness-sheet-weight" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.rowCount !== 9 || item.answerSource !== sourceItemId || item.fractionCount < 14 || item.unitCount < 9) fail(`${label}/${index + 1}: 답의 정사각형 넓이·1m² 무게·넓이 비·새 무게 계산이 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.rowCount !== 3 || item.answerSource || state.answerLeak || item.text.includes(expectedItem.answer) || !item.text.includes("같은 두께")) fail(`${label}/${index + 1}: 문제 조건이 빠졌거나 풀이·답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.answerData !== answer.items[index]?.answerData) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 철판 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 4-1 원문 철판·세로 분수·넓이 단위·독립 계산·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 4-1 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
