...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e3-example3-browser-audit");
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      items: items.map(item => {
        ...
        const evidence = item.querySelector("[data-source62-fraction-e3-example3-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E3Example3Structure || "",
          cards: board?.dataset.source62E3Example3Cards || "",
          bestOrder: board?.dataset.bestOrder || "",
          validArrangements: Number(board?.dataset.validArrangements || 0),
          answerFraction: board?.dataset.answerFraction || "",
          cardValues: [...item.querySelectorAll(".source62-mixed-card-board .source41-number-card")].map(card => (card.innerText || "").trim()).join(":"),
          cardCount: item.querySelectorAll(".source62-mixed-card-board .source41-number-card").length,
          fractionCount: item.querySelectorAll(".source62-mixed-card-board .math-fraction").length,
          solvedCount: item.querySelectorAll(".source62-mixed-card-board.is-solved").length,
          solutionRows: item.querySelectorAll(".source62-mixed-card-solution .source61-math-row").length,
          font: board ? getComputedStyle(board).fontFamily : "",
          source: evidence?.dataset.sourceItem || "",
          kind: evidence?.dataset.source62FractionE3Example3Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          evidenceCards: evidence?.dataset.cards || "",
          evidenceOrder: evidence?.dataset.bestOrder || "",
          answerSource: item.querySelector(".source62-e3-example3-answer")?.dataset.answerSource || "",
          text,
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 수 카드·대분수 식이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "six-cards-smallest-mixed-quotient" || !row || item.bestOrder !== row?.order || item.answerFraction !== row?.answer || item.validArrangements !== 180 || item.evidenceCards !== item.cards || item.evidenceOrder !== item.bestOrder) fail(`${label}/${index + 1}: 문제와 답의 카드·최솟값 자료가 다릅니다.`);
    if (item.cardCount !== 6 || item.cardValues !== item.cards || item.fractionCount < (answerView ? 3 : 2)) fail(`${label}/${index + 1}: 원문 수 카드 여섯 장 또는 대분수 식이 빠졌습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "six-cards-smallest-mixed-quotient" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionRows !== 3 || item.answerSource !== sourceItemId) fail(`${label}/${index + 1}: 답의 카드 배치·전수 비교표가 없습니다.`);
    } else if (item.solvedCount || item.solutionRows || item.answerSource || state.answerLeak) fail(`${label}/${index + 1}: 문제에 최솟값 배치나 풀이가 노출되었습니다.`);
  }
  return state;
}
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 3-3 수 카드·대분수 배치·전수 비교·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 3-3 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
