...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(os.tmpdir(), "source-6-1-ratio-e5-browser-audit");
const sourceIds = [
...
async function inspect(page, generated, sourceId, difficulty, view, viewportName) {
  ...
  const state = await page.evaluate(() => {
    ...
    const diagrams = [...document.querySelectorAll("svg.source61-ratio-e5-diagram")];
    ...
    const equalAreaErrors = diagrams.map(diagram => {
      const left = diagram.querySelector(".source61-e5-equal-area-a");
      const right = diagram.querySelector(".source61-e5-equal-area-b");
      if (!left && !right) return "";
      if (!left || !right) return "같은 넓이 직사각형 한쪽 누락";
      const leftArea = Number(left.getAttribute("width")) * Number(left.getAttribute("height"));
      const rightArea = Number(right.getAttribute("width")) * Number(right.getAttribute("height"));
      return Math.abs(leftArea - rightArea) <= 0.01 ? "" : `${leftArea}:${rightArea}`;
    }).filter(Boolean);
    const poleCaseErrors = diagrams.flatMap(diagram => [...diagram.querySelectorAll(".source61-e5-pole-case")].map(group => {
      const rects = [...group.querySelectorAll("rect")].map(rect => Number(rect.getAttribute("width")));
      const total = Number(group.dataset.totalLength);
      const firstRate = Number(group.dataset.firstBuriedRate);
      const secondBuried = Number(group.dataset.secondBuriedLength);
      if (rects.length !== 4 || !total || !firstRate || !secondBuried) return "말뚝 경우 자료 누락";
      const firstActual = rects[1] / rects[0] * 100;
      const secondActual = rects[3] / rects[2] * total;
      return Math.abs(firstActual - firstRate) <= 0.01 && Math.abs(secondActual - secondBuried) <= 0.01 ? "" : `${firstActual}:${secondActual}`;
    })).filter(Boolean);
    return {
      ...
      overlaps,
      diagrams: diagrams.map(node => ({ structure: node.dataset.source61RatioE5Structure || "", values: node.dataset.source61RatioE5Values || "", width: node.getBoundingClientRect().width, height: node.getBoundingClientRect().height, bbox: (() => { try { const b = node.getBBox(); return { width: b.width, height: b.height }; } catch (_) { return { width: 0, height: 0 }; } })(), font: getComputedStyle(node.querySelector("text") || node).fontFamily })),
      resultCount: document.querySelectorAll(".source61-e5-result-label,[data-result-highlight]").length,
      ...
      marker: document.querySelector("[data-source61-ratio-e5-kind]")?.dataset.sourceItem || "",
      equalAreaErrors,
      poleCaseErrors,
      poleSchematicCount: document.querySelectorAll('[data-pole-condition-schematic="true"]').length,
      poleCaseCount: document.querySelectorAll(".source61-e5-pole-case").length
    };
  });
  ...
  if (sourceId === sourceIds[3] && state.equalAreaErrors.length) fail(`${label}: 넓이가 같다는 조건과 실제 직사각형 넓이가 다릅니다 (${state.equalAreaErrors.join(",")})`);
  if (sourceId === sourceIds[4] && (state.poleSchematicCount !== 1 || state.poleCaseErrors.length || state.poleCaseCount !== (view === "answer" ? 2 : 0))) fail(`${label}: 말뚝 조건 모형 또는 두 답 경우의 실제 비율이 다릅니다.`);
...
async function captureA4(page, filename, label, view) {
  ...
  const printState = await page.evaluate(expectedView => {
    ...
    items.forEach((item, index) => {
      ...
      item.querySelectorAll("svg.source61-ratio-e5-diagram").forEach(svg => {
...
async function inspectActualRoute(page, sourceId, viewportName, view) {
  const state = await page.evaluate(expected => {
    ...
    const diagrams = items.flatMap(item => [...item.querySelectorAll("svg.source61-ratio-e5-diagram")]);
    ...
    const diagramBoxes = diagrams.map(svg => {
      ...
      return {
        ...
        height: box.height,
        bbox,
        structure: svg.dataset.source61RatioE5Structure || "",
        values: svg.dataset.source61RatioE5Values || "",
    ...
    const answerNodes = items.flatMap(item => [...item.querySelectorAll(".source61-ratio-e5-answer")]);
    const equalAreaErrors = diagrams.map(diagram => {
      const left = diagram.querySelector(".source61-e5-equal-area-a");
      const right = diagram.querySelector(".source61-e5-equal-area-b");
      if (!left && !right) return "";
    ...
    const poleCaseErrors = diagrams.flatMap(diagram => [...diagram.querySelectorAll(".source61-e5-pole-case")].map(group => {
      ...
      const secondBuried = Number(group.dataset.secondBuriedLength);
      if (rects.length !== 4 || !total || !firstRate || !secondBuried) return "말뚝 경우 자료 누락";
      return Math.abs(rects[1] / rects[0] * 100 - firstRate) <= 0.01 && Math.abs(rects[3] / rects[2] * total - secondBuried) <= 0.01 ? "" : "말뚝 비율 불일치";
    })).filter(Boolean);
    return {
      ...
      equalAreaErrors,
      poleCaseErrors,
      poleSchematicCount: items.reduce((total, item) => total + item.querySelectorAll('[data-pole-condition-schematic="true"]').length, 0),
      poleCaseCount: items.reduce((total, item) => total + item.querySelectorAll(".source61-e5-pole-case").length, 0)
    };
  }, { sourceId, view });
  ...
  if (sourceId === sourceIds[3] && state.equalAreaErrors.length) fail(`${label}: 넓이가 같다는 조건과 실제 직사각형 넓이가 다릅니다 (${state.equalAreaErrors.join(",")})`);
  if (sourceId === sourceIds[4] && (state.poleSchematicCount !== 3 || state.poleCaseErrors.length || state.poleCaseCount !== (view === "answer" ? 6 : 0))) fail(`${label}: 말뚝 조건 모형 또는 두 답 경우의 실제 비율이 다릅니다.`);
  ...
  if (view === "answer" && sourceId === sourceIds[9] && state.answerMathFractionCount !== 3) fail(`${label}: 분수 정답 3개가 공통 세로 분수로 표시되지 않았습니다.`);
...
(async () => {
  ...
  try {
    ...
    for (let variant = 0; variant < sourceIds.length; variant += 1) for (const difficulty of difficulties) {
      const generated = api.generate({ sourceItemId: sourceIds[variant], generatorKey: "sourceGrade6RatioE5", reviewLocked: false }, 0, difficulty, 615000 + variant * 10000 + (difficulty + 1) * 100 + 17, variant);
