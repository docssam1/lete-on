...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e3-example2-browser-audit");
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 12000) fail(`${label}: ${name}가 비었거나 문제·풀이 내용이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-common-fraction-quotients,#problemView .source62-common-fraction-board.is-solved,#problemView .source62-common-fraction-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-common-fraction-board");
        const boardBox = board?.getBoundingClientRect();
        const descendants = board ? [...board.querySelectorAll(":scope > *,.source62-common-fraction-values > span,.source62-common-fraction-goal > *,.source62-common-fraction-quotients > span")] : [];
        const evidence = item.querySelector("[data-source62-fraction-e3-example2-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E3Example2Structure || "",
          values: board?.dataset.source62E3Example2Values || "",
          denominator: board?.dataset.commonDenominator || "",
          numerators: board?.dataset.commonNumerators || "",
          answerFraction: board?.dataset.answerFraction || "",
          valueCells: item.querySelectorAll(".source62-common-fraction-values > span").length,
          goalCells: item.querySelectorAll(".source62-common-fraction-goal > *").length,
          quotientCells: item.querySelectorAll(".source62-common-fraction-quotients > span").length,
          quotientValues: [...item.querySelectorAll(".source62-common-fraction-quotients > span")].map(cell => Number((cell.innerText || "").match(/=(\d+)$/)?.[1] || 0)),
          solvedCount: item.querySelectorAll(".source62-common-fraction-board.is-solved").length,
          solutionRows: item.querySelectorAll(".source62-common-fraction-solution .source61-math-row").length,
          ...
          kind: evidence?.dataset.source62FractionE3Example2Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          evidenceValues: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e3-example2-answer")?.dataset.answerSource || "",
          text,
  ...
  for (const [index, item] of state.items.entries()) {
    const row = expected.get(item.values);
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 세 분수·목표·몫 표가 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "largest-common-fraction-divisor" || !row || item.denominator !== row?.denominator || item.numerators !== row?.numerators || item.answerFraction !== row?.answer || item.evidenceValues !== item.values) fail(`${label}/${index + 1}: 문제와 답의 세 분수·공통분모 자료가 다릅니다.`);
    if (item.valueCells !== 3 || item.goalCells !== 3 || item.fractionCount < (answerView ? 10 : 6)) fail(`${label}/${index + 1}: 원문 세 분수 또는 자연수 몫 조건이 빠졌습니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 분수와 몫 표의 공통 글꼴이 아닙니다.`);
    if (item.source !== sourceItemId || item.kind !== "largest-common-fraction-divisor" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.quotientCells !== 3 || item.quotientValues.join(":") !== row.quotients.join(":") || item.solutionRows !== 3 || item.answerSource !== sourceItemId) fail(`${label}/${index + 1}: 답의 같은 분모와 세 자연수 몫이 다릅니다.`);
    } else if (item.solvedCount || item.quotientCells || item.solutionRows || item.answerSource || state.answerLeak) fail(`${label}/${index + 1}: 문제에 세 몫이나 풀이가 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.answerFraction !== answer.items[index]?.answerFraction) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 세 분수 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 3-2 세 분수·공통분모·최대공약수·세 몫·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 3-2 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
