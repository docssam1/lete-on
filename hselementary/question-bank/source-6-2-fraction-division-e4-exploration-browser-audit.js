...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e4-exploration-browser-audit");
const sourceItemId = "6-2-u1-e4-exploration";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["7:10:18:5:13:5:64:5:965:8", { distancePerLitre: "36:7", answerData: "747:32", answer: "23 11/32L" }],
  ["3:4:9:2:7:3:11:1:273:4", { distancePerLitre: "6:1", answerData: "87:8", answer: "10 7/8L" }],
  ["7:8:25:6:18:5:90:7:585:7", { distancePerLitre: "100:21", answerData: "333:20", answer: "16 13/20L" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 10000) fail(`${label}: ${name}가 비었거나 기름·거리·답 계산이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-unit-rate-trip-board.is-solved,#problemView .source62-unit-rate-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-unit-rate-trip-board");
        ...
        const evidence = item.querySelector("[data-source62-fraction-e4-exploration-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E4ExplorationStructure || "",
          values: board?.dataset.source62E4ExplorationValues || "",
          distancePerLitre: board?.dataset.distancePerLitre || "",
          answerData: board?.dataset.addedFuel || "",
          rowCount: item.querySelectorAll(".source61-math-row").length,
          solvedCount: item.querySelectorAll(".source62-unit-rate-trip-board.is-solved").length,
          solutionCount: item.querySelectorAll(".source62-unit-rate-solution").length,
          ...
          kind: evidence?.dataset.source62FractionE4ExplorationKind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e4-exploration-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedRows) fail(`${label}/${index + 1}: 기름·거리 계산판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "fuel-distance-unit-rate" || !expectedItem || item.distancePerLitre !== expectedItem?.distancePerLitre || item.answerData !== expectedItem?.answerData) fail(`${label}/${index + 1}: 문제와 답의 기름·거리 자료가 다릅니다.`);
    if (item.fractionCount < 5 || item.thinBars || item.expressionCount < 4 || item.brokenExpressions || item.unitCount < 5) fail(`${label}/${index + 1}: 분수선·단위 또는 줄바꿈되지 않는 수식 묶음이 빠졌습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "fuel-distance-unit-rate" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.rowCount !== 9 || item.answerSource !== sourceItemId || item.fractionCount < 12 || item.unitCount < 9) fail(`${label}/${index + 1}: 답의 1L당 거리·전체 거리·주유량 계산이 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.rowCount !== 4 || item.answerSource || state.answerLeak || item.text.includes(expectedItem.answer)) fail(`${label}/${index + 1}: 문제에 풀이 또는 답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.answerData !== answer.items[index]?.answerData) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 기름·거리 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 개념탐구 4 원문 기름·거리·세로 분수·단위·독립 계산·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 개념탐구 4 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
