...
(async () => {
  ...
  page.on("console", message => { if (message.type() === "error") failures.push(`콘솔 오류: ${message.text()}`); });
  ...
  if (previewBox && (previewBox.width > 320 || previewBox.height > 365)) failures.push(`데스크톱 미리보기가 너무 큽니다: ${previewBox.width}×${previewBox.height}`);
