...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e4-mission2-browser-audit");
const sourceItemId = "6-2-u1-e4-mission-2";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["9:20:97:12:13:5:90:1", { passage: "128:15", perMinute: "128:39", target: "3840:13", answer: "3840/13km" }],
  ["7:20:25:6:5:2:90:1", { passage: "271:60", perMinute: "271:150", target: "813:5", answer: "813/5km" }],
  ["5:12:27:4:10:3:90:1", { passage: "43:6", perMinute: "43:20", target: "387:2", answer: "387/2km" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 15000 || inkPixels(file) < 5000) fail(`${label}: ${name}가 비었거나 열차·다리·분수 계산이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-train-bridge-board.is-solved,#problemView .source62-train-bridge-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-train-bridge-board");
        ...
        const evidence = item.querySelector("[data-source62-fraction-e4-mission2-kind]");
        ...
        return {
          ...
          clippedRows: boardBox ? [...board.querySelectorAll(".source61-math-row,.source62-train-bridge-rule")].filter(element => { const box = element.getBoundingClientRect(); return box.left < boardBox.left - 1 || box.right > boardBox.right + 1; }).length : 1,
          structure: board?.dataset.source62E4Mission2Structure || "",
          values: board?.dataset.source62E4Mission2Values || "",
          passage: board?.dataset.passageDistance || "",
          perMinute: board?.dataset.distancePerMinute || "",
          target: board?.dataset.targetDistance || "",
          rowCount: item.querySelectorAll(".source61-math-row").length,
          ruleCount: item.querySelectorAll(".source62-train-bridge-rule").length,
          solvedCount: item.querySelectorAll(".source62-train-bridge-board.is-solved").length,
          solutionCount: item.querySelectorAll(".source62-train-bridge-solution").length,
          ...
          kind: evidence?.dataset.source62FractionE4Mission2Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e4-mission2-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedRows) fail(`${label}/${index + 1}: 열차·다리 계산판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "train-completely-crosses-bridge" || !expectedItem || item.passage !== expectedItem?.passage || item.perMinute !== expectedItem?.perMinute || item.target !== expectedItem?.target) fail(`${label}/${index + 1}: 문제와 답의 완전 통과 거리 자료가 다릅니다.`);
    if (item.fractionCount < 5 || item.thinBars || item.expressionCount < 7 || item.brokenExpressions || item.unitCount < 6 || item.ruleCount !== 1) fail(`${label}/${index + 1}: 기약분수·단위·완전 통과 규칙 또는 줄바꿈되지 않는 수식 묶음이 빠졌습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "train-bridge-complete-passage" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.rowCount !== 12 || item.answerSource !== sourceItemId || item.fractionCount < 14 || item.unitCount < 10 || !item.text.includes("다른 방법 확인")) fail(`${label}/${index + 1}: 답의 완전 통과 거리·1분 거리·시간 배수 계산이 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.rowCount !== 4 || item.answerSource || state.answerLeak || item.text.includes(expectedItem.answer) || !item.text.includes("열차 길이 + 다리 길이")) fail(`${label}/${index + 1}: 문제 조건이 빠졌거나 풀이·답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.target !== answer.items[index]?.target) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 열차·다리 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 2 원문 완전 통과 거리·기약분수·단위·독립 계산·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
