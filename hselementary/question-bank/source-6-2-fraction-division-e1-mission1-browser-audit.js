...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-mission1-browser-audit");
const sourceItemId = "6-2-u1-e1-mission-1";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["3/1", "4/1", "5/1"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspectView(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ isAnswer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: isAnswer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-complex-equation-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-complex-equation-board");
        const equation = board?.querySelector(".source62-complex-equation");
        const solution = item.querySelector(".source62-complex-equation-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-mission1-kind]");
        ...
        return {
          ...
          expression: board?.dataset.source62E1Mission1Expression || "",
          structure: board?.dataset.source62E1Mission1Structure || "",
          equationParts: equation?.children.length || 0,
          equationOverflow: equation ? equation.scrollWidth > equation.clientWidth + 1 : true,
          partOverflow: [...(equation?.children || [])].some(part => part.scrollWidth > part.clientWidth + 1),
          problemBlanks: item.querySelectorAll(".source62-complex-blank-fraction").length,
          solutionVisible: isAnswer ? visible(solution) : !solution,
          solutionOutside: isAnswer ? outside(solution) : false,
          solutionClipped: isAnswer ? clipped(solution) : false,
          ...
          kind: evidence?.dataset.source62FractionE1Mission1Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-mission1-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || item.itemClipped || !item.boardVisible || item.boardOutside || item.boardClipped || item.boardWidth < 250 || item.equationOverflow || item.partOverflow) fail(`${label} ${index + 1}: 문항이나 분수식이 비었거나 화면 밖·잘림 상태입니다.`);
    if (item.structure !== "two-brackets-and-blank-reciprocal" || !item.expression || item.equationParts !== 5) fail(`${label} ${index + 1}: 원본의 두 괄호와 1/빈칸 구조가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "complex-fraction-blank" || item.difficulty !== difficultyNames[String(difficulty)]) fail(`${label} ${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (item.rawFraction || item.invalid || item.mathFractions < (answerView ? 9 : 5)) fail(`${label} ${index + 1}: 가로 분수·깨진 값이 있거나 세로 분수 표시가 부족합니다.`);
    if (answerView) {
      const values = item.values.split(",").map(Number);
      const answerSignature = `${values[14]}/${values[15]}`;
      if (!item.solutionVisible || item.solutionOutside || item.solutionClipped || item.solutionRows !== 4 || item.answerSource !== state.sourceId || item.problemBlanks !== 0 || !expectedAnswers.has(answerSignature)) fail(`${label} ${index + 1}: 답 풀이표의 배치·원문 연결·완성 식이 다릅니다.`);
    } else if (!item.solutionVisible || state.answerLeak || item.problemBlanks !== 1) fail(`${label} ${index + 1}: 문제의 빈칸 수가 다르거나 답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 분수식 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 1 3난이도 PC/모바일 문제·답, 두 괄호·1/빈칸·세로 분수·넘침·답 누출 검사; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 Mission 1 브라우저·인쇄 감사 통과: 3난이도×PC/모바일×문제/답 · 화면 12장 · A4 2개 전 ${renderedPages}쪽`);
})().catch(error => {
