...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(os.tmpdir(), "source-6-1-ratio-e6-browser-audit");
const sourceIds = [
...
async function inspect(page, generated, sourceId, difficulty, view, viewportName) {
  ...
  const state = await page.evaluate(() => {
    ...
    const diagrams = [...document.querySelectorAll("svg.source61-ratio-e6-diagram")];
    ...
    const flowErrors = diagrams.filter(diagram => diagram.classList.contains("source61-e6-flow")).map(diagram => {
      const paths = diagram.querySelectorAll("path").length;
      return diagram.dataset.notToScale === "true" && paths >= 4 ? "" : `용기 ${paths}개 경로 또는 축척 안내 누락`;
    }).filter(Boolean);
    const routeErrors = diagrams.filter(diagram => diagram.classList.contains("source61-e6-route")).map(diagram => {
      const labels = [...diagram.querySelectorAll("text")].map(node => node.textContent || "").join(" ");
      const road = diagram.querySelector('path[d^="M 42 97"]');
      return road && ["학교", "공원", "평지", "내리막", "오르막"].every(label => labels.includes(label)) ? "" : "세 구간의 점·선분·이름 누락";
    }).filter(Boolean);
    return {
      ...
      overlaps,
      diagrams: diagrams.map(node => ({ structure: node.dataset.source61RatioE6Structure || "", values: node.dataset.source61RatioE6Values || "", width: node.getBoundingClientRect().width, height: node.getBoundingClientRect().height, bbox: (() => { try { const b = node.getBBox(); return { width: b.width, height: b.height }; } catch (_) { return { width: 0, height: 0 }; } })(), font: getComputedStyle(node.querySelector("text") || node).fontFamily })),
      resultCount: document.querySelectorAll(".source61-e6-result-label,[data-result-highlight]").length,
      ...
      marker: document.querySelector("[data-source61-ratio-e6-kind]")?.dataset.sourceItem || "",
      flowCount: document.querySelectorAll(".source61-e6-flow").length,
      routeCount: document.querySelectorAll(".source61-e6-route").length,
      flowErrors,
      routeErrors
    };
  });
  ...
  if ([sourceIds[1], sourceIds[5], sourceIds[6], sourceIds[7]].includes(sourceId) && (state.flowCount !== 1 || state.flowErrors.length)) fail(`${label}: 소금물 변화의 용기 그림이 조건과 다릅니다 (${state.flowErrors.join(",")})`);
  if (sourceId === sourceIds[4] && (state.routeCount !== 1 || state.routeErrors.length)) fail(`${label}: 학교·공원과 세 구간의 위치·방향 표시가 다릅니다 (${state.routeErrors.join(",")})`);
  ...
  if (/\d+\.\d{5,}/.test(state.text)) fail(`${label}: 긴 소수 꼬리가 표시되었습니다.`);
...
async function captureA4(page, filename, label, view) {
  ...
  const printState = await page.evaluate(expectedView => {
    ...
    items.forEach((item, index) => {
      ...
      item.querySelectorAll("svg.source61-ratio-e6-diagram").forEach(svg => {
...
async function inspectActualRoute(page, sourceId, viewportName, view) {
  const state = await page.evaluate(expected => {
    ...
    const diagrams = items.flatMap(item => [...item.querySelectorAll("svg.source61-ratio-e6-diagram")]);
    ...
    const diagramBoxes = diagrams.map(svg => {
      ...
      return {
        ...
        height: box.height,
        bbox,
        structure: svg.dataset.source61RatioE6Structure || "",
        values: svg.dataset.source61RatioE6Values || "",
    ...
    const answerNodes = items.flatMap(item => [...item.querySelectorAll(".source61-ratio-e6-answer")]);
    const flowErrors = diagrams.filter(diagram => diagram.classList.contains("source61-e6-flow")).map(diagram => {
      const paths = diagram.querySelectorAll("path").length;
      return diagram.dataset.notToScale === "true" && paths >= 4 ? "" : `용기 ${paths}개 경로 또는 축척 안내 누락`;
    }).filter(Boolean);
    ...
    return {
      ...
      flowCount: items.reduce((total, item) => total + item.querySelectorAll(".source61-e6-flow").length, 0),
      routeCount: items.reduce((total, item) => total + item.querySelectorAll(".source61-e6-route").length, 0),
      flowErrors,
      routeErrors
    };
  }, { sourceId, view });
  ...
  if ([sourceIds[1], sourceIds[5], sourceIds[6], sourceIds[7]].includes(sourceId) && (state.flowCount !== 3 || state.flowErrors.length)) fail(`${label}: 실제 경로의 소금물 용기 그림이 조건과 다릅니다 (${state.flowErrors.join(",")})`);
  if (sourceId === sourceIds[4] && (state.routeCount !== 3 || state.routeErrors.length)) fail(`${label}: 실제 경로의 학교·공원과 세 구간 표시가 다릅니다 (${state.routeErrors.join(",")})`);
  ...
  if (/\d+\.\d{5,}/.test(state.bodyText)) fail(`${label}: 긴 소수 꼬리가 표시되었습니다.`);
...
(async () => {
  ...
  try {
    ...
    for (let variant = 0; variant < sourceIds.length; variant += 1) for (const difficulty of difficulties) {
      const generated = api.generate({ sourceItemId: sourceIds[variant], generatorKey: "sourceGrade6RatioE6", reviewLocked: false }, 0, difficulty, 616000 + variant * 10000 + (difficulty + 1) * 100 + 17, variant);
