...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-exploration-browser-audit");
const sourceItemId = "6-2-u1-e2-exploration";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["48/11", "72/11", "50/9"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ answer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-average-speed-solution,#problemView .source62-average-speed-row>em").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-average-speed-board");
        const rows = [...item.querySelectorAll(".source62-average-speed-row")];
        const solution = item.querySelector(".source62-average-speed-solution");
        const evidence = item.querySelector("[data-source62-fraction-e2-exploration-kind]");
        const values = (evidence?.dataset.values || "").split(",").map(Number);
        const answerValue = values.length === 18 ? `${values[16]}${values[17] === 1 ? "" : `/${values[17]}`}` : "";
        return {
          ...
          structure: board?.dataset.source62E2ExplorationStructure || "",
          expression: board?.dataset.source62E2ExplorationExpression || "",
          fractionCount: item.querySelectorAll(".math-fraction").length,
          solvedDistances: item.querySelectorAll(".source62-average-speed-row>em").length,
          ...
          kind: evidence?.dataset.source62FractionE2ExplorationKind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerValue,
          answerSource: item.querySelector(".source62-e2-exploration-answer")?.dataset.answerSource || "",
          sequenceVisible: ["처음", "다음", "마지막", "남은 거리"].every(word => (item.innerText || "").includes(word)),
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 250 || item.rowOverflow) fail(`${label}/${index + 1}: 문항이나 세 이동 구간이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "remaining-distance-average-speed" || !item.expression || item.rowCount !== 3 || item.fractionCount < 4 || !item.sequenceVisible) fail(`${label}/${index + 1}: 원본의 전체·남은 거리 이동 순서가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "remaining-distance-average-speed" || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·유형·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (!item.solutionVisible || item.solutionOutside || item.solutionRows !== 5 || item.solvedDistances !== 3 || item.answerSource !== state.sourceId || !expectedAnswers.has(item.answerValue)) fail(`${label}/${index + 1}: 답의 세 거리·세 시간 풀이표가 다릅니다.`);
    } else if (!item.solutionVisible || item.solvedDistances || state.answerLeak) fail(`${label}/${index + 1}: 문제에 중간 거리나 답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 이동 조건 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 개념탐구 2 세 이동 구간·남은 거리·구간별 시간·넘침·답 누출; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 개념탐구 2 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
