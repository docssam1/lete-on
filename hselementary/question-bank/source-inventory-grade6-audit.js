...
check(readyItems.length === 46 && lockedItems.length === 587, `6학년 원문 유형의 공개 46개·잠금 587개 구성이 다릅니다: ${readyItems.length}/${lockedItems.length}`);
check(readyItems.every(item => ["sourceGrade6FractionDivisionE1", "sourceGrade6FractionDivisionE2", "sourceGrade6PrismsPyramidsE1", "sourceGrade6PrismsPyramidsE2", "sourceGrade6PrismsPyramidsE3", "sourceGrade6PrismsPyramidsE4", "sourceGrade6DecimalDivisionE1"].includes(item.generatorKey) && Number.isInteger(item.variant) && item.answerVisualStatus === "verified" && item.verifiedVariantCount === (item.sourceItemId === "6-1-u2-e4-example-4-1" ? 1 : 3)), "검증 완료한 6-1 원문 46유형의 생성기·답 그림·고정 문항 연결이 다릅니다.");
...
for (const semesterId of ["6-1", "6-2"]) {
  ...
  semester.units.forEach(unit => {
    ...
    check(actual.every(type => type.reviewLocked ? type.generatorKey === "" : ["sourceGrade6FractionDivisionE1", "sourceGrade6FractionDivisionE2", "sourceGrade6PrismsPyramidsE1", "sourceGrade6PrismsPyramidsE2", "sourceGrade6PrismsPyramidsE3", "sourceGrade6PrismsPyramidsE4", "sourceGrade6DecimalDivisionE1"].includes(type.generatorKey)), `${unit.id}: 원문 유형의 잠금과 생성기 연결이 다릅니다.`);
...
console.log(`6학년 공개 분류표·화면 연결 감사 통과: ${items.length}개 원문 문제 = ${items.length}개 세부 유형 · 생성 가능 46 · 검수 잠금 587 · 기존 생성 문제 보존`);
