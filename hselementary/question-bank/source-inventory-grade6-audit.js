...
const readyGeneratorKeys = [
  ...
  "sourceGrade6DecimalDivisionE1", "sourceGrade6DecimalDivisionE1Mission3", "sourceGrade6DecimalDivisionE1Mission4", "sourceGrade6DecimalDivisionE2", "sourceGrade6DecimalDivisionE2Example2", "sourceGrade6DecimalDivisionE3", "sourceGrade6DecimalDivisionE4",
...
for (const [generatorKey, expectedCount, label] of [
  ...
  mappedItems.forEach(item => {
    ...
    const readinessItem = (["sourceGrade6DecimalDivisionE1Mission3", "sourceGrade6DecimalDivisionE1Mission4", "sourceGrade6DecimalDivisionE2Example2"].includes(generatorKey) ? readinessU3 : ["sourceGrade6GraphsE1", "sourceGrade6GraphsE2", "sourceGrade6GraphsE3", "sourceGrade6GraphsE4"].includes(generatorKey) ? readinessU5 : ["sourceGrade6VolumeSurfaceE3", "sourceGrade6SurfaceE1", "sourceGrade6VolumeE2", "sourceGrade6VolumeE3Mission3"].includes(generatorKey) ? readinessU6 : readiness).items.find(candidate => candidate.sourceItemId === item.sourceItemId);
    if (["sourceGrade6VolumeSurfaceE3", "sourceGrade6VolumeE3Mission3", "sourceGrade6DecimalDivisionE1Mission3", "sourceGrade6DecimalDivisionE1Mission4", "sourceGrade6DecimalDivisionE2Example2"].includes(generatorKey)) {
...
check(readyItems.length === 223 && lockedItems.length === 410, `6학년 원문 유형의 공개 223개·잠금 410개 구성이 다릅니다: ${readyItems.length}/${lockedItems.length}`);
check(readyItems.every(item => readyGeneratorKeys.includes(item.generatorKey) && Number.isInteger(item.variant) && item.answerVisualStatus === "verified" && item.verifiedVariantCount === (item.sourceItemId === "6-1-u2-e4-example-4-1" ? 1 : 3)), "검증 완료한 6학년 원문 223유형의 생성기·답 그림·고정 문항 연결이 다릅니다.");
