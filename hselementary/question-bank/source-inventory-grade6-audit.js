...
check(readyItems.length === 248 && lockedItems.length === 385, `6학년 원문 유형의 공개 248개·잠금 385개 구성이 다릅니다: ${readyItems.length}/${lockedItems.length}`);
check(readyItems.every(item => readyGeneratorKeys.includes(item.generatorKey) && Number.isInteger(item.variant) && item.answerVisualStatus === "verified" && item.verifiedVariantCount === (item.sourceItemId === "6-1-u2-e4-example-4-1" ? 1 : 3)), "검증 완료한 6학년 원문 248유형의 생성기·답 그림·고정 문항 연결이 다릅니다.");
