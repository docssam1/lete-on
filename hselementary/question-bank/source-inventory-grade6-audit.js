...
const readyGeneratorKeys = [
  ...
  "sourceGrade6RatioE1", "sourceGrade6RatioE2", "sourceGrade6RatioE3", "sourceGrade6RatioE4"
];
...
check(readyItems.length === 117 && lockedItems.length === 516, `6학년 원문 유형의 공개 117개·잠금 516개 구성이 다릅니다: ${readyItems.length}/${lockedItems.length}`);
check(readyItems.every(item => readyGeneratorKeys.includes(item.generatorKey) && Number.isInteger(item.variant) && item.answerVisualStatus === "verified" && item.verifiedVariantCount === (item.sourceItemId === "6-1-u2-e4-example-4-1" ? 1 : 3)), "검증 완료한 6-1 원문 117유형의 생성기·답 그림·고정 문항 연결이 다릅니다.");
...
console.log(`6학년 공개 분류표·화면 연결 감사 통과: ${items.length}개 원문 문제 = ${items.length}개 세부 유형 · 생성 가능 117 · 검수 잠금 516 · 기존 생성 문제 보존`);
