...
if (e4.length !== 11 || types.length !== 96 || inventory.items.filter(item => item.implementationStatus === "ready").length !== 91 || inventory.items.filter(item => item.implementationStatus === "review-locked").length !== 5) failures.push("원문 96유형과 공개 91/잠금 5 수가 다릅니다.");
...
console.log(`5-1 약수와 배수 개념탐구 4 감사 통과: 원문 96유형 · 공개 91/잠금 5 · ${checked.toLocaleString()}회 독립 계산·답 형식·화면 조건 검사`);
