...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(__dirname, "tmp", "decimal-division-e4-example1-browser-audit");
...
function buildGenerator() {
  ...
  delete require.cache[require.resolve("./source-grade6-decimal-e4-example1.js")];
...
function inspectContract(generated, poolIndex, difficulty) {
  ...
  if (values.length !== 9 || values.some((value, index) => !close(value, expectedPools[poolIndex][index])) || attr(problemSvg, "data-source61-e4ex1-values") !== attr(answerSvg, "data-source61-e4ex1-values")) fail(`${label}: 문제·답 값 자료가 고정 문항과 다릅니다.`);
  if (attr(problemSvg, "data-source61-e4ex1-points") !== attr(answerSvg, "data-source61-e4ex1-points")) fail(`${label}: 문제와 답의 점 좌표가 다릅니다.`);
  if (attr(problemSvg, "data-source61-e4ex1-cells") !== attr(answerSvg, "data-source61-e4ex1-cells")) fail(`${label}: 문제와 답의 격자 칸 목록이 다릅니다.`);
  const points = parsePoints(attr(problemSvg, "data-source61-e4ex1-points"));
  if (!points.N || !points.G || !points.D) fail(`${label}: 삼각형의 세 꼭짓점이 없습니다.`);
  else {
    if (!(points.N.x < points.G.x && points.G.x < points.D.x) || !close(points.N.y, points.D.y) || !(points.G.y < points.N.y)) fail(`${label}: 삼각형 ㄱㄴㄷ의 격자점 순서가 다릅니다.`);
  }
  if (problemSvg.includes("source61-e4ex1-target") || !answerSvg.includes("source61-e4ex1-target is-solved")) fail(`${label}: 문제와 답의 삼각형 넓이 강조가 분리되지 않았습니다.`);
...
async function inspectMarkup(page, generated, view, viewportName, difficulty, poolIndex) {
  ...
  const state = await page.evaluate(() => {
    ...
    const svg = document.querySelector("svg.source61-e4ex1-diagram");
    ...
    return {
      ...
      texts, overlaps,
      phase: svg?.dataset.source61E4ex1Phase || "",
      model: svg?.dataset.source61E4ex1Model || "",
      points: svg?.dataset.source61E4ex1Points || "",
      pool: svg?.dataset.pool || "",
      cellCount: svg?.querySelectorAll(".source61-e4ex1-cell").length || 0,
      triangleCount: svg?.querySelectorAll(".source61-e4ex1-triangle").length || 0,
      pointLabelCount: svg?.querySelectorAll(".source61-e4ex1-point-label").length || 0,
      targetCount: svg?.querySelectorAll(".source61-e4ex1-target.is-solved").length || 0,
      resultCount: svg?.querySelectorAll(".source61-e4ex1-result").length || 0,
  ...
  if (state.model !== "equal-rectangle-grid-triangle-area-ratio" || state.triangleCount !== 1 || state.pointLabelCount !== 3 || state.cellCount !== expectedPools[poolIndex][5]) fail(`${label}: 격자 칸 또는 삼각형 ㄱㄴㄷ 구조가 다릅니다.`);
  ...
  if (view === "problem" && (state.phase !== "problem" || state.pool !== String(poolIndex) || state.targetCount || state.resultCount || state.answerSource)) fail(`${label}: 문제 화면에 답 전용 넓이 강조가 노출됩니다.`);
  if (view === "answer" && (state.phase !== "answer" || state.pool !== String(poolIndex) || state.targetCount !== 1 || state.resultCount !== 1 || state.answerSource !== sourceItemId)) fail(`${label}: 답 화면의 삼각형 넓이 강조와 결과 표시가 다릅니다.`);
  return state;
}
...
async function inspectActualRoute(browser, baseUrl, viewportName, viewport) {
  ...
  try {
    ...
    const routeState = await page.evaluate(expectedId => ({
      ...
      matchingCount: [...document.querySelectorAll(".tree-type")].filter(node => node.textContent?.includes("도형 전체 넓이가 삼각형 넓이의 몇 배인지 구하기")).length,
      disabledMatchingCount: [...document.querySelectorAll(".tree-type")].filter(node => node.textContent?.includes("도형 전체 넓이가 삼각형 넓이의 몇 배인지 구하기") && node.querySelector("input")?.disabled).length,
    ...
    const check = async view => page.evaluate(expectedView => {
      ...
      const diagrams = [...(active?.querySelectorAll("svg.source61-e4ex1-diagram") || [])];
      return {
        ...
        sourceCount: isProblem ? diagrams.filter(svg => svg.dataset.source61E4ex1Phase === "problem").length : [...active.querySelectorAll("[data-answer-source]")].filter(node => node.dataset.answerSource === "6-1-u3-e4-example-1").length,
        solvedCount: active?.querySelectorAll(".source61-e4ex1-target.is-solved").length || 0,
        overflow: document.documentElement.scrollWidth > innerWidth + 2,
...
(async () => {
  ...
  const summary = `${failures.length ? "실패" : "통과"}: 6-1-u3-e4-example-1 3문항×3난이도×PC1440/mobile390 문제·답, 격자 칸·삼각형 ㄱㄴㄷ·문제/답 좌표 동일·15px 글꼴·겹침/잘림·실제 앱 3문항 3쪽·A4 검사; 화면 ${screenshots}장, A4 ${pdfs}개, 렌더 ${renderedPdfPages}쪽\n${failures.join("\n")}\n`;
