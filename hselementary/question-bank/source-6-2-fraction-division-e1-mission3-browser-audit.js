...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-mission3-browser-audit");
const sourceItemId = "6-2-u1-e1-mission-3";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["16/15", "28/25", "99/91"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ answer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-expression-sequence-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-expression-sequence-board");
        const sequence = board?.querySelector(".source62-expression-sequence");
        const solution = item.querySelector(".source62-expression-sequence-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-mission3-kind]");
        return {
          ...
          structure: board?.dataset.source62E1Mission3Structure || "",
          sequence: board?.dataset.source62E1Mission3Sequence || "",
          terms: sequence?.querySelectorAll(".source62-expression-sequence-term").length || 0,
          termOverflow: [...(sequence?.querySelectorAll(".source62-expression-sequence-term") || [])].some(term => term.scrollWidth > term.clientWidth + 1),
          ...
          kind: evidence?.dataset.source62FractionE1Mission3Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-mission3-answer")?.dataset.answerSource || "",
          fractions: item.querySelectorAll(".math-fraction").length,
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 250 || item.termOverflow) fail(`${label}/${index + 1}: 문항이나 분수식 수열이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "same-numerator-neighbor-denominators" || !item.sequence || item.terms !== 3) fail(`${label}/${index + 1}: 원본의 처음 세 식 구조가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "fraction-expression-sequence-quotient" || item.difficulty !== difficultyNames[String(difficulty)]) fail(`${label}/${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (item.rawFraction || item.invalid || item.fractions < (answerView ? 15 : 6)) fail(`${label}/${index + 1}: 분수 표시가 부족하거나 깨진 값이 있습니다.`);
    if (answerView) {
      const values = item.values.split(",").map(Number);
      if (!item.solutionVisible || item.solutionOutside || item.solutionRows !== 3 || item.answerSource !== state.sourceId || !expectedAnswers.has(`${values[7]}/${values[8]}`)) fail(`${label}/${index + 1}: 답 풀이표나 최종 몫이 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.sequence !== answer.items[index]?.sequence) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답 수열이 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 3 수열·분수·넘침·답 누출; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
