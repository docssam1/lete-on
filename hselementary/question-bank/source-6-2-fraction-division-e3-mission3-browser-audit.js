...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e3-mission3-browser-audit");
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 12000) fail(`${label}: ${name}가 비었거나 카드·분수 내용이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-mixed-card-board.is-solved,#problemView .source62-mixed-card-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-mixed-card-board");
        const boardBox = board?.getBoundingClientRect();
        const descendants = board ? [...board.querySelectorAll(":scope > *,.source41-number-card,.source62-mixed-card-expression > *")] : [];
        const evidence = item.querySelector("[data-source62-fraction-e3-mission3-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E3Mission3Structure || "", cards: board?.dataset.source62E3Mission3Cards || "", bestOrder: board?.dataset.bestOrder || "", validArrangements: Number(board?.dataset.validArrangements || 0), answerFraction: board?.dataset.answerFraction || "",
          cardValues: [...item.querySelectorAll(".source62-mixed-card-board .source41-number-card")].map(card => (card.innerText || "").trim()).join(":"), cardCount: item.querySelectorAll(".source62-mixed-card-board .source41-number-card").length, fractionCount: item.querySelectorAll(".source62-mixed-card-board .math-fraction").length,
          solvedCount: item.querySelectorAll(".source62-mixed-card-board.is-solved").length, solutionRows: item.querySelectorAll(".source62-mixed-card-solution .source61-math-row").length,
          font: board ? getComputedStyle(board).fontFamily : "", source: evidence?.dataset.sourceItem || "", kind: evidence?.dataset.source62FractionE3Mission3Kind || "", candidateCount: Number(evidence?.dataset.candidateCount || 0), difficulty: evidence?.dataset.difficultyDesign || "", evidenceCards: evidence?.dataset.cards || "", evidenceOrder: evidence?.dataset.bestOrder || "", answerSource: item.querySelector(".source62-e3-mission3-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    const row = expected.get(item.cards);
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 수 카드·대분수÷진분수 식이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "five-cards-largest-mixed-proper-quotient" || !row || item.bestOrder !== row?.order || item.answerFraction !== row?.answer || item.validArrangements !== 30 || item.evidenceCards !== item.cards || item.evidenceOrder !== item.bestOrder) fail(`${label}/${index + 1}: 문제와 답의 카드·최댓값 자료가 다릅니다.`);
    if (item.cardCount !== 5 || item.cardValues !== item.cards || item.fractionCount < (answerView ? 3 : 2)) fail(`${label}/${index + 1}: 원문 수 카드 다섯 장 또는 대분수÷진분수 식이 빠졌습니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 카드와 분수 식의 공통 글꼴이 아닙니다.`);
    if (item.source !== sourceItemId || item.kind !== "five-cards-largest-mixed-proper-quotient" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionRows !== 4 || item.answerSource !== sourceItemId) fail(`${label}/${index + 1}: 답의 카드 배치·전수 비교표가 없습니다.`);
    } else if (item.solvedCount || item.solutionRows || item.answerSource || state.answerLeak) fail(`${label}/${index + 1}: 문제에 최댓값 배치나 풀이가 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.cards !== answer.items[index]?.cards || problem.items[index]?.bestOrder !== answer.items[index]?.bestOrder || problem.items[index]?.answerFraction !== answer.items[index]?.answerFraction) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 카드 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 3 수 카드·대분수÷진분수 배치·전수 비교·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
