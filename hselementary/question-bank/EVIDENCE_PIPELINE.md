...
| `negative-control` | 임시 복사본에서 핵심 자료 하나를 고의로 변경 | 관련 검사가 실패한 뒤 임시본을 삭제하고 원본은 그대로 유지 |
...
- 모양을 그리거나 칠하는 문항은 정답지에도 완성된 정답 그림을 표시합니다. `그림 1개` 같은 글만으로 대신하지 않습니다.
- 문제 그림과 정답 그림은 같은 점·선분·칸·높이 자료에서 만들고, 정답 그림에만 필요한 선·색·수를 더합니다.
- 입체, 전개도, 단면처럼 무한 난수화가 원문 구조나 단일 답을 흔들 수 있는 유형은 `fixed-verified-pool`로 운영합니다.
- 고정 묶음은 유형당 3문항을 우선 목표로 하며, 각 문항의 문제 그림·정답 그림·풀이·단일 답·PC·모바일·A4 검수가 모두 끝나야 공개합니다.
- 3문항을 만들지 못한 유형은 확인된 문항만 공개하거나 계속 잠그며, 검증되지 않은 난수 문항으로 수를 채우지 않습니다.
- 화면에서 더 많은 문항을 요청해도 고정 묶음은 실제 검증 문항 수까지만 서로 다르게 출제하고, 같은 문항을 반복해 수를 채우지 않습니다.
