...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(os.tmpdir(), "source-6-1-ratio-e1-browser-audit");
const sourceIds = [
...
async function inspect(page, generated, sourceId, difficulty, view, viewportName) {
  ...
  const state = await page.evaluate(() => {
    ...
    const diagrams = [...document.querySelectorAll("svg.source61-ratio-e1-diagram")];
    ...
    return {
      ...
      marker: document.querySelector("[data-source61-ratio-e1-kind]")?.dataset.sourceItem || ""
    };
  });
  ...
  if (view === "problem") {
    ...
    if (sourceId === "6-1-u4-e1-example-1-2") {
      const values = String(state.diagrams[0]?.values || "").split(",").map(Number);
      const leaked = [values[3], values[4]].filter(Number.isFinite).filter(value => new RegExp(`(^|\\D)${value}(?=\\D|$)`).test(state.text));
      if (leaked.length) fail(`${label}: 문제 화면에 구해야 할 두 양 ${leaked.join(", ")}이 노출되었습니다.`);
    }
  } else {
    if (state.source !== sourceId || state.marker !== sourceId || state.resultCount < 1) fail(`${label}: 답 자료의 source 또는 정답 강조가 없습니다.`);
    if (state.diagrams.some(svg => !svg.font.includes("Pretendard") || !svg.font.includes("Malgun Gothic") || !svg.font.includes("Arial"))) fail(`${label}: 공통 수학 글꼴이 없습니다.`);
  }
  return state;
}
...
async function captureA4(page, filename, label, view) {
  ...
  const printState = await page.evaluate(expectedView => {
    ...
    items.forEach((item, index) => {
      ...
      item.querySelectorAll("svg.source61-ratio-e1-diagram").forEach(svg => {
...
async function inspectActualRoute(page, sourceId, viewportName, view) {
  const state = await page.evaluate(expected => {
    ...
    const diagrams = items.flatMap(item => [...item.querySelectorAll("svg.source61-ratio-e1-diagram")]);
    ...
    const diagramBoxes = diagrams.map(svg => {
      ...
      return {
        ...
        height: box.height,
        bbox,
        structure: svg.dataset.source61RatioE1Structure || "",
        values: svg.dataset.source61RatioE1Values || "",
    ...
    const answerNodes = items.flatMap(item => [...item.querySelectorAll(".source61-ratio-e1-answer")]);
    const valueLeaks = isProblem && expectedSourceId === "6-1-u4-e1-example-1-2" ? items.flatMap((item, index) => {
      const values = String(item.querySelector("svg.source61-ratio-e1-diagram")?.dataset.source61RatioE1Values || "").split(",").map(Number);
      return [values[3], values[4]].filter(Number.isFinite).filter(value => new RegExp(`(^|\\D)${value}(?=\\D|$)`).test(item.innerText || "")).map(value => `${index + 1}번:${value}`);
    }) : [];
    return {
  ...
  if (view === "problem" && state.valueLeaks.length) fail(`${label}: 문제 화면에 구해야 할 두 양이 노출되었습니다. (${state.valueLeaks.join(", ")})`);
  ...
  if (state.diagramBoxes.some(svg => !svg.font.includes("Pretendard") || !svg.font.includes("Malgun Gothic") || !svg.font.includes("Arial"))) fail(`${label}: 공통 수학 글꼴이 없습니다.`);
  return state;
}
...
(async () => {
  ...
  try {
    ...
    for (let variant = 0; variant < sourceIds.length; variant += 1) for (const difficulty of difficulties) {
      const generated = api.generate({ sourceItemId: sourceIds[variant], generatorKey: "sourceGrade6RatioE1", reviewLocked: false }, 0, difficulty, 611000 + variant * 10000 + (difficulty + 1) * 100 + 17, variant);
  ...
  if (screenshots !== 144 || pdfs !== 18 || expectedPdfPages < pdfs || renderedPdfPages !== expectedPdfPages) fail(`산출물 수 ${screenshots}/${pdfs}/${renderedPdfPages}/${expectedPdfPages}, 화면 144장·PDF 18개·PDF 전쪽 렌더여야 합니다.`);
  const summary = `${failures.length ? "실패" : "통과"}: 실제 주소 9유형×PC1440/mobile390 문제·답 + 잠금 3유형 차단 + 실제 주소 A4 문제·답, 9유형×3난이도 직접 생성 문제·답, 동일 자료·정답 누출·겹침·글꼴·넘침 검사; 화면 ${screenshots}장, A4 ${pdfs}개, 렌더 ${renderedPdfPages}/${expectedPdfPages}쪽\n${failures.join("\n")}\n`;
