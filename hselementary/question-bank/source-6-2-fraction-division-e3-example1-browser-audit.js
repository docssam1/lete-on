...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e3-example1-browser-audit");
const sourceItemId = "6-2-u1-e3-example-1";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["6:16", { product: "96", pairCount: 7, pairs: "1:96|2:48|3:32|4:24|6:16|8:12|12:8", answer: "7쌍" }],
  ["5:18", { product: "90", pairCount: 8, pairs: "1:90|2:45|3:30|5:18|6:15|9:10|10:9|15:6", answer: "8쌍" }],
  ["8:15", { product: "120", pairCount: 9, pairs: "1:120|2:60|3:40|4:30|5:24|6:20|8:15|10:12|12:10", answer: "9쌍" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      items: items.map(item => {
        ...
        const evidence = item.querySelector("[data-source62-fraction-e3-example1-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E3Example1Structure || "",
          expression: board?.dataset.source62E3Example1Expression || "",
          ...
          kind: evidence?.dataset.source62FractionE3Example1Kind || "",
          ...
          answerSource: item.querySelector(".source62-e3-example1-answer")?.dataset.answerSource || "",
          text,
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 식·조건·순서쌍 표가 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "proper-fraction-natural-pair-count" || !row || item.product !== row?.product || item.pairCount !== row?.pairCount || item.pairs !== row?.pairs || item.evidencePairs !== row?.pairs) fail(`${label}/${index + 1}: 문제와 답의 자연수 순서쌍 자료가 다릅니다.`);
    if (item.conditionCount !== 2 || item.equationCount !== 1 || item.fractionCount < (answerView ? 2 : 3)) fail(`${label}/${index + 1}: 원문 식·진분수·서로 다른 자연수 조건이 빠졌습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "proper-fraction-natural-pair-count" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.pairCells !== row.pairCount || item.solutionRows !== 3 || item.answerSource !== sourceItemId || !item.text.includes(row.answer)) fail(`${label}/${index + 1}: 답의 전체 순서쌍 표와 세 확인 단계가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 3-1 식·진분수·서로 다른 자연수·전체 순서쌍·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 3-1 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
