...
const outputDir = process.env.HSE_SCREENSHOT_DIR
  || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-browser-audit");
const sourceItemId = "6-2-u1-e1-example-1";
const difficulties = [-1, 0, 1];
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["51/58", "33/40", "27/34"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
function inkPixels(pngPath) {
  ...
  let count = 0;
  for (let y = Math.floor(png.height * .04); y < Math.floor(png.height * .96); y += 1) {
...
async function inspectView(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ isAnswer, expectedDifficulty, sourceId }) => {
    ...
    return {
      ...
      items: items.map(item => {
        const line = item.querySelector(".source62-number-line");
        const labels = [...(line?.querySelectorAll(".source62-number-line__label") || [])];
        const labelBoxes = labels.map(label => {
          const box = label.getBoundingClientRect();
          return { left: box.left, right: box.right, top: box.top, bottom: box.bottom, width: box.width, height: box.height, text: label.innerText || "" };
        });
        const evidence = item.querySelector("[data-source62-fraction-e1-kind]");
        ...
        return {
          ...
          lineOutside: outside(line),
          lineClipped: clipped(line),
          lineWidth: lineBox?.width || 0,
          lineHeight: lineBox?.height || 0,
          structure: line?.dataset.source62E1Structure || "",
          geometry: line?.dataset.source62E1Geometry || "",
          tickCount: line?.querySelectorAll("[data-tick-index]").length || 0,
          labelCount: labels.length,
          labelOutside: labelBoxes.some(box => box.left < (lineBox?.left || 0) - 2 || box.right > (lineBox?.right || 0) + 2),
          labelOverlap: labelBoxes.some((left, index) => labelBoxes.slice(index + 1).some(right => {
            const width = Math.max(0, Math.min(left.right, right.right) - Math.max(left.left, right.left));
            const height = Math.max(0, Math.min(left.bottom, right.bottom) - Math.max(left.top, right.top));
            return width * height / Math.max(1, Math.min(left.width * left.height, right.width * right.height)) > .15;
          })),
          font: line ? getComputedStyle(line).fontFamily : "",
          solved: Boolean(line?.classList.contains("is-solved")),
          solvedLabels: line?.querySelectorAll(".source62-number-line__label.is-solved").length || 0,
          mathFractions: item.querySelectorAll(".math-fraction").length,
          source: evidence?.dataset.sourceItem || "",
          kind: evidence?.dataset.source62FractionE1Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-answer")?.dataset.answerSource || "",
          pool: item.querySelector(".source62-e1-answer")?.dataset.verifiedPoolIndex || "",
          text: item.innerText || "",
          rawFraction: /\b\d+\s*\/\s*\d+\b/.test(item.innerText || ""),
          invalid: /undefined|null|NaN|Infinity|SyntaxError/.test(item.innerText || "")
        };
      }),
      leakedAnswer: isAnswer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-number-line.is-solved,#problemView .source62-number-line__label.is-solved").length,
      expectedDifficulty,
      sourceId
    };
  }, { isAnswer: answerView, expectedDifficulty: difficultyNames[String(difficulty)], sourceId: sourceItemId });
  checkedViews += 1;
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || item.itemClipped || !item.lineVisible || item.lineOutside || item.lineClipped || item.lineWidth < 250 || item.lineHeight < 140) fail(`${label} ${index + 1}: 문항 또는 수직선이 비었거나 화면 밖·잘림 상태입니다.`);
    if (item.structure !== "fraction-division-number-line" || !item.geometry || item.tickCount !== 11 || item.labelCount !== 4) fail(`${label} ${index + 1}: 수직선 구조·눈금·표시가 원본 계약과 다릅니다.`);
    if (item.labelOutside || item.labelOverlap) fail(`${label} ${index + 1}: 수직선의 분수나 가·나 표시가 겹치거나 밖으로 나갔습니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label} ${index + 1}: 수직선 글꼴이 공통 글꼴로 고정되지 않았습니다.`);
    if (item.source !== sourceItemId || item.kind !== "fraction-division-number-line" || item.difficulty !== state.expectedDifficulty) fail(`${label} ${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (item.rawFraction || item.invalid || item.mathFractions < (answerView ? 7 : 2)) fail(`${label} ${index + 1}: 가로 분수·깨진 값이 있거나 세로 분수 표시가 부족합니다.`);
    if (answerView) {
      if (!item.solved || item.solvedLabels !== 2 || item.answerSource !== sourceItemId) fail(`${label} ${index + 1}: 답 수직선에 가·나 값이 정확히 표시되지 않았습니다.`);
      const values = item.values.split(",").map(Number);
      const answer = `${values[10]}/${values[11]}`;
      if (!expectedAnswers.has(answer)) fail(`${label} ${index + 1}: 고정 문항 답 ${answer}이 계약과 다릅니다.`);
    } else if (item.solved || item.solvedLabels || state.leakedAnswer) fail(`${label} ${index + 1}: 문제에 답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) {
      if (problem.items[index]?.geometry !== answer.items[index]?.geometry) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 수직선 좌표가 다릅니다.`);
    }
...
(async () => {
  ...
  const summary = `${failures.length ? "실패" : "통과"}: 6-2 예제 1-1 3난이도 PC/모바일 문제·답, 11눈금·분수·겹침·잘림·답 누출 검사; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`;
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), summary, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 1-1 브라우저·인쇄 감사 통과: 3난이도×PC/모바일×문제/답 · 화면 12장 · A4 2개 전 ${renderedPages}쪽`);
})().catch(error => {
