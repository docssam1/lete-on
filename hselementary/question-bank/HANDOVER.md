...
- Implemented types: 857 (deterministic runtime availability check)
- Review-locked types: 87
...
- Grade 4-2 unit 2: triangles, all 44 source items mapped; 5 directly source-matched types ready and 39 locked for source-faithful rebuilding
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 857 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks all 857 public runtime types across 100 seeds per difficulty, including the source-mapped public types
- Numeric display policy: `numeric-display-audit.js` checks all 857 public runtime types across 100 seeds per difficulty; the runtime sweep separately checks availability
...
- 4-2 triangle source routing, publication gate, and independent answer check: `triangle-4-2-audit.js` maps all 44 source items and permits only the five directly source-matched structures to run
