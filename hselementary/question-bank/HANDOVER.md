...
- Grade 4-2 unit 2: triangles, all 44 source items mapped; 30 directly source-matched types ready and 14 locked for source-faithful rebuilding
...
The current ready set has passed its unit-specific regression coverage. `runtime-availability-audit.js` generates all 882 runtime-ready types at three difficulty offsets across 20 seeds. The 4-1 source-native mapping has 309 ready items, and the 4-2 fraction unit adds 66 ready source-native items. The 4-2 triangle source audit maps all 44 items and currently publishes 30 source-matched types; the other 14 remain locked. Triangle-count Exploration 1 reconstructs the source's eight maximal printed segments and ten vertices, confirms 40 triangles by both vertex triples and supporting-line triples, and independently splits them into 21 using the left-side junction and 19 not using it. Exploration 2 enumerates every triple on the 4-by-3 dot board, groups rotations and reflections by the same three side lengths, and confirms nine distinct obtuse triangles. Example 2-2 and Mission 2 enumerate the pentagram's five source segments and all ten intersection-based triangles, confirming five acute and five obtuse triangles. Example 2-4 and Mission 3 enumerate every valid angle pair and each confirm ten methods, including cases where one selected angle is already obtuse. Example 1-2 confirms 16 triangles containing the marked cell, and Example 1-3 independently confirms 63 and 12. Mission 4 confirms 37, Mission 5 confirms 24 rather than the handwritten 19, and Mission 6 confirms 64. The isosceles source set adds the 38cm concave perimeter, 320cm strip perimeter, and 3cm short-side types. The equilateral source set now publishes its exploration, Missions 1-6, and Examples 4-1 through 4-4. The exploration exhaustively checks moving two of twelve matchsticks, finds 48 valid placements, and reduces them to five shapes under rotation and reflection. Mission 2 is independently 75 degrees, Mission 5 uses six exact equilateral triangles and `(large-small)/3`, Example 4-3 uses seven exact equilateral triangles, and Example 4-4 reflects the original rectangle across the fold line and confirms 15cm for the printed 8cm/27cm source rather than the handwritten 22cm. The remaining completed groups retain their dedicated audits and source gates described below.

The full non-browser regression suite has 70 dedicated audits, and the 2026-08-29 final run passed all 70. The 30 currently public 4-2 triangle types passed all 60 desktop/mobile states with three generated prompts per type; final evidence, including direct problem and solution captures for Exploration 1, is stored at `E:\Codex\visualizations\2026\08\29\hse-4-2-triangle-public-30-final`. The matchstick exploration also passed direct desktop/mobile visual review after removing an overlong duplicate SVG title; final captures are stored at `E:\Codex\visualizations\2026\08\29\hse-4-2-u4-matchstick-direct-final`. Division-remainder, bar-graph, and plane-transformation browser evidence remains in the dated visualization folders recorded by their dedicated audits.
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 882 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks all 882 public runtime types across 100 seeds per difficulty, including the source-mapped public types
- Numeric display policy: `numeric-display-audit.js` checks all 882 public runtime types across 100 seeds per difficulty; the runtime sweep separately checks availability
...
- 4-2 triangle source routing, publication gate, and independent answer check: `triangle-4-2-audit.js` maps all 44 source items and permits only the 30 directly source-matched types to run
