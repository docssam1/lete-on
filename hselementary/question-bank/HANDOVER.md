...
- Implemented types: 686 (deterministic runtime availability check)
- Review-locked types: 228 (180 source-inventoried 4-1 items plus 48 source-complex 4-2 items)
...
The current ready set has passed its unit-specific regression coverage. `runtime-availability-audit.js` generates every one of the 686 runtime-ready types at all three difficulty offsets across 20 seeds each. The legacy generator-family audits remain available; 4-1 exposes 140 source-native variants across all six large-number exploration groups, the first four complete angle groups, 9 safe items from angle group 5, 10 safe items from angle group 6, and all 11 multiplication exploration group 1 items, plus 9 other exact source matches. Graph audits reverse-check values against SVG coordinates; geometry audits enumerate answer candidates and enforce one visible, inferable answer. Graph and diagram units additionally receive desktop and mobile (375px) checks for overflow, missing questions, missing solutions, accidental lock states, and graph-label overlap.
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 686 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks all 686 public runtime types across 100 seeds per difficulty, including all 149 public 4-1 mappings
- Numeric display policy: `numeric-display-audit.js` checks all 686 public runtime types across 100 seeds per difficulty; the runtime sweep separately checks availability
