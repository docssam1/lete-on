...
- Implemented types: 728 (deterministic runtime availability check)
- Review-locked types: 186 (138 source-inventoried 4-1 items plus 48 source-complex 4-2 items)
...
- Grade 4-1 unit 4: 44 source items, 11 ready and 33 review-locked; exploration group 1 publishes 10 source-native items and keeps ambiguous Mission 1 locked, plus 1 earlier exact-match generator in group 4
...
The current ready set has passed its unit-specific regression coverage. `runtime-availability-audit.js` generates every one of the 728 runtime-ready types at all three difficulty offsets across 20 seeds each. The legacy generator-family audits remain available; 4-1 exposes 183 source-native variants across all six large-number exploration groups, the first four complete angle groups, 9 safe items from angle group 5, 10 safe items from angle group 6, both complete 11-item multiplication exploration groups, and both complete 11-item division exploration groups, 10 safe items from plane-transformation group 1, plus 8 other exact source matches. The second multiplication group exhaustively checks card arrangements and digit-condition candidates and rejects the incorrect pencil maximum for example 2-4. Division group 3 exhaustively checks discarded-item, date, supplement, and bag-weight candidates and rejects two incorrect handwritten answers. Division group 4 exhaustively checks torn-division candidates, digit-length ranges, all number-card arrangements, equal quotient-and-remainder candidates, and consecutive-number sums. Graph audits reverse-check values against SVG coordinates; geometry audits enumerate answer candidates and enforce one visible, inferable answer. Graph and diagram units additionally receive desktop and mobile (375px) checks for overflow, missing questions, missing solutions, accidental lock states, and graph-label overlap.

The full regression suite has 61 dedicated audits. The source-item taxonomy gate requires each mapped exploration, example, and Mission problem to have its own source ID and page locator. The 2026-08-28 run passed the 4-1 inventory, child-readable Korean type-language, all six source-native large-number groups, the first four complete angle groups, the 9 safe items in angle group 5, the 10 safe items in angle group 6, both 11-item multiplication exploration groups, both 11-item division exploration groups, the 10 safe items in plane-transformation group 1, crosswalk, runtime taxonomy, and availability gates together with the existing 4-2 and shared audits. Exploration group 4 corrects the answer sheet's `11cm` unit error to the independently calculated `11km`. Exploration group 5 independently corrects the nearest-number answer and uses the source Mission 2 instead of the different edition shown in the answer PDF; it also fixes Example 5-3 at 72 cases and interprets Mission 6 as matching the counted number's three respective place digits `5, 8, 4`. Exploration group 6 corrects answer-sheet errors in Example 6-3 and 6-4 and Mission 1, 2, and 6; the 180-degree card calculation uses the source's actual second-smallest number `10258`, producing `15057`. The first angle group rebuilds two visually ambiguous ray diagrams with exact safe coordinates, calculates 30 and 42 vertical-angle pairs from line pairs, and excludes always-true statement ⑦ from the Mission 3 false-statement answer. The second angle group preserves all 11 source structures, including the shifted equilateral-triangle result `90°`, the three reflex-angle answers `100°`, `150°`, and `230°`, and the six Mission answers `111°`, `36°`, `52°`, `80°`, `40°`, and `55°`. The third angle group independently corrects source-answer conflicts at `1800°`, `263°`, `1620°`, and `50°`, and verifies the remaining source answers including Mission 6 at `400°`. The fourth angle group verifies 11 exterior-angle and star-geometry source structures, including `52°`, `65°`, `360°`, `18°`, `180°`, `27°`, `53°`, `63°`, `95°`, `82°`, and `540°`; it additionally rejects compressed auxiliary-line layouts and star labels or aspect ratios that are hard to read. The fifth angle group publishes 9 source structures after 13,500 independent generations, corrects Example 5-2 from the handwritten `130°` to `110°`, adds explicit right-angle and fold/reflection marks where the source drawing was incomplete, and keeps Example 5-1 and Mission 4 locked because their stated markings do not force one answer. The sixth angle group publishes 10 source structures after 15,000 independent generations and checks all 720 whole-minute clock times for numberless-clock direction. It keeps Example 6-4 locked because the stated night condition conflicts with the official `1:30`, and redraws Mission 6's overlapped `70°` label so it cannot be read as `10°`. Multiplication exploration group 1 publishes all 11 source structures after 16,500 independent generations, corrects the handwritten packaging result from `4295` to `4290`, subtracts the truck length to obtain the physical tunnel length `622m` instead of `638m`, and rejects the answer PDF's different-edition tape Mission 3 in favor of the source rope result `97m 82cm`. Division exploration group 3 publishes all 11 source structures after 16,500 independent generations, corrects the handwritten discarded-pear answer from `501` to `21` and the Mission 5 bag answer from `4250g` to `450g`, and exhaustively checks discarded-item, date, supplement, and bag-weight candidates. Division exploration group 4 publishes all 11 source structures after 16,500 independent generations, replaces the generic Mission 6 with the source's 13 consecutive three-digit numbers, and exhaustively checks torn-division candidates, digit-length ranges, number-card arrangements, equal quotient-and-remainder candidates, and consecutive-number sums. Desktop, mobile, and print views passed diagram, overlap, and overflow checks. In addition to answer checks, the bank rejects visible square-root/combinatorics wording that does not fit the elementary explanation policy, lower-unit zero labels such as `4cm 0mm`, raw SVG fractions, and long floating-point tails such as `31.400000000000002`.
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 728 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks all 728 public runtime types across 100 seeds per difficulty, including all 191 public 4-1 mappings
- Numeric display policy: `numeric-display-audit.js` checks all 728 public runtime types across 100 seeds per difficulty; the runtime sweep separately checks availability
