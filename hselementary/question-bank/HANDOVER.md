...
- Implemented types: 665 (deterministic runtime availability check)
- Review-locked types: 249 (201 source-inventoried 4-1 items plus 48 source-complex 4-2 items)
...
The current ready set has passed its unit-specific regression coverage. `runtime-availability-audit.js` generates every one of the 665 runtime-ready types at all three difficulty offsets across 20 seeds each. The legacy generator-family audits remain available; 4-1 exposes 119 source-native variants across all six large-number exploration groups, the first four complete angle groups, and 9 safe items from angle group 5, plus 9 other exact source matches. Graph audits reverse-check values against SVG coordinates; geometry audits enumerate answer candidates and enforce one visible, inferable answer. Graph and diagram units additionally receive desktop and mobile (375px) checks for overflow, missing questions, missing solutions, accidental lock states, and graph-label overlap.

The full regression suite has 55 dedicated audits. The source-item taxonomy gate requires each mapped exploration, example, and Mission problem to have its own source ID and page locator. The 2026-08-27 run passed the 4-1 inventory, child-readable Korean type-language, all six source-native large-number groups, the first four complete angle groups, the 9 safe items in angle group 5, crosswalk, runtime taxonomy, and availability gates together with the existing 4-2 and shared audits. Exploration group 4 corrects the answer sheet's `11cm` unit error to the independently calculated `11km`. Exploration group 5 independently corrects the nearest-number answer and uses the source Mission 2 instead of the different edition shown in the answer PDF; it also fixes Example 5-3 at 72 cases and interprets Mission 6 as matching the counted number's three respective place digits `5, 8, 4`. Exploration group 6 corrects answer-sheet errors in Example 6-3 and 6-4 and Mission 1, 2, and 6; the 180-degree card calculation uses the source's actual second-smallest number `10258`, producing `15057`. The first angle group rebuilds two visually ambiguous ray diagrams with exact safe coordinates, calculates 30 and 42 vertical-angle pairs from line pairs, and excludes always-true statement ⑦ from the Mission 3 false-statement answer. The second angle group preserves all 11 source structures, including the shifted equilateral-triangle result `90°`, the three reflex-angle answers `100°`, `150°`, and `230°`, and the six Mission answers `111°`, `36°`, `52°`, `80°`, `40°`, and `55°`. The third angle group independently corrects source-answer conflicts at `1800°`, `263°`, `1620°`, and `50°`, and verifies the remaining source answers including Mission 6 at `400°`. The fourth angle group verifies 11 exterior-angle and star-geometry source structures, including `52°`, `65°`, `360°`, `18°`, `180°`, `27°`, `53°`, `63°`, `95°`, `82°`, and `540°`; it additionally rejects compressed auxiliary-line layouts and star labels or aspect ratios that are hard to read. The fifth angle group publishes 9 source structures after 13,500 independent generations, corrects Example 5-2 from the handwritten `130°` to `110°`, adds explicit right-angle and fold/reflection marks where the source drawing was incomplete, and keeps Example 5-1 and Mission 4 locked because their stated markings do not force one answer. Desktop, mobile, and print views passed diagram, overlap, and overflow checks. In addition to answer checks, the bank rejects visible square-root/combinatorics wording that does not fit the elementary explanation policy, lower-unit zero labels such as `4cm 0mm`, raw SVG fractions, and long floating-point tails such as `31.400000000000002`.
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 665 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks the generator-family catalog across 100 seeds per difficulty; the 128 public 4-1 mappings are a verified subset
