...
- Total runtime types: 949 across 6 semesters, 36 major units, and 174 subunits
- Implemented types: 890 (deterministic runtime availability check)
- Review-locked types: 59
- Exact source-item mapping: 492 items: all 329 items in 4-1 plus 66 fraction, 44 triangle, 44 decimal, and 9 quadrilateral items in 4-2. Do not describe the remaining entries as original problem items until every exploration, example, and Mission problem has a unique source locator.
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 890 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks all 890 public runtime types across 100 seeds per difficulty, including the source-mapped public types
- Numeric display policy: `numeric-display-audit.js` checks all 890 public runtime types across 100 seeds per difficulty; the runtime sweep separately checks availability
