...
- Implemented types: 736 (deterministic runtime availability check)
- Review-locked types: 178 (130 source-inventoried 4-1 items plus 48 source-complex 4-2 items)
...
- Grade 4-1 unit 4: 44 source items, 19 ready and 25 review-locked; exploration group 1 publishes 10 source-native items and keeps ambiguous Mission 1 locked, exploration group 2 publishes 8 items and locks 3 ambiguous or contradictory items, plus 1 earlier exact-match generator in group 4
...
The current ready set has passed its unit-specific regression coverage. `runtime-availability-audit.js` generates all 736 runtime-ready types at three difficulty offsets across 20 seeds. The source-native mapping has 199 items. Plane-transformation group 2 adds 8 independent source structures and keeps 3 items locked: Example 2-2 conflicts with its answer drawing, while Example 2-3 and Mission 2 do not force one answer. Its dedicated audit runs 12,000 independent generations and checks visible SVG cells, unique choices, cycles, reverse operations, order changes, and `1+4+5+6+9=25`.

The full regression suite has 62 dedicated audits. The 2026-08-28 run passed all 62, including the new plane-transformation group 2 source audit, 4-1 inventory, child-readable Korean type-language, runtime taxonomy, availability, elementary-language, numeric-display, math-notation, and the existing shared audits. Separate browser QA passed all 32 desktop/mobile problem-and-solution states, all 8 A4 PDFs and their 13 physical pages, and direct URL checks for all 3 locked types. The exploration print uses one complete question per A4 page. Evidence is stored at `E:\Codex\visualizations\2026\08\28\hse-4-1-u4-e2-qa-final-luna\REPORT.md`.
...
- Runtime availability policy: `runtime-availability-audit.js` checks all 736 public types across 20 seeds per difficulty
...
- Elementary explanation policy: `elementary-language-audit.js` checks all 736 public runtime types across 100 seeds per difficulty, including all 199 public 4-1 mappings
- Numeric display policy: `numeric-display-audit.js` checks all 736 public runtime types across 100 seeds per difficulty; the runtime sweep separately checks availability
