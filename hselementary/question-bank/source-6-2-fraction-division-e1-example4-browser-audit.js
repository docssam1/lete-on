...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e1-example4-browser-audit");
const sourceItemId = "6-2-u1-e1-example-4";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["3/5:9/5:9/20", "5/2:15/4:2/1", "9/8:9/4:3/4"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspectView(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ isAnswer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: isAnswer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-system-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-equation-system");
        const solution = item.querySelector(".source62-system-solution");
        const evidence = item.querySelector("[data-source62-fraction-e1-example4-kind]");
        ...
        return {
          ...
          system: board?.dataset.source62E1Example4System || "",
          structure: board?.dataset.source62E1Example4Structure || "",
          equations: board?.querySelectorAll(".source62-system-equation").length || 0,
          equationOverflow: [...(board?.querySelectorAll(".source62-system-equation") || [])].some(row => row.scrollWidth > row.clientWidth + 1),
          ...
          kind: evidence?.dataset.source62FractionE1Example4Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          values: evidence?.dataset.values || "",
          answerSource: item.querySelector(".source62-e1-example4-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || item.itemClipped || !item.boardVisible || item.boardOutside || item.boardClipped || item.boardWidth < 250 || item.equationOverflow) fail(`${label} ${index + 1}: 문항이나 세 식이 비었거나 화면 밖·잘림 상태입니다.`);
    if (item.structure !== "three-fraction-equations" || !item.system || item.equations !== 3) fail(`${label} ${index + 1}: 원본의 세 식 구조가 다릅니다.`);
    ...
    if (item.source !== state.sourceId || item.kind !== "three-equation-fraction-system" || item.difficulty !== difficultyNames[String(difficulty)]) fail(`${label} ${index + 1}: 원문·유형·난이도 연결이 다릅니다.`);
    if (item.rawFraction || item.invalid || item.mathFractions < (answerView ? 7 : 3)) fail(`${label} ${index + 1}: 가로 분수·깨진 값이 있거나 세로 분수 표시가 부족합니다.`);
    if (answerView) {
      const values = item.values.split(",").map(Number);
      const answerSignature = `${values[8]}/${values[9]}:${values[10]}/${values[11]}:${values[12]}/${values[13]}`;
      if (!item.solutionVisible || item.solutionOutside || item.solutionClipped || item.solutionRows !== 3 || item.answerSource !== state.sourceId || !expectedAnswers.has(answerSignature)) fail(`${label} ${index + 1}: 답 풀이표의 배치·원문 연결·세 값이 다릅니다.`);
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.system !== answer.items[index]?.system) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 세 식 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 1-4 3난이도 PC/모바일 문제·답, 세 식·세로 분수·넘침·답 누출 검사; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 1-4 브라우저·인쇄 감사 통과: 3난이도×PC/모바일×문제/답 · 화면 12장 · A4 2개 전 ${renderedPages}쪽`);
})().catch(error => {
