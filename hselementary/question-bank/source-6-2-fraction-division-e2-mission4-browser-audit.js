...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-mission4-browser-audit");
const sourceItemId = "6-2-u1-e2-mission-4";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["3:4:9:4:6:30", { days: "30", perDay: "3", total: "90", answer: "90개" }],
  ["2:3:8:3:4:30", { days: "30", perDay: "4", total: "120", answer: "120개" }],
  ["5:6:10:3:2:28", { days: "28", perDay: "4", total: "112", answer: "112개" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-doll-production-solution,#problemView .source62-doll-production-board.is-solved").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-doll-production-board");
        const evidence = item.querySelector("[data-source62-fraction-e2-mission4-kind]");
        const facts = item.querySelector(".source62-doll-production-facts");
        const flow = item.querySelector(".source62-doll-production-flow");
        const boardBox = board?.getBoundingClientRect();
        const clippedDescendants = boardBox ? [...board.querySelectorAll(":scope .source62-doll-production-facts,:scope .source62-doll-production-facts > span,:scope .source62-doll-production-flow,:scope .source62-doll-production-flow > span")].filter(element => {
          const box = element.getBoundingClientRect();
        ...
        return {
          ...
          factsOverflow: facts ? facts.scrollWidth > facts.clientWidth + 1 : true,
          flowOverflow: flow ? flow.scrollWidth > flow.clientWidth + 1 : true,
          clippedDescendants,
          structure: board?.dataset.source62E2Mission4Structure || "",
          expression: board?.dataset.source62E2Mission4Expression || "",
          unitTime: board?.dataset.unitTime || "",
          dailyWork: board?.dataset.dailyWork || "",
          monthDays: board?.dataset.monthDays || "",
          perDay: board?.dataset.dollsPerDay || "",
          total: board?.dataset.totalDolls || "",
          factCount: item.querySelectorAll(".source62-doll-production-facts > span").length,
          flowStates: item.querySelectorAll(".source62-doll-production-flow > span").length,
          operators: item.querySelectorAll(".source62-doll-production-flow > i").length,
          solvedCount: item.querySelectorAll(".source62-doll-production-board.is-solved").length,
          solutionRows: item.querySelectorAll(".source62-doll-production-solution .source61-math-row").length,
          ...
          kind: evidence?.dataset.source62FractionE2Mission4Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e2-mission4-answer")?.dataset.answerSource || "",
          text,
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.factsOverflow || item.flowOverflow || item.clippedDescendants) fail(`${label}/${index + 1}: 문항이나 제작 일정판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "daily-rate-month-total" || !row || item.monthDays !== row?.days || item.perDay !== row?.perDay || item.total !== row?.total || !item.unitTime || !item.dailyWork) fail(`${label}/${index + 1}: 제작 시간과 달력의 같은 자료 모델이 빠졌습니다.`);
    if (item.factCount !== 3 || item.flowStates !== 3 || item.operators !== 2 || item.fractionCount < 2) fail(`${label}/${index + 1}: 한 개 제작 시간, 하루 작업, 달의 날 수 또는 계산 흐름이 다릅니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 제작 일정판의 공통 글꼴이 아닙니다.`);
    if (item.source !== sourceItemId || item.kind !== "daily-rate-month-total" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 후보·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionRows !== 3 || item.answerSource !== sourceItemId || !item.text.includes(row.answer) || !item.text.includes(`${row.perDay}개`)) fail(`${label}/${index + 1}: 답 자료판의 하루와 한 달 생산량, 세 계산 단계가 다릅니다.`);
    } else if (item.solvedCount || item.solutionRows || item.answerSource || state.answerLeak || item.text.includes(row.answer) || item.text.includes(`${row.perDay}개`)) fail(`${label}/${index + 1}: 문제에 하루 또는 한 달 생산량이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 제작 일정 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 4 제작 시간·하루 생산량·달력·답 누출·분수·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
