...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-example1-browser-audit");
const sourceItemId = "6-2-u1-e2-example-1";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedAnswers = new Set(["28/5", "5", "7"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(({ answer, sourceId }) => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-area-solution,#problemView .source62-area-target.is-solved").length,
      items: items.map(item => {
        const diagram = item.querySelector(".source62-area-segment");
        const evidence = item.querySelector("[data-source62-fraction-e2-example1-kind]");
        const values = (evidence?.dataset.values || "").split(",").map(Number);
        const answerValue = values.length === 10 ? `${values[8]}${values[9] === 1 ? "" : `/${values[9]}`}` : "";
        const pointLabels = [...item.querySelectorAll(".source62-area-point")].map(label => label.textContent.trim());
        const measureRoles = [...item.querySelectorAll("[data-measure-role]")].map(label => label.dataset.measureRole);
        return {
          itemVisible: visible(item),
          itemOutside: outside(item),
          diagramVisible: visible(diagram),
          diagramOutside: outside(diagram),
          diagramWidth: diagram?.getBoundingClientRect().width || 0,
          diagramOverflow: diagram ? diagram.scrollWidth > diagram.clientWidth + 1 : true,
          structure: diagram?.dataset.source62E2Example1Structure || "",
          expression: diagram?.dataset.source62E2Example1Expression || "",
          points: diagram?.dataset.geometryPoints || "",
          segments: diagram?.dataset.geometrySegments || "",
          targetLength: diagram?.dataset.targetLength || "",
          pointLabels,
          measureRoles,
          mathFractionCount: item.querySelectorAll(".math-fraction,.svg-measure-fraction").length,
          targetQuestion: item.querySelectorAll(".source62-area-target-question").length,
          targetSolved: item.querySelectorAll(".source62-area-target.is-solved").length,
          triangleSolved: item.querySelectorAll(".source62-area-triangle.is-solved").length,
          solutionRows: item.querySelectorAll(".source62-area-solution .source61-math-row").length,
          font: diagram ? getComputedStyle(diagram.querySelector("text") || diagram).fontFamily : "",
          source: evidence?.dataset.sourceItem || "",
          kind: evidence?.dataset.source62FractionE2Example1Kind || "",
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerValue,
          answerSource: item.querySelector(".source62-e2-example1-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.diagramVisible || item.diagramOutside || item.diagramWidth < 250 || item.diagramOverflow) fail(`${label}/${index + 1}: 문항이나 도형이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "rectangle-triangle-area-segment" || !item.expression || !item.points || item.segments !== "0-1:top;1-2:target;2-3:right-lower;3-4:bottom;4-0:left;4-2:diagonal" || !item.targetLength) fail(`${label}/${index + 1}: 점·선분 모델이 원본 구조와 다릅니다.`);
    if (item.pointLabels.join(",") !== "ㄱ,ㄹ,ㅁ,ㄷ,ㄴ" || item.measureRoles.join(",") !== "width,total-height,target" || item.mathFractionCount < 1) fail(`${label}/${index + 1}: 꼭짓점 또는 가로·높이·분수 표기가 빠졌습니다.`);
    if (!item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 도형 길이의 공통 글꼴이 아닙니다.`);
    if (item.source !== state.sourceId || item.kind !== "rectangle-triangle-area-segment" || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·유형·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.targetQuestion || item.targetSolved !== 1 || item.triangleSolved !== 1 || item.solutionRows !== 2 || item.answerSource !== state.sourceId || !expectedAnswers.has(item.answerValue)) fail(`${label}/${index + 1}: 답 그림의 선분 강조·두 계산 단계가 다릅니다.`);
    } else if (item.targetQuestion !== 1 || item.targetSolved || item.triangleSolved || item.solutionRows || state.answerLeak) fail(`${label}/${index + 1}: 문제에 답 길이나 풀이가 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) {
      if (problem.items[index]?.expression !== answer.items[index]?.expression || problem.items[index]?.points !== answer.items[index]?.points || problem.items[index]?.segments !== answer.items[index]?.segments) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 점·선분 자료가 다릅니다.`);
    }
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 2-1 점·선분 모델·분수 길이·넓이·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 2-1 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
