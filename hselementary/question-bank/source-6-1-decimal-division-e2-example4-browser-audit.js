...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(__dirname, "tmp", "decimal-division-e2-example4-browser-audit");
...
function buildGenerator() {
  ...
  delete require.cache[require.resolve("./source-grade6-decimal-e2-example4.js")];
...
function inspectContract(generated, poolIndex, difficulty) {
  ...
  const values = attr(problemSvg, "data-source61-e2ex4-values").split(",").map(Number);
  ...
  if (values.length !== 9 || values.some((value, index) => !close(value, expectedPools[poolIndex][index])) || attr(problemSvg, "data-source61-e2ex4-values") !== attr(answerSvg, "data-source61-e2ex4-values")) fail(`${label}: 문제·답 값 자료가 고정 문항과 다릅니다.`);
  if (attr(problemSvg, "data-source61-e2ex4-points") !== attr(answerSvg, "data-source61-e2ex4-points")) fail(`${label}: 문제와 답의 점 좌표가 다릅니다.`);
  const points = parsePoints(attr(problemSvg, "data-source61-e2ex4-points"));
  if (!points.IL || !points.IR || !points.TL || !points.TA || !points.TR || !points.FL || !points.FR || !points.OT) fail(`${label}: 이동 전후 필수 점이 없습니다.`);
  else {
    if (!(points.IL.x < points.IR.x && points.IR.x < points.TL.x && points.TL.x < points.TA.x && points.TA.x < points.TR.x)) fail(`${label}: 처음 직사각형·간격·삼각형의 좌우 순서가 다릅니다.`);
    if (!close(points.TA.x, (points.TL.x + points.TR.x) / 2) || !close(points.IL.y, points.IR.y) || !close(points.TL.y, points.TR.y)) fail(`${label}: 직각이등변삼각형 또는 기준선 좌표가 다릅니다.`);
    if (!close(points.FR.x, points.OT.x) || !close(points.FR.y, points.TL.y) || !(points.OT.y < points.FR.y)) fail(`${label}: 이동 뒤 직사각형 오른쪽 변과 겹친 삼각형이 다릅니다.`);
  }
  if (problemSvg.includes("source61-e2ex4-overlap") || problemSvg.includes("is-final") || !answerSvg.includes("source61-e2ex4-overlap is-solved") || !answerSvg.includes("source61-e2ex4-rectangle is-final")) fail(`${label}: 문제와 답의 이동·겹침 표시가 분리되지 않았습니다.`);
...
async function inspectMarkup(page, generated, view, viewportName, difficulty, poolIndex) {
  ...
  const state = await page.evaluate(() => {
    ...
    const svg = document.querySelector("svg.source61-e2ex4-diagram");
    ...
    return {
      ...
      texts, overlaps,
      phase: svg?.dataset.source61E2ex4Phase || "",
      model: svg?.dataset.source61E2ex4Model || "",
      points: svg?.dataset.source61E2ex4Points || "",
      pool: svg?.dataset.pool || "",
      rectangleCount: svg?.querySelectorAll(".source61-e2ex4-rectangle").length || 0,
      triangleCount: svg?.querySelectorAll(".source61-e2ex4-triangle").length || 0,
      rightAngleCount: svg?.querySelectorAll(".source61-e2ex4-right-angle").length || 0,
      overlapCount: svg?.querySelectorAll(".source61-e2ex4-overlap.is-solved").length || 0,
      finalCount: svg?.querySelectorAll(".source61-e2ex4-rectangle.is-final").length || 0,
      resultCount: svg?.querySelectorAll(".source61-e2ex4-result").length || 0,
  ...
  if (state.model !== "moving-rectangle-right-isosceles-overlap" || state.triangleCount !== 1 || state.rightAngleCount !== 1) fail(`${label}: 직사각형·직각이등변삼각형 구조가 없습니다.`);
  ...
  if (view === "problem" && (state.phase !== "problem" || state.pool !== String(poolIndex) || state.rectangleCount !== 1 || state.overlapCount || state.finalCount || state.resultCount || state.answerSource)) fail(`${label}: 문제 화면에 답 전용 이동·겹침 표시가 노출됩니다.`);
  if (view === "answer" && (state.phase !== "answer" || state.pool !== String(poolIndex) || state.rectangleCount !== 2 || state.overlapCount !== 1 || state.finalCount !== 1 || state.resultCount !== 1 || state.answerSource !== sourceItemId)) fail(`${label}: 답 화면의 이동 뒤 직사각형·겹침·결과 표시가 다릅니다.`);
  return state;
}
...
async function inspectActualRoute(browser, baseUrl, viewportName, viewport) {
  ...
  try {
    ...
    const check = async view => page.evaluate(expectedView => {
      ...
      const diagrams = [...(active?.querySelectorAll("svg.source61-e2ex4-diagram") || [])];
      return {
        ...
        sourceCount: isProblem ? diagrams.filter(svg => svg.dataset.source61E2ex4Phase === "problem").length : [...active.querySelectorAll("[data-answer-source]")].filter(node => node.dataset.answerSource === "6-1-u3-e2-example-4").length,
        solvedCount: active?.querySelectorAll(".source61-e2ex4-overlap.is-solved").length || 0,
        overflow: document.documentElement.scrollWidth > innerWidth + 2,
...
(async () => {
  ...
  const summary = `${failures.length ? "실패" : "통과"}: 6-1-u3-e2-example-4 3문항×3난이도×PC1440/mobile390 문제·답, 이동 전후 점·선분·직각 표시·교집합·문제/답 좌표 동일·15px 글꼴·겹침/잘림·실제 앱 3문항 3쪽·A4 검사; 화면 ${screenshots}장, A4 ${pdfs}개, 렌더 ${renderedPdfPages}쪽\n${failures.join("\n")}\n`;
