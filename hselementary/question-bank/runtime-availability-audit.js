...
if (types.length !== 964) failures.push(`런타임 유형은 964개여야 하나 ${types.length}개입니다.`);
if (ready.length !== 894) failures.push(`생성 가능 유형은 894개여야 하나 ${ready.length}개입니다.`);
if (locked.length !== 70) failures.push(`검수 대기 유형은 70개여야 하나 ${locked.length}개입니다.`);
...
console.log(`런타임 생성 가능성 감사 통과: 전체 964 · 생성 가능 894 · 검수 대기 70 · ${generatedCount.toLocaleString()}회 생성`);
