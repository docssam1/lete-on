...
if (ready.length !== 1383) failures.push(`생성 가능 유형은 1383개여야 하나 ${ready.length}개입니다.`);
if (locked.length !== 510) failures.push(`검수 대기 유형은 510개여야 하나 ${locked.length}개입니다.`);
if (sourceGrade6.length !== 633) failures.push(`6학년 원문 세부 유형은 633개여야 하나 ${sourceGrade6.length}개입니다.`);
if (sourceGrade6.filter(type => !type.reviewLocked).length !== 214 || sourceGrade6.filter(type => type.reviewLocked).length !== 419) failures.push("6학년 원문 세부 유형의 생성 가능·잠금 수가 다릅니다.");
...
console.log(`런타임 생성 가능성 감사 통과: 전체 1893 · 생성 가능 1383 · 검수 대기 510(6학년 원문 419 포함) · ${generatedCount.toLocaleString()}회 생성`);
