...
if (ready.length !== 1392) failures.push(`생성 가능 유형은 1392개여야 하나 ${ready.length}개입니다.`);
...
if (sourceGrade6.filter(type => !type.reviewLocked).length !== 223 || sourceGrade6.filter(type => type.reviewLocked).length !== 410) failures.push("6학년 원문 세부 유형의 생성 가능·잠금 수가 다릅니다.");
