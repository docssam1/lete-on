...
if (ready.length !== 1439) failures.push(`생성 가능 유형은 1439개여야 하나 ${ready.length}개입니다.`);
if (locked.length !== 454) failures.push(`검수 대기 유형은 454개여야 하나 ${locked.length}개입니다.`);
if (sourceGrade6.length !== 633) failures.push(`6학년 원문 세부 유형은 633개여야 하나 ${sourceGrade6.length}개입니다.`);
if (sourceGrade6.filter(type => !type.reviewLocked).length !== 270 || sourceGrade6.filter(type => type.reviewLocked).length !== 363) failures.push("6학년 원문 세부 유형의 생성 가능·잠금 수가 다릅니다.");
