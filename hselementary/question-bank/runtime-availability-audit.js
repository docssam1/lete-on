...
if (types.length !== 946) failures.push(`런타임 유형은 946개여야 하나 ${types.length}개입니다.`);
if (ready.length !== 887) failures.push(`생성 가능 유형은 887개여야 하나 ${ready.length}개입니다.`);
if (locked.length !== 59) failures.push(`검수 대기 유형은 59개여야 하나 ${locked.length}개입니다.`);
...
console.log(`런타임 생성 가능성 감사 통과: 전체 946 · 생성 가능 887 · 검수 대기 59 · ${generatedCount.toLocaleString()}회 생성`);
