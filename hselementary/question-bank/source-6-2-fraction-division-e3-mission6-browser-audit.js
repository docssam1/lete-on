...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e3-mission6-browser-audit");
const sourceItemId = "6-2-u1-e3-mission-6";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["2", { pairs: "4:2|6:3|8:2|8:4", products: "8:18:16:32", count: 4, answer: "74" }],
  ["3", { pairs: "6:2|9:3", products: "12:27", count: 2, answer: "39" }],
  ["8", { pairs: "8:2", products: "16", count: 1, answer: "16" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 12000) fail(`${label}: ${name}가 비었거나 세 분수·순서쌍·곱의 합이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      items: items.map(item => {
        ...
        const inlineExpressions = [...item.querySelectorAll(".math-inline-expression")];
        const evidence = item.querySelector("[data-source62-fraction-e3-mission6-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E3Mission6Structure || "",
          expression: board?.dataset.source62E3Mission6Expression || "",
          checkedCount: Number(board?.dataset.checkedPairCount || 0),
          pairCount: Number(board?.dataset.pairCount || 0),
          pairs: board?.dataset.pairs || "",
          products: board?.dataset.products || "",
          answer: board?.dataset.answerSum || "",
          ...
          inlineExpressionCount: inlineExpressions.length,
          brokenInlineExpressions: inlineExpressions.filter(expression => getComputedStyle(expression).display !== "inline-flex" || getComputedStyle(expression).whiteSpace !== "nowrap" || expression.getClientRects().length !== 1).length,
          ...
          kind: evidence?.dataset.source62FractionE3Mission6Kind || "",
          ...
          evidenceProducts: evidence?.dataset.products || "",
          evidenceAnswer: evidence?.dataset.answer || "",
          answerSource: item.querySelector(".source62-e3-mission6-answer")?.dataset.answerSource || "",
          text,
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (item.structure !== "one-digit-fraction-expression-product-sum" || !row || item.checkedCount !== 64 || item.pairCount !== row?.count || item.pairs !== row?.pairs || item.products !== row?.products || item.answer !== row?.answer || item.evidencePairs !== row?.pairs || item.evidenceProducts !== row?.products || item.evidenceAnswer !== row?.answer) fail(`${label}/${index + 1}: 문제와 답의 64가지 확인 자료·순서쌍·곱의 합이 다릅니다.`);
    if (item.conditionCount !== 2 || item.equationCount !== 1 || item.fractionCount < 3 || item.thinBars) fail(`${label}/${index + 1}: 원문 세 분수·한 자리 수·자연수 조건 또는 분수선이 빠졌습니다.`);
    if (item.inlineExpressionCount !== (answerView ? 1 : 2) || item.brokenInlineExpressions) fail(`${label}/${index + 1}: 세 분수로 된 식이 한 덩어리로 표시되지 않았습니다.`);
    ...
    if (item.source !== sourceItemId || item.kind !== "one-digit-fraction-expression-product-sum" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.pairCells !== row.count || item.solutionRows !== 3 || item.answerSource !== sourceItemId || !item.text.includes(row.answer)) fail(`${label}/${index + 1}: 답의 전체 순서쌍·각 곱·합과 세 확인 단계가 다릅니다.`);
    } else if (item.solvedCount || item.pairCells || item.solutionRows || item.answerSource || state.answerLeak || item.text.includes(row.answer)) fail(`${label}/${index + 1}: 문제에 조건에 맞는 순서쌍이나 답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.expression !== answer.items[index]?.expression || problem.items[index]?.pairs !== answer.items[index]?.pairs || problem.items[index]?.products !== answer.items[index]?.products || problem.items[index]?.answer !== answer.items[index]?.answer) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 식·순서쌍·곱의 합 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 6 원문 세 분수·한 자리 수 조건·64가지 순서쌍·각 곱·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
