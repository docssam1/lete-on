...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e4-example2-browser-audit");
const sourceItemId = "6-2-u1-e4-example-2";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expected = new Map([
  ["4:1:29:8:13:6:26:1", { referenceArea: "29:2", answerData: "174:1", answer: "174m²" }],
  ["3:1:7:4:7:6:8:1", { referenceArea: "21:4", answerData: "36:1", answer: "36m²" }],
  ["5:1:11:6:11:9:8:1", { referenceArea: "55:6", answerData: "60:1", answer: "60m²" }]
]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
function renderPdf(pdf, label) {
  ...
  for (const name of rendered) {
    ...
    if (fs.statSync(file).size < 20000 || inkPixels(file) < 10000) fail(`${label}: ${name}가 비었거나 벽 넓이·페인트 계산이 충분히 인쇄되지 않았습니다.`);
  }
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-paint-area-board.is-solved,#problemView .source62-paint-area-solution").length,
      items: items.map(item => {
        const board = item.querySelector(".source62-paint-area-board");
        ...
        const evidence = item.querySelector("[data-source62-fraction-e4-example2-kind]");
        ...
        return {
          ...
          structure: board?.dataset.source62E4Example2Structure || "",
          values: board?.dataset.source62E4Example2Values || "",
          referenceArea: board?.dataset.referenceArea || "",
          answerData: board?.dataset.targetArea || "",
          rowCount: item.querySelectorAll(".source61-math-row").length,
          solvedCount: item.querySelectorAll(".source62-paint-area-board.is-solved").length,
          solutionCount: item.querySelectorAll(".source62-paint-area-solution").length,
          ...
          powerCount: item.querySelectorAll("sup,.math-power").length,
          source: evidence?.dataset.sourceItem || "",
          kind: evidence?.dataset.source62FractionE4Example2Kind || "",
          candidateCount: Number(evidence?.dataset.candidateCount || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e4-example2-answer")?.dataset.answerSource || "",
  ...
  for (const [index, item] of state.items.entries()) {
    ...
    if (!item.itemVisible || item.itemOutside || !item.boardVisible || item.boardOutside || item.boardWidth < 280 || item.boardOverflow || item.clippedRows) fail(`${label}/${index + 1}: 벽 넓이·페인트 계산판이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "paint-area-unit-rate" || !expectedItem || item.referenceArea !== expectedItem?.referenceArea || item.answerData !== expectedItem?.answerData) fail(`${label}/${index + 1}: 문제와 답의 벽 길이·페인트 양·넓이 자료가 다릅니다.`);
    if (item.fractionCount < 4 || item.thinBars || item.expressionCount < 8 || item.brokenExpressions || item.unitCount < 8 || item.powerCount < 1) fail(`${label}/${index + 1}: 세로 분수·넓이 단위 또는 줄바꿈되지 않는 수식 묶음이 빠졌습니다. (분수 ${item.fractionCount}, 얇은 선 ${item.thinBars}, 수식 ${item.expressionCount}, 깨진 수식 ${item.brokenExpressions}, 단위 ${item.unitCount}, 위첨자 ${item.powerCount})`);
    ...
    if (item.source !== sourceItemId || item.kind !== "paint-area-unit-rate" || item.candidateCount !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 답·난이도 또는 분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionCount !== 1 || item.rowCount !== 10 || item.answerSource !== sourceItemId || item.fractionCount < 10 || item.unitCount < 10) fail(`${label}/${index + 1}: 답의 기준 벽 넓이·1L당 넓이·페인트 양의 비·목표 넓이 계산이 없습니다.`);
    } else if (item.solvedCount || item.solutionCount || item.rowCount !== 4 || item.answerSource || state.answerLeak || item.text.includes(expectedItem.answer) || !item.text.includes("같은 방법")) fail(`${label}/${index + 1}: 문제 조건이 빠졌거나 풀이·답이 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) if (problem.items[index]?.values !== answer.items[index]?.values || problem.items[index]?.answerData !== answer.items[index]?.answerData) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 벽·페인트 자료가 다릅니다.`);
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: 예제 4-2 원문 벽·페인트·세로 분수·넓이 단위·독립 계산·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
  if (failures.length) throw new Error([...new Set(failures)].join("\n"));
  console.log(`6-2 예제 4-2 브라우저·인쇄 감사 통과: 화면 ${screenshots}장 · A4 ${pdfs}개 전 ${renderedPages}쪽`);
})().catch(error => {
