...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(questionBankDir, "tmp", "source-6-2-fraction-division-e2-mission1-browser-audit");
const sourceItemId = "6-2-u1-e2-mission-1";
const difficultyNames = { "-1": "guided", "0": "source", "1": "independent-reasoning" };
const expectedSignatures = new Set(["16:5:1:4:4:9", "15:4:3:8:5:12", "14:3:2:7:7:12"]);
const failures = [];
let screenshots = 0;
let pdfs = 0;
let renderedPages = 0;
...
async function inspect(page, difficulty, answerView, viewportLabel) {
  const state = await page.evaluate(answer => {
    ...
    return {
      ...
      answerLeak: answer ? 0 : document.querySelectorAll("#problemView [data-answer-source],#problemView .source62-overlap-area-solution,#problemView .source62-overlap-area.is-solved").length,
      items: items.map(item => {
        const diagram = item.querySelector(".source62-overlap-area");
        const evidence = item.querySelector("[data-source62-fraction-e2-mission1-kind]");
        const values = (evidence?.dataset.values || "").split(",").map(Number);
        const viewBox = diagram?.viewBox?.baseVal;
        const clippedSvgChildren = diagram && viewBox ? [...diagram.querySelectorAll("polygon,text")].filter(element => {
        ...
        return {
          ...
          structure: diagram?.dataset.source62E2Mission1Structure || "",
          expression: diagram?.dataset.source62E2Mission1Expression || "",
          rectanglePoints: diagram?.dataset.rectanglePoints || "",
          trianglePoints: diagram?.dataset.trianglePoints || "",
          overlapPoints: diagram?.dataset.overlapPoints || "",
          overlapVertices: Number(diagram?.dataset.overlapVertexCount || 0),
          polygons: diagram?.querySelectorAll("polygon").length || 0,
          labels: diagram?.querySelectorAll("text").length || 0,
          clippedSvgChildren,
          relationRows: item.querySelectorAll(".source62-overlap-relation > span").length,
          solutionRows: item.querySelectorAll(".source62-overlap-area-solution .source61-math-row").length,
          solvedCount: item.querySelectorAll(".source62-overlap-area.is-solved").length,
          fractionCount: item.querySelectorAll(".math-fraction").length,
          font: diagram ? getComputedStyle(diagram).fontFamily : "",
          source: evidence?.dataset.sourceItem || "",
          kind: evidence?.dataset.source62FractionE2Mission1Kind || "",
          contract: evidence?.dataset.resultContract || "",
          rectangleCandidates: Number(evidence?.dataset.rectangleCandidates || 0),
          triangleCandidates: Number(evidence?.dataset.triangleCandidates || 0),
          difficulty: evidence?.dataset.difficultyDesign || "",
          answerSource: item.querySelector(".source62-e2-mission1-answer")?.dataset.answerSource || "",
          values,
  ...
  for (const [index, item] of state.items.entries()) {
    if (!item.itemVisible || item.itemOutside || !item.diagramVisible || item.diagramOutside || item.diagramWidth < 280 || item.clippedSvgChildren) fail(`${label}/${index + 1}: 문항이나 겹친 도형이 화면 밖이거나 잘렸습니다.`);
    if (item.structure !== "rectangle-triangle-overlap" || !expectedSignatures.has(item.expression) || !item.rectanglePoints || !item.trianglePoints || !item.overlapPoints || item.overlapVertices < 3 || item.polygons !== 3 || item.labels !== 3) fail(`${label}/${index + 1}: 직사각형·삼각형·교집합 점 모델이 다릅니다.`);
    if (item.relationRows !== 3 || item.fractionCount < 6 || !item.font.includes("Pretendard") || !item.font.includes("Malgun Gothic") || !item.font.includes("Arial")) fail(`${label}/${index + 1}: 넓이 관계나 공통 글꼴·분수 표시가 다릅니다.`);
    if (item.source !== sourceItemId || item.kind !== "overlap-area-relations" || item.contract !== "single-value" || item.rectangleCandidates !== 1 || item.triangleCandidates !== 1 || item.difficulty !== difficultyNames[String(difficulty)] || item.values.length !== 12 || item.values.some(value => !Number.isFinite(value)) || item.rawFraction || item.invalid) fail(`${label}/${index + 1}: 원문·단일 후보·난이도·분수 표시가 다릅니다.`);
    if (answerView) {
      if (item.solvedCount !== 1 || item.solutionRows !== 3 || item.answerSource !== sourceItemId) fail(`${label}/${index + 1}: 답 그림의 색칠 강조와 세 계산 단계가 다릅니다.`);
    } else if (item.solvedCount || item.solutionRows || state.answerLeak) fail(`${label}/${index + 1}: 문제에 답 그림이나 풀이가 노출되었습니다.`);
  }
  return state;
}
...
async function inspectType(browser, baseUrl, difficulty, viewport, viewportLabel) {
  ...
  try {
    ...
    for (let index = 0; index < 3; index += 1) {
      ...
      if (left?.expression !== right?.expression || left?.rectanglePoints !== right?.rectanglePoints || left?.trianglePoints !== right?.trianglePoints || left?.overlapPoints !== right?.overlapPoints) fail(`${viewportLabel}/${difficulty}/${index + 1}: 문제와 답의 겹친 도형 자료가 다릅니다.`);
    }
...
(async () => {
  ...
  fs.writeFileSync(path.join(outputDir, "audit-result.txt"), `${failures.length ? "실패" : "통과"}: Mission 1 직사각형·삼각형·교집합·분수 관계·답 누출·넘침; 화면 ${screenshots}장, A4 ${pdfs}개 ${renderedPages}쪽\n${failures.join("\n")}\n`, "utf8");
