...
const outputDir = process.env.HSE_SCREENSHOT_DIR || path.join(os.tmpdir(), "lete-on-hse-5-1-factor-multiple-e5-browser-audit");
const failures = [];
let screenshots = 0;
let pdfs = 0;
...
async function review(browser, type, viewport, label, difficulty) {
  ...
  if (isBroken(state) || new Set(state.prompts).size !== state.prompts.length) fail(`${key}: PC·모바일 문제 화면이 비었거나 넘치거나 같은 문항이 반복됩니다.`);
...
(async () => {
  ...
  if (types.length !== 96 || ready.length !== 12 || locked.length !== 2) fail("E5 공개 12·잠금 2 구성이 다릅니다.");
  for (const type of ready) if (api.generatorKey(type) !== "factorMultipleE5") fail(`${type.id}: 공개 유형 생성기가 다릅니다.`);
  ...
  await browser.close();
  if (screenshots !== 146 || pdfs !== 72) fail(`검수 산출물 수가 다릅니다: 화면 ${screenshots}, A4 ${pdfs}`);
  if (failures.length) throw new Error(failures.join("\n"));
  console.log(`5-1 약수와 배수 개념탐구 5 브라우저 감사 통과: 공개 12/잠금 2 · PC·모바일 ${screenshots}화면 · A4 ${pdfs}파일 · ${outputDir}`);
})().catch(error => {
  console.error(`5-1 약수와 배수 개념탐구 5 브라우저 감사 실패: ${error.stack || error}`);
  process.exit(1);
});
