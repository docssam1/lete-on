(function () {
  ...
  const state = {tab:'grade',round:1,responses:new Map(),selected:{bank:new Set(),wrong:new Set()},query:'',area:'',section:'all',difficulty:'same',snapshot:null,report:null,preview:null,composing:false,busy:false,printRevision:0,approvedIdentity:null,localName:''};
  ...
  function changeTab(tab,focus=false){
    state.tab=tab;state.query='';state.area='';state.section='all';$('studioSearch').value='';$('sectionFilter').value='all';
    ...
    message('');if(tab==='grade')renderGrade();else{renderAreaFilter();renderBank();}
  }
  ...
  function baseRows(){return [...rows.values()].filter(row=>row.round===state.round&&(state.tab!=='wrong'||activeWrong(row)));}
  function renderAreaFilter(){
    const areas=new Map(baseRows().map(row=>[row.taxonomy.areaId,row.taxonomy.areaLabel]));
  ...
  function renderBank(){
    const wrong=state.tab==='wrong';$('bankTitle').textContent=wrong?'오답에서 다음 학습 고르기':'회차별 문제은행';$('bankScope').textContent=wrong?`${state.round}회에서 오답으로 입력한 본시험 문항만 표시합니다. 미입력·추가 연습은 포함하지 않습니다.`:`${state.round}회 원문의 조건과 그림을 확인한 뒤 유형을 고르세요.`;
    const visible=visibleRows();$('resultCount').textContent=`${visible.length}개 원문 유형`;
  ...
  function renderSelection(){
    ...
    $('selectionCount').textContent=`선택 ${selected.size}개 원문 유형${hidden?` · 현재 결과 밖 ${hidden}개`:''}`;
  ...
  async function showPreview(id){
    ...
    const requestedIdentity=verifiedIdentity();let question;try{question=await sourceFor(row);}catch(e){return message(e.message,true);}if(!sameIdentity(requestedIdentity,verifiedIdentity()))return;if(!question?.prompt)return message('원문을 불러오지 못했습니다.',true);
  ...
  function approvedWatermark(){const identity=window.HFChallengeAccess?.watermarkIdentity?.();return `G.FIELD  LETE-ON${identity?.name?'  '+identity.name:localPreview?'  교사용 검토':''}`;}
  ...
  async function buildPractice(){
    ...
    try{
      ...
      const snapshot=freeze(copy({kind:'practice',title:state.tab==='wrong'?'오답 유사문제 학습지':'유형별 유사문제 학습지',student:identity(),approvedIdentity:startingIdentity,difficulty,entries,createdAt:new Date().toISOString()}));
  ...
  function setFolios(){[...$('worksheetPages').children].forEach((element,index)=>{const footer=element.querySelector('.page-folio');if(footer)footer.textContent=String(index+1);if(!element.classList.contains('blank'))element.dataset.watermark=approvedWatermark();});}
  ...
  async function renderWorksheet(){
    ...
    document.body.dataset.printReady='false';$('printWorksheet').disabled=true;
    ...
    $('worksheetTitle').textContent=snapshot.title;$('worksheetMeta').textContent=`확정 ${entries.length}문항 · ${levelLabels[snapshot.difficulty]} · 현재 화면의 입력을 바꿔도 이 학습지는 유지됩니다.`;
    ...
    try{await waitImages($('worksheetPages'));if(revision!==state.printRevision||!snapshotAllowed(snapshot))return;document.body.dataset.printReady='true';$('printWorksheet').disabled=false;}catch(error){message(error.message+' 인쇄를 중단했습니다.',true);}
  }
  ...
  async function renderDiagnosisPrint(snapshot,revision){
    ...
    try{await waitImages($('worksheetPages'));if(revision!==state.printRevision||!sameIdentity(snapshot.approvedIdentity,verifiedIdentity()))return;document.body.dataset.printReady='true';$('printWorksheet').disabled=false;}catch(error){message(error.message,true);}
  }
  function ready(){
    ...
    const initialParams=new URLSearchParams(location.search),initialRound=Number(initialParams.get('round'));if([1,2,3,4].includes(initialRound)){state.round=initialRound;$('studioRound').value=String(initialRound);}updateExamLink();
    ...
    $('studioRound').addEventListener('change',()=>{state.round=Number($('studioRound').value);updateExamLink();state.area='';if(state.tab==='grade')renderGrade();else{renderAreaFilter();renderBank();}message('');});
    ...
    $('areaFilter').addEventListener('change',()=>{state.area=$('areaFilter').value;renderBank();});
    $('sectionFilter').addEventListener('change',()=>{state.section=$('sectionFilter').value;renderBank();});
    ...
    $('closeWorksheet').addEventListener('click',()=>{$('worksheetPanel').hidden=true;document.body.dataset.worksheetOpen='false';document.body.dataset.printReady='false';});
    $('printWorksheet').addEventListener('click',async()=>{
      await renderWorksheet();if(document.body.dataset.printReady!=='true')return;
      window.print();
    });
    window.addEventListener('hfchallengeaccesschange',()=>{
      ...
      if($('questionPreview').open)$('questionPreview').close();$('diagnosisPanel').hidden=true;$('worksheetPages').innerHTML='';$('worksheetPanel').hidden=true;document.body.dataset.printReady='false';
