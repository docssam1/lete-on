...
활용 1회 원본은 1~4쪽의 20문항과 5~7쪽의 번호별 풀이가 서로 대응하지 않는다. 따라서 뒤쪽 풀이와 영상 대본을 답안 근거로 사용하지 않고 각 문항을 독립 계산한다. 1회는 20문항이 검산을 통과했다. 16번은 실제 그림의 정사각형 7개에 맞게 공개 재제작 문구도 7개로 정정했고, 그림 좌표 전수 열거로 정답 후보가 하나임을 재확인했다. 회차는 모바일·A4 시각 게이트를 통과하기 전까지 계속 `review_pending`으로 둔다.
