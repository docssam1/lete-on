...
감사 수치만 담는 공개 안전 목록은 `mock/premier-release-catalog.js`에 별도로 둔다. 활용 8회·파이널 3회·최종 4회, 총 15회가 모두 `review_pending`, `href: null` 상태이며 문항·정답·원본 경로·자산 주소는 포함하지 않는다. 현재 감사 결과는 전체 300문항 중 정답 검산 227문항, 잠금 73문항이다. 한 회차라도 잠금 문항이나 모바일 시각 게이트가 남아 있으면 회차 전체를 공개하지 않는다.
