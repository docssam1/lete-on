...
현재 상태는 **Hyper Focus 전용 원격 프로젝트·DB migration·세 Edge Function·private Storage·활용 1회 page image 전달 자산 배포 완료, 웹 클라이언트 운영 연결 활성**입니다. 프로젝트 `gfield-hyper-focus`(`uqtkxhchtbcizzteuvsq`, 서울 리전)는 기존 필즈더클래식 프로젝트와 분리되어 있습니다. `supabase-config.js`의 `enabled`와 `features.secureMockDelivery`는 `true`입니다. 운영 학생과 회차 권한은 아직 0건이며 활용 1회 보호 답안 자산도 없으므로, 첫 학생 등록 뒤에는 영상 해설 기반 O/X 경로부터 검증합니다.
...
- 운영 검증 없이 새 회차를 `published`로 바꾸거나 private Storage 경로를 공개 목록에 넣지 않습니다.
...
4. `DOCSSAM` 관리자는 현재 사용자 결정에 따라 TOTP 없이 Supabase 비밀번호로 로그인합니다. 관리자 함수는 Auth가 서명한 `admin` 역할, 활성 관리자 행, 권한 변경 시각을 서버에서 다시 확인합니다. TOTP를 다시 도입할 때는 관리자 로그인 화면·Edge 정책·원격 검증을 한 배포에서 함께 바꿉니다.
5. 실제 수강이 승인된 학생만 관리자 화면에서 새 계정으로 만들고 승인번호를 한 번만 전달합니다. 현재 운영 학생은 0명이며 임의의 테스트 학생을 영구 등록하지 않습니다. 소규모 Hyper Focus 운영에서는 `GF-7265`처럼 `GF-` 뒤 숫자 4자리 형식을 사용하고, 기존 20자리 코드는 전환 기간 동안 계속 허용합니다. 서버에는 원문 승인번호를 별도 저장하지 않습니다.
...
- 관리자 비밀번호 로그인 뒤 서명 역할·활성 관리자 행·권한 변경 시각이 모두 맞을 때만 관리 함수가 통과함
- 정지·보관·권한 회수는 즉시 반영됨
...
- 관리자 계정과 운영 DB는 준비됐고 TOTP는 사용자 결정에 따라 생략함. leaked-password protection은 Dashboard에서 별도로 켤 것을 권장함
- 운영 학생과 회차 권한이 0건이므로 교차 학생 차단·번호 회전·상품 승인·회수의 실제 학생 E2E는 미실행
- `page_images` manifest v2, private `page` 자산 4장, 활용 1회 회차 데이터, 세 Edge Function은 원격 배포됨. 보호 답안 자산은 아직 없음
- `mock/index.html`·`mock/viewer.html` 화면 연결, 영상 채점, 제출 잠금, 회차별 RLS, 포털 15회 목록, 관리자 8·3·4 상품 토글 계약은 정적 검증 완료. 명시적 2·3회차 재응시 흐름은 미구현
...
- 운영 학생 명단은 0명이며 새 학생은 원격 관리자 화면에서만 발급함. 공개 레거시 관리자 해시 경로는 제거함
- 로컬 모의 서버 기반 Chromium에서 데스크톱·390px 모바일 화면은 통과했지만 실제 학생 계정 기반 운영 브라우저 E2E는 미완료
