...
학생 화면은 `학교 핵심 → 사고력 클리닉 → 경시 가교` 세 단계만 보여 준다. Grade 6의 실제 공개 클리닉 7개만 `공개`로 표시하고, Grade 5·7·8은 문항 검수 전까지 `영역별 설계 공개`로 둔다. SASMO·Math Kangaroo·AMC 연결은 준비 방향이며 예상 등급이나 승급 컷으로 사용하지 않는다.
