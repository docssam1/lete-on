...
3. `6.EE.B` is released. Preserve the proof above rather than redoing it without a code change.
4. Upgrade `6.EE.C` from the current 12-item clinic to the same 36-workbook-item plus 8-distinct-recheck contract. Use `SOL 울트라` for the math-authoring and independent-answer-verification phase: do not merely expand item count. Keep a fixed answer ledger, independently recompute every answer, and provide Korean, English, and Simplified Chinese curriculum-appropriate wording. Then use `TERRA 높은` for integration, responsive/print QA, deployment, and live verification.
