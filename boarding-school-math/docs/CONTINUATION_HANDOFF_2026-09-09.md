...
4. Before expanding into a new grade, inventory every remaining Grade 6 public clinic route and identify any route that is still a 12-item clinic. Upgrade only one verified route at a time to the same 36-workbook-item plus eight-distinct-recheck contract. Use `SOL 울트라` for authoring, answer verification, and any geometry coordinate/point-and-segment calculation; then use `TERRA 높은` for integration, responsive/print QA, deployment, and live verification.
