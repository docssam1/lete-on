(function (root, factory) {
  ...
  const q = {
    ...
    plants: tr("같은 날 심은 강낭콩의 키는 4주 뒤 각각 몇 cm인가?", "How tall is each bean plant four weeks after the plants were planted on the same day?", "同一天种下的菜豆，四周后每株有多高？"),
    ...
    screen: tr("우리 반 학생들은 지난 토요일에 화면이 있는 기기를 몇 분 동안 사용했는가?", "How many minutes of screen time did students have last Saturday?", "学生上周六使用屏幕多少分钟？"),
    heart: tr("우리 반 학생들이 1분 동안 운동한 뒤 잰 심박수는 각각 얼마인가?", "What is each student's heart rate after one minute of exercise?", "每名学生运动一分钟后的心率是多少？"),
    rain: tr("지난 14일 동안 날마다 내린 비의 양은 얼마였는가?", "How much rain fell on each of the last 14 days?", "过去14天每天的降雨量是多少？")
  };
  ...
  const WORKBOOK_ITEMS = freeze([
    ...
    variation("spa-w10", "variability", "foundation", q.books, [
      variabilityChoice("A", tr("조사 기간인 지난달", "The month surveyed", "调查的月份"), false),
    ...
    variation("spa-w11", "variability", "foundation", q.plants, [
      variabilityChoice("A", tr("조사 기간인 4주", "The four-week period", "四周这一时间段"), false),
      variabilityChoice("B", tr("길이 단위인 cm", "The unit, centimeters", "长度单位：厘米"), false),
      variabilityChoice("C", tr("4주 뒤 각 강낭콩의 키", "Height of each plant", "每株植物的高度"), true)
    ]),
    variation("spa-w12", "variability", "core", q.goals, [
      variabilityChoice("A", tr("경기마다 넣은 골 수", "Goals scored in each game", "每场比赛的进球数"), true),
      variabilityChoice("B", tr("조사 기간인 이번 시즌", "The time period, this season", "本赛季这一时间范围"), false),
      variabilityChoice("C", tr("경기 종목", "The sport being played", "比赛项目"), false)
    ]),
    variation("spa-w13", "variability", "core", q.shoes, [
      variabilityChoice("A", tr("조사 대상인 6학년", "Grade 6", "六年级"), false),
    ...
    variation("spa-w14", "variability", "core", q.screen, [
      variabilityChoice("A", tr("조사한 날짜인 지난 토요일", "The date, last Saturday", "上周六这一日期"), false),
      variabilityChoice("B", tr("시간 단위인 분", "The unit, minutes", "时间单位：分钟"), false),
      variabilityChoice("C", tr("각 학생의 화면 사용 시간", "Screen time for each student", "每名学生的屏幕使用时间"), true)
    ]),
    variation("spa-w15", "variability", "core", q.heart, [
      variabilityChoice("A", tr("운동 뒤 잰 각 학생의 심박수", "Each student's heart rate after exercise", "每名学生运动后的心率"), true),
      variabilityChoice("B", tr("정해진 운동 시간 1분", "The one-minute exercise time", "一分钟的运动时间"), false),
      variabilityChoice("C", tr("측정 단위", "The unit of measure", "测量单位"), false)
    ]),
    variation("spa-w16", "variability", "core", q.rain, [
      variabilityChoice("A", tr("조사 기간인 14일", "The 14-day observation period", "14天的观察期"), false),
  ...
  const RECHECK_ITEMS = freeze([
    classify("spa-r01", "recheck", "core", tr("우리 반 학생들의 생일은 각각 몇 월인가?", "In which months are students in our class born?", "我们班学生分别出生在哪个月？"), true),
    ...
    variation("spa-r04", "recheck", "core", q.rain, [
      ...
      variabilityChoice("C", tr("조사 기간인 14일", "The 14 observed days", "观测的14天"), false)
    ]),
  ...
  const ERROR_GUIDES = freeze({
    ...
    "one-measure-only": { label:tr("평균이나 범위 중 하나만 구함", "Checked only center or only variability", "只检查了中心位置或离散程度中的一个"), prompt:tr("평균과 범위를 모두 구한 뒤 두 결과를 한 문장으로 설명하게 하세요.", "Find both the mean and range, then use both results in one statement.", "先求平均数和极差，再用一句话同时说明两个结果。") }
  });
  ...
  const PACK = freeze({
    ...
    ui: freeze({ sectionOrder:["questions","variability","distributions","measures","synthesis","recheck"], sectionLabels:{
      ...
      synthesis:tr("5 · 평균과 범위로 두 자료 설명하기", "5 · Explain center and variability together", "5 · 综合说明中心位置和离散程度"),
  ...
  function renderVisual(candidate, locale) {
    ...
    if (candidate.kind === "measure-role") return '<div class="measure-card"><strong>' + esc(text(candidate.question, locale)) + '</strong><span>' + esc(text(tr("이 값은 자료의 무엇을 알려 줄까?", "What does this one number describe?", "这个数描述什么？"), locale)) + '</span></div>';
