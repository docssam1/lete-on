(function (root, factory) {
  ...
  function renderVisual(candidate, locale) {
    ...
    if (candidate.kind === "fraction-rate") { const labels=locale === "ko" ? { distance:"거리", time:"시간", unit:"단위율 · km/h" } : locale === "zh-Hans" ? { distance:"距离", time:"时间", unit:"单位率 · 千米/小时" } : { distance:"distance", time:"time", unit:"unit rate · km/h" }; return '<div class="g7rpa-rate-model" role="img" aria-label="rate division model"><span class="g7rpa-quantity"><small>' + labels.distance + '</small><strong>' + fractionText(data.distance) + ' <em>km</em></strong></span><b aria-hidden="true">÷</b><span class="g7rpa-quantity"><small>' + labels.time + '</small><strong>' + fractionText(data.time) + ' <em>h</em></strong></span><i>' + labels.unit + '</i></div>'; }
