...
seed('seed-turtle','꼬북칩','스낵',2),seed('seed-nacho','나쵸','스낵',2),seed('seed-muji','무인양품 초코과자','초콜릿',2,'',1,'봉',.8,true,false,'약 4/5 남음'),seed('seed-cornpop','옥수수스프맛 팝콘','스낵',2),seed('seed-tofu','두부과자','스낵',2,'2026-10-08'),seed('seed-rice','쌀과자','스낵',2),seed('seed-ansung','안성탕면','기타',2,'',1,'개'),seed('seed-jjol','쫄병','스낵',2),seed('seed-chickenpop','치킨팝','스낵',2),
...
[2,'꼬북칩',['꼬북칩','거북칩','거북찹']],[2,'나쵸',['나쵸','나초','nacho']],[2,'무인양품 초코과자',['무인양품초코과자','무인초코과자','무인초커과자','무인양품비스킷','무인양품바나나칩']],[2,'옥수수스프맛 팝콘',['옥수수스프맛팝콘','옥수수수프맛팝콘','코옥수수스프맛팝콘']],[2,'두부과자',['두부과자']],[2,'쌀과자',['쌀과자']],[2,'안성탕면',['안성탕면']],[2,'쫄병',['쫄병']],[2,'치킨팝',['치킨팝']],
...
if(!localStorage.getItem(JAPAN_BOX_FIX)){
  ...
  for(const product of japanBoxes){
    const existing=items.find(x=>canon(x.name)===canon(product.name));
    if(existing){existing.name=product.name;existing.category=product.category;existing.shelf=1;existing.expiry=product.expiry;existing.quantity=Math.max(1,Number(existing.quantity||0));existing.unit='박스';existing.remaining=Number(existing.remaining||1);existing.opened=Boolean(existing.opened);existing.needsReview=false;existing.note=product.note;existing.updatedAt=new Date().toISOString();}
    else items.push({...product,updatedAt:new Date().toISOString()});
  }
