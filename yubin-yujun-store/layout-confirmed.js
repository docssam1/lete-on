...
const PUDDING_FIX='yubinYujunStore.addPucchinPudding.2026-07-24.v1',PUDDING_SNAPSHOT='yubinYujunStore.beforeAddPucchinPudding.2026-07-24.v1';
...
const pucchinPudding=seed('seed-pucchin-pudding','글리코 쵸콧토 푸칭푸딩 커스터드','기타',2,'2026-11-08',3,'팩',1,false,false,'1팩 6개입 · 상온 보관 가능');
const desired=[
...
seed('seed-turtle','꼬북칩','스낵',2),seed('seed-nacho','나쵸','스낵',2),seed('seed-muji','무인양품 초코과자','초콜릿',2,'',1,'봉',.8,true,false,'약 4/5 남음'),seed('seed-cornpop','옥수수스프맛 팝콘','스낵',2),seed('seed-tofu','두부과자','스낵',2,'2026-10-08'),seed('seed-rice','쌀과자','스낵',2),seed('seed-ansung','안성탕면','기타',2,'',1,'개'),seed('seed-jjol','쫄병','스낵',2),seed('seed-chickenpop','치킨팝','스낵',2),...secondFloorBatch,pucchinPudding,
...
if(!localStorage.getItem(PUDDING_FIX)){
  items=parse(KEY);localStorage.setItem(PUDDING_SNAPSHOT,JSON.stringify(items));
  const existing=items.find(x=>canon(x.name)===canon(pucchinPudding.name));
  if(existing){Object.assign(existing,{name:pucchinPudding.name,category:'기타',shelf:2,expiry:'2026-11-08',quantity:3,unit:'팩',remaining:1,opened:false,needsReview:false,note:'1팩 6개입 · 상온 보관 가능',updatedAt:new Date().toISOString()});}
  else items.push({...pucchinPudding,updatedAt:new Date().toISOString()});
  localStorage.setItem(KEY,JSON.stringify(items));localStorage.setItem(PUDDING_FIX,'done');
}
