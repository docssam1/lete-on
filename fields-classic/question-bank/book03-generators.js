...
const TANGRAM_PIECES = Object.freeze([
  ...
  { id: 3, kind: "평행사변형", area: 2 },
  { id: 4, kind: "작은 삼각형", area: 1 },
  { id: 5, kind: "정사각형", area: 2 },
  { id: 6, kind: "작은 삼각형", area: 1 },
  { id: 7, kind: "중간 삼각형", area: 2 }
]);
...
function tangramShapeComposition({ difficulty = 2 }) {
  ...
  const fixedPieceIds = sample(fixedByDifficulty[difficulty] || fixedByDifficulty[2]);
  const rotation = sample([0, 90, 180, 270]);
  const mirrored = Math.random() < 0.5;
  const baseVisual = {
    kind: "book3",
    subtype: "tangram-composition-source",
    template: "five-piece-square",
    pieceIds: [3, 4, 5, 6, 7],
    fixedPieceIds,
    rotation,
    mirrored,
  ...
  return {
    prompt: "가장 큰 삼각형 2개를 뺀 나머지 칠교 5조각으로 아래 정사각형을 완성하고, 조각 사이의 선을 그으세요. 이미 놓인 조각은 움직이지 않습니다.",
    visual: { ...baseVisual, complete: false },
    answerVisual: { ...baseVisual, complete: true },
    answer: "그림과 같이 5조각으로 정사각형 완성",
    solution: "정사각형 조각을 기준으로 두 작은 삼각형, 중간 삼각형, 평행사변형을 차례로 맞추면 빈틈이나 겹침 없이 정사각형이 됩니다.",
...
function tangramPieceArea({ difficulty = 2 }) {
  const unitArea = randomInt(1, difficulty === 3 ? 4 : 3);
  const askMode = difficulty === 1 ? "piece" : difficulty === 2 ? "three-piece-square" : "five-piece-square";
  const selected = askMode === "piece"
    ? [sample(TANGRAM_PIECES).id]
    : askMode === "three-piece-square"
      ? [4, 6, 7]
      : [3, 4, 5, 6, 7];
  const baseArea = TANGRAM_PIECES.filter((item) => selected.includes(item.id)).reduce((sum, item) => sum + item.area, 0);
  const answer = baseArea * unitArea;
  const rotation = sample([0, 90, 180, 270]);
  const mirrored = Math.random() < 0.5;
  const pieceLabel = selected.length === 1 ? `${selected[0]}번 조각` : `${selected.length}조각으로 만든 정사각형`;
  return {
    prompt: `작은 정사각형 한 칸의 넓이가 ${unitArea}일 때, ${pieceLabel}의 넓이를 구하세요.`,
    visual: {
      kind: "book3",
      subtype: "tangram-area-source",
      template: "four-by-four-tangram",
      askMode,
    ...
    solution: selected.length === 1
      ? `${selected[0]}번 조각은 작은 정사각형 ${baseArea}칸과 넓이가 같습니다. ${baseArea} × ${unitArea} = ${answer}입니다.`
      : `사용한 ${selected.length}조각의 넓이를 모눈으로 세면 작은 정사각형 ${baseArea}칸과 같습니다. ${baseArea} × ${unitArea} = ${answer}입니다.`,
    meta: {
      family: "tangram-area",
      askMode,
      selected,
      pieceAreas: Object.fromEntries(TANGRAM_PIECES.map((item) => [item.id, item.area])),
      unitArea,
      baseArea,
      rotation,
      mirrored,
...
function unitGridArea({ difficulty = 2 }) {
  const templates = {
    ...
    2: [
      ...
      [[0,1],[2,0],[4,1],[2,2]],
      [[0,1],[1,0],[2,1],[3,2],[2,3],[1,2],[0,3]],
      [[0,2],[1,1],[2,2],[3,1],[4,2],[3,3],[1,3]]
    ],
    3: [
      [[0,2],[1,1],[2,2],[4,2],[5,1],[4,3],[2,3],[2,4],[1,3],[0,3]],
      [[0,1],[1,2],[2,2],[2,0],[3,0],[3,2],[4,2],[5,1],[5,3],[3,3],[3,5],[2,5],[2,3],[0,3]],
      [[0,2],[1,1],[2,2],[3,1],[4,2],[3,3],[4,4],[2,4],[1,3],[0,4]],
      [[0,1],[1,0],[2,1],[2,2],[4,2],[5,1],[5,3],[3,3],[3,5],[1,5],[1,3],[0,3]]
    ]
  };
  let points = sample(templates[difficulty] || templates[2]).map(([x, y]) => [x, y]);
  const mirrored = Math.random() < 0.5;
  if (mirrored) {
    const maxX = Math.max(...points.map(([x]) => x));
    points = points.map(([x, y]) => [maxX - x, y]).reverse();
  }
  const areaTwice = Math.abs(points.reduce((sum, [x, y], index) => {
  ...
  return {
    ...
    visual: { kind: "book3", subtype: "unit-grid-area", width, height, points },
    answer: Number.isInteger(answer) ? String(answer) : `${Math.floor(answer)}와 1/2`,
    solution: `도형 안의 온칸을 세고, 대각선으로 나뉜 반칸 두 개를 온칸 하나로 묶습니다. 넓이는 ${Number.isInteger(answer) ? answer : `${Math.floor(answer)}와 1/2`}입니다.`,
    meta: { family: "unit-grid-area", points, width, height, areaTwice, connected: true, sourceReady: true }
  };
}
...
const SOURCE_PARTITION_LEVELS = Object.freeze({
  1: Object.freeze([
    { template: "triangle-3", shape: "triangle", parts: 3 },
    { template: "triangle-4", shape: "triangle", parts: 4 }
  ]),
  2: Object.freeze([
    { template: "triangle-6", shape: "triangle", parts: 6 },
    { template: "triangle-8", shape: "triangle", parts: 8 },
    { template: "hexagon-12", shape: "hexagon", parts: 12 }
  ]),
  3: Object.freeze([
    { template: "triangle-12", shape: "triangle", parts: 12 },
    { template: "hexagon-18", shape: "hexagon", parts: 18 }
  ])
});

const INCOMPLETE_PARTITION_LEVELS = Object.freeze({
  1: Object.freeze([
    { template: "triangle-3", shape: "triangle", parts: 3, internalLines: 3, visibleLines: 2 },
    { template: "triangle-4", shape: "triangle", parts: 4, internalLines: 3, visibleLines: 2 }
  ]),
  2: Object.freeze([
    { template: "triangle-6", shape: "triangle", parts: 6, internalLines: 6, visibleLines: 4 },
    { template: "triangle-8", shape: "triangle", parts: 8, internalLines: 7, visibleLines: 4 }
  ]),
  3: Object.freeze([
    { template: "triangle-8", shape: "triangle", parts: 8, internalLines: 7, visibleLines: 3 },
    { template: "triangle-12", shape: "triangle", parts: 12, internalLines: 15, visibleLines: 8 }
  ])
});
...
function equalPartShadedFraction({ difficulty = 2 }) {
  const choices = difficulty === 1
    ? [
        { template: "circle-radial", shape: "circle", parts: 2 },
        { template: "circle-radial", shape: "circle", parts: 3 },
        { template: "circle-radial", shape: "circle", parts: 4 },
        { template: "circle-radial", shape: "circle", parts: 5 },
        { template: "circle-radial", shape: "circle", parts: 6 }
      ]
    : difficulty === 2
      ? [
          { template: "circle-radial", shape: "circle", parts: 8 },
          { template: "circle-radial", shape: "circle", parts: 9 },
          { template: "triangle-grid", shape: "triangle", parts: 4 },
          { template: "triangle-grid", shape: "triangle", parts: 9 },
          { template: "hexagon-radial", shape: "hexagon", parts: 6 }
        ]
      : [
          { template: "circle-radial", shape: "circle", parts: 16 },
          { template: "triangle-grid", shape: "triangle", parts: 9 },
          { template: "triangle-grid", shape: "triangle", parts: 16 },
          { template: "hexagon-radial", shape: "hexagon", parts: 12 }
        ];
  const { template, shape, parts } = sample(choices);
  const shaded = randomInt(1, parts - 1);
  const rotation = randomInt(0, parts - 1);
  return {
    prompt: "도형을 모양과 크기가 같은 조각으로 나누었습니다. 색칠한 부분을 분수로 나타내세요.",
    visual: { kind: "book3", subtype: "equal-fraction-source", template, shape, parts, shaded, rotation },
    answer: fractionText(shaded, parts),
    solution: `전체는 ${parts}조각이고 색칠한 부분은 ${shaded}조각이므로 ${fractionText(shaded, parts)}입니다.`,
    meta: { family: "equal-fraction", template, shape, parts, shaded, rotation, equalParts: true, sourceReady: true }
  };
}

function equalPartitionDrawing({ difficulty = 2 }) {
  const { template, shape, parts } = sample(SOURCE_PARTITION_LEVELS[difficulty] || SOURCE_PARTITION_LEVELS[2]);
  const shaded = difficulty === 1 ? 1 : randomInt(1, Math.max(1, Math.floor(parts / 3)));
  const rotation = randomInt(0, parts - 1);
  const visual = { kind: "book3", subtype: "equal-partition-source", template, shape, parts, shaded: 0, rotation, complete: false, targetParts: parts };
  const answerVisual = { ...visual, shaded, complete: true };
  return {
    ...
    responseKind: "drawing",
    solution: `안내점을 지나도록 선을 그어 모양과 크기가 같은 ${parts}조각을 만들고, 그중 ${shaded}조각을 색칠합니다.`,
    meta: { family: "equal-partition-drawing", template, shape, parts, shaded, rotation, equalParts: true, sourceReady: true }
  };
}

function incompletePartitionFraction({ difficulty = 2 }) {
  const choice = sample(INCOMPLETE_PARTITION_LEVELS[difficulty] || INCOMPLETE_PARTITION_LEVELS[2]);
  const { template, shape, parts, internalLines, visibleLines } = choice;
  const shaded = randomInt(1, Math.max(1, Math.floor(parts / 3)));
  const rotation = randomInt(0, parts - 1);
  const lineShift = randomInt(0, internalLines - 1);
  return {
    prompt: "빠진 선을 이어 모양과 크기가 같은 작은 조각으로 완성한 뒤, 색칠한 부분을 분수로 나타내세요.",
    visual: { kind: "book3", subtype: "incomplete-partition-source", template, shape, parts, shaded, rotation, complete: false, visibleLines, lineShift },
    answerVisual: { kind: "book3", subtype: "incomplete-partition-source", template, shape, parts, shaded, rotation, complete: true, visibleLines: internalLines, lineShift },
    answer: fractionText(shaded, parts),
    solution: `빠진 선을 모두 이으면 전체가 ${parts}조각이 됩니다. 색칠한 조각은 ${shaded}개이므로 ${fractionText(shaded, parts)}입니다.`,
    meta: { family: "incomplete-fraction", template, shape, parts, shaded, visibleLines, internalLines, lineShift, rotation, equalParts: true, sourceReady: true }
  };
}
...
function coloredIndicesForCode(target, rows, columns, columnWeights, randomizeSingle = false) {
  const colored = [];
  columnWeights.forEach((weight, column) => {
    const digit = Math.floor(target / weight) % (rows + 1);
    const rowOrder = randomizeSingle && digit === 1 ? shuffle(Array.from({ length: rows }, (_, index) => index)) : Array.from({ length: rows }, (_, index) => index);
    rowOrder.slice(0, digit).forEach((row) => colored.push(row * columns + column));
  });
  return colored.sort((a, b) => a - b);
}

function coloredCellNumberCode({ difficulty = 2 }) {
  const ternary = difficulty === 3 || (difficulty === 2 && Math.random() < 0.35);
  const rows = ternary ? 2 : 1;
  const columns = ternary ? 4 : 5;
  const columnWeights = ternary ? [27, 9, 3, 1] : [16, 8, 4, 2, 1];
  const maximum = columnWeights.reduce((sum, value) => sum + value * rows, 0);
  const minimum = difficulty === 1 ? 1 : ternary ? 3 : 2;
  const target = randomInt(minimum, maximum);
  const colored = coloredIndicesForCode(target, rows, columns, columnWeights, true);
  const weights = Array.from({ length: rows }, () => columnWeights).flat();
  const mode = difficulty === 1 ? "read" : Math.random() < 0.35 ? "color" : "read";
  const exampleValues = ternary ? [1, 2, 3, 4, 6, 8, 9, 10] : [1, 2, 3, 4, 5, 7, 8, 14];
  const examples = difficulty === 1 ? [] : exampleValues.map((value) => ({
    value,
    colored: coloredIndicesForCode(value, rows, columns, columnWeights)
  }));
  const visual = {
    kind: "book3",
    subtype: "cell-code",
    ...
    weights,
    columnWeights,
    colored: mode === "read" ? colored : [],
    target,
    mode,
    examples,
    showWeights: difficulty === 1
  };
  return {
    prompt: mode === "read"
      ? "보기의 규칙에 따라 색칠한 칸이 나타내는 수를 구하세요."
      : `보기의 규칙에 따라 ${target}이 되도록 빈 도형의 칸을 색칠하세요.`,
    visual,
    answerVisual: mode === "color" ? { ...visual, colored, reveal: true } : null,
    answer: mode === "read" ? String(target) : `${target}이 되도록 색칠`,
    responseKind: mode === "color" ? "drawing" : "text",
    solution: mode === "read"
      ? `색칠한 칸의 값은 ${colored.map((index) => weights[index]).join(", ")}입니다. 모두 더하면 ${colored.map((index) => weights[index]).join(" + ")} = ${target}입니다.`
      : `${target}을 ${colored.map((index) => weights[index]).join(" + ")}로 나타낼 수 있으므로 답 그림처럼 색칠합니다.`,
    meta: { family: "colored-cell-number-code", rows, columns, weights, columnWeights, colored, target, answer: target, mode, sourceReady: true }
  };
}
...
function symbolValueCode({ difficulty = 2, template: forcedTemplate = null } = {}) {
  ...
  while (circle === star) circle = randomInt(4, difficulty === 3 ? 20 : 14);
  ...
  const countRows = rowSymbolIndexes.map((indexes) => symbols.map((_, symbolIndex) => indexes.filter((index) => index === symbolIndex).length));
  const totalForCounts = (counts) => counts.reduce((sum, count, index) => sum + count * values[index], 0);
  const rows = countRows.map((counts, index) => ({ counts, symbols: rowSymbolIndexes[index].map((symbolIndex) => symbols[symbolIndex]), total: totalForCounts(counts) }));
  const sourceTargets = template === "practice"
    ? [[0, 0, 0, 1], [2, 2, 0, 1], [2, 0, 0, 1, 1]]
    : [[2, 0], [2, 0, 2, 0], [1, 1, 1, 1, 1], [2, 1, 0]];
  const targetIndexes = difficulty === 1
    ? [sample([0, 1, 2])]
    : sample(difficulty === 3 ? sourceTargets.filter((target) => target.length >= 4) : sourceTargets);
  const targetCounts = symbols.map((_, symbolIndex) => targetIndexes.filter((index) => index === symbolIndex).length);
  const targetSymbols = targetIndexes.map((symbolIndex) => symbols[symbolIndex]);
  const answer = totalForCounts(targetCounts);
  return {
    prompt: "보기에서 같은 도형은 같은 수를 나타냅니다. 아래 도형 묶음이 나타내는 수를 구하세요.",
    visual: { kind: "book3", subtype: "symbol-value-code", rows, targetSymbols },
    answer: String(answer),
    solution: template === "practice"
      ? `첫째 상자와 둘째 상자를 비교하면 ○ 하나가 ${rows[1].total - rows[0].total}이므로 ○=${circle}입니다. ☆=${rows[0].total - circle}, ◇=${rows[2].total - circle - star}입니다. 따라서 ${targetSymbols.map((symbol) => values[symbols.indexOf(symbol)]).join(" + ")} = ${answer}입니다.`
...
function magicSquareFourComplete({ difficulty = 2 }) {
  ...
  const blankIndices = difficulty === 1
    ? shuffle(sourceMasks.flat()).filter((value, index, values) => values.indexOf(value) === index).slice(0, 4).sort((a, b) => a - b)
    : [...sample(sourceMasks)];
  const hidden = new Set(blankIndices);
  const shown = solution.flat().map((value, index) => hidden.has(index) ? null : value);
  return {
    prompt: "1부터 16까지의 수를 한 번씩 써서 가로, 세로, 대각선에 놓인 네 수의 합이 모두 34가 되도록 빈칸을 채우세요.",
    visual: { kind: "book3", subtype: "magic-grid", size: 4, cards: [], shown, lineSum: 34 },
    answerVisual: { kind: "book3", subtype: "magic-grid", size: 4, cards: [], shown: solution.flat(), lineSum: 34 },
    answer: solution.map((row) => row.join(" ")).join(" / "),
    responseKind: "visual-fill",
    solution: `빈칸에 들어갈 수를 1부터 16까지 중에서 찾습니다. 가로와 세로의 네 수를 더해 34가 되도록 채운 뒤 두 대각선도 34인지 확인합니다. 완성한 마방진은 ${solution.map((row) => row.join(", ")).join(" / ")}입니다.`,
    meta: { family: "magic-four-complete", solution, blankIndices, lineSum: 34, sourceReady: true }
  };
}
...
const BOOK3_EQUAL_LINE_EIGHT_LAYOUTS = (() => {
  const layouts = [];
  eachPermutation([1, 2, 3, 4, 5, 6, 7, 8], (values) => {
    const lineSum = values[0] + values[1] + values[2];
    const lines = [[0,1,2], [2,3,4], [4,5,6], [6,7,0]];
    if (lines.every((line) => line.reduce((sum, index) => sum + values[index], 0) === lineSum)) {
      layouts.push({ values, lineSum });
    }
  });
  return layouts;
})();

function book3IndexCombinations(values, count) {
  if (count === 0) return [[]];
  if (values.length < count) return [];
  return values.flatMap((value, index) => book3IndexCombinations(values.slice(index + 1), count - 1)
    .map((tail) => [value, ...tail]));
}

function equalLineSumEightCardsCompleteBook3({ difficulty = 2 }) {
  const lineSum = sample([12, 15]);
  const pool = BOOK3_EQUAL_LINE_EIGHT_LAYOUTS.filter((layout) => layout.lineSum === lineSum);
  const sourceClues = lineSum === 12 ? [0, 4, 6] : [0, 2, 4];
  const allIndexes = Array.from({ length: 8 }, (_, index) => index);
  const clueSets = difficulty === 1
    ? allIndexes.filter((index) => !sourceClues.includes(index)).map((index) => [...sourceClues, index].sort((a, b) => a - b))
    : difficulty === 2
      ? [sourceClues]
      : book3IndexCombinations(allIndexes, 3).filter((indexes) => indexes.filter((index) => index % 2 === 0).length <= 2);
  const choices = [];
  for (const solution of pool) {
    for (const givenIndices of clueSets) {
      const candidates = pool.filter((candidate) => givenIndices.every((index) => candidate.values[index] === solution.values[index]));
      if (candidates.length === 1) choices.push({ solution, givenIndices, candidateCount: 1 });
    }
  }
  if (!choices.length) return equalLineSumEightCardsCompleteBook3({ difficulty: 2 });
  const choice = sample(choices);
  const shown = choice.solution.values.map((value, index) => choice.givenIndices.includes(index) ? value : null);
  return {
    prompt: `1부터 8까지의 수를 한 번씩 사용하여 가로와 세로 각 줄에 있는 세 수의 합이 모두 ${lineSum}이 되도록 빈칸을 채우세요.`,
    visual: { kind: "book3", subtype: "equal-line-eight-complete", cards: [1,2,3,4,5,6,7,8], shown, lineSum },
    answerVisual: { kind: "book3", subtype: "equal-line-eight-complete", cards: [], shown: choice.solution.values, lineSum },
    answer: choice.solution.values.join(", "),
    responseKind: "visual-fill",
    solution: `각 줄의 합 ${lineSum}에서 이미 놓인 두 수를 빼며 빈칸을 채웁니다. 1부터 8까지를 한 번씩 썼는지 확인하면 왼쪽 위부터 시계 방향으로 ${choice.solution.values.join(", ")}입니다.`,
    meta: { family: "equal-line-eight-complete-book3", layout: choice.solution.values, shown, lineSum, givenIndices: choice.givenIndices, candidateCount: choice.candidateCount }
  };
}
...
export const BOOK03_GENERATORS = {
  ...
  triangleEdgeSumSix,
  triangleEdgeSumNine,
  equalLineSumEightCardsCompleteBook3,
